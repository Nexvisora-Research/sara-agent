...
_loaded_env_paths = load_sara_dotenv(sara_home=_sara_home, project_env=_project_env)
...
class _SafeWriter:
    ...
    When sara-agent runs as a systemd service, Docker container, or headless
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[sara-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            load_soul_identity (bool): If True, still use ~/.sara/SOUL.md as the primary
        ...
        self._cache_ttl = "5m"
        try:
            from sara_cli.config import load_config as _load_pc_cfg
        ...
        # both live under ~/.sara/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from sara_logging import setup_logging, setup_verbose_logging
        setup_logging(sara_home=_sara_home)
        ...
        if self.api_mode == "anthropic_messages":
            ...
            try:
                from sara_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    ...
                    from sara_cli.models import copilot_default_headers
                    ...
                    if _explicit and _explicit not in ("auto", "openrouter", "custom"):
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `sara model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `sara model` to "
                            "select a provider, or run `sara setup` for first-time "
                            "configuration."
                        )
            ...
            try:
                # If the OpenAI SDK isn't installed, avoid importing it when
                # the user intends to talk to a local Ollama endpoint. This
                # prevents ModuleNotFoundError for users who only run local
                # LLMs (Ollama/llama.cpp/vLLM) and don't need the OpenAI
                # SDK. If OpenAI is required, the user should install the
                # `openai` package into the environment.
                import importlib
                _openai_spec = importlib.util.find_spec("openai")
                base = str(client_kwargs.get("base_url", "") or "").lower()
                wants_ollama = (
                    (self.provider or "").startswith("ollama")
                    or "ollama" in base
                    or "11434" in base
                )
                if _openai_spec is None and wants_ollama:
                    if not self.quiet_mode:
                        print("ℹ️  OpenAI SDK not installed — skipping OpenAI client init for Ollama/local provider.")
                    # Do not create an OpenAI client; Ollama/local flows
                    # use direct HTTP probes elsewhere in the codebase.
                    self.client = None
                else:
        ...
        # Session logs go into ~/.sara/sessions/ alongside gateway sessions
        sara_home = get_sara_home()
        self.logs_dir = sara_home / "sessions"
        ...
        try:
            from sara_cli.config import load_config as _load_agent_config
                ...
                if _mem_provider_name:
                    ...
                    if self._memory_manager.providers:
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "sara"
                        except Exception:
                            pass
        ...
        if _config_context_length is None and _custom_providers:
            try:
                from sara_cli.config import get_custom_provider_context_length
        ...
        if _engine_name != "compressor":
            ...
            if _selected_engine is None:
                try:
                    from sara_cli.plugins import get_plugin_context_engine
        ...
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"by sara Agent.  Choose a model with at least "
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            self._session_db.create_session(
                session_id=self.session_id,
                source=self.platform or os.environ.get("sara_SESSION_SOURCE", "cli"),
                model=self.model,
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least sara' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from sara_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from sara_cli.providers import determine_api_mode
        ...
        if hasattr(self, "context_compressor") and self.context_compressor:
            ...
            try:
                from sara_cli.config import load_config, get_compatible_custom_providers
                _sm_cfg = load_config()
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``sara chat -q``. In that mode,
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `sara setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by sara "
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``sara_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``sara_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("sara_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``sara_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("sara_API_CALL_STALE_TIMEOUT")
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        if normalized_provider == "copilot":
            try:
                from sara_cli.models import _should_use_copilot_responses_api
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("sara_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        try:
            ...
            if env_var_enabled("sara_DUMP_REQUEST_STDOUT"):
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        # Some execution modes (cron) still want sara_HOME persona while keeping
        ...
        # Pointer to the sara-agent skill + docs for user questions about sara itself.
        prompt_parts.append(sara_AGENT_HELP_GUIDANCE)
        ...
        # sara_KANBAN_TASK env var). Normal chat sessions never see
        # this block.
        ...
        if not self.skip_context_files:
            ...
            # mode).  The gateway process runs from the sara-agent install
        ...
        from sara_time import now as _sara_now
        now = _sara_now()
    ...
    def _copilot_headers_for_request(self, *, is_vision: bool) -> dict:
        from sara_cli.copilot_auth import copilot_request_headers
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from sara_cli.copilot_auth import resolve_copilot_token
    ...
    def _interruptible_streaming_api_call(
        ...
        def _call_chat_completions():
            ...
            # wins over the sara_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("sara_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # sara_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("sara_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode sara_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("sara_STREAM_RETRIES", 2))
        ...
        _stream_stale_timeout_base = float(os.getenv("sara_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set sara_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        try:
            ...
            try:
                from sara_cli.model_normalize import normalize_model_for_provider
        ...
        def _shrink_data_url(url: str) -> Optional[str]:
            ...
            try:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="sara_shrink_", suffix=suffix, delete=False,
                )
                try:
                    tmp.write(raw)
                    tmp.close()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "sara",
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        if (
            ...
            try:
                from sara_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from sara_cli.models import lmstudio_model_reasoning_options
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        # compression (not a fresh /new). Plugin engines (e.g. sara-lcm) use
        # boundary_reason="compression" to preserve DAG lineage across the
        # rollover instead of re-initializing fresh per-session state.
        # See sara-lcm#68. Built-in ContextCompressor ignores kwargs.
        try:
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from sara_cli.plugins import get_pre_tool_call_block_message
    ...
    def run_conversation(
        ...
        # ``sara logs --session <id>`` can filter a single conversation.
        from sara_logging import set_session_context
        ...
        # are reused.  The system prompt is sara's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            # cache prefix.  The system prompt is reserved for sara internals.
            if effective_system:
                api_messages = [{"role": "system", "content": effective_system}] + api_messages
            ...
            while retry_count < max_retries:
                ...
                try:
                    ...
                    if env_var_enabled("sara_DUMP_REQUESTS"):
                    ...
                    if (
                        ...
                        from sara_constants import display_sara_home as _dhh_fn
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for sara-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: sara config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: sara config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            if _provider == "openai-codex" and status_code == 401:
                                ...
                                self._vprint(f"{self.log_prefix}      2. Then run `sara auth` to re-authenticate.", force=True)
                            else:
                                self._vprint(f"{self.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                                self._vprint(f"{self.log_prefix}      • Is the key valid? Run: sara setup", force=True)
