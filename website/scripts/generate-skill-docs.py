...
def rewrite_relative_links(body: str, meta: dict[str, Any]) -> str:
    ...
    base = f"https://github.com/NexvisoraResearch/sara-agent/blob/main/{source_dir}/{meta['rel_path']}"
...
def render_skill_page(
    ...
    sara_meta = (fm.get("metadata") or {}).get("sara") or {}
    tags = sara_meta.get("tags") or []
    related = sara_meta.get("related_skills") or []
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `sara skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with sara Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "sara ships with a large built-in skill library copied into `~/.sara/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
