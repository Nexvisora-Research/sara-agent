...
description: "Run sara Agent directly on an Android phone with Termux"
---

# sara on Android with Termux

This is the tested path for running sara Agent directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop sara from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

sara now ships a Termux-aware installer path:
...
- links `sara` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `sara` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/sara" "$PREFIX/bin/sara"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `sara` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.sara/.env`.
...
### `sara doctor` says ripgrep or Node is missing
