...
This guide covers adding a new messaging platform to the sara gateway. A platform adapter connects sara to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.sara/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core sara code. Your plugin is a directory with two files:

```
~/.sara/plugins/my-platform/
...
| `sara status` | Shows plugin platforms with `(plugin)` tag |
| `sara gateway setup` | Plugin platforms appear in setup menu |
| `sara tools` / `sara skills` | Plugin platforms in per-platform config |
...
:::note
This checklist is for adding a platform directly to the sara core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
...
1. **`sara_cli/config.py`** — Add all `NEWPLAT_*` vars to `_EXTRA_ENV_KEYS`
2. **`sara_cli/gateway.py`** — Add entry to `_PLATFORMS` list with key, label, emoji, token_var, setup_instructions, and vars
3. **`sara_cli/platforms.py`** — Add `PlatformInfo` entry with label and default_toolset (used by `skills_config` and `tools_config` TUIs)
4. **`sara_cli/setup.py`** — Add `_setup_newplat()` function (can delegate to `gateway.py`) and add tuple to the messaging platforms list
5. **`sara_cli/status.py`** — Add platform detection entry: `"NewPlat": ("NEWPLAT_TOKEN", "NEWPLAT_HOME_CHANNEL")`
6. **`sara_cli/dump.py`** — Add `"newplat": "NEWPLAT_TOKEN"` to platform detection dict
...
1. **`toolsets.py`** — Add `"sara-newplat"` toolset definition with `_sara_CORE_TOOLS`
2. **`toolsets.py`** — Add `"sara-newplat"` to the `"sara-gateway"` includes list
...
| `website/docs/reference/toolsets-reference.md` | sara-newplat toolset |
