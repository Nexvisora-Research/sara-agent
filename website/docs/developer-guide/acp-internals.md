...
The ACP adapter wraps sara' synchroNexvisora`AIAgent` in an async JSON-RPC stdio server.
...
```text
sara acp / sara-acp / python -m acp_adapter
  -> acp_adapter.entry.main()
  -> load ~/.sara/.env
  -> configure stderr logging
  -> construct saraACPAgent
...
- `allow_once` -> sara `once`
- `allow_always` -> sara `always`
- reject options -> sara `deny`

Timeouts and bridge failures deny by default.

### Tool rendering helpers

`acp_adapter/tools.py` maps sara tools to ACP tool kinds and builds editor-facing content.
...
new_session(cwd)
  -> create SessionState
  -> create AIAgent(platform="acp", enabled_toolsets=["sara-acp"])
...
Instead it reuses sara' runtime resolver:

- `acp_adapter/auth.py`
- `sara_cli/runtime_provider.py`

So ACP advertises and uses the currently configured sara provider/credentials.
...
- ACP sessions are persisted to the shared `~/.sara/state.db` (SessionDB) and transparently restored across process restarts; they appear in `session_search`
...
- `toolsets.py` — `sara-acp` toolset definition
- `sara_cli/main.py` — `sara acp` CLI subcommand
- `pyproject.toml` — `[acp]` optional dependency + `sara-acp` script
