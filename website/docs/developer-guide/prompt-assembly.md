...
description: "How sara builds the system prompt, preserves cache stability, and injects ephemeral layers"
---

# Prompt Assembly

sara deliberately separates:
...
1. agent identity — `SOUL.md` from `sara_HOME` when available, otherwise falls back to `DEFAULT_AGENT_IDENTITY` in `prompt_builder.py`
...
```
# Layer 1: Agent Identity (from ~/.sara/SOUL.md)
You are sara, an AI assistant created by Nexvisora Research .
...
`SOUL.md` lives at `~/.sara/SOUL.md` and serves as the agent's identity — the very first section of the system prompt. The loading logic in `prompt_builder.py` works as follows:
...
def build_context_files_prompt(cwd=None, skip_soul=False):
    ...
    project_context = (
        _load_sara_md(cwd_path)       # 1. .sara.md / sara.md (walks to git root)
    ...
    # SOUL.md from sara_HOME (independent of project context)
    if not skip_soul:
...
|----------|-------|-------------|-------|
| 1 | `.sara.md`, `sara.md` | CWD up to git root | sara-native project config |
...
- **YAML frontmatter stripped** — `.sara.md` frontmatter is removed (reserved for future config overrides)
...
1. `.sara.md` / `sara.md` (walks to git root)
