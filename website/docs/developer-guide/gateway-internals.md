...
The messaging gateway is the long-running process that connects sara to 14+ external messaging platforms through a unified architecture.
...
1. `resolve_command()` from `sara_cli/commands.py` maps input to canonical name (handles aliases, prefix matching)
...
|--------|-----------------|
| `~/.sara/.env` | API keys, bot tokens, platform credentials |
| `~/.sara/config.yaml` | Model settings, tool configuration, display options |
...
Hooks are discovered from `gateway/builtin_hooks/` (always active) and `~/.sara/hooks/` (user-installed). Each hook is a directory with a `HOOK.yaml` manifest and `handler.py`.
...
- `sara gateway start` / `sara gateway stop` — manual control
- `systemctl` (Linux) or `launchctl` (macOS) — service management
- PID file at `~/.sara/gateway.pid` — profile-scoped process tracking

**Profile-scoped vs global**: `start_gateway()` uses profile-scoped PID files. `sara gateway stop` stops only the current profile's gateway. `sara gateway stop --all` uses global `ps aux` scanning to kill all gateway processes (used during updates).
