...
description: "How to add a new inference provider to sara Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

sara can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `sara model` menu entries
...
1. `sara_cli/auth.py` decides how credentials are found.
2. `sara_cli/runtime_provider.py` turns that into runtime data:
   - `provider`
   - `api_mode`
   - `base_url`
   - `api_key`
...
4. `sara_cli/models.py` and `sara_cli/main.py` make the provider show up in the CLI. (`sara_cli/setup.py` delegates to `main.py` automatically — no changes needed there.)
...
:::tip
`sara_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `sara setup`.
:::
...
- `PROVIDER_REGISTRY` in `sara_cli/auth.py`
- `_PROVIDER_LABELS` in `sara_cli/models.py`
- `_PROVIDER_ALIASES` in both `sara_cli/auth.py` and `sara_cli/models.py`
- CLI `--provider` choices in `sara_cli/main.py`
...
## Step 2: Add auth metadata in `sara_cli/auth.py`
...
- OAuth / auth-store path: nexvisora, OpenAI Codex

Questions to answer here:

- What env vars should sara check, and in what priority order?
...
## Step 3: Add model catalog and aliases in `sara_cli/models.py`
...
## Step 4: Resolve runtime data in `sara_cli/runtime_provider.py`
...
Be careful with API-key precedence. sara already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `sara_cli/main.py`

A provider is not discoverable until it shows up in the interactive `sara model` flow.

Update these in `sara_cli/main.py`:
...
:::tip
`sara_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `sara model` and `sara setup` automatically.
:::
...
When you add a native provider, double-check that sara is only sending fields that provider actually understands.
...
```bash
source venv/bin/activate
python -m sara_cli.main chat -q "Say hello" --provider your-provider --model your-model
```
...
- [ ] `ProviderConfig` added in `sara_cli/auth.py`
- [ ] aliases added in `sara_cli/auth.py` and `sara_cli/models.py`
- [ ] model catalog added in `sara_cli/models.py`
- [ ] runtime branch added in `sara_cli/runtime_provider.py`
- [ ] CLI wiring added in `sara_cli/main.py` (setup.py inherits automatically)
...
### 7. Updating `sara model` but not `sara setup`
