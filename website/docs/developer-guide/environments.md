...
description: "Building RL training environments, running evaluation benchmarks, and generating SFT data with the sara-Agent Atropos integration"
---

# Environments, Benchmarks & Data Generation

sara Agent includes a full environment framework that connects its tool-calling capabilities to the [Atropos](https://github.com/NexvisoraResearch/atropos) RL training framework. This enables three workflows:
...
The Python environment framework documented here lives under the repo's `environments/` directory and is the implementation-level API for sara/Atropos integration. This is separate from the user-facing `rl_*` tools, which operate as an orchestration surface for remote RL training workflows.
:::
...
classDiagram
    ...
    BaseEnv <|-- saraAgentBaseEnv
    saraAgentBaseEnv <|-- TerminalTestEnv
    saraAgentBaseEnv <|-- saraSweEnv
    saraAgentBaseEnv <|-- TerminalBench2EvalEnv
...
The sara-agent layer (`environments/sara_base_env.py`). Adds:
- **Terminal backend configuration** — sets `TERMINAL_ENV` for sandboxed execution (local, Docker, Modal, Daytona, SSH, Singularity)
- **Tool resolution** — `_resolve_tools_for_group()` calls sara-agent's `get_tool_definitions()` to get the right tool schemas based on enabled/disabled toolsets
- **Agent loop integration** — `collect_trajectory()` runs `saraAgentLoop` and scores the result
...
Your environment inherits from `saraAgentBaseEnv` and implements five methods:
...
`saraAgentLoop` (`environments/agent_loop.py`) is the reusable multi-turn agent engine. It runs the same tool-calling pattern as sara-agent's main loop:
...
| **Generic** | `call_tool(name, args)` — escape hatch for any sara-agent tool |
...
parser = get_parser("sara")  # or "mistral", "llama3_json", "qwen", "deepseek_v3", etc.
content, tool_calls = parser.parse(raw_model_output)
```

Available parsers: `sara`, `mistral`, `llama3_json`, `qwen`, `qwen3_coder`, `deepseek_v3`, `deepseek_v3_1`, `kimi_k2`, `longcat`, `glm45`, `glm47`.
...
Dataset: [NexvisoraResearch/terminal-bench-2](https://huggingface.co/datasets/NexvisoraResearch/terminal-bench-2) on HuggingFace.
...
TBLite is a thin subclass of TerminalBench2 — only the dataset and timeouts differ. Created by the OpenThoughts Agent team (Snorkel AI + Bespoke Labs). Dataset: [NexvisoraResearch/openthoughts-tblite](https://huggingface.co/datasets/NexvisoraResearch/openthoughts-tblite).
...
pip install "sara-agent[yc-bench]"
...
```bash
python environments/sara_swe_env/sara_swe_env.py serve \
...
- Set `tool_call_parser` in config to match your model's format (e.g., `"sara"`, `"qwen"`, `"mistral"`)
...
```python
from environments.sara_base_env import saraAgentBaseEnv, saraAgentEnvConfig
from atroposlib.envs.server_handling.server_manager import APIServerConfig

class MyEnvConfig(saraAgentEnvConfig):
    my_custom_field: str = "default_value"

class MyEnv(saraAgentBaseEnv):
    name = "my-env"
...
### saraAgentEnvConfig Fields

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `enabled_toolsets` | `List[str]` | `None` (all) | Which sara toolsets to enable |
...
| `tool_call_parser` | `str` | `"sara"` | Parser for Phase 2 raw output |
...
env:
  ...
  dataset_name: "NexvisoraResearch/terminal-bench-2"
  tokenizer_name: "NexvisoraResearch/sara-3-Llama-3.1-8B"
  use_wandb: true
...
- Python >= 3.11
- `atroposlib`: `pip install git+https://github.com/NexvisoraResearch/atropos.git`
- An LLM API key (OpenRouter, OpenAI, or self-hosted VLLM/SGLang)

### For Modal-sandboxed benchmarks (TB2, TBLite)

- [Modal](https://modal.com) account and CLI: `pip install "sara-agent[modal]"`
- `MODAL_TOKEN_ID` and `MODAL_TOKEN_SECRET` environment variables

### For YC-Bench

- `pip install "sara-agent[yc-bench]"` (installs the yc-bench CLI + SQLAlchemy)
...
```
environments/
├── sara_base_env.py          # Abstract base class (saraAgentBaseEnv)
├── agent_loop.py               # Multi-turn agent engine (saraAgentLoop)
├── tool_context.py             # Per-rollout tool access for reward functions
├── patches.py                  # Async-safety patches for Modal backend
│
├── tool_call_parsers/          # Phase 2 client-side parsers
│   ├── sara_parser.py        # sara/ChatML <tool_call> format
...
├── sara_swe_env/             # SWE-bench training environment
│
