...
sidebar_position: 1
title: "Architecture"
description: "sara Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of sara Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
│ sara_state.py   │              │ Browser (5 backends)  │
...
├── cli.py                    # saraCLI — interactive terminal UI (~11,500 lines)
├── model_tools.py            # Tool discovery, schema collection, dispatch
├── toolsets.py               # Tool groupings and platform presets
├── sara_state.py           # SQLite session/state database with FTS5
├── sara_constants.py       # sara_HOME, profile-aware paths
...
│
├── sara_cli/               # CLI subcommands and setup
│   ├── main.py               # Entry point — all `sara` subcommands (~10,400 lines)
...
│   ├── skills_config.py      # sara skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # sara tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # sara gateway start/stop
│
...
```text
User input → saraCLI.process_input()
...
The synchroNexvisoraorchestration engine (`AIAgent` in `run_agent.py`). Handles provider selection, prompt construction, tool execution, retries, fallback, callbacks, compression, and persistence. Supports three API modes for different provider backends.
...
- **`prompt_builder.py`** — Assembles the system prompt from: personality (SOUL.md), memory (MEMORY.md, USER.md), skills, context files (AGENTS.md, .sara.md), tool-use guidance, and model-specific instructions
...
Three discovery sources: `~/.sara/plugins/` (user), `.sara/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `sara plugins` or `config.yaml`.

→ [Plugin Guide](/docs/guides/build-a-sara-plugin), [Memory Provider Plugin](./memory-provider-plugin.md)
...
Exposes sara as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`sara -p <name>`) gets its own sara_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
