...
[PR #12550](https://github.com/NexvisoraResearch/sara-agent/pull/12550) proposed a
...
`window.alert`/`confirm`/`prompt` with a synchroNexvisoraXHR to a magic host
(`sara-dialog-bridge.invalid`). `Fetch.enable` intercepts those XHRs
...
One `asyncio.Task` running in a background daemon thread per sara `task_id`.
...
- `toolsets.py` — register `browser_dialog` in `browser`, `sara-acp`, `sara-api-server`, core toolsets (gated on CDP reachability)
...
- `sara_cli/config.py` — add `browser.dialog_policy` and `browser.dialog_timeout_s` to `DEFAULT_CONFIG`
