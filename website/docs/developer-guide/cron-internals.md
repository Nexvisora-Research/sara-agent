...
description: "How sara stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `sara_cli/cron.py` | CLI `sara cron` subcommands |
...
Jobs are stored in `~/.sara/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `sara cron` commands are run or during active CLI sessions.
...
```python
# ~/.sara/scripts/check_competitors.py
...
2. **Environment variable** — `sara_CRON_SCRIPT_TIMEOUT`
...
| Local file | `local` | Save to `~/.sara/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `sara cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `sara cron` CLI provides direct job management:

```bash
sara cron list                    # Show all jobs
sara cron create                  # Interactive job creation (alias: add)
sara cron edit <job_id>           # Edit job configuration
sara cron pause <job_id>          # Pause a running job
sara cron resume <job_id>         # Resume a paused job
sara cron run <job_id>            # Trigger immediate execution
sara cron remove <job_id>         # Delete a job
```
