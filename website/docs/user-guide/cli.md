...
sidebar_position: 1
title: "CLI Interface"
description: "Master the sara Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

sara Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip
sara also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `sara --tui` — see the [TUI](tui.md) guide.
:::
...
sara -s sara-agent-dev,github-auth
sara chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
sara --continue             # Resume the most recent CLI session (-c)
sara --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
sara chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
sara -w                         # Interactive mode in worktree
sara -w -q "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/img/docs/cli-layout.svg" alt="Stylized preview of the sara CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The sara CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
When resuming a previous session (`sara -c` or `sara --resume <id>`), a "Previous Conversation" panel appears between the banner and the input prompt, showing a compact recap of the conversation history. See [Sessions — Conversation Recap on Resume](sessions.md#conversation-recap-on-resume) for details and configuration.
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend sara to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. sara supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for sara replies |
...
```bash
sara -s sara-agent-dev,github-auth
sara chat -s github-pr-workflow -s github-auth
```

sara loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.sara/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.sara/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain sara..."
  # Add your own!
```
...
The very first time you press Enter while sara is working, sara prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend sara to the background — just like any terminal process. The shell prints a confirmation:

```
sara Agent has been suspended. Run `fg` to bring sara Agent back.
```
...
Resume this session with:
  sara --resume 20260225_143052_a1b2c3
...
```bash
sara --continue                          # Resume the most recent CLI session
sara -c                                  # Short form
sara -c "my project"                     # Resume a named session (latest in lineage)
sara --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
sara --resume "refactoring auth"         # Resume by title
sara -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `sara sessions rename <id> <title>` from the command line. Use `sara sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in sara's SQLite state database under `~/.sara/state.db`. The database keeps:
...
sara immediately confirms the task and gives you back the prompt:
