...
title: "Git Worktrees"
description: "Run multiple sara agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

sara Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with sara so each session has a clean, isolated working directory.

## Why Use Worktrees with sara?

sara treats the **current working directory** as the project root:

- CLI: the directory where you run `sara` or `sara chat`
...
git worktree add ../repo-feature feature/sara-experiment
```

This creates:

- A new directory: `../repo-feature`
- A new branch: `feature/sara-experiment` checked out in that directory

Now you can `cd` into the new worktree and run sara there:

```bash
cd ../repo-feature

# Start sara in the worktree
sara
```
...
git worktree add ../repo-experiment-a feature/sara-a
git worktree add ../repo-experiment-b feature/sara-b
```
...
- Works on its own branch (`feature/sara-a` vs `feature/sara-b`).
...
- sara checkpoint data under `~/.sara/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per sara experiment**
...
- **Name branches after the experiment**
  - e.g. `feature/sara-checkpoints-docs`, `feature/sara-refactor-tests`.
- **Commit frequently**
...
- **Avoid running sara from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `sara -w` (Automatic Worktree Mode)

sara has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `sara/sara-<hash>`).
...
```bash
sara -w -q "Fix issue #123"
```

For parallel agents, open multiple terminals and run `sara -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each sara session its own clean checkout.
