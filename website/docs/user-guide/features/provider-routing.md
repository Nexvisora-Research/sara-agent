...
When using [OpenRouter](https://openrouter.ai) as your LLM provider, sara Agent supports **provider routing** — fine-grained control over which underlying AI providers handle your requests and how they're prioritized.
...
Add a `provider_routing` section to your `~/.sara/config.yaml`:
...
- **CLI mode** — configured in `~/.sara/config.yaml`, loaded at startup
