...
sara Agent has three layers of resilience that keep your sessions running when providers hit issues:
...
When your main LLM provider encounters errors — rate limits, server overload, auth failures, connection drops — sara can automatically switch to a backup provider:model pair mid-session without losing your conversation.
...
`sara fallback` reuses the provider picker from `sara model` — same provider list, same credential prompts, same validation. Press `a` to add a fallback, `↑`/`↓` to reorder, `d` to remove, `q` to save and exit. Changes persist under `model.fallback_providers` in `config.yaml`.

If you'd rather edit the YAML directly, add a `fallback_model` section to `~/.sara/config.yaml`:
...
`fallback_model` (singular) is the legacy single-fallback key — sara still honors it for back-compat. `fallback_providers` (plural, list) supports multiple fallbacks tried in order; `sara fallback` writes to this key. When both are set, sara merges them with `fallback_providers` taking priority.
:::
...
| Google Gemini (OAuth) | `google-gemini-cli` | `sara model` (Google OAuth; optional: `sara_GEMINI_PROJECT_ID`) |
...
| Qwen Portal (OAuth) | `qwen-oauth` | `sara model` (Qwen Portal OAuth; optional: `sara_QWEN_BASE_URL`) |
| MiniMax (OAuth) | `minimax-oauth` | `sara model` (MiniMax portal OAuth) |
...
sara uses separate lightweight models for side tasks. Each task has its own provider resolution chain that acts as a built-in fallback system.
...
When a task's provider is set to `"auto"` (the default), sara tries providers in order until one works:
...
If the resolved provider fails at call time, sara also has an internal retry: if the provider is not OpenRouter and no explicit `base_url` is set, it tries OpenRouter as a last-resort fallback.
...
For `auxiliary.session_search`, sara also supports:
...
`base_url` takes precedence over `provider`. sara uses the configured `api_key` for authentication, falling back to `OPENAI_API_KEY` if not set. It does **not** reuse `OPENROUTER_API_KEY` for custom endpoints.
...
If no provider is available for compression, sara drops middle conversation turns without generating a summary rather than failing the session.
