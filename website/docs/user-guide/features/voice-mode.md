...
sidebar_position: 10
title: "Voice Mode"
description: "Real-time voice conversations with sara Agent — CLI, Telegram, Discord (DMs, text channels, and voice channels)"
---

# Voice Mode

sara Agent supports full voice interaction across CLI and messaging platforms. Talk to the agent using your microphone, hear spoken replies, and have live voice conversations in Discord voice channels.

If you want a practical setup walkthrough with recommended configurations and real usage patterns, see [Use Voice Mode with sara](/docs/guides/use-voice-mode-with-sara).

## Prerequisites

Before using voice features, make sure you have:

1. **sara Agent installed** — `pip install sara-agent` (see [Installation](/docs/getting-started/installation))
2. **An LLM provider configured** — run `sara model` or set your preferred provider credentials in `~/.sara/.env`
3. **A working base setup** — run `sara` to verify the agent responds to text before enabling voice

:::tip
The `~/.sara/` directory and default `config.yaml` are created automatically the first time you run `sara`. You only need to create `~/.sara/.env` manually for API keys.
:::
...
pip install "sara-agent[messaging]"

# Premium TTS (ElevenLabs)
pip install "sara-agent[tts-premium]"

# Local TTS (NeuTTS, optional)
python -m pip install -U neutts[all]

# Everything at once
pip install "sara-agent[all]"
```
...
Voice mode is available in both the **classic CLI** (`sara chat`) and the **TUI** (`sara --tui`). Behavior is identical across both — same slash commands, same VAD silence detection, same streaming TTS, same hallucination filter. The TUI additionally forwards crash-forensic logs to `~/.sara/logs/` so push-to-talk failures on exotic audio backends can be reported with a full stack trace rather than disappearing silently.
...
```bash
sara                # Start the interactive CLI
```
...
1. Start the CLI with `sara` and enable voice mode with `/voice on`
...
:::tip
The record key is configurable via `voice.record_key` in `~/.sara/config.yaml` (default: `ctrl+b`).
:::
...
```bash
sara gateway        # Start the gateway (connects to configured platforms)
sara gateway setup  # Interactive setup wizard for first-time configuration
```
...
**Server channels:** The bot only responds when you @mention it (e.g. `@sarabyt4 hello`). Make sure you select the **bot user** from the mention popup, not the role with the same name.

:::tip
To disable the mention requirement in server channels, add to `~/.sara/.env`:
```bash
...
```bash
sara gateway        # Start with existing configuration
```
...
3. Or set `DISCORD_REQUIRE_MENTION=false` in `~/.sara/.env`
...
- Review gateway logs: `tail -f ~/.sara/logs/gateway.log`
