...
Credential pools let you register multiple API keys or OAuth tokens for the same provider. When one key hits a rate limit or billing quota, sara automatically rotates to the next healthy key — keeping your session alive without switching providers.
...
If you already have an API key set in `.env`, sara auto-discovers it as a 1-key pool. To benefit from pooling, add more keys:

```bash
# Add a second OpenRouter key
sara auth add openrouter --api-key sk-or-v1-your-second-key

# Add a second Anthropic key
sara auth add anthropic --type api-key --api-key sk-ant-api03-your-second-key

# Add an Anthropic OAuth credential (requires Claude Max plan + extra usage credits)
sara auth add anthropic --type oauth
...
Run `sara auth` with no subcommand for an interactive wizard:
...
| Command | Description |
|---------|-------------|
| `sara auth` | Interactive pool management wizard |
| `sara auth list` | Show all pools and credentials |
| `sara auth list <provider>` | Show a specific provider's pool |
| `sara auth add <provider>` | Add a credential (prompts for type and key) |
| `sara auth add <provider> --type api-key --api-key <key>` | Add an API key non-interactively |
| `sara auth add <provider> --type oauth` | Add an OAuth credential via browser login |
| `sara auth remove <provider> <index>` | Remove credential by 1-based index |
| `sara auth reset <provider>` | Clear all cooldowns/exhaustion status |

## Rotation Strategies

Configure via `sara auth` → "Set rotation strategy" or in `config.yaml`:
...
When you set up a custom endpoint via `sara model`, it auto-generates a name like "Together.ai" or "Local (localhost:8080)". This name becomes the pool key.

```bash
# After setting up a custom endpoint via sara model:
sara auth list
# Shows:
...
sara auth add Together.ai --api-key sk-together-second-key
```
...
sara automatically discovers credentials from multiple sources and seeds the pool on startup:
...
| Manual entries | Added via `sara auth add` | Persisted in auth.json |

Auto-seeded entries are updated on each pool load — if you remove an env var, its pool entry is automatically pruned. Manual entries (added via `sara auth add`) are never auto-pruned.
...
2. **`sara_cli/auth_commands.py`** — CLI commands and interactive wizard
3. **`sara_cli/runtime_provider.py`** — Pool-aware credential resolution
4. **`run_agent.py`** — Error recovery: 429/402/401 → pool rotation → fallback

## Storage

Pool state is stored in `~/.sara/auth.json` under the `credential_pool` key:
