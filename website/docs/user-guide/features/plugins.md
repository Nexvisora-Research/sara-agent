...
title: "Plugins"
description: "Extend sara with custom tools, hooks, and integrations via the plugin system"
---

# Plugins

sara has a plugin system for adding custom tools, hooks, and integrations without modifying core code.

**→ [Build a sara Plugin](/docs/guides/build-a-sara-plugin)** — step-by-step guide with a complete working example.

## Quick overview

Drop a directory into `~/.sara/plugins/` with a `plugin.yaml` and Python code:
...
Start sara — your tools appear alongside built-in tools. The model can call them immediately.

### Minimal working example

Here is a complete plugin that adds a `hello_world` tool and logs every tool call via a hook.

**`~/.sara/plugins/hello-world/plugin.yaml`**
...
**`~/.sara/plugins/hello-world/__init__.py`**

```python
"""Minimal sara plugin — registers a tool and a hook."""
...
Drop both files into `~/.sara/plugins/hello-world/`, restart sara, and the model can immediately call `hello_world`. The hook prints a log line after every tool invocation.

Project-local plugins under `./.sara/plugins/` are disabled by default. Enable them only for trusted repositories by setting `sara_ENABLE_PROJECT_PLUGINS=true` before starting sara.
...
| Add CLI commands | `ctx.register_cli_command(name, help, setup_fn, handler_fn)` — adds `sara <plugin> <subcommand>` |
| Inject messages | `ctx.inject_message(content, role="user")` — see [Injecting Messages](#injecting-messages) |
| Ship data files | `Path(__file__).parent / "data" / "file.yaml"` |
| Bundle skills | `ctx.register_skill(name, path)` — namespaced as `plugin:skill`, loaded via `skill_view("plugin:skill")` |
| Gate on env vars | `requires_env: [API_KEY]` in plugin.yaml — prompted during `sara plugins install` |
| Distribute via pip | `[project.entry-points."sara_agent.plugins"]` |

## Plugin discovery

| Source | Path | Use case |
|--------|------|----------|
| Bundled | `<repo>/plugins/` | Ships with sara — see [Built-in Plugins](/docs/user-guide/features/built-in-plugins) |
| User | `~/.sara/plugins/` | Personal plugins |
| Project | `.sara/plugins/` | Project-specific plugins (requires `sara_ENABLE_PROJECT_PLUGINS=true`) |
| pip | `sara_agent.plugins` entry_points | Distributed packages |
| Nix | `services.sara-agent.extraPlugins` / `extraPythonPackages` | NixOS declarative installs — see [Nix Setup](/docs/getting-started/nix-setup#plugins) |

Later sources override earlier ones on name collision, so a user plugin with the same name as a bundled plugin replaces it.

## Plugins are opt-in

**Every plugin — user-installed, bundled, or pip — is disabled by default.** Discovery finds them (so they show up in `sara plugins` and `/plugins`), but nothing loads until you add the plugin's name to `plugins.enabled` in `~/.sara/config.yaml`. This stops anything with hooks or tools from running without your explicit consent.
...
```bash
sara plugins                    # interactive toggle (space to check/uncheck)
sara plugins enable <name>      # add to allow-list
sara plugins disable <name>     # remove from allow-list + add to disabled
```

After `sara plugins install owner/repo`, you're asked `Enable 'name' now? [y/N]` — defaults to no. Skip the prompt for scripted installs with `--enable` or `--no-enable`.

### Migration for existing users

When you upgrade to a version of sara that has opt-in plugins (config schema v21+), any user plugins already installed under `~/.sara/plugins/` that weren't already in `plugins.disabled` are **automatically grandfathered** into `plugins.enabled`. Your existing setup keeps working. Bundled plugins are NOT grandfathered — even existing users have to opt in explicitly.
...
sara has three kinds of plugins:

| Type | What it does | Selection | Location |
|------|-------------|-----------|----------|
| **General plugins** | Add tools, hooks, slash commands, CLI commands | Multi-select (enable/disable) | `~/.sara/plugins/` |
...
On NixOS, plugins can be installed declaratively via the module options — no `sara plugins install` needed. See the **[Nix Setup guide](/docs/getting-started/nix-setup#plugins)** for full details.
...
```bash
sara plugins                               # unified interactive UI
sara plugins list                          # table: enabled / disabled / not enabled
sara plugins install user/repo             # install from Git, then prompt Enable? [y/N]
sara plugins install user/repo --enable    # install AND enable (no prompt)
sara plugins install user/repo --no-enable # install but leave disabled (no prompt)
sara plugins update my-plugin              # pull latest
sara plugins remove my-plugin              # uninstall
sara plugins enable my-plugin              # add to allow-list
sara plugins disable my-plugin             # remove from allow-list + add to disabled
```

### Interactive UI

Running `sara plugins` with no arguments opens a composite interactive screen:
...
The default for a newly-installed or bundled plugin is `not enabled`. `sara plugins list` shows all three distinct states so you can tell what's been explicitly turned off vs. what's just waiting to be enabled.
...
See the **[full guide](/docs/guides/build-a-sara-plugin)** for handler contracts, schema format, hook behavior, error handling, and common mistakes.
