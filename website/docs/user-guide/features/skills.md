...
All skills live in **`~/.sara/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point sara at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling sara to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.sara/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
sara chat --toolsets skills -q "What skills do you have?"
sara chat --toolsets skills -q "Show me the axolotl skill"
```
...
When a missing value is encountered, sara asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `sara setup` or `~/.sara/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `sara config migrate` prompts for unconfigured settings, and `sara config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.sara/skills/                  # Single source of truth
...
If you maintain skills outside of sara — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell sara to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.sara/config.yaml`:
...
- **Read-only**: External dirs are only scanned for skill discovery. When the agent creates or edits a skill, it always writes to `~/.sara/skills/`.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, sara ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.sara/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
```bash
sara skills browse                              # Browse all hub skills (official first)
sara skills browse --source official            # Browse only official optional skills
sara skills search kubernetes                   # Search all sources
sara skills search react --source skills-sh     # Search the skills.sh directory
sara skills search https://mintlify.com/docs --source well-known
sara skills inspect openai/skills/k8s           # Preview before installing
sara skills install openai/skills/k8s           # Install with security scan
sara skills install official/security/1password
sara skills install skills-sh/vercel-labs/json-render/json-render-react --force
sara skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
sara skills list --source hub                   # List hub-installed skills
sara skills check                               # Check installed hub skills for upstream updates
sara skills update                              # Reinstall hub skills with upstream changes when needed
sara skills audit                               # Re-scan all hub skills for security
sara skills uninstall k8s                       # Remove a hub skill
sara skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
sara skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
sara skills publish skills/my-skill --to github --repo owner/repo
sara skills snapshot export setup.json          # Export skill config
sara skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with sara. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `sara skills search <query> --source skills-sh`. sara resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
sara currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the sara repository itself and install with builtin trust.
...
This is Vercel's public skills directory. sara can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
sara skills search https://mintlify.com/docs --source well-known
sara skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
sara skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

sara can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
sara skills tap add myorg/skills-repo
```
...
- sara source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

sara supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

sara source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

sara can search and convert agent entries from LobeHub's public catalog into installable sara skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- sara source id: `lobehub`

#### 8. Direct URL (`url`)

Install a single-file `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). sara fetches the URL, parses the YAML frontmatter, security-scans it, and installs.
...
```bash
sara skills install https://sharethis.chat/SKILL.md
sara skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
sara skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `sara skills update` re-fetches from the same URL automatically when you want to refresh.
...
`sara skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with sara | Always trusted |
...
```bash
sara skills check          # Report which installed hub skills changed upstream
sara skills update         # Reinstall only the skills with updates available
sara skills update react   # Update one specific installed hub skill
```
...
## Bundled skill updates (`sara skills reset`)

sara ships with a set of bundled skills in `skills/` inside the repo. On install and on every `sara update`, a sync pass copies those into `~/.sara/skills/` and records a manifest at `~/.sara/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, sara recomputes the hash of your local copy and compares it to the origin hash:
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `sara_HOME`, so `sara -p coder skills reset <name>` only affects that profile.
:::
