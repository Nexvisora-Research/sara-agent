...
Schedule tasks to run automatically with natural language or cron expressions. sara exposes cron management through a single `cronjob` tool with action-style operations instead of separate schedule/list/remove tools.
...
:::warning
Cron-run sessions cannot recursively create more cron jobs. sara disables cron management tools inside cron executions to prevent runaway scheduling loops.
:::
...
```bash
sara cron create "every 2h" "Check server status"
sara cron create "every 1h" "Summarize new feed items" --skill blogwatcher
sara cron create "every 1h" "Use both skills and combine the result" \
  --skill blogwatcher \
  --skill maps \
  --name "Skill combo"
```
...
sara will use the unified `cronjob` tool internally.
...
sara cron create "every 1d at 09:00" \
...
```bash
sara cron edit <job_id> --schedule "every 4h"
sara cron edit <job_id> --prompt "Use the revised task"
sara cron edit <job_id> --skill blogwatcher --skill maps
sara cron edit <job_id> --add-skill maps
sara cron edit <job_id> --remove-skill blogwatcher
sara cron edit <job_id> --clear-skills
```
...
```bash
sara cron list
sara cron pause <job_id>
sara cron resume <job_id>
sara cron run <job_id>
sara cron remove <job_id>
sara cron status
sara cron tick
```
...
sudo sara gateway install --system   # Linux: boot-time system service for servers
sara gateway             # Or run in foreground
...
1. loads jobs from `~/.sara/cron/jobs.json`
...
A file lock at `~/.sara/cron/.tick.lock` prevents overlapping scheduler ticks from double-running the same job batch.
...
| `"local"` | Save to local files only (`~/.sara/cron/output/`) | Default on CLI |
...
If the agent's final response starts with `[SILENT]`, delivery is suppressed entirely. The output is still saved locally for audit (in `~/.sara/cron/output/`), but no message is sent to the delivery target.
...
Or set the `sara_CRON_SCRIPT_TIMEOUT` environment variable. The resolution order is: env var → config.yaml → 120s default.
...
The agent's final response is automatically delivered — you do **not** need to include `send_message` in the cron prompt for that same destination. If a cron run calls `send_message` to the exact target the scheduler will already deliver to, sara skips that duplicate send and tells the model to put the user-facing content in the final response instead. Use `send_message` only for additional or different targets.
...
Cron runs each job in a fresh agent session with no chat platform attached. By default the cron agent gets **the toolset you configured for the `cron` platform in `sara tools`** — not the CLI default, not everything under the sun.
...
When `enabled_toolsets` is set on a job it wins; otherwise the `sara tools` cron-platform config wins; otherwise sara falls back to the built-in defaults. This matters for cost control: carrying `moa`, `browser`, `delegation` into every tiny "fetch news" job bloats the tool-schema prompt on every LLM call.

### Skipping the agent entirely: `wakeAgent`

If your cron job attaches a pre-check script (via `script=`), the script can decide at runtime whether sara should even invoke the agent. Emit a final stdout line of the form:
...
Jobs are stored in `~/.sara/cron/jobs.json`. Output from job runs is saved to `~/.sara/cron/output/{job_id}/{timestamp}.md`.

Jobs may store `model` and `provider` as `null`. When those fields are omitted, sara resolves them at execution time from the global configuration. They only appear in the job record when a per-job override is set.
