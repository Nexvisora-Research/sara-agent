...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of sara's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple sara instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
sara memory setup    # select "honcho" from the provider list
```
...
Honcho is configured in `~/.honcho/config.json` (global) or `$sara_HOME/honcho.json` (profile-local). The setup wizard handles this for you.
...
- `per-session` — each `sara` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — sara syncs them back at session init.
...
```bash
sara honcho status          # Connection status, config, and key settings
sara honcho setup           # Interactive setup wizard
sara honcho strategy        # Show or set session strategy
sara honcho peer            # Update peer names for multi-agent setups
sara honcho mode            # Show or set recall mode
sara honcho tokens          # Show or set context token budget
sara honcho identity        # Show Honcho peer identity
sara honcho sync            # Sync host blocks for all profiles
sara honcho enable          # Enable Honcho
sara honcho disable         # Disable Honcho
```

## Migrating from `sara honcho`

If you previously used the standalone `sara honcho setup`:
...
No re-login or re-setup needed. Run `sara memory setup` and select "honcho" — the wizard detects your existing config.
