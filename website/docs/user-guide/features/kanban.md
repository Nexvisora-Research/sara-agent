...
description: "Durable SQLite-backed task board for coordinating multiple sara profiles"
---
...
sara Kanban is a durable task board, shared across all your sara profiles, that lets multiple named agents collaborate on work without fragile in-process subagent swarms. Every task is a row in `~/.sara/kanban.db`; every handoff is a row anyone can read and write; every worker is a full OS process with its own identity.
...
For the full design rationale, comparative analysis against Cline Kanban / Paperclip / NanoClaw / Google Gemini Enterprise, and the eight canonical collaboration patterns, see `docs/sara-kanban-v1-spec.pdf` in the repository.
...
- **Workspace** — the directory a worker operates in. Three kinds:
  - `scratch` (default) — fresh tmp dir under `~/.sara/kanban/workspaces/<id>/`.
...
# 3. Create a task
sara kanban create "research AI funding landscape" --assignee researcher
...
Override the config flag at runtime via `sara_KANBAN_DISPATCH_IN_GATEWAY=0`
for debugging. Standard gateway supervision applies: run `sara gateway
...
until one comes up — `sara kanban create` warns about this at creation
time.

Running `sara kanban daemon` as a separate process is **deprecated**;
...
sara kanban create "nightly ops review" \
    --assignee ops \
...
```bash
sara kanban complete t_abc t_def t_hij --result "batch wrap"
sara kanban archive  t_abc t_def t_hij
sara kanban unblock  t_abc t_def
sara kanban block    t_abc "need input" --ids t_def t_hij
```

## How workers interact with the board

When the dispatcher spawns a worker, it sets `sara_KANBAN_TASK` in the child's env. That env var is the gate for a dedicated **kanban toolset** — 7 tools that the normal agent schema never sees:
...
**Why tools and not just shelling to `sara kanban`?** Three reasons:

1. **Backend portability.** Workers whose terminal tool points at a remote backend (Docker / Modal / Singularity / SSH) would run `sara kanban complete` inside the container where `sara` isn't installed and the DB isn't mounted. The kanban tools run in the agent's own Python process and always reach `~/.sara/kanban.db` regardless of terminal backend.
...
**Zero schema footprint on normal sessions.** A regular `sara chat` session has zero `kanban_*` tools in its schema. The `check_fn` on each tool only returns True when `sara_KANBAN_TASK` is set, which only happens when the dispatcher spawned this process. No tool bloat for users who never touch kanban.
...
2. `cd $sara_KANBAN_WORKSPACE` and do the work there.
...
```bash
sara skills install devops/kanban-worker
```
...
sara kanban create "translate README to Japanese" \
    --assignee linguist \
    --skill translation

# Multiple skills
sara kanban create "audit auth flow" \
    --assignee reviewer \
...
```bash
sara skills install devops/kanban-orchestrator
```
...
- A **Kanban** tab showing one column per status: `triage`, `todo`, `ready`, `running`, `blocked`, `done` (plus `archived` when the toggle is on).
  - `triage` is the parking column for rough ideas a specifier is expected to flesh out. Tasks created with `sara kanban create --triage` (or via the Triage column's inline create) land here and the dispatcher leaves them alone until a human or specifier promotes them to `todo` / `ready`.
...
└──────────┬─────────────┘                                    │
           ...
           ▼                                                  │
┌────────────────────────┐                                    │
│  ~/.sara/kanban.db   │ ───── append task_events ──────────┘
│  (WAL, shared)         │
└────────────────────────┘
```
...
If you run `sara dashboard --host 0.0.0.0`, every plugin route — kanban included — becomes reachable from the network. **Don't do that on a shared host.** The board contains task bodies, comments, and workspace paths; an attacker reaching these routes gets read access to your entire collaboration surface and can also create / reassign / archive tasks.

Tasks in `~/.sara/kanban.db` are profile-agnostic on purpose (that's the coordination primitive). If you open the dashboard with `sara -p <profile> dashboard`, the board still shows tasks created by any other profile on the host. Same user owns all profiles, but this is worth knowing if multiple personas coexist.
...
The plugin uses the standard sara dashboard plugin contract — see [Extending the Dashboard](./extending-the-dashboard) for the full manifest reference, shell slots, page-scoped slots, and the Plugin SDK. Extra columns, custom card chrome, tenant-filtered layouts, or full `tab.override` replacements are all expressible without forking this plugin.
...
```
sara kanban init                                     # create kanban.db + print daemon hint
sara kanban create "<title>" [--body ...] [--assignee <profile>]
                                ...
                                [--skill <name>]...
                                [--json]
sara kanban list [--mine] [--assignee P] [--status S] [--tenant T] [--archived] [--json]
sara kanban show <id> [--json]
sara kanban assign <id> <profile>                    # or 'none' to unassign
sara kanban link <parent_id> <child_id>
sara kanban unlink <parent_id> <child_id>
sara kanban claim <id> [--ttl SECONDS]
sara kanban comment <id> "<text>" [--author NAME]

# Bulk verbs — accept multiple ids:
sara kanban complete <id>... [--result "..."]
sara kanban block <id> "<reason>" [--ids <id>...]
sara kanban unblock <id>...
sara kanban archive <id>...

sara kanban tail <id>                                # follow a single task's event stream
sara kanban watch [--assignee P] [--tenant T]        # live stream ALL events to the terminal
        [--kinds completed,blocked,…] [--interval SECS]
sara kanban heartbeat <id> [--note "..."]            # worker liveness signal for long ops
sara kanban runs <id> [--json]                       # attempt history (one row per run)
sara kanban assignees [--json]                       # profiles on disk + per-assignee task counts
sara kanban dispatch [--dry-run] [--max N]           # one-shot pass
        [--failure-limit N] [--json]
sara kanban daemon --force                           # DEPRECATED — standalone dispatcher (use `sara gateway start` instead)
        [--failure-limit N] [--pidfile PATH] [-v]
sara kanban stats [--json]                           # per-status + per-assignee counts
sara kanban log <id> [--tail BYTES]                  # worker log from ~/.sara/kanban/logs/
sara kanban notify-subscribe <id>                    # gateway bridge hook (used by /kanban in the gateway)
        --platform <name> --chat-id <id> [--thread-id <id>] [--user-id <id>]
sara kanban notify-list [<id>] [--json]
sara kanban notify-unsubscribe <id>
        --platform <name> --chat-id <id> [--thread-id <id>]
sara kanban context <id>                             # what a worker sees
sara kanban gc [--event-retention-days N]            # workspaces + old events + old logs
...
For worked examples of each, see `docs/sara-kanban-v1-spec.pdf`.
...
```bash
sara kanban create "monthly report" \
    --assignee researcher \
    --tenant business-a \
    --workspace dir:~/tenants/business-a/data/
```

Workers receive `$sara_TENANT` and namespace their memory writes by prefix. The board, the dispatcher, and the profile definitions are all shared; only the data is scoped.
...
```bash
sara kanban notify-subscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
sara kanban notify-list
sara kanban notify-unsubscribe t_abcd \
    --platform telegram --chat-id 12345678 --thread-id 7
```
...
**Bulk close caveat.** `sara kanban complete a b c --summary X` is refused — structured handoff is per-run, so copy-pasting the same summary to N tasks is almost always wrong. Bulk close *without* `--summary` / `--metadata` still works for the common "I finished a pile of admin tasks" case.
...
Every transition appends a row to `task_events`. Each row carries an optional `run_id` so UIs can group events by attempt. Kinds group into three clusters so filtering is easy (`sara kanban watch --kinds completed,gave_up,timed_out`):
...
| `heartbeat` | `{note?}` | Worker called `sara kanban heartbeat $TASK` to signal liveness during long operations. |
...
`sara kanban tail <id>` shows these for a single task. `sara kanban watch` streams them board-wide.

## Out of scope

Kanban is deliberately single-host. `~/.sara/kanban.db` is a local SQLite file and the dispatcher spawns workers on the same machine. Running a shared board across two hosts is not supported — there's no coordination primitive for "worker X on host A, worker Y on host B," and the crash-detection path assumes PIDs are host-local. If you need multi-host, run an independent board per host and use `delegate_task` / a message queue to bridge them.

## Design spec

The complete design — architecture, concurrency correctness, comparison with other systems, implementation plan, risks, open questions — lives in `docs/sara-kanban-v1-spec.pdf`. Read that before filing any behavior-change PR.
