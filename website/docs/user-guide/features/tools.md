...
description: "Overview of sara Agent's tools — what's available, how toolsets work, and terminal backends"
---
...
sara ships with a broad built-in tool registry covering web search, browser automation, terminal execution, file editing, memory, delegation, RL training, messaging delivery, Home Assistant, and more.
...
sara chat --toolsets "web,terminal"
...
See [Toolsets Reference](/docs/reference/toolsets-reference) for the full set, including platform presets such as `sara-cli`, `sara-telegram`, and dynamic MCP toolsets like `mcp-<server>`.
...
# Configure
sara config set terminal.backend singularity
sara config set terminal.singularity_image ~/python.sif
```
...
modal setup
sara config set terminal.backend modal
```
...
sara config set terminal.backend vercel_sandbox
sara config set terminal.vercel_runtime node24
```

Authenticate with all three of `VERCEL_TOKEN`, `VERCEL_PROJECT_ID`, and `VERCEL_TEAM_ID`. This access-token setup is the supported path for deployments and normal long-running sara processes on Render, Railway, Docker, and similar hosts. Supported runtimes are `node24`, `node22`, and `python3.13`; sara defaults to `/vercel/sandbox` as the remote workspace root.
...
With `container_persistent: true`, sara uses Vercel snapshots to preserve filesystem state across sandbox recreation for the same task. This can include sara-synced credentials, skills, and cache files inside the sandbox. Snapshots do not preserve live processes, PID space, or the same live sandbox identity.

Background terminal commands use sara' generic non-local process flow: spawn, poll, wait, log, and kill work through the normal process tool while the sandbox is alive, but sara does not provide native Vercel detached-process recovery after cleanup or restart.
...
If a command needs sudo, you'll be prompted for your password (cached for the session). Or set `SUDO_PASSWORD` in `~/.sara/.env`.

:::warning
On messaging platforms, if sudo fails, the output includes a tip to add `SUDO_PASSWORD` to `~/.sara/.env`.
:::
