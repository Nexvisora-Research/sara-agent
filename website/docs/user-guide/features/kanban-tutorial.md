...
A walkthrough of the four use-cases the sara Kanban system was designed for, with the dashboard open in a browser. If you haven't read the [Kanban overview](./kanban) yet, start there — this assumes you know what a task, run, assignee, and dispatcher are.

## Setup

```bash
sara kanban init           # optional; first `sara kanban <anything>` auto-inits
sara dashboard             # opens http://127.0.0.1:9119 in your browser
# click Kanban in the left nav
```

The dashboard is the most comfortable place to learn the system. Everything you see here is also available via `sara kanban <verb>` on the CLI — the two surfaces share the same SQLite database at `~/.sara/kanban.db`.
...
```bash
SCHEMA=$(sara kanban create "Design auth schema" \
    --assignee backend-dev --tenant auth-project --priority 2 \
    --body "Design the user/session/token schema for the auth module." \
    --json | jq -r .id)

API=$(sara kanban create "Implement auth API endpoints" \
    --assignee backend-dev --tenant auth-project --priority 2 \
    --parent $SCHEMA \
    --body "POST /register, POST /login, POST /refresh, POST /logout." \
    --json | jq -r .id)

sara kanban create "Write auth integration tests" \
...
```bash
sara kanban claim $SCHEMA

# (you design the schema, commit, etc.)

sara kanban complete $SCHEMA \
...
```bash
sara kanban show $SCHEMA
sara kanban runs $SCHEMA
...
for lang in Spanish French German; do
    sara kanban create "Translate homepage to $lang" \
        --assignee translator --tenant content-ops
done
for i in 1 2 3 4 5; do
    sara kanban create "Transcribe Q3 customer call #$i" \
        --assignee transcriber --tenant content-ops
done
for sku in 1001 1002 1003 1004; do
    sara kanban create "Generate product description: SKU-$sku" \
...
sara kanban claim $IMPL
sara kanban block $IMPL "Review: password strength check missing, reset link isn't single-use (can be replayed within 30min)"

# Engineer iterates, resolves, completes
sara kanban unblock $IMPL
sara kanban claim $IMPL
sara kanban complete $IMPL \
...
```bash
sara kanban create "Deploy to staging (missing creds)" \
...
The bulk-close guard exists because this data is per-run. `sara kanban complete a b c --summary X` is refused — copy-pasting the same summary to three tasks is almost always wrong. Bulk close without the handoff flags still works for the common "I finished a pile of admin tasks" case.
...
- `sara kanban --help` — every subcommand, every flag.
- `sara kanban watch --kinds completed,gave_up,timed_out` — live stream terminal events across the whole board.
- `sara kanban notify-subscribe <task> --platform telegram --chat-id <id>` — get a gateway ping when a specific task finishes.
