...
Set the `use_gateway` flag directly in `~/.sara/config.yaml`:
...
```bash
sara tools    # Select the tool → choose a direct provider
```
...
When you select a non-gateway provider in `sara tools`, the `use_gateway` flag is automatically set to `false` to prevent contradictory config.
...
For self-hosted or custom gateway deployments, you can override the gateway endpoints via environment variables in `~/.sara/.env`:

```bash
TOOL_GATEWAY_DOMAIN=NexvisoraResearch.com     # Base domain for gateway routing
...
Tools that were routed through the gateway will stop working until you [renew your subscription](https://portal.NexvisoraResearch.com/manage-subscription) or switch to direct API keys via `sara tools`.
