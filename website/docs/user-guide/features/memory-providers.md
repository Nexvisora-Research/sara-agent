...
sara Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
sara memory setup      # interactive picker + configuration
sara memory status     # check what's active
sara memory off        # disable external provider
```

You can also select the active memory provider via `sara plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.sara/config.yaml`:
...
When a memory provider is active, sara automatically:
...
**Setup Wizard:**
```bash
sara honcho setup        # (legacy command) 
# or
sara memory setup        # select "honcho"
```

**Config:** `$sara_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$sara_HOME/honcho.json` > `~/.sara/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/sara-ai/sara-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/sara).
...
:::tip Migrating from `sara honcho`
If you previously used `sara honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per sara profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All sara profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per sara profile. Host key `sara` → default; `sara.<profile>` for others. |
...
```bash
sara profile create coder --clone
```

`--clone` creates a `sara.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.
...
Scans every sara profile, creates host blocks for any profile without one, inherits settings from the default `sara` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
See the [config reference](https://github.com/sara-ai/sara-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/sara).
...
# Then configure sara
sara memory setup    # select "openviking"
# Or manually:
sara config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.sara/.env
```
...
**Setup:**
```bash
sara memory setup    # select "mem0"
# Or manually:
sara config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.sara/.env
```

**Config:** `$sara_HOME/mem0.json`

| Key | Default | Description |
|-----|---------|-------------|
| `user_id` | `sara-user` | User identifier |
| `agent_id` | `sara` | Agent identifier |
...
**Setup:**
```bash
sara memory setup    # select "hindsight"
# Or manually:
sara config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.sara/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p sara ui start`

**Config:** `$sara_HOME/hindsight/config.json`

| Key | Default | Description |
|-----|---------|-------------|
| `mode` | `cloud` | `cloud` or `local` |
| `bank_id` | `sara` | Memory bank identifier |
...
See [plugin README](https://github.com/NexvisoraResearch/sara-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
sara memory setup    # select "holographic"
# Or manually:
sara config set memory.provider holographic
```

**Config:** `config.yaml` under `plugins.sara-memory-store`

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$sara_HOME/memory_store.db` | SQLite database path |
...
**Setup:**
```bash
sara memory setup    # select "retaindb"
# Or manually:
sara config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.sara/.env
```
...
# Then configure sara
sara memory setup    # select "byterover"
# Or manually:
sara config set memory.provider byterover
```

**Key features:**
- Automatic pre-compression extraction (saves insights before context compression discards them)
- Knowledge tree stored at `$sara_HOME/byterover/` (profile-scoped)
...
**Setup:**
```bash
sara memory setup    # select "supermemory"
# Or manually:
sara config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.sara/.env
```

**Config:** `$sara_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `sara` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `sara-{identity}` → `sara-coder`) to isolate memories per sara profile
...
- **Local storage providers** (Holographic, ByteRover) use `$sara_HOME/` paths which differ per profile
- **Config file providers** (Honcho, Mem0, Hindsight, Supermemory) store config in `$sara_HOME/` so each profile has its own credentials
