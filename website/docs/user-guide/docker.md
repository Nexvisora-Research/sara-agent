...
sidebar_position: 7
title: "Docker"
description: "Running sara Agent in Docker and using Docker as a terminal backend"
---

# sara Agent — Docker

There are two distinct ways Docker intersects with sara Agent:

1. **Running sara IN Docker** — the agent itself runs inside a container (this page's primary focus)
...
If this is your first time running sara Agent, create a data directory on the host and start the container interactively to run the setup wizard:

```sh
mkdir -p ~/.sara
docker run -it --rm \
  -v ~/.sara:/opt/data \
  NexvisoraResearch/sara-agent setup
```

This drops you into the setup wizard, which will prompt you for your API keys and write them to `~/.sara/.env`. You only need to do this once. It is highly recommended to set up a chat system for the gateway to work with at this point.
...
docker run -d \
  ...
  -v ~/.sara:/opt/data \
  -p 8642:8642 \
  NexvisoraResearch/sara-agent gateway run
```
...
docker run -d \
  ...
  NexvisoraResearch/sara-agent dashboard
```
...
docker run -it --rm \
  -v ~/.sara:/opt/data \
  NexvisoraResearch/sara-agent
```

Or if you have already opened a terminal in your running container (via Docker Desktop for instance), just run:

```sh
/opt/sara/.venv/bin/sara
```

## Persistent volumes

The `/opt/data` volume is the single source of truth for all sara state. It maps to your host's `~/.sara/` directory and contains:

| Path | Contents |
|------|----------|
| `.env` | API keys and secrets |
| `config.yaml` | All sara configuration |
...
:::warning
Never run two sara **gateway** containers against the same data directory simultaneously — session files and memory stores are not designed for concurrent write access. Running a dashboard container alongside the gateway is safe since the dashboard only reads data.
:::

## Multi-profile support

sara supports [multiple profiles](../reference/profile-commands.md) — separate `~/.sara/` directories that let you run independent agents (different SOUL, skills, memory, sessions, credentials) from a single installation. **When running under Docker, using sara' built-in multi-profile feature is not recommended.**
...
- **Independent lifecycle** — upgrade, restart, pause, or roll back each agent separately (`docker restart sara-work` leaves `sara-personal` untouched).
...
services:
  sara-work:
    image: NexvisoraResearch/sara-agent:latest
    container_name: sara-work
    restart: unless-stopped
    command: gateway run
    ports:
      - "8642:8642"
  ...
  sara-personal:
    image: NexvisoraResearch/sara-agent:latest
    container_name: sara-personal
    restart: unless-stopped
    command: gateway run
    ports:
      - "8643:8642"
    volumes:
      - ~/.sara-personal:/opt/data
```
...
services:
  ...
  dashboard:
    ...
    volumes:
      - ~/.sara:/opt/data
    environment:
      - GATEWAY_HEALTH_URL=http://sara:8642
    networks:
      - sara-net
    depends_on:
      - sara
...
The sara container needs moderate resources. Recommended minimums:
...
- Python 3 with all sara dependencies (`uv pip install -e ".[all]"`)
...
- Then runs `sara` with whatever arguments you pass
...
```sh
docker pull NexvisoraResearch/sara-agent:latest
docker rm -f sara
docker run -d \
  --name sara \
  --restart unless-stopped \
...
Check logs: `docker logs sara`. Common causes:
- Missing or invalid `.env` file — run interactively first to complete setup
- Port conflicts if running with exposed ports

### "Permission denied" errors

The container's entrypoint drops privileges to the non-root `sara` user (UID 10000) via `gosu`. If your host `~/.sara/` is owned by a different UID, set `sara_UID`/`sara_GID` to match your host user, or ensure the data directory is writable:
...
```sh
docker logs --tail 50 sara          # Recent logs
docker run -it --rm NexvisoraResearch/sara-agent:latest version     # Verify version
docker stats sara                    # Resource usage
```
