...
- Give it a name like "sara-agent"
- Select scopes:
...
- Give it a title like "sara-agent-&lt;machine-name>"
...
elif [ -f ~/.sara/.env ] && grep -q "^GITHUB_TOKEN=" ~/.sara/.env; then
  export GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.sara/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
  echo "AUTH_METHOD=curl"
