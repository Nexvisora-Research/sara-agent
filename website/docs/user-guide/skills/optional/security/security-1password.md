...
| | |
|---|---|
| Source | Optional — install with `sara skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by sara Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during sara terminal calls (desktop app flow only)
...
### Service Account (recommended for sara)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `~/.sara/.env` (the skill will prompt for this on first load).
...
## sara Execution Pattern (desktop app flow)

sara terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/sara-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/sara-op.sock"
