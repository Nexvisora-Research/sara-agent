...
| | |
|---|---|
| Source | Optional — install with `sara skills install official/productivity/here-now` |
...
- Skill helper paths:
  - `${sara_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${sara_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${sara_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client sara
```
...
```bash
PUBLISH="${sara_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client sara
```
...
```bash
DRIVE="${sara_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `sara`)    |
