...
| | |
|---|---|
| Source | Optional — install with `sara skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into sara or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to sara. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.sara/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.sara/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is sara integration, either:

- configure the server in `~/.sara/config.yaml` using the `native-mcp` skill, or
...
### sara cannot see the deployed server

The server-building part may be correct while the sara config is not. Load the `native-mcp` skill and configure the server in `~/.sara/config.yaml`, then restart sara.
