---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into sara Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into sara Agent"
---
...
Migrate a user's OpenClaw customization footprint into sara Agent. Imports sara-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sara skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | sara Agent (Nexvisora Research ) |
| License | MIT |
| Tags | `Migration`, `OpenClaw`, `sara`, `Memory`, `Persona`, `Import` |
| Related skills | [`sara-agent`](/docs/user-guide/skills/bundled/autonomous-ai-agents/autonomous-ai-agents-sara-agent) |
...
# OpenClaw -> sara Migration

Use this skill when a user wants to move their OpenClaw setup into sara Agent with minimal manual cleanup.
...
```bash
sara claw migrate              # Full interactive migration
sara claw migrate --dry-run    # Preview what would be migrated
sara claw migrate --preset user-data   # Migrate without secrets
sara claw migrate --overwrite  # Overwrite existing conflicts
sara claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `sara setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_sara.py` to:

- import `SOUL.md` into the sara home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into sara memory entries
- merge OpenClaw command approval patterns into sara `command_allowlist`
- migrate sara-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.sara/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen sara workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.sara/tts/`
- archive non-secret docs that do not have a direct sara destination
...
- `scripts/openclaw_to_sara.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py`

Do not guess a shorter path like `~/.sara/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.sara/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of sara-compatible secrets, currently:
...
sara CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in sara. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a sara workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing sara skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.sara/skills/migration/openclaw-migration/scripts/openclaw_to_sara.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of sara unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty sara targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean sara destination. Unsupported auth blobs must still be reported as skipped.
...
- sara persona state imported
- sara memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.sara/skills/openclaw-imports/`
