...
Give the agent its own dedicated email inbox via AgentMail. Send, receive, and manage email autonomously using agent-owned email addresses (e.g. sara-agent@agentmail.to).

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sara skills install official/email/agentmail` |
...
Add to `~/.sara/config.yaml` (paste your actual key — MCP env vars are not expanded from .env):
```yaml
mcp_servers:
  agentmail:
    command: "npx"
...
1. Create a dedicated inbox:
   - Use `create_inbox` with a username (e.g. `sara-agent`)
   - The agent gets address: `sara-agent@agentmail.to`
2. Send an email:
...
```
1. create_inbox (username: "sara-outreach")
...
```
sara --toolsets mcp -q "Create an AgentMail inbox called test-agent and tell me its email address"
```
