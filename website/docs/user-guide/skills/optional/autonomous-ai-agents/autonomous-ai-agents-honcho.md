...
description: "Configure and use Honcho memory with sara -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with sara -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `sara skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for sara

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every sara profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/sara#running-honcho-locally-with-sara

### Verify

```bash
sara honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this sara profile's AI peer
...
Honcho models conversations as interactions between **peers**. sara creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this sara instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each sara run |
| `global` | Single session across all directories |

Manual override: `sara honcho map my-project-name`
...
Each sara profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block sara.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `sara.coder` host block in `honcho.json`
...
```bash
sara honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for sara when Honcho memory is active.
...
Config file: `$sara_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `sara honcho setup`. Ensure `memory.provider: honcho` is in `~/.sara/config.yaml`.

### Memory not persisting across sessions
Check `sara honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `sara profile create <name> --clone`. For existing profiles: `sara honcho sync`.
...
| Command | Description |
|---------|-------------|
| `sara honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `sara honcho status` | Show resolved config, connection test, peer info for active profile |
| `sara honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `sara honcho disable` | Disable Honcho for the active profile |
| `sara honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `sara honcho peers` | Show peer identities across all profiles |
| `sara honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `sara honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `sara honcho sessions` | List known directory-to-session-name mappings |
| `sara honcho map <name>` | Map current working directory to a Honcho session name |
| `sara honcho identity` | Seed AI peer identity or show both peer representations |
| `sara honcho sync` | Create host blocks for all sara profiles that don't have one yet |
| `sara honcho migrate` | Step-by-step migration guide from OpenClaw native memory to sara + Honcho |
| `sara memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `sara memory status` | Show active memory provider and config |
| `sara memory off` | Disable external memory provider |
