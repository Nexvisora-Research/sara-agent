...
Connect sara to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
sara supports two WeCom integration modes:
...
3. sara decrypts the message, queues it for the agent
...
(Use `sara gateway start` only after `sara gateway install` has registered the systemd/launchd service.)
