...
sidebar_position: 4
title: "Slack"
description: "Set up sara Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect sara Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your sara instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. sara uses the modern
...
The fastest path is to paste a manifest sara generates for you. It
...
### Option A: From a sara-generated manifest (recommended)

1. Generate the manifest:
   ```bash
   sara slack manifest --write
   ```
   This writes `~/.sara/slack-manifest.json` and prints paste-in
   instructions.
...
4. Enter an app name (e.g., "sara Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, sara can chat but **cannot reliably read user-uploaded attachments**.
...
3. You'll be prompted to create an **App-Level Token**:
   - Name it something like `sara-socket` (the name doesn't matter)
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a sara configuration issue.
:::
...
sara uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
sara gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
sara gateway              # Foreground
sara gateway install      # Install as a user service
sudo sara gateway install --system   # Linux only: boot-time system service
```
...
Every sara command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
sara command with its description.

Under the hood: sara ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/NexvisoraResearch/sara-agent/blob/main/sara_cli/commands.py)
...
When sara adds new commands (e.g. after `sara update`), regenerate
...
3. Paste the new contents of `~/.sara/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/sara <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/sara btw run the tests` — sara routes it the same way as `/btw
run the tests`. Free-form questions also work: `/sara what's the
...
```bash
sara slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how sara behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@sara Agent what time is it?`). In channels, sara replies in a thread attached to that message. |
| **Threads** | If you @mention sara inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.sara/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to sara in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before sara will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where sara will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @sara Agent`).

---

## Multi-Workspace Support

sara can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, sara also loads tokens from an **OAuth token file** at:

```
~/.sara/slack_tokens.json
```
...
- When a message arrives, sara uses the correct workspace-specific client to respond.
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @sara Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @sara Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. sara now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @sara Agent`)
...
- Tokens should be stored in `~/.sara/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your sara config directory
