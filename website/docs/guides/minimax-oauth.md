...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in sara Agent — no API key required"
---

# MiniMax OAuth

sara Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and sara automatically refreshes your session.
...
# → sara opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.sara/auth.json` and are refreshed automatically before each session.
...
```bash
sara auth add minimax-oauth --region cn
```
...
```bash
sara auth add minimax-oauth --no-browser
```

sara will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

sara implements a PKCE device-code flow against the MiniMax OAuth endpoints:

1. sara generates a PKCE verifier / challenge pair and a random state value.
...
4. sara polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.sara/auth.json` under the `minimax-oauth` key.
...
```bash
sara config set model MiniMax-M2.7
sara config set provider minimax-oauth
```

## Configuration Reference

After login, `~/.sara/config.yaml` will contain entries similar to:
...
```bash
sara --provider minimax-oauth    # canonical
sara --provider minimax-portal   # alias
sara --provider minimax-global   # alias
sara --provider minimax_oauth    # alias (underscore form)
```
...
```bash
sara_INFERENCE_PROVIDER=minimax-oauth sara
```
...
sara refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, sara marks the session as requiring re-login.

**Fix:** run `sara auth add minimax-oauth` again to start a fresh login.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, sara raises a timeout error.

**Fix:** re-run `sara auth add minimax-oauth` (or `sara model`). The flow starts fresh.

### State mismatch (possible CSRF)

sara detected that the `state` value returned by the authorization server does not match what it sent.
...
If `sara` cannot open a browser window, use `--no-browser`:

```bash
sara auth add minimax-oauth --no-browser
```

sara prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `sara model` and select MiniMax (OAuth), or run `sara auth add minimax-oauth`.
...
```bash
sara auth remove minimax-oauth
```
...
- [sara doctor](../reference/cli-commands.md)
