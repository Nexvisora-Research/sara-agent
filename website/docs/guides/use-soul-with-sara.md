---
sidebar_position: 7
title: "Use SOUL.md with sara"
description: "How to use SOUL.md to shape sara Agent's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with sara

`SOUL.md` is the **primary identity** for your sara instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want sara to feel like the same assistant every time you talk to it — or if you want to replace the sara persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm sara should be
- what sara should avoid stylistically
- how sara should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who sara is and how sara speaks
...
sara now uses only the global SOUL file for the current instance:

```text
~/.sara/SOUL.md
```

If you run sara with a custom home directory, it becomes:
...
sara automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, sara does not overwrite it
- if the file exists but is empty, sara adds nothing from it to the prompt

## How sara uses it

When sara starts a session, it reads `SOUL.md` from `sara_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how sara feels.
...
sara already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How sara should sound.

# Avoid
What sara should not do.

# Defaults
How sara should behave when ambiguity appears.
```
...
Then restart sara or start a new session.
...
### I edited SOUL.md but sara still sounds the same

Check:
- you edited `~/.sara/SOUL.md` or `$sara_HOME/SOUL.md`
...
### sara is ignoring parts of my SOUL.md
