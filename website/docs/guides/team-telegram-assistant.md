...
This tutorial walks you through setting up a Telegram bot powered by sara Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **sara Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/docs/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.sara/.env`

:::tip
A $5/month VPS is plenty for running the gateway. sara itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team sara Assistant`)
   - **Username** — must end in `bot` (e.g., `myteam_sara_bot`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by sara Agent. DM me for help with code, research, debugging, and more.
   ```
...
# View live logs
journalctl --user -u sara-gateway -f
...
sara gateway start
sara gateway stop
tail -f ~/.sara/logs/gateway.log
```

:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `sara gateway install` to update the plist.
:::
...
```bash
sara gateway stop && sara gateway start
```
...
sara pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.sara/.env`:
...
Control how much detail the bot shows when using tools. In `~/.sara/config.yaml`:
...
Customize how the bot communicates by editing `~/.sara/SOUL.md`:

For a full guide, see [Use SOUL.md with sara](/docs/guides/use-soul-with-sara).
...
```bash
# From the CLI
sara cron list          # View all scheduled jobs
sara cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u sara-gateway` (Linux) or `~/.sara/logs/gateway.log` (macOS) |
| Cron job output | `~/.sara/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.sara/cron/jobs.json` |
| Pairing data | `~/.sara/pairing/` |
| Session history | `~/.sara/sessions/` |
