...
title: "Build a sara Plugin"
description: "Step-by-step guide to building a complete sara plugin with tools, hooks, data files, and skills"
---

# Build a sara Plugin

This guide walks through building a complete sara plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.
...
```bash
mkdir -p ~/.sara/plugins/calculator
cd ~/.sara/plugins/calculator
```
...
This tells sara: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** sara may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `sara my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but sara continues fine
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.sara/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.sara/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `sara plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
This is the only hook whose return value matters. When a `pre_llm_call` callback returns a dict with a `"context"` key (or a plain string), sara injects that text into the **current turn's user message**. This is the mechanism for memory plugins, RAG integrations, guardrails, and any plugin that needs to provide the model with additional context.

#### Return format

```python
# Dict with context key
return {"context": "Recalled memories:\n- User prefers dark mode\n- Last project: sara-agent"}
...
- **The system prompt is sara's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `sara <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for sara my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: sara my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for sara my-plugin."""
...
After registration, users can run `sara my-plugin status`, `sara my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `sara name` in a terminal |
...
```toml
# pyproject.toml
[project.entry-points."sara_agent.plugins"]
my-plugin = "my_plugin_package"
```

```bash
pip install sara-plugin-calculator
# Plugin auto-discovered on next sara startup
```
...
```python
# Wrong — will break if sara passes extra context
def handler(args):
    ...
