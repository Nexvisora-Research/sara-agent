---
sidebar_position: 8
title: "Use Voice Mode with sara"
description: "A practical guide to setting up and using sara voice mode across CLI, Telegram, Discord, and Discord voice channels"
---

# Use Voice Mode with sara
...
- you want sara sitting in a Discord voice channel for live conversation
...
There are really three different voice experiences in sara.
...
## Step 1: make sure normal sara works first
...
sara supports both local and cloud speech stacks.
...
### If you use `sara setup`

If you choose NeuTTS in the setup wizard, sara checks whether `neutts` is already installed. If it is missing, the wizard tells you NeuTTS needs the Python package `neutts` and the system package `espeak-ng`, offers to install them for you, installs `espeak-ng` with your platform package manager, and then runs:
...
4. sara transcribes and responds
...
- asking sara to structure your thoughts in real time

#### Accessibility / low-typing sessions

If typing is inconvenient, voice mode is one of the fastest ways to stay in the full sara loop.

## Tuning CLI behavior

### Silence threshold

If sara starts/stops too aggressively, tune:
...
sara stays a normal chat bot, but can speak replies.
...
- you want sara to function like a portable research or ops assistant
...
sara joins a Discord VC, listens to user speech, transcribes it, runs the normal agent pipeline, and speaks replies back into the channel.
...
- sara detects speech boundaries
- transcripts are posted in the associated text channel
- sara responds in text and audio
...
1. get text sara working
2. install `sara-agent[voice]`
