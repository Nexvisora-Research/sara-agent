---
sidebar_position: 6
title: "Use MCP with sara"
description: "A practical guide to connecting MCP servers to sara Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with sara

This guide shows how to actually use MCP with sara Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native sara tool
- you want sara to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect sara to internal APIs, databases, or company systems without modifying sara core

Do not use MCP when:
- a built-in sara tool already solves the job well
...
- sara remains the agent
- MCP servers contribute tools
- sara discovers those tools at startup or reload time
...
If you installed sara with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- sara banner/status should show MCP integration when configured
- ask sara what tools it has available
...
There are two categories of MCP-exposed functionality in sara:
...
So sara will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want sara to reason over a bounded workspace.
...
Now sara can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing sara core.
...
Because sara now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
