...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  sara Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to sara instantly when PRs are opened or updated.
:::
...
- **sara Agent installed** — see the [Installation guide](/docs/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  sara gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.sara/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure sara can access GitHub. Start a chat:
...
```
Run: gh pr list --repo NexvisoraResearch/sara-agent --state open --limit 3
```
...
Still in the chat, ask sara to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo NexvisoraResearch/sara-agent
```
...
A skill gives sara consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.sara/skills/code-review
```

Create `~/.sara/skills/code-review/SKILL.md`:
...
Verify it loaded — start `sara` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach sara your team's standards:
...
2. Teach sara your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
sara gateway status    # Is the gateway running?
sara cron list         # Is the job enabled?
```
...
- **[Build a Plugin](/docs/guides/build-a-sara-plugin)** — wrap the review logic into a shareable plugin
