...
{ inputs, ... }: {
  perSystem = { pkgs, lib, self', ... }:
    let
      sara-agent = self'.packages.default;
      saraVenv = sara-agent.saraVenv;

      configMergeScript = pkgs.callPackage ./configMergeScript.nix { };

      # Auto-generated config key reference — always in sync with Python
      configKeys = pkgs.runCommand "sara-config-keys" {} ''
        set -euo pipefail
        export HOME=$TMPDIR
        ${saraVenv}/bin/python3 -c '
import json, sys
...
' > $out
    ...
    in {
      ...
      checks = {
        ...
        in pkgs.runCommand "sara-cross-eval" { } (
          if failures != [] then
        ...
        package-contents = pkgs.runCommand "sara-package-contents" { } ''
          set -e
          echo "=== Checking binaries ==="
          test -x ${sara-agent}/bin/sara || (echo "FAIL: sara binary missing"; exit 1)
          test -x ${sara-agent}/bin/sara-agent || (echo "FAIL: sara-agent binary missing"; exit 1)
          echo "PASS: All binaries present"

          echo "=== Checking version ==="
          ${sara-agent}/bin/sara version 2>&1 | grep -qi "sara" || (echo "FAIL: version check"; exit 1)
        ...
        entry-points-sync = pkgs.runCommand "sara-entry-points-sync" { } ''
          set -e
          echo "=== Checking entry points match pyproject.toml [project.scripts] ==="
          for bin in sara sara-agent sara-acp; do
            test -x ${sara-agent}/bin/$bin || (echo "FAIL: $bin binary missing from Nix package"; exit 1)
        ...
        cli-commands = pkgs.runCommand "sara-cli-commands" { } ''
          set -e
          export HOME=$(mktemp -d)

          echo "=== Checking sara --help ==="
          ${sara-agent}/bin/sara --help 2>&1 | grep -q "gateway" || (echo "FAIL: gateway subcommand missing"; exit 1)
          ${sara-agent}/bin/sara --help 2>&1 | grep -q "config" || (echo "FAIL: config subcommand missing"; exit 1)
        ...
        bundled-skills = pkgs.runCommand "sara-bundled-skills" { } ''
          set -e
          echo "=== Checking bundled skills ==="
          test -d ${sara-agent}/share/sara-agent/skills || (echo "FAIL: skills directory missing"; exit 1)
          echo "PASS: skills directory exists"

          SKILL_COUNT=$(find ${sara-agent}/share/sara-agent/skills -name "SKILL.md" | wc -l)
          test "$SKILL_COUNT" -gt 0 || (echo "FAIL: no SKILL.md files found in skills directory"; exit 1)
          echo "PASS: $SKILL_COUNT bundled skills found"

          grep -q "sara_BUNDLED_SKILLS" ${sara-agent}/bin/sara || \
            (echo "FAIL: sara_BUNDLED_SKILLS not in wrapper"; exit 1)
          echo "PASS: sara_BUNDLED_SKILLS set in wrapper"
        ...
        bundled-plugins = pkgs.runCommand "sara-bundled-plugins" { } ''
          set -e
          echo "=== Checking bundled plugins ==="
          test -d ${sara-agent}/share/sara-agent/plugins || (echo "FAIL: plugins directory missing"; exit 1)
          echo "PASS: plugins directory exists"

          test -f ${sara-agent}/share/sara-agent/plugins/platforms/irc/plugin.yaml || \
            (echo "FAIL: irc plugin manifest missing"; exit 1)
          echo "PASS: irc plugin manifest present"

          grep -q "sara_BUNDLED_PLUGINS" ${sara-agent}/bin/sara || \
            (echo "FAIL: sara_BUNDLED_PLUGINS not in wrapper"; exit 1)
          echo "PASS: sara_BUNDLED_PLUGINS set in wrapper"
        ...
        bundled-tui = pkgs.runCommand "sara-bundled-tui" { } ''
          set -e
          echo "=== Checking bundled TUI ==="
          test -d ${sara-agent}/ui-tui || (echo "FAIL: ui-tui directory missing"; exit 1)
          echo "PASS: ui-tui directory exists"

          test -f ${sara-agent}/ui-tui/dist/entry.js || (echo "FAIL: compiled entry.js missing"; exit 1)
          echo "PASS: compiled entry.js present"

          test -d ${sara-agent}/ui-tui/node_modules || (echo "FAIL: node_modules missing"; exit 1)
          echo "PASS: node_modules present"

          grep -q "sara_TUI_DIR" ${sara-agent}/bin/sara || \
            (echo "FAIL: sara_TUI_DIR not in wrapper"; exit 1)
          echo "PASS: sara_TUI_DIR set in wrapper"
        ...
        # Verify sara_NODE is set in wrapper and points to Node 20+
        # (string-width uses the /v regex flag which requires Node 20+)
        sara-node = pkgs.runCommand "sara-node-version" { } ''
          set -e
          echo "=== Checking sara_NODE in wrapper ==="
          grep -q "sara_NODE" ${sara-agent}/bin/sara || \
            (echo "FAIL: sara_NODE not set in wrapper"; exit 1)
          echo "PASS: sara_NODE present in wrapper"

          sara_NODE=$(sed -n "s/^export sara_NODE='\(.*\)'/\1/p" ${sara-agent}/bin/sara)
          test -x "$sara_NODE" || (echo "FAIL: sara_NODE=$sara_NODE not executable"; exit 1)
          echo "PASS: sara_NODE executable at $sara_NODE"

          NODE_MAJOR=$("$sara_NODE" --version | sed 's/^v//' | cut -d. -f1)
          test "$NODE_MAJOR" -ge 20 || \
            (echo "FAIL: Node v$NODE_MAJOR < 20, TUI needs /v regex flag support"; exit 1)
          echo "PASS: Node v$NODE_MAJOR >= 20"

          echo "=== All sara_NODE checks passed ==="
          mkdir -p $out
          echo "ok" > $out/result
        '';

        # Verify sara_MANAGED guard works on all mutation commands
        managed-guard = pkgs.runCommand "sara-managed-guard" { } ''
          set -e
          export HOME=$(mktemp -d)
          ...
          check_blocked() {
            local label="$1"
            shift
            OUTPUT=$(sara_MANAGED=true "$@" 2>&1 || true)
          ...
          echo "=== Checking sara_MANAGED guards ==="
          check_blocked "config set" ${sara-agent}/bin/sara config set model foo
          check_blocked "config edit" ${sara-agent}/bin/sara config edit
        ...
        extra-python-packages = let
          ...
          saraWithExtra = sara-agent.override {
            extraPythonPackages = [ testPkg ];
          };
        in pkgs.runCommand "sara-extra-python-packages" { } ''
          set -e
          echo "=== Checking extraPythonPackages PYTHONPATH injection ==="

          grep -q "PYTHONPATH" ${saraWithExtra}/bin/sara || \
            (echo "FAIL: PYTHONPATH not in wrapper"; exit 1)
          echo "PASS: PYTHONPATH present in wrapper"

          grep -q "${testPkg}" ${saraWithExtra}/bin/sara || \
          ...
          if grep -q "PYTHONPATH" ${sara-agent}/bin/sara; then
        ...
        in pkgs.runCommand "sara-config-roundtrip" {
          ...
          merge_and_load() {
            local sara_home="$1"
            export sara_HOME="$sara_home"
            ${configMergeScript} ${nixSettings} "$sara_home/config.yaml"
            ${saraVenv}/bin/python3 -c '
import json, sys
