# nix/nixosModules.nix — NixOS module for sara-agent
#
# Two modes:
#   container.enable = false (default) → native systemd service
#   container.enable = true            → OCI container (persistent writable layer)
#
# Container mode: sara runs from /nix/store bind-mounted read-only into a
...
# container recreation. Environment variables are written to $sara_HOME/.env
# and read by sara at startup — no container recreation needed for env changes.
#
# Tool resolution: the sara wrapper uses --suffix PATH for nix store tools,
...
{ inputs, ... }: {
  ...
  let
    cfg = config.services.sara-agent;
    effectivePackage = if cfg.extraPythonPackages == [ ] then cfg.package
      else cfg.package.override { inherit (cfg) extraPythonPackages; };
    sara-agent = inputs.self.packages.${pkgs.stdenv.hostPlatform.system}.default;

    # Deep-merge config type (from 0xrsydn/nix-sara-agent)
    deepConfigType = lib.types.mkOptionType {
      name = "sara-config-attrs";
      description = "sara YAML config (attrset), merged deeply via lib.recursiveUpdate.";
    ...
    generatedConfigFile = pkgs.writeText "sara-config.yaml" configJson;
    ...
    documentDerivation = pkgs.runCommand "sara-documents" { } (
      ''
        mkdir -p $out
      '' + lib.concatStringsSep "\n" (
        lib.mapAttrsToList (name: value:
          ...
          else "cat > $out/${name} <<'sara_DOC_EOF'\n${value}\nsara_DOC_EOF"
        ) cfg.documents
      )
    );

    containerName = "sara-agent";
    containerDataDir = "/data";     # stateDir mount point inside container
    containerHomeDir = "/home/sara";
    ...
    # sara user + sudo on first boot (writable layer persists), then
    # drops privileges. Supports arbitrary base images (Debian, Alpine, etc).
    containerEntrypoint = pkgs.writeShellScript "sara-container-entrypoint" ''
      set -eu

      sara_UID="''${sara_UID:?sara_UID must be set}"
      sara_GID="''${sara_GID:?sara_GID must be set}"

      # ── Group: ensure a group with GID=$sara_GID exists ──
      # Check by GID (not name) to avoid collisions with pre-existing groups
      # (e.g. GID 100 = "users" on Ubuntu)
      EXISTING_GROUP=$(getent group "$sara_GID" 2>/dev/null | cut -d: -f1 || true)
      if [ -n "$EXISTING_GROUP" ]; then
        GROUP_NAME="$EXISTING_GROUP"
      else
        GROUP_NAME="sara"
        if command -v groupadd >/dev/null 2>&1; then
          groupadd -g "$sara_GID" "$GROUP_NAME"
        elif command -v addgroup >/dev/null 2>&1; then
          addgroup -g "$sara_GID" "$GROUP_NAME" 2>/dev/null || true
        fi
      fi

      # ── User: ensure a user with UID=$sara_UID exists ──
      PASSWD_ENTRY=$(getent passwd "$sara_UID" 2>/dev/null || true)
      if [ -n "$PASSWD_ENTRY" ]; then
        TARGET_USER=$(echo "$PASSWD_ENTRY" | cut -d: -f1)
        TARGET_HOME=$(echo "$PASSWD_ENTRY" | cut -d: -f6)
      else
        TARGET_USER="sara"
        TARGET_HOME="/home/sara"
        if command -v useradd >/dev/null 2>&1; then
          useradd -u "$sara_UID" -g "$sara_GID" -m -d "$TARGET_HOME" -s /bin/bash "$TARGET_USER"
        elif command -v adduser >/dev/null 2>&1; then
          adduser -u "$sara_UID" -D -h "$TARGET_HOME" -s /bin/sh -G "$GROUP_NAME" "$TARGET_USER" 2>/dev/null || true
        fi
      fi
      mkdir -p "$TARGET_HOME"
      chown "$sara_UID:$sara_GID" "$TARGET_HOME"
      chmod 0750 "$TARGET_HOME"

      # Ensure sara_HOME is owned by the target user
      if [ -n "''${sara_HOME:-}" ] && [ -d "$sara_HOME" ]; then
        chown -R "$sara_UID:$sara_GID" "$sara_HOME"
      fi
      ...
      if [ ! -f /var/lib/sara-tools-provisioned ] && command -v apt-get >/dev/null 2>&1; then
        ...
        touch /var/lib/sara-tools-provisioned
      fi

      if command -v sudo >/dev/null 2>&1 && [ ! -f /etc/sudoers.d/sara ]; then
        mkdir -p /etc/sudoers.d
        echo "$TARGET_USER ALL=(ALL) NOPASSWD:ALL" > /etc/sudoers.d/sara
      ...
      if command -v setpriv >/dev/null 2>&1; then
        exec setpriv --reuid="$sara_UID" --regid="$sara_GID" --init-groups "$@"
    ...
    # so they can update without recreation. Env vars go through $sara_HOME/.env.
    ...
    # Default: /var/lib/sara/workspace → /data/workspace.
  ...
  in {
    options.services.sara-agent = with lib; {
      enable = mkEnableOption "sara Agent gateway service";

      # ── Package ──────────────────────────────────────────────────────────
      package = mkOption {
        type = types.package;
        default = sara-agent;
        description = "The sara-agent package to use.";
      };
      ...
      stateDir = mkOption {
        type = types.str;
        default = "/var/lib/sara";
        description = "State directory. Contains .sara/ subdir (sara_HOME).";
      };
      ...
      settings = mkOption {
        ...
        default = { };
        description = ''
          Declarative sara config (attrset). Deep-merged across module
      ...
      environmentFiles = mkOption {
        ...
        description = ''
          ...
          Contents are merged into $sara_HOME/.env at activation time.
          sara reads this file on every startup via load_sara_dotenv().
        '';
      };
      ...
      environment = mkOption {
        ...
        default = { };
        description = ''
          Non-secret environment variables. Merged into $sara_HOME/.env
          ...
          options = {
            ...
            auth = mkOption {
              ...
              description = ''
                ...
                (remote MCP servers). Tokens are stored in $sara_HOME/mcp-tokens/.
              '';
            };
      ...
      extraArgs = mkOption {
        type = types.listOf types.str;
        default = [ ];
        description = "Extra command-line arguments for `sara gateway`.";
      };
      ...
      extraPackages = mkOption {
        ...
        description = ''
          ...
          Implemented via the sara user's per-user profile
      ...
      extraPlugins = mkOption {
        ...
        default = [ ];
        description = ''
          Directory-based plugin packages to symlink into the sara plugins
          directory. Each package should contain a plugin.yaml and __init__.py
          at its root. sara discovers these automatically on startup.
        '';
        example = literalExpression ''
          [
      ...
      extraPythonPackages = mkOption {
        ...
        description = ''
          ...
          sara_agent.plugins entry-point group. Each package must be built
          with the same Python interpreter as sara (python312).
        '';
        example = literalExpression ''
          [
      ...
      addToSystemPackages = mkOption {
        ...
        default = false;
        description = ''
          Add the sara CLI to environment.systemPackages and export
          sara_HOME system-wide (via environment.variables) so interactive
      ...
      container = {
        ...
        hostUsers = mkOption {
          ...
          default = [ ];
          description = ''
            Interactive users who get a ~/.sara symlink to the service
            stateDir. These users are automatically added to the sara group.
          '';
          example = [ "sidbin" ];
        };
      };
    ...
    config = lib.mkIf cfg.enable (lib.mkMerge [
      ...
      (lib.mkIf (cfg.mcpServers != { }) {
        services.sara-agent.settings.mcp_servers = lib.mapAttrs (_name: srv:
          # Stdio transport
      ...
      # ── Host CLI ──────────────────────────────────────────────────────
      # Add the sara CLI to system PATH and export sara_HOME system-wide
      # so interactive shells share state (sessions, skills, cron) with the
      # gateway service instead of creating a separate ~/.sara/.
      (lib.mkIf cfg.addToSystemPackages {
        environment.systemPackages = [ effectivePackage ];
        environment.variables.sara_HOME = "${cfg.stateDir}/.sara";
      })
      ...
      # ── Assertions ─────────────────────────────────────────────────────
      {
        ...
        in [{
          ...
          message = "services.sara-agent.extraPlugins: duplicate plugin names detected: ${toString names}. If using fetchFromGitHub, set name = \"plugin-name\" to disambiguate.";
        }];
      }
      ...
      # Wire extraPackages into the sara user's per-user profile so the
      ...
      (lib.mkIf (cfg.container.enable && !cfg.addToSystemPackages && cfg.container.hostUsers != []) {
        warnings = [
          ''
            services.sara-agent: container.enable is true and container.hostUsers
            is set, but addToSystemPackages is false. Without a host-installed sara
            binary, container routing will not work for interactive users.
            Set addToSystemPackages = true or ensure sara is on PATH.
          ''
        ];
      })
      ...
      # ── Directories ───────────────────────────────────────────────────
      {
        systemd.tmpfiles.rules = [
          ...
          "d ${cfg.stateDir}/.sara        2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sara/cron   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sara/sessions 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sara/logs   2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sara/memories 2770 ${cfg.user} ${cfg.group} - -"
          "d ${cfg.stateDir}/.sara/plugins 2770 ${cfg.user} ${cfg.group} - -"
      ...
      {
        system.activationScripts."sara-agent-setup" = lib.stringAfter ([ "users" ] ++ lib.optional (config.system.activationScripts ? setupSecrets) "setupSecrets") ''
          # Ensure directories exist (activation runs before tmpfiles)
          mkdir -p ${cfg.stateDir}/.sara
          mkdir -p ${cfg.stateDir}/home
          mkdir -p ${cfg.workingDirectory}
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir} ${cfg.stateDir}/.sara ${cfg.stateDir}/home ${cfg.workingDirectory}
          chmod 2770 ${cfg.stateDir} ${cfg.stateDir}/.sara ${cfg.workingDirectory}
          ...
          find ${cfg.stateDir}/.sara -maxdepth 1 \
            \( -name "*.db" -o -name "*.db-wal" -o -name "*.db-shm" -o -name "SOUL.md" \) \
            -exec chmod g+rw {} + 2>/dev/null || true
          for _subdir in cron sessions logs memories plugins; do
            mkdir -p "${cfg.stateDir}/.sara/$_subdir"
            chown ${cfg.user}:${cfg.group} "${cfg.stateDir}/.sara/$_subdir"
            chmod 2770 "${cfg.stateDir}/.sara/$_subdir"
            find "${cfg.stateDir}/.sara/$_subdir" -type f \
              -exec chmod g+rw {} + 2>/dev/null || true
          done
          ...
          ${if cfg.configFile != null then ''
            install -o ${cfg.user} -g ${cfg.group} -m 0640 -D ${configFile} ${cfg.stateDir}/.sara/config.yaml
          '' else ''
            ${configMergeScript} ${generatedConfigFile} ${cfg.stateDir}/.sara/config.yaml
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sara/config.yaml
            chmod 0640 ${cfg.stateDir}/.sara/config.yaml
          ''}

          # Managed mode marker (so interactive shells also detect NixOS management)
          touch ${cfg.stateDir}/.sara/.managed
          chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sara/.managed
          chmod 0644 ${cfg.stateDir}/.sara/.managed
          ...
          ${if cfg.container.enable then ''
            cat > ${cfg.stateDir}/.sara/.container-mode <<'sara_CONTAINER_MODE_EOF'
    # Written by NixOS activation script. Do not edit manually.
    backend=${cfg.container.backend}
    container_name=${containerName}
    exec_user=${cfg.user}
    sara_bin=${containerDataDir}/current-package/bin/sara
    sara_CONTAINER_MODE_EOF
            chown ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sara/.container-mode
            chmod 0644 ${cfg.stateDir}/.sara/.container-mode
          '' else ''
            rm -f ${cfg.stateDir}/.sara/.container-mode

            # Remove symlink bridge for hostUsers
            ${lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.sara";
              in ''
                if [ -L "${symlinkPath}" ] && [ "$(readlink "${symlinkPath}")" = "${cfg.stateDir}/.sara" ]; then
                  rm -f "${symlinkPath}"
                  echo "sara-agent: removed symlink ${symlinkPath}"
                fi
              '') cfg.container.hostUsers)}
          ''}

          # ── Symlink bridge for interactive users ───────────────────────
          # Create ~/.sara -> stateDir/.sara for each hostUser so the
          ...
          ${lib.optionalString cfg.container.enable
            (lib.concatStringsSep "\n" (map (user:
              let
                userHome = config.users.users.${user}.home;
                symlinkPath = "${userHome}/.sara";
                target = "${cfg.stateDir}/.sara";
              in ''
                if [ -d "${symlinkPath}" ] && [ ! -L "${symlinkPath}" ]; then
                  # Real directory — back it up, then create symlink.
                  # (ln -sfn cannot atomically replace a directory.)
                  _backup="${symlinkPath}.bak.$(date +%s)"
                  echo "sara-agent: backing up existing ${symlinkPath} to $_backup"
          ...
          ${lib.optionalString (cfg.authFile != null) ''
            ${if cfg.authFileForceOverwrite then ''
              install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sara/auth.json
            '' else ''
              if [ ! -f ${cfg.stateDir}/.sara/auth.json ]; then
                install -o ${cfg.user} -g ${cfg.group} -m 0600 ${cfg.authFile} ${cfg.stateDir}/.sara/auth.json
              fi
            ''}
          ''}
          ...
          # sara reads $sara_HOME/.env at startup via load_sara_dotenv(),
          # so this is the single source of truth for both native and container mode.
          ${lib.optionalString (cfg.environment != {} || cfg.environmentFiles != []) ''
            ENV_FILE="${cfg.stateDir}/.sara/.env"
            install -o ${cfg.user} -g ${cfg.group} -m 0640 /dev/null "$ENV_FILE"
            cat > "$ENV_FILE" <<'sara_NIX_ENV_EOF'
    ${envFileContent}
    sara_NIX_ENV_EOF
            ${lib.concatStringsSep "\n" (map (f: ''
              if [ -f "${f}" ]; then
        ...
        find ${cfg.stateDir}/.sara/plugins -maxdepth 1 -type l -name 'nix-managed-*' -delete 2>/dev/null || true
        ...
        ${lib.concatStringsSep "\n" (map (plugin:
          ...
          in ''
            ...
            ln -sfn ${plugin} ${cfg.stateDir}/.sara/plugins/nix-managed-${name}
            chown -h ${cfg.user}:${cfg.group} ${cfg.stateDir}/.sara/plugins/nix-managed-${name}
          '') cfg.extraPlugins)}
        '';
      }
      ...
      (lib.mkIf (!cfg.container.enable) {
        systemd.services.sara-agent = {
          description = "sara Agent Gateway";
          ...
          serviceConfig = {
            ...
            # $sara_HOME/.env by the activation script. load_sara_dotenv()
            # reads them at Python startup — no systemd EnvironmentFile needed.

            ExecStart = lib.concatStringsSep " " ([
              "${effectivePackage}/bin/sara"
              "gateway"
            ] ++ cfg.extraArgs);
            ...
            # so interactive users in the sara group can read/write them.
            UMask = "0007";
      ...
      (lib.mkIf cfg.container.enable {
        ...
        systemd.services.sara-agent = {
          description = "sara Agent Gateway (container)";
          wantedBy = [ "multi-user.target" ];
          after = [ "network-online.target" ]
          ...
          preStart = ''
            ...
            if [ "$NEED_CREATE" = "true" ]; then
              ...
              sara_UID=$(${pkgs.coreutils}/bin/id -u ${cfg.user})
              sara_GID=$(${pkgs.coreutils}/bin/id -g ${cfg.user})
              ...
              ${containerBin} create \
                ...
                --env sara_UID="$sara_UID" \
                --env sara_GID="$sara_GID" \
                --env sara_HOME=${containerDataDir}/.sara \
                --env sara_MANAGED=true \
                --env HOME=${containerHomeDir} \
                --env MESSAGING_CWD=${containerWorkDir} \
                ${lib.concatStringsSep " " cfg.container.extraOptions} \
                ${cfg.container.image} \
                ${containerDataDir}/current-package/bin/sara gateway run --replace ${lib.concatStringsSep " " cfg.extraArgs}
