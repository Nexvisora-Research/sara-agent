...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("sara_HOME", os.path.expanduser("~/.sara")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("sara_HOME", Path.home() / ".sara")) / "skills" / "red-teaming" / "godmode"
...
sara_HOME = Path(os.getenv("sara_HOME", Path.home() / ".sara"))
CONFIG_PATH = sara_HOME / "config.yaml"
PREFILL_PATH = sara_HOME / "prefill.json"
...
def _get_current_model() -> tuple:
    """Read current model and provider from sara config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.sara/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart sara for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart sara for changes to take effect.")
