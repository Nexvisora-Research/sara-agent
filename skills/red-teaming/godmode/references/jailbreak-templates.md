...
## 5. GODMODE FAST — sara 4 405B (Zero Refusal)

**Model:** `NexvisoraResearch/sara-4-405b`
**Strategy:** sara models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with sara Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.sara/config.yaml`:
