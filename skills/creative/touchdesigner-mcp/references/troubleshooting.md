...
## 5. sara Configuration

### Config location

`$sara_HOME/config.yaml` (defaults to `~/.sara/config.yaml` when `sara_HOME` is unset)
...
Restart the sara session for changes to take effect. The MCP connection is
