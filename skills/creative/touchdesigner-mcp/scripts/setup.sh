...
sara_CFG="${sara_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure sara config has twozero_td MCP entry ──
if [[ ! -f "$sara_CFG" ]]; then
    echo -e " ${FAIL} sara config not found at ${sara_CFG}"
    manual_steps+=("Create ${sara_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$sara_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in sara config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to sara config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${sara_CFG} manually"); }
    manual_steps+=("Restart sara session to pick up config change")
fi
