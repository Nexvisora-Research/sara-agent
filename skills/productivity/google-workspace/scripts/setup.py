#!/usr/bin/env python3
"""Google Workspace OAuth2 setup for sara Agent.
...
# Ensure sibling modules (_sara_home) are importable when run standalone.
_SCRIPTS_DIR = str(Path(__file__).resolve().parent)
if _SCRIPTS_DIR not in sys.path:
    sys.path.insert(0, _SCRIPTS_DIR)

from _sara_home import display_sara_home, get_sara_home

sara_HOME = get_sara_home()
TOKEN_PATH = sara_HOME / "google_token.json"
CLIENT_SECRET_PATH = sara_HOME / "google_client_secret.json"
PENDING_AUTH_PATH = sara_HOME / "google_oauth_pending.json"
...
def _format_missing_scopes(missing_scopes: list[str]) -> str:
    ...
    return (
        ...
        f"{bullets}\n"
        "Run the Google Workspace setup again from this same sara profile to refresh consent."
    )
...
def install_deps():
    ...
    try:
        ...
        print("  pip install 'sara-agent[google]'")
...
def store_client_secret(path: str):
    """Copy and validate client_secret.json to sara home."""
...
def exchange_auth_code(code: str):
    ...
    print(f"Profile-scoped token location: {display_sara_home()}/google_token.json")
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace OAuth setup for sara")
