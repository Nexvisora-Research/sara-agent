#!/usr/bin/env python3
"""Google Workspace API CLI for sara Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing sara-facing JSON contract and falls back to the Python client
...
from _sara_home import get_sara_home
...
def _gws_binary() -> str | None:
    override = os.getenv("sara_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for sara Agent")
