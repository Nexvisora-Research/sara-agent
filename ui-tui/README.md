...
React + Ink terminal UI for sara. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `sara_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
- Input history is stored in `~/.sara/.sara_history` or under `sara_HOME`.
...
```text
ui-tui/
  packages/sara-ink/   forked Ink renderer (local dep)
  src/
    ...
    types/
      sara-ink.d.ts    type declarations for @sara/ink
...
tui_gateway/
  ...
  slash_worker.py        persistent saraCLI subprocess for slash commands
```
