...
export default class Ink {
  ...
  // synchronexvisoraly in handleResize would leave the screen blank for the ~80ms
  ...
  // Fold synchroNexvisorare-entry (selection fanout, onFrame callback)
  ...
  constructor(private readonly options: Options) {
    ...
    // synchronexvisoraly. Deferring to a microtask runs onRender after layout
    ...
    // existing synchroNexvisoralastFrame() tests are unaffected.
  ...
  onRender() {
    ...
    if (this.altScreenActive && hasDiff) {
      ...
      // synchronexvisoraly in handleResize would blank the screen for the ~80ms
      // render() takes.
  ...
  async copySelectionNoClear(): Promise<string> {
    ...
    if (text) {
      try {
        ...
        if (process.env.sara_TUI_DEBUG_CLIPBOARD) {
          console.error(
            '[clipboard] no path reached the clipboard (headless + no tmux?) — set sara_TUI_FORCE_OSC52=1 to force the escape sequence'
          )
        }
      } catch (err) {
        if (process.env.sara_TUI_DEBUG_CLIPBOARD) {
   ...
   * with no side effects — call it synchronexvisoraly at click time so the
  ...
  unmount(error?: Error | number | null): void {
    ...
    // Clean up terminal modes synchronexvisoraly before process exit.
