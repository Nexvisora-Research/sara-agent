import type { RunExternalProcess } from '@sara/ink'
...
export async function runExternalSetup({ args, ctx, done, launcher, suspend }: RunExternalSetupOptions) {
  ...
  transcript.sys(`launching \`sara ${args.join(' ')}\`…`)
  ...
  if (result.error) {
    transcript.sys(`error launching sara: ${result.error}`)
  ...
  if (result.code !== 0) {
    transcript.sys(`sara ${args[0]} exited with code ${result.code}`)
    patchUiState({ status: 'setup required' })
