"""Slash command definitions and autocomplete for the sara CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of sara config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal sara works on across turns until achieved", "Session",
    ...
    CommandDef("busy", "Control what Enter does while sara is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.sara/skills/ for newly installed or removed skills",
    ...
    CommandDef("update", "Update sara Agent to the latest version", "Info",
               gateway_only=True),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`sara_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/sara`` subcommand mapping, and
    ...
    """
    try:
        from sara_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``sara skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``sara skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``sara skills list`` and
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/sara <verb>`` subcommand.
    ...
    can still reach them via ``/sara <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/sara`` is always reserved as the first entry so the
    legacy ``/sara <subcommand>`` form keeps working for anything that
    ...
    # Reserve /sara as the catch-all top-level command.
    entries.append(("sara", "Talk to sara or run a subcommand", "[subcommand] [args]"))
    seen.add("sara")
...
def slack_app_manifest(request_url: str = "https://sara-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /sara handler.

    Maps both canonical names and aliases so /sara bg do stuff works
    the same as /sara background do stuff.

    Plugin-registered slash commands are included so ``/sara <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from sara_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
