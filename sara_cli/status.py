"""
Status command for sara CLI.

Shows the status of all sara Agent components.
"""
...
from sara_cli.auth import AuthError, resolve_provider
from sara_cli.colors import Colors, color
from sara_cli.config import get_env_path, get_env_value, get_sara_home, load_config
from sara_cli.models import provider_label
from sara_cli.nexvisora_subscription import get_nexvisora_subscription_features
from sara_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``sara config``'s
...
def show_status(args):
    """Show status of all sara Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ⚕ sara Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nexvisora_logged_in = bool(nexvisora_status.get("logged_in"))
    nexvisora_error = nexvisora_status.get("error")
    nexvisora_label = "logged in" if nexvisora_logged_in else "not logged in (run: sara auth add Nexvisora--type oauth)"
    print(
        f"  {'NexvisoraPortal':<12}  {check_mark(nexvisora_logged_in)} "
        f"{nexvisora_label}"
    )
    portal_url = nexvisora_status.get("portal_base_url") or "(unknown)"
    access_exp = _format_iso_timestamp(nexvisora_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(nexvisora_status.get("agent_key_expires_at"))
    refresh_label = "yes" if nexvisora_status.get("has_refresh_token") else "no"
    if nexvisora_logged_in or portal_url != "(unknown)" or nexvisora_error:
        print(f"    Portal URL: {portal_url}")
    if nexvisora_logged_in or nexvisora_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if nexvisora_logged_in or nexvisora_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if nexvisora_logged_in or nexvisora_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if nexvisora_error and not nexvisora_logged_in:
        print(f"    Error:      {nexvisora_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'OpenAI Codex':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: sara model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: sara auth add minimax-oauth)'}"
    )
    ...
    # =========================================================================
    # NexvisoraSubscription Features
    # =========================================================================
    if managed_nexvisora_tools_enabled():
        features = get_nexvisora_subscription_features(config)
        print()
        print(color("◆ NexvisoraTool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.nexvisora_auth_present:
            print("  NexvisoraPortal   ✗ not logged in")
        else:
            print("  NexvisoraPortal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_nexvisora:
                state = "active via Nexvisorasubscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.nexvisora_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.nexvisora_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_nexvisora)} {state}")
    elif nexvisora_logged_in:
        # Logged into Nexvisorabut on the free tier — show upgrade nudge
        print()
        print(color("◆ NexvisoraTool Gateway", Colors.CYAN, Colors.BOLD))
        print("  Your free-tier Nexvisoraaccount does not include Tool Gateway access.")
        print("  Upgrade your subscription to unlock managed web, image, TTS, and browser tools.")
        try:
            portal_url = nexvisora_status.get("portal_base_url", "").rstrip("/")
            if portal_url:
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: sara model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from sara_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'sara-agent[vercel]')"
    ...
    try:
        from sara_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
    ...
    jobs_file = get_sara_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    sessions_file = get_sara_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'sara doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'sara setup' to configure", Colors.DIM))
    print()
