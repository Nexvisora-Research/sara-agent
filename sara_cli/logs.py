"""``sara logs`` — view and filter sara log files.
...
Usage examples::
    ...
    sara logs                    # last 50 lines of agent.log
    sara logs -f                 # follow agent.log in real time
    sara logs errors             # last 50 lines of errors.log
    sara logs gateway -n 100    # last 100 lines of gateway.log
    sara logs --level WARNING    # only WARNING+ lines
    sara logs --session abc123   # filter by session ID substring
    sara logs --component tools  # only tool-related lines
    sara logs --since 1h         # lines from the last hour
    sara logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_sara_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when sara runs — try 'sara chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_sara_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_sara_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_sara_home()}/logs/")
        return

    print(f"Log files in {display_sara_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'sara chat' to generate logs)")
