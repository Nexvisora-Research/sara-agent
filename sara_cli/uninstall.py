"""
sara Agent Uninstaller.

Provides options for:
- Full uninstall: Remove everything including configs and data
- Keep data: Remove code but keep ~/.sara/ (configs, sessions, logs)
"""
...
def remove_path_from_shell_configs():
    """Remove sara PATH entries from shell configuration files."""
    ...
    for config_path in configs:
        try:
            ...
            # Remove lines containing sara-agent or sara PATH entries
            new_lines = []
            skip_next = False
            
            for line in content.split('\n'):
                # Skip the "# sara Agent" comment and following line
                if '# sara Agent' in line or '# sara-agent' in line:
                    skip_next = True
                    continue
                if skip_next and ('sara' in line.lower() and 'PATH' in line):
                    skip_next = False
                    continue
                skip_next = False
                
                # Remove any PATH line containing sara
                if 'sara' in line.lower() and ('PATH=' in line or 'path=' in line.lower()):
                    continue
...
def remove_wrapper_script():
    """Remove the sara wrapper script if it exists."""
    wrapper_paths = [
        Path.home() / ".local" / "bin" / "sara",
        Path("/usr/local/bin/sara"),
    ]
    ...
    for wrapper in wrapper_paths:
        if wrapper.exists():
            try:
                # Check if it's our wrapper (contains sara_cli reference)
                content = wrapper.read_text()
                if 'sara_cli' in content or 'sara-agent' in content:
                    wrapper.unlink()
...
def uninstall_gateway_service():
    ...
    - All platforms: standalone ``sara gateway run`` processes
    ...
    try:
        from sara_cli.gateway import kill_gateway_processes, find_gateway_pids
    ...
    # 3. macOS: uninstall launchd plist
        ...
        try:
            from sara_cli.gateway import get_launchd_plist_path
...
def _is_default_sara_home(sara_home: Path) -> bool:
    """Return True when ``sara_home`` points at the default (non-profile) root."""
    try:
        from sara_constants import get_default_sara_root
        return sara_home.resolve() == get_default_sara_root().resolve()
    except Exception:
        return False
...
def _uninstall_profile(profile) -> None:
    ...
    remove its alias wrapper, and wipe its sara_HOME directory.

    We shell out to ``sara -p <name> gateway stop|uninstall`` because
    service names, unit paths, and plist paths are all derived from the
    current sara_HOME and can't be easily switched in-process.
    """
    import sys as _sys
    name = profile.name
    ...
    #    Use `python -m sara_cli.main` so we don't depend on a `sara`
    #    wrapper that may be half-removed mid-uninstall.
    sara_invocation = [_sys.executable, "-m", "sara_cli.main", "--profile", name]
    for subcmd in ("stop", "uninstall"):
        try:
            subprocess.run(
                sara_invocation + ["gateway", subcmd],
                capture_output=True,
                text=True,
                timeout=60,
                check=False,
    ...
    # 3. Wipe the profile's sara_HOME directory.
    try:
...
def run_uninstall(args):
    ...
    Options:
    - Full uninstall: removes code + ~/.sara/ (configs, data, logs)
    - Keep data: removes code but keeps ~/.sara/ for future reinstall
    """
    project_root = get_project_root()
    sara_home = get_sara_home()

    # Detect named profiles when uninstalling from the default root —
    # offer to clean them up too instead of leaving zombie sara_HOMEs
    # and systemd units behind.
    is_default_profile = _is_default_sara_home(sara_home)
    named_profiles = _discover_named_profiles() if is_default_profile else []

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│            ⚕ sara Agent Uninstaller                  │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print(f"  Config:  {sara_home / 'config.yaml'}")
    print(f"  Secrets: {sara_home / '.env'}")
    print(f"  Data:    {sara_home / 'cron/'}, {sara_home / 'sessions/'}, {sara_home / 'logs/'}")
    print()
    ...
    # their alias wrappers, and wiping their sara_HOME dirs. Otherwise
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL sara data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the sara code but keep your configuration and data.")
    ...
    log_info("Removing sara command...")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.sara/, just remove the sara-agent subdir
            if sara_home in project_root.parents or project_root.parent == sara_home:
    ...
    # 5. Optionally remove ~/.sara/ data directory (and named profiles)
    if full_uninstall:
        # 5a. Stop and remove each named profile's gateway service and
        #     alias wrapper. The profile sara_HOME dirs live under
        ...
        try:
            if sara_home.exists():
                shutil.rmtree(sara_home)
                log_success(f"Removed {sara_home}")
        except Exception as e:
            log_warn(f"Could not fully remove {sara_home}: {e}")
            log_info("You may need to manually remove it")
    else:
        log_info(f"Keeping configuration and data in {sara_home}")
    ...
    if not full_uninstall:
        ...
        print(color("  curl -fsSL https://raw.githubusercontent.com/NexvisoraResearch/sara-agent/main/scripts/install.sh | bash", Colors.DIM))
        print()
    
    print(color("Reload your shell to complete the process:", Colors.YELLOW))
    print("  source ~/.bashrc  # or ~/.zshrc")
    print()
    print("Thank you for using sara Agent! ⚕")
    print()
