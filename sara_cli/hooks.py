"""sara hooks — inspect and manage shell-script hooks.

Usage::

    sara hooks list
    sara hooks test <event> [--for-tool X] [--payload-file F]
    sara hooks revoke <command>
    sara hooks doctor

Consent records live under ``~/.sara/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.sara/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``sara hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: sara hooks {list|test|revoke|doctor}")
        print("Run 'sara hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.sara/config.yaml.")
        print("See `sara hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `sara hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `sara hooks test` and `sara hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (sara wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `sara hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `sara hooks doctor` would execute
