...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or sara -c.",
    ...
    "Ctrl+Z suspends sara to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "sara -c resumes your most recent CLI session. sara -c \"project name\" resumes by title.",
    "sara -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "sara -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "sara chat -t web,terminal enables only specific toolsets for a focused session.",
    "sara chat -s github-pr-workflow preloads a skill at launch.",
    "sara chat -q \"query\" runs a single non-interactive query and exits.",
    "sara chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "sara chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "sara --yolo bypasses all dangerous command approval prompts for the entire session.",
    "sara chat --source telegram tags the session for filtering in sara sessions list.",
    "sara -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "sara doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "sara dump outputs a compact setup summary — great for bug reports.",
    "sara config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "sara config edit opens config.yaml in your default editor.",
    "sara config check scans for missing or stale configuration options.",
    "sara sessions browse opens an interactive session picker with search.",
    "sara sessions stats shows session counts by platform and database size.",
    "sara sessions prune --older-than 30 cleans up old sessions.",
    "sara skills search react --source skills-sh searches the skills.sh public directory.",
    "sara skills check scans installed hub skills for upstream updates.",
    "sara skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "sara skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "sara mcp add github --command npx adds MCP servers from the command line.",
    "sara mcp serve runs sara itself as an MCP server for other agents.",
    "sara auth add lets you add multiple API keys for credential pool rotation.",
    "sara completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "sara logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "sara backup creates a zip backup of your entire sara home directory.",
    "sara profile create coder creates an isolated profile that becomes its own command.",
    "sara profile create work --clone copies your current config and keys to a new profile.",
    "sara update syncs new bundled skills to ALL profiles automatically.",
    "sara gateway install sets up sara as a system service (systemd/launchd).",
    "sara memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "sara webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: sara tools disables unused tools, sara skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "sara models routes vision, compression, and aux tasks to cheaper models — cuts background token cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as sara -w).",
    ...
    "sara_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call sara tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'sara profile create coder' creates the 'coder' command.",
    "sara profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, sara prints a resume command with session ID and stats.",
    "sara sessions export backup.jsonl exports all sessions for backup or analysis.",
    "sara -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "sara skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .sara/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: sara cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "sara runs on 18 platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, email, and more.",
    "sara gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to sara via a local macOS server.",
    ...
    "Context files (.sara.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "SOUL.md at ~/.sara/SOUL.md is the agent's primary identity — customize it to shape behavior.",
    "sara loads project context from .sara.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    "MCP servers auto-generate toolsets at runtime — sara tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.sara/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "sara chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "sara plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "sara acp runs sara as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "sara_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "sara login supports OAuth-based auth for Nexvisoraand OpenAI Codex providers.",
    ...
    "sara status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.sara/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make sara your own.",
    "SOUL.md is auto-seeded with a default personality on first run. Edit ~/.sara/SOUL.md to customize.",
    ...
    "Before a gateway session reset, sara auto-flushes important facts to memory in the background.",
    ...
    "The skills audit log at ~/.sara/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Each profile gets its own subprocess HOME at sara_HOME/home/ — isolated git, ssh, npm, gh configs.",
    "sara_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in sara_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.sara/interrupt_debug.log with timestamps.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'sara profile create ls' would be rejected.",
    "sara profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "sara claw migrate --dry-run previews OpenClaw migration without writing anything.",
    ...
    "sara_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via sara tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.sara/.env.",
    "Skills from trusted repos (NexvisoraResearch) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.sara/skills/.hub/quarantine/ holds skills pending security review.",
]
