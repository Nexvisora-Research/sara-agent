...
def is_managed_tool_gateway_ready(_tool_name: str) -> bool:
    try:
        module = importlib.import_module("tools.managed_tool_gateway")
    except ModuleNotFoundError:
        return False
    helper = getattr(module, "is_managed_tool_gateway_ready", None)
    if callable(helper):
        return bool(helper(_tool_name))
    return False
...
def get_nexvisora_subscription_features(
    ...
    # Tool Gateway via `sara model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from sara_cli.setup import prompt_choice
    except Exception:
        return set()
