"""
Single source of truth for provider identity in sara Agent.
...
2. **sara overlays** — transport type, auth patterns, aggregator flags,
...
# -- sara overlay ----------------------------------------------------------
# sara-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class saraOverlay:
    """sara-specific provider metadata layered on top of models.dev."""
...
sara_OVERLAYS: Dict[str, saraOverlay] = {
    "openrouter": saraOverlay(
    ...
    "openai-codex": saraOverlay(
        transport="codex_responses",
        auth_type="oauth_external",
        base_url_override="https://chatgpt.com/backend-api/codex",
    ),
    "qwen-oauth": saraOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="sara_QWEN_BASE_URL",
    ),
    "google-gemini-cli": saraOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": saraOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "copilot-acp": saraOverlay(
        transport="codex_responses",
        auth_type="external_process",
        base_url_override="acp://copilot",
        base_url_env_var="COPILOT_ACP_BASE_URL",
    ),
    "github-copilot": saraOverlay(
        transport="openai_chat",
        extra_env_vars=("COPILOT_GITHUB_TOKEN", "GH_TOKEN"),
    ),
    "anthropic": saraOverlay(
    ...
    "alibaba-coding-plan": saraOverlay(
        transport="openai_chat",
    ...
    "opencode": saraOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": saraOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "ollama-cloud": saraOverlay(
        transport="openai_chat",
        base_url_env_var="OLLAMA_BASE_URL",
    ),
    "ollama-local": saraOverlay(
        transport="openai_chat",
...
class ProviderDef:
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "sara", "user-config"
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    overlay = sara_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + sara extra
    ...
    if overlay is not None:
        # sara-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in sara_OVERLAYS
    if provider == "bedrock":
