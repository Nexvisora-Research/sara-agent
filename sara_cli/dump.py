"""
Dump command for sara CLI.

Outputs a compact, plain-text summary of the user's sara setup
...
from sara_cli.config import get_sara_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``sara dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from sara_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(sara_home: Path) -> int:
...
def _cron_summary(sara_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = sara_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"sara_home:      {display_sara_home()}")
    ...
    toolsets = config.get("toolsets", ["sara-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(sara_home)}")
    lines.append(f"  skills:             {_count_skills(sara_home)}")
