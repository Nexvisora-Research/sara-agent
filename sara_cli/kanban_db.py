...
The board lives at ``$sara_HOME/kanban.db`` (profile-agnostic on purpose:
...
coding workloads.  See ``docs/sara-kanban-v1-spec.pdf`` for the full
...
def kanban_db_path() -> Path:
    """Return the path to ``kanban.db`` inside the active sara_HOME."""
    from sara_constants import get_sara_home
    return get_sara_home() / "kanban.db"


def workspaces_root() -> Path:
    """Return the directory under which ``scratch`` workspaces are created."""
    from sara_constants import get_sara_home
    return get_sara_home() / "kanban" / "workspaces"
...
def init_db(db_path: Optional[Path] = None) -> Path:
    ...
    Kept as a public entry point so CLI ``sara kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``sara --skills ...`` alongside the built-in
    ...
    # `sara --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``sara kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``sara kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``sara kanban complete <id>``) works without requiring
...
def resolve_workspace(task: Task) -> Path:
    ...
    - ``scratch``: a fresh dir under ``$sara_HOME/kanban/workspaces/<id>/``.
    - ``dir:<path>``: the path stored in ``workspace_path``.  Created
      ...
      root.  Users who want a sara_HOME-relative workspace should
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``sara kanban
...
def _default_spawn(task: Task, workspace: str) -> Optional[int]:
    """Fire-and-forget ``sara -p <profile> chat -q ...`` subprocess.
    ...
    env = dict(os.environ)
    if task.tenant:
        env["sara_TENANT"] = task.tenant
    env["sara_KANBAN_TASK"] = task.id
    env["sara_KANBAN_WORKSPACE"] = workspace
    # sara_PROFILE is the author the kanban_comment tool defaults to.
    # `sara -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["sara_PROFILE"] = task.assignee
    ...
    # Redirect output to a per-task log under sara_HOME/kanban/logs/.
    from sara_constants import get_sara_home
    log_dir = get_sara_home() / "kanban" / "logs"
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`sara` executable not found on PATH. "
            "Install sara Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``sara kanban daemon`` is systemd-friendly.
...
def worker_log_path(task_id: str) -> Path:
    ...
    from sara_constants import get_sara_home
    return get_sara_home() / "kanban" / "logs" / f"{task_id}.log"
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``~/.sara/profiles/`` directly so this module has no import
    dependency on ``sara_cli.profiles`` (which pulls in a large chunk
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``sara kanban assignees`` for the terminal.
