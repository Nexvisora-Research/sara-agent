...
- ``sara_cli.providers``        -- canonical provider identity + overlays
- ``sara_cli.model_normalize``  -- per-provider name formatting
...
from sara_cli.providers import (
...
_sara_MODEL_WARNING = (
    "Nexvisora Researchsara 3 & 4 models are NOT agentic and are not designed "
    "for use with sara Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nexvisora Researchsara 3 / sara 4 chat families.
# The previous substring check (`"sara" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``sara-brain:qwen3-14b-ctx16k`` that just
# happen to carry "sara" in their tag but are fully tool-capable.
#
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from sara_cli.model_switch import
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'sara model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'sara doctor' — config issues detected:"
    ...
    # Mirrors the same logic in sara_cli.runtime_provider.resolve_runtime_provider;
    ...
    sara_warn = _check_sara_model_warning(new_model)
    if sara_warn:
        warnings.append(sara_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from sara_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from sara_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Ollama local (localhost:11434) — probe common local endpoint and
    # surface a dedicated "ollama-local" provider row in the picker when
    # a local Ollama server is reachable. This makes switching to local
    # Ollama instances more discoverable in the /model modal.
    if "ollama-local" not in curated:
        try:
            ollama_base = os.environ.get("OLLAMA_BASE_URL", "http://127.0.0.1:11434")
            from sara_cli.models import fetch_api_models
            live_local = fetch_api_models(None, ollama_base, timeout=1.5) or []
        except Exception:
            live_local = []
        curated["ollama-local"] = live_local
    ...
    # --- 1. Check sara-mapped providers ---
    for sara_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(sara_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (sara_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from sara_cli.auth import _load_auth_store
                store = _load_auth_store()
                if store and sara_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a sara release.
        model_ids = curated.get(sara_id, [])
        if sara_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(sara_id, model_ids)
        total = len(model_ids)
    ...
    # Build reverse mapping: models.dev ID → sara provider ID.
    # sara_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use sara IDs ("copilot").
    _mdev_to_sara = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in sara_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve sara slug — e.g. "github-copilot" → "copilot"
        sara_slug = _mdev_to_sara.get(pid, pid)
        if sara_slug.lower() in seen_slugs:
            continue
        ...
        if not has_creds:
            try:
                ...
                if store and (
                    pid in providers_store or sara_slug in providers_store
                    or pid in pool_store or sara_slug in pool_store
                ):
                    has_creds = True
        ...
        if not has_creds:
            try:
                ...
                pool = load_pool(sara_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", sara_slug, exc)
        ...
        # configured.
        if not has_creds and sara_slug == "anthropic":
            try:
                ...
                if (sara_creds and sara_creds.get("accessToken")) or \
        ...
        if sara_slug in {"copilot", "copilot-acp"}:
            model_ids = provider_model_ids(sara_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                from bedrock_adapter import bedrock_model_ids_or_none
                _ids = bedrock_model_ids_or_none()
                model_ids = _ids if _ids is not None else (curated.get(sara_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(sara_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by sara slug, fall back to overlay key
            model_ids = curated.get(sara_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if sara_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(sara_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": sara_slug,
            "name": get_label(sara_slug),
            "is_current": sara_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "sara",
        ...
        seen_slugs.add(sara_slug.lower())
        _record_builtin_endpoint(sara_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or sara_OVERLAYS (keeps /model in sync
    # with `sara model`).
    try:
        from sara_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is sara's canonical write key (matches
            ...
            # sara writes ``models:`` as a dict keyed by model id
            # (see sara_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # sara's own writer uses; a hyphen variant is accepted
            ...
            # active model. sara's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, sara_cli/config.py) already consume that dict.
    ...
    # If we discovered a local Ollama catalog, append it as a visible
    # provider row (grouped endpoint) so the picker shows "Ollama (local)".
    try:
        if curated.get("ollama-local"):
            ol_models = curated.get("ollama-local") or []
            if ol_models:
                ol_top = ol_models[:max_models]
                results.append({
                    "slug": "ollama-local",
                    "name": "Ollama (local)",
                    "is_current": ("ollama-local" == current_provider) or ("11434" in (current_base_url or "")),
                    "is_user_defined": False,
                    "models": ol_top,
                    "total_models": len(ol_models),
                    "source": "ollama-local",
                })
                seen_slugs.add("ollama-local")
    except Exception:
        pass

    # Ensure a static 'Ollama (local)' provider row is present so the
    # picker shows a local Ollama option even when no server responds.
    try:
        if "ollama-local" not in seen_slugs:
            results.append({
                "slug": "ollama-local",
                "name": "Ollama (local)",
                "is_current": ("ollama-local" == current_provider) or ("11434" in (current_base_url or "")),
