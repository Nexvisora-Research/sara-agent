"""
sara Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m sara_cli.main web          # Start on http://127.0.0.1:9119
    python -m sara_cli.main web --port 8080
"""
...
WEB_DIST = Path(os.environ["sara_WEB_DIST"]) if "sara_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="sara Agent", version=__version__)
...
# or sara_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
# streamed to a per-action log file under ``~/.sara/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_sara_home() / "logs"

# Short ``name`` (from the URL) → absolute log file path.
_ACTION_LOG_FILES: Dict[str, str] = {
    "gateway-restart": "gateway-restart.log",
    "sara-update": "sara-update.log",
}
...
def _spawn_sara_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``sara <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``sara_cli.main`` module so the action
    ...
    cmd = [sys.executable, "-m", "sara_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "sara_NONINTERACTIVE": "1"},
    }
...
async def restart_gateway():
    """Kick off a ``sara gateway restart`` in the background."""
    try:
        proc = _spawn_sara_action(["gateway", "restart"], "gateway-restart")
...
@app.post("/api/sara/update")
async def update_sara():
    """Kick off ``sara update`` in the background."""
    try:
        proc = _spawn_sara_action(["update"], "sara-update")
    except Exception as exc:
        _log.exception("Failed to spawn sara update")
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sara supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def get_model_info():
    ...
    try:
        ...
        caps = {}
        try:
            from models_dev import get_model_capabilities
...
# in sara_cli/config.py — listed here for deterministic ordering in the UI.
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.sara/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    sara resolves Anthropic creds in this order at runtime:
    1. ``~/.sara/.anthropic_oauth.json`` — sara-managed PKCE flow
    ...
    try:
        from anthropic_adapter import (
            read_sara_oauth_credentials,
            read_claude_code_credentials,
            _sara_OAUTH_FILE,
        )
    except ImportError:
        read_claude_code_credentials = None  # type: ignore
        read_sara_oauth_credentials = None  # type: ignore
        _sara_OAUTH_FILE = None  # type: ignore

    sara_creds = None
    if read_sara_oauth_credentials:
        try:
            sara_creds = read_sara_oauth_credentials()
        except Exception:
            sara_creds = None
    if sara_creds and sara_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "sara_pkce",
            "source_label": f"sara PKCE ({_sara_OAUTH_FILE})",
            "token_preview": _truncate_token(sara_creds.get("accessToken")),
            "expires_at": sara_creds.get("expiresAt"),
            "has_refresh_token": bool(sara_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into sara even
    when they also have a separate sara-managed PKCE login.
    """
    try:
        from anthropic_adapter import read_claude_code_credentials
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "flow": "pkce",
        "cli_command": "sara auth add anthropic",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "sara auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "sara auth add qwen-oauth",
    ...
    },
    {
        ...
        "name": "MiniMax (OAuth)",
        "flow": "pkce",
        "cli_command": "sara auth add minimax-oauth",
...
async def list_oauth_providers():
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("sara_pkce", "claude_code", ...)
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    # Anthropic and claude-code clear the same sara-managed PKCE file
    # AND forget the Claude Code import. We don't touch ~/.claude/* directly
    # — that's owned by the Claude Code CLI; users can re-auth there if they
    # want to undo a disconnect.
    if provider_id in ("anthropic", "claude-code"):
        try:
            from anthropic_adapter import _sara_OAUTH_FILE
            if _sara_OAUTH_FILE.exists():
                _sara_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from sara_cli.auth import clear_provider_auth
...
# Guarded so sara web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both sara file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``sara auth add anthropic``.
    """
    from anthropic_adapter import _sara_OAUTH_FILE
    payload = {
    ...
    _sara_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _sara_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def get_logs(
    ...
    from sara_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_sara_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from sara_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    profiles: List[Dict[str, Any]] = []
    default_home = profiles_mod._get_default_sara_home()
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from sara_cli import profiles as profiles_mod
    try:
...
def _profile_setup_command(name: str) -> str:
    ...
    return "sara setup" if name == "default" else f"{name} setup"
...
async def get_skills():
    ...
    from sara_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from sara_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
#
# The endpoint spawns the same ``sara --tui`` binary the CLI uses, behind
...
from sara_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Default: whatever ``sara --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``sara_TUI_RESUME`` env var —
    matching what ``sara_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    `sidecar_url` (when set) is forwarded as ``sara_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).
    """
    from sara_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    if resume:
        env["sara_TUI_RESUME"] = resume

    if sidecar_url:
        env["sara_TUI_SIDECAR_URL"] = sidecar_url
...
# sara_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    def _serve_index():
        ...
        token_script = (
            f'<script>window.__sara_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__sara_DASHBOARD_EMBEDDED_CHAT__={chat_js};</script>"
        )
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",   "label": "sara Teal",  "description": "Classic dark teal — the canonical sara look"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.sara/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.sara/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_sara_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.sara/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as sara_cli.plugins):
    1. User plugins:    ~/.sara/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.sara/plugins/  (only if sara_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from sara_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_sara_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("sara_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".sara" / "plugins", "project"))
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from sara_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_sara_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from sara_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from sara_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sara_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from sara_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"sara_dashboard_plugin_{plugin['name']}"
...
def start_server(
    ...
    *,
    ...
    print(f"  sara Web UI → http://{host}:{port}")
