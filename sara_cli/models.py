...
Add, remove, or reorder entries here — both `sara setup` and
`sara` provider-selection will pick up the change automatically.
"""
...
_sara_USER_AGENT = f"sara-cli/{_sara_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `sara model`
    flow without maintaining a separate static list.
    """
    from sara_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $sara_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $sara_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. sara refreshes the cache from
    ...
    """
    try:
        from models_dev import _load_disk_cache
...
# this list:  sara model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `sara model` interactive picker
# ---------------------------------------------------------------------------
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``sara model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``sara auth add openai-codex`` without
    ``sara model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    sara-agent is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from sara_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — sara-agent
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from sara_constants import AI_GATEWAY_BASE_URL
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return sara-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; sara's picker expects
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``sara model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from sara_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def _resolve_static_model_alias(
    ...
    try:
        from sara_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to sara' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``sara_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether sara should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``sara auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.sara/.env``.
    ...
    """
    try:
        from sara_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        from sara_cli.auth import read_credential_pool
        from sara_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a sara release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a sara release). Falls back to the hardcoded catalog if no token
...
def _fetch_anthropic_models(timeout: float = 5.0) -> Optional[list[str]]:
    ...
    """
    try:
        from anthropic_adapter import resolve_anthropic_token, _is_oauth_token
    except ImportError:
        return None
    ...
    if is_oauth:
        ...
        from anthropic_adapter import _COMMON_BETAS, _OAUTH_ONLY_BETAS, _CONTEXT_1M_BETA
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _sara_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: sara' default Claude IDs elsewhere use
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _sara_USER_AGENT}
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_sara_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "custom":
        ...
        message = (
            ...
            f"sara will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so sara cannot verify the model name."
    ...
    if normalized == "bedrock":
        try:
            from bedrock_adapter import discover_bedrock_models, resolve_bedrock_region
