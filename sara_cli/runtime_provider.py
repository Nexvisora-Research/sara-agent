...
from sara_cli import auth as auth_mod
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from sara_cli.models import copilot_model_api_mode
    ...
    *,
    ...
    if provider == "openai-codex":
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from sara_cli.models import azure_foundry_model_api_mode
        ...
        if provider in ("opencode-zen", "opencode-go"):
            ...
            # Refs #16878.
            from sara_cli.models import opencode_model_api_mode
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("sara_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'sara model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.sara/.env or run 'sara model' to configure."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "anthropic":
        ...
        api_key = explicit_api_key
        if not api_key:
            from anthropic_adapter import resolve_anthropic_token
    ...
    *,
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "sara-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
    ...
    if provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (sara canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most sara-compatible importers).
