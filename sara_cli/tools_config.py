"""
Unified tool configuration for sara Agent.

`sara tools` and `sara setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.sara/config.yaml under
...
from sara_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _sara_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `sara tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``sara tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from sara_cli.plugins import discover_plugins, get_plugin_toolsets
...
from sara_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "sara_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "sara_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "sara_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "sara_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "sara_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_sara_home()}/sara-agent && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in sara-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/NexvisoraResearch/sara-agent:latest"
            )
            return
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.sara/config.yaml")

    elif post_setup_key == "spotify":
        # Run the full `sara auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.sara/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from sara_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: sara auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `sara auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: sara auth spotify")
        except Exception as exc:
        ...
        try:
            from sara_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: sara plugins enable observability/langfuse")
        _print_info("    Restart sara for tracing to take effect.")
        _print_info("    Verify: sara plugins list")
...
def _get_platform_tools(
    ...
    *,
    ...
    # "sara-cli" (which include all _sara_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "sara-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `sara tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"sara-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("sara-")}
    ...
    # shipped as a bundled plugin but user must opt in via `sara tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `sara tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by sara tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (sara-cli, sara-telegram, etc.)
    ...
    # Opening `sara tools` is the user's opt-in to reconfigure tools, so treat
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `sara tools` and `sara setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print()
        return
    print(color("⚕ sara Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: https://sara-agent.NexvisoraResearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    print()
    from sara_constants import display_sara_home
    print(color(f"  Tool configuration saved to {display_sara_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'sara' or gateway restart.", Colors.DIM))
    print()
