"""
Profile management for multiple isolated sara instances.

Each profile is a fully independent sara_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.sara/profiles/<name>/`` by default.

The "default" profile is ``~/.sara`` itself — backward compatible,
zero migration needed.

Usage::

    sara profile create coder          # fresh profile + bundled skills
    sara profile create coder --clone  # also copy config, .env, SOUL.md, skills
    sara profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    sara -p coder chat                 # or via flag
    sara profile use coder             # set as sticky default
    sara profile delete coder          # remove profile + alias + service
"""
...
_PROFILE_DIRS = [
    ...
    # See sara_constants.get_subprocess_home() and issue #4426.
    "home",
]
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.sara`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.sara) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "sara-agent",         # repo checkout (multi-GB)
...
_RESERVED_NAMES = frozenset({
    "sara", "default", "test", "tmp", "root", "sudo",
})

# sara subcommands that cannot be used as profile names/aliases
_sara_SUBCOMMANDS = frozenset({
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the sara root, NOT to the current sara_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where sara_HOME points outside
    ``~/.sara``, profiles live under ``sara_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_sara_home() / "profiles"


def _get_default_sara_home() -> Path:
    """Return the default (pre-profile) sara_HOME path.

    In standard deployments this is ``~/.sara``.
    In Docker/custom deployments where sara_HOME is outside ``~/.sara``
    (e.g. ``/opt/data``), returns sara_HOME directly.
    """
    from sara_constants import get_default_sara_root
    return get_default_sara_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_sara_home() / "active_profile"
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its sara_HOME directory."""
    if name == "default":
        return _get_default_sara_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, sara subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _sara_SUBCOMMANDS:
        return f"'{name}' conflicts with a sara subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_sara_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.sara)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.sara/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches sara_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "sara_HOME": str(profile_dir)},
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.sara).\n"
            "To remove everything, use: sara uninstall"
        )
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set sara_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("sara_HOME")
    try:
        os.environ["sara_HOME"] = str(profile_dir)
        from sara_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if old_home is not None:
            os.environ["sara_HOME"] = old_home
        elif "sara_HOME" in os.environ:
            del os.environ["sara_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.sara/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: sara profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from sara_HOME.

    Returns ``"default"`` if sara_HOME is not set or points to ``~/.sara``.
    Returns the profile name if sara_HOME points into ``~/.sara/profiles/<name>``.
    Returns ``"custom"`` if sara_HOME is set to an unrecognized path.
    """
    from sara_constants import get_sara_home
    sara_home = get_sara_home()
    resolved = sara_home.resolve()

    default_resolved = _get_default_sara_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.sara itself — its parent is ~/ and its
        # directory name is ".sara", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: sara profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.sara itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.sara). "
            "Specify a different name: sara profile import <archive> --name <name>"
        )
    ...
    with tempfile.TemporaryDirectory(prefix="sara_profile_import_") as tmpdir:
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"sara.{old_name}"
    new_host = f"sara.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_sara_home() / "honcho.json",
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for sara profile names."""
    return '''# sara Agent profile completion
# Add to ~/.bashrc: eval "$(sara completion bash)"

_sara_profiles() {
    local profiles_dir="$HOME/.sara/profiles"
...
_sara_completion() {
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_sara_profiles)" -- "$cur"))
        return
    fi
...
complete -F _sara_completion sara
'''


def generate_zsh_completion() -> str:
    """Generate a zsh completion script for sara profile names."""
    return '''#compdef sara
# sara Agent profile completion
# Add to ~/.zshrc: eval "$(sara completion zsh)"

_sara() {
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.sara/profiles" ]]; then
        profiles+=("${(@f)$(ls $HOME/.sara/profiles 2>/dev/null)}")
    fi
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a sara_HOME path string.

    Called early in the CLI entry point, before any sara modules
    are imported, to set the sara_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: sara profile create {profile_name}"
        )
