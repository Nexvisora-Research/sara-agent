"""
Gateway subcommand for sara CLI.

Handles: sara gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
# display_sara_home is imported lazily at call sites to avoid ImportError
# when sara_constants is cached from a pre-update version during `sara update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "sara-gateway*",
...
def _request_gateway_self_restart(pid: int) -> bool:
    """Ask a running gateway ancestor to restart itself asynchronexvisoraly."""
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    pids: list[int] = []
    patterns = [
        "sara_cli.main gateway",
        "sara_cli.main --profile",
        "sara_cli.main -p",
        "sara_cli/main.py gateway",
        "sara_cli/main.py --profile",
        "sara_cli/main.py -p",
        "sara gateway",
        "gateway/run.py",
    ]
    current_home = str(get_sara_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"sara_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "sara_HOME=" in command and f"sara_HOME={current_home}" not in command:
            return False
        return True
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``sara update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            sara profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to sara profiles via PID files."""
    _exclude = set(exclude_pids or set())
    processes: list[ProfileGatewayProcess] = []
    try:
        from gateway.status import get_running_pid
        from sara_cli.profiles import list_profiles
    except Exception:
        return processes
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "sara_cli.main"]
    if profile != "default":
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}sara gateway status\n"
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from sara_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `sara gateway`")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (sara_HOME-scoped).
...
from sara_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "sara-gateway"
SERVICE_DESCRIPTION = "sara Agent Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current sara_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (sara_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from sara_constants import get_default_sara_root
    home = get_sara_home().resolve()
    default = get_default_sara_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary sara_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(sara_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when sara_HOME is a named profile.

    For ``~/.sara/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        sara_home: Optional explicit sara_HOME path. Defaults to the current
            ``get_sara_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from sara_constants import get_default_sara_root
    home = Path(sara_home or str(get_sara_home())).resolve()
    default = get_default_sara_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this sara_HOME.

    Default ``~/.sara`` returns ``sara-gateway`` (backward compatible).
    Profile ``~/.sara/profiles/coder`` returns ``sara-gateway-coder``.
    Any other sara_HOME appends a short hash for uniqueness.
    """
...
# Legacy service names from older sara installs that predate the
# sara-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (sara-gateway-*.service) and unrelated third-party
# "sara" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("sara.service",)
...
def _find_legacy_sara_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy sara gateway units.

    Detects unit files installed by older sara versions that used a
    different service name (e.g. ``sara.service`` before the rename to
    ``sara-gateway.service``). When both a legacy unit and the current
    ``sara-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``sara-gateway-coder.service`` and unrelated third-party
      ``sara-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``sara.service`` running an unrelated
...
def has_legacy_sara_units() -> bool:
    """Return True when any legacy sara gateway unit files exist."""
    return bool(_find_legacy_sara_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy sara gateway unit files if any are installed.

    Idempotent: prints nothing when no legacy units are detected. Safe to
    call from any status/install/setup path.
    """
    legacy = _find_legacy_sara_units()
    if not legacy:
        return
    print_warning("Legacy sara gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current sara-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    sara gateway migrate-legacy")


def remove_legacy_sara_units(
    interactive: bool = True,
    dry_run: bool = False,
) -> tuple[int, list[Path]]:
    """Stop, disable, and remove legacy sara gateway unit files.

    Iterates over whatever ``_find_legacy_sara_units()`` returns — which is
    ...
    legacy = _find_legacy_sara_units()
    if not legacy:
        print("No legacy sara gateway units found.")
        return 0, []
    ...
    print()
    print("Legacy sara gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: sara gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo sara gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    sara gateway uninstall")
    print_info("    sudo sara gateway uninstall --system")
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so sara can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo sara gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo sara gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo sara gateway start --system")
            return scope, False
...
def _launchd_user_home() -> Path:
    ...
    Profile-mode sara often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.sara`` → ``ai.sara.gateway.plist`` (backward compatible).
    Profile ``~/.sara/profiles/coder`` → ``ai.sara.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.sara.gateway-{suffix}" if suffix else "ai.sara.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.sara/sara-agent  -> /home/alice/.sara/sara-agent
      /opt/sara                 -> /opt/sara  (kept as-is)
...
def _sara_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current sara_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_sara_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.sara                    → /home/alice/.sara
      /root/.sara/profiles/coder     → /home/alice/.sara/profiles/coder
      /opt/custom-sara               → /opt/custom-sara  (kept as-is)
    """
    current_sara = get_sara_home().resolve()
    current_default = (Path.home() / ".sara").resolve()
    target_default = Path(target_home_dir) / ".sara"

    # Default ~/.sara → remap to target user's default
    if current_sara == current_default:
        return str(target_default)

    # Profile or subdir of ~/.sara → preserve the relative structure
    try:
        relative = current_sara.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.sara) — keep as-is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        sara_home = _sara_home_for_target_user(home_dir)
        profile_arg = _profile_arg(sara_home)
...
Environment="sara_HOME={sara_home}"
Restart=always
RestartSec=60
...
"""
    ...
    sara_home = str(get_sara_home().resolve())
    profile_arg = _profile_arg(sara_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current sara install")
    return True
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (sara.service from pre-rename installs)
    # before installing the new sara-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}sara gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}sara gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        # unit immediately so `sara gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}sara gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}sara gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}sara gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}sara gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.sara.gateway-{suffix}" if suffix else "ai.sara.gateway"
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current sara install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  sara gateway status             # Check status")
    from sara_constants import display_sara_home as _dhh
...
def launchd_stop():
    ...
    # `sara gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current sara install")
    else:
        print("⚠ Service definition is stale relative to the current sara install")
        print("  Run: sara gateway start")
    ...
    if deep:
        log_file = get_sara_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and sara will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @sara:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. sara) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where sara delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your sara agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address sara will use (e.g., sara@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. sara connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: sara pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and sara will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``sara setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.sara/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_sara_home() / "whatsapp" / "session" / "creds.json"
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'sara pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: sara pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'sara gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from sara_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from sara_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. sara will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. sara will store the returned account_id/token in ~/.sara/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `sara gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_sara_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `sara pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `sara pairing approve`.")
    elif access_idx == 1:
...
def _setup_signal():
    ...
    if shutil.which("signal-cli"):
        ...
        print_info("    signal-cli link -n \"saraAgent\"")
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``sara_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from sara_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.sara/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.sara/.env: {', '.join(required)}")
    else:
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: sara gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: sara gateway install")
                else:
                    print_info("  You can install later: sara gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo sara gateway install --system")
                    print_info("  Or run in foreground:  sara gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: sara gateway run")
                print_info("  For persistence:   tmux new -s sara 'sara gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: sara gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup sara gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: sara gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'sara gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: sara gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: sara gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s sara 'sara gateway run'")
                print()
            ...
            print()
            print("  sara gateway run                              # direct foreground")
            print("  tmux new -s sara 'sara gateway run'         # persistent via tmux")
            print("  nohup sara gateway run > ~/.sara/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: sara gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: sara gateway run")
            sys.exit(1)
        ...
        if is_termux():
            ...
            print("Stop manual runs with: sara gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: sara gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: sara gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                ...
                if is_termux():
                    ...
                    print("  sara gateway install")
                    print("  sudo sara gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  sara gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup sara gateway run > ~/.sara/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s sara 'sara gateway run'         # persistent via tmux")
                    print("  nohup sara gateway run > ~/.sara/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  sara gateway install  # Install as user service")
                    print("  sudo sara gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy sara gateway unit files from
        # pre-rename installs (e.g. sara.service). Profile units and
        # unrelated third-party services are never touched.
        ...
        if not supports_systemd_services() and not is_macos():
            ...
            return
        remove_legacy_sara_units(interactive=not yes, dry_run=dry_run)
