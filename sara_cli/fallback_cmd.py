"""
sara fallback — manage the fallback provider chain.
...
Subcommands:
  sara fallback [list]   Show the current fallback chain (default when no subcommand)
  sara fallback add      Pick provider + model via the same picker as `sara model`,
                           then append the selection to the chain
  sara fallback remove   Pick an entry to delete from the chain
  sara fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.sara/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from sara_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `sara model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  sara fallback add")
        print()
        return
    ...
    print("  Docs: https://sara-agent.NexvisoraResearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `sara model`, then append the selection to the chain."""
    from sara_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `sara model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `sara fallback list` to view, or `sara fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``sara fallback [subcommand]``."""
