"""
Doctor command for sara CLI.

Diagnoses issues with sara Agent setup.
"""
...
from sara_cli.config import get_project_root, get_sara_home, get_env_path
from sara_constants import display_sara_home

PROJECT_ROOT = get_project_root()
sara_HOME = get_sara_home()
_DHH = display_sara_home()  # user-facing display path (e.g. ~/.sara or ~/.sara/profiles/coder)

# Load environment variables from ~/.sara/.env so API key checks work
...
from sara_cli.models import _sara_USER_AGENT
from sara_cli.vercel_auth import describe_vercel_auth
from sara_constants import OPENROUTER_MODELS_URL
...
from sara_constants import is_termux as _is_termux
...
def _has_provider_env_config(content: str) -> bool:
    """Return True when ~/.sara/.env contains provider auth/base URL settings."""
...
def run_doctor(args):
    ...
    # checks (like cronjob management) should see the same context as `sara`.
    os.environ.setdefault("sara_INTERACTIVE", "1")
    ...
    print(color("│                 🩺 sara Doctor                        │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Check ~/.sara/.env (primary location for user config)
    env_path = sara_HOME / '.env'
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'sara setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'sara setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'sara setup' to create one")
                issues.append("Run 'sara setup' to create .env")
    
    # Check ~/.sara/config.yaml (primary) or project cli-config.yaml (fallback)
    config_path = sara_HOME / 'config.yaml'
    if config_path.exists():
        ...
        try:
            ...
            try:
                ...
                pass
            try:
                from sara_cli.config import get_compatible_custom_providers as _compatible_custom_providers
            ...
            if provider and provider != "auto":
                if catalog_provider is None or (
                    ...
                    issues.append(
                        ...
                        f"Fix: run 'sara config set model.provider <valid_provider>'"
                    )
            ...
            if runtime_provider and runtime_provider not in ("auto", "custom", "openrouter"):
                try:
                    from sara_cli.auth import PROVIDER_REGISTRY, get_auth_status
                    ...
                    if pconfig and getattr(pconfig, "auth_type", "") == "api_key":
                        ...
                        if not configured:
                            check_fail(
                                ...
                                "(check ~/.sara/.env or run 'sara setup')",
                            )
                            issues.append(
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'sara setup' or set the provider's API key in {_DHH}/.env, "
                                f"or switch providers with 'sara config set model.provider <name>'"
                            )
                except Exception:
                    pass
    ...
    if config_path.exists():
        try:
            from sara_cli.config import check_config_version, migrate_config
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'sara setup' to migrate config")
                else:
                    issues.append("Run 'sara doctor --fix' or 'sara setup' to migrate config")
            else:
        ...
        try:
            ...
            if stale_root_keys:
                ...
                if should_fix:
                    ...
                    issues.append("Stale root-level provider/base_url in config.yaml — run 'sara doctor --fix'")
        except Exception:
            pass
    ...
    if _safe_which("codex"):
        ...
        # Native OAuth uses sara' own device-code flow — the Codex CLI is
        # only needed if you want to import existing tokens from
        # ~/.codex/auth.json.  Downgrade to info so users running
        # `sara auth openai-codex` aren't told they're missing something.
        check_info(
    ...
    if sara_home.exists():
        ...
        if should_fix:
            sara_home.mkdir(parents=True, exist_ok=True)
    ...
    for subdir_name in expected_subdirs:
        subdir_path = sara_home / subdir_name
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give sara a custom personality)")
        if should_fix:
            soul_path.parent.mkdir(parents=True, exist_ok=True)
            soul_path.write_text(
                "# sara Agent Persona\n\n"
                "<!-- Edit this file to customize how sara communicates. -->\n\n"
    ...
    memories_dir = sara_home / "memories"
    ...
    state_db_path = sara_home / "state.db"
    ...
    wal_path = sara_home / "state.db-wal"
    if wal_path.exists():
        try:
            ...
            if wal_size > 50 * 1024 * 1024:  # 50 MB
                ...
                if should_fix:
                    ...
                    issues.append("Large WAL file — run 'sara doctor --fix' to checkpoint")
    ...
    # =========================================================================
    # Check: Command installation (sara bin symlink)
    # =========================================================================
    if sys.platform != "win32":
        ...
        for _venv_name in ("venv", ".venv"):
            _candidate = PROJECT_ROOT / _venv_name / "bin" / "sara"
        ...
        _cmd_link = _cmd_link_dir / "sara"

        if _venv_bin is None:
            check_warn(
                "Venv entry point not found",
                "(sara not in venv/bin/ or .venv/bin/ — reinstall with pip install -e '.[all]')"
            )
            ...
            if _cmd_link.is_symlink():
                ...
                if _target == _expected:
                    check_ok(f"{_cmd_link_display}/sara → correct target")
                else:
                    check_warn(
                        f"{_cmd_link_display}/sara points to wrong target",
                        f"(→ {_target}, expected → {_expected})"
                    )
                    if should_fix:
                        _cmd_link.unlink()
                        _cmd_link.symlink_to(_venv_bin)
                        check_ok(f"Fixed symlink: {_cmd_link_display}/sara → {_venv_bin}")
                        fixed_count += 1
                    else:
                        issues.append(f"Broken symlink at {_cmd_link_display}/sara — run 'sara doctor --fix'")
            elif _cmd_link.exists():
                # It's a regular file, not a symlink — possibly a wrapper script
                check_ok(f"{_cmd_link_display}/sara exists (non-symlink)")
            else:
                check_fail(
                    f"{_cmd_link_display}/sara not found",
                    "(sara command may not work outside the venv)"
                )
                if should_fix:
                    _cmd_link_dir.mkdir(parents=True, exist_ok=True)
                    _cmd_link.symlink_to(_venv_bin)
                    check_ok(f"Created symlink: {_cmd_link_display}/sara → {_venv_bin}")
                    fixed_count += 1
                    ...
                    issues.append(f"Missing {_cmd_link_display}/sara symlink — run 'sara doctor --fix'")
    ...
    if terminal_env == "vercel_sandbox":
        ...
        if importlib.util.find_spec("vercel") is not None:
            ...
            check_fail("vercel SDK not installed", "(pip install 'sara-agent[vercel]')")
            issues.append("Install the Vercel optional dependency: pip install 'sara-agent[vercel]'")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                issues.append(
                    ...
                    "Fix: run 'sara config set model.provider <provider>' to switch providers, "
    ...
    try:
        from bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'sara setup' to configure missing API keys for full tool access")
    except Exception as e:
    ...
    if hub_dir.exists():
        ...
        check_warn("Skills Hub directory not initialized", "(run: sara skills list)")
    ...
    try:
        import yaml as _yaml
        _mem_cfg_path = sara_HOME / "config.yaml"
    ...
    if not _active_memory_provider:
        ...
        try:
            ...
            if not _honcho_cfg_path.exists():
                check_warn("Honcho config not found", "run: sara memory setup")
            elif not hcfg.enabled:
                check_info(f"Honcho disabled (set enabled: true in {_honcho_cfg_path} to activate)")
            elif not (hcfg.api_key or hcfg.base_url):
                check_fail("Honcho API key or base URL not set", "run: sara memory setup")
                issues.append("No Honcho API key — run 'sara memory setup'")
            else:
        ...
        try:
            ...
            if mem0_key:
                ...
                check_fail("Mem0 API key not set", "(set MEM0_API_KEY in .env or run sara memory setup)")
        ...
        try:
            ...
            if _provider and _provider.is_available():
                ...
                check_warn(f"{_active_memory_provider} configured but not available", "run: sara memory status")
            else:
                check_warn(f"{_active_memory_provider} plugin not found", "run: sara memory setup")
        except Exception as _e:
    ...
    # =========================================================================
    try:
        from sara_cli.profiles import list_profiles, _get_wrapper_dir, profile_exists
        import re as _re
            ...
            if wrapper_dir.is_dir():
                for wrapper in wrapper_dir.iterdir():
                    ...
                    try:
                        ...
                        if "sara -p" in content:
                            _m = _re.search(r"sara -p (\S+)", content)
    ...
    if should_fix and fixed_count > 0:
        ...
        print()
        if not should_fix:
            print(color("  Tip: run 'sara doctor --fix' to auto-fix what's possible.", Colors.DIM))
    else:
        print(color("─" * 60, Colors.GREEN))
