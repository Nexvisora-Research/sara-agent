"""
Interactive setup wizard for sara Agent.
...
_AGENTS_DIR = Path(__file__).parent.parent.resolve() / ".agents"
...
_DOCS_BASE = "https://sara-agent.NexvisoraResearch.com/docs"
...
# display_sara_home imported lazily at call sites (stale-module safety during sara update)
...
from sara_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("⚕ sara Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure sara using environment variables or config commands:")
    print_info("  sara config set model.provider custom")
    print_info("  sara config set model.base_url http://localhost:8080/v1")
    print_info("  sara config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'sara setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from sara_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'sara setup')")


def _print_setup_summary(config: dict, sara_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'sara setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from sara_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'sara setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'sara setup tts'"))
    else:
    ...
    # Spotify (OAuth via sara auth spotify — check auth.json, not env vars)
    try:
        from sara_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'sara setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {sara_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('sara setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('sara setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('sara setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('sara setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('sara setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('sara config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('sara config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('sara config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('sara', Colors.GREEN)}              Start chatting")
    print(f"   {color('sara gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('sara doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `sara tools` and `sara setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``sara model``)
    ...
    provider added to ``sara model`` is automatically available here.
    ...
    # Delegate to the shared sara model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from sara_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: sara model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from sara_cli.auth_commands import auth_add_command
            ...
            print_info(
                "sara can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one sara can auto-use for vision,")
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"sara setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'sara setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where sara runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using sara via Telegram/Discord, this is where")
        print_info(
        ...
        try:
            ...
            def is_managed_tool_gateway_ready(_tool_name: str) -> bool:
                return False
        ...
        print_info("Requires the optional SDK: pip install 'sara-agent[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'sara-agent[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into sara_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("sara_MAX_ITERATIONS")
    ...
    print_info("  Run `sara setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # sara_MAX_ITERATIONS from agent.max_turns at startup.
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where sara delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `sara update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://sara-agent.NexvisoraResearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under sara_HOME, and print
    ...
    """
    try:
        from sara_cli.slack_cli import _build_full_manifest
        from sara_constants import get_sara_home

        manifest = _build_full_manifest(
            bot_name="sara",
            bot_description="Your sara agent on Slack",
        )
        target = Path(get_sara_home()) / "slack-manifest.json"
        ...
        print_info(
            "   Re-run `sara slack manifest --write` anytime to refresh after "
            "sara adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where sara delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where sara delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  sara config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects sara to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from sara_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://sara-agent.NexvisoraResearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://sara-agent.NexvisoraResearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  sara config edit")
    print_info("   Open config in your editor:  sara config edit")
...
def setup_gateway(config: dict):
    ...
    from sara_cli.gateway import _all_platforms, _platform_status, _configure_platform
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     sara config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_sara_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  sara gateway")
        else:
            from sara_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   sara gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   sara gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `sara setup tools` and `sara tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    """
    try:
        from sara_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from sara_cli.config
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_sara migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_sara", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and sara.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure sara to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point sara at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point sara at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point sara at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point sara at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to sara equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing sara config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in sara.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ sara's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(sara_home: Path) -> bool:
    ...
    print_info("sara can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: sara claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: sara claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing sara configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing sara config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in sara (use sara claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      sara setup           — full or quick (auto-detected)
      sara setup model     — just model/provider
      sara setup tts       — just text-to-speech
      sara setup terminal  — just terminal backend
      sara setup gateway   — just messaging platforms
      sara setup tools     — just tool configuration
      sara setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ⚕ sara Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ⚕ sara Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your sara Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, sara_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have sara configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'sara setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(sara_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up sara?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, sara_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {sara_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'sara config edit'")
    ...
    save_config(config)
    _print_setup_summary(config, sara_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch sara chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, sara_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'sara setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    sara setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  sara setup gateway")
    print()
...
def _run_quick_setup(config: dict, sara_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'sara setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect sara to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'sara setup gateway'.")
