"""
Configuration management for sara Agent.

Config files are stored in ~/.sara/ for easy access:
- ~/.sara/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.sara/.env         - API keys and secrets

This module provides:
- sara config          - Show current configuration
- sara config edit     - Open config in editor
- sara config set      - Set a specific value
- sara config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `sara plugins enable
    # observability/langfuse` or `sara tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "sara_LANGFUSE_ENV",
    "sara_LANGFUSE_RELEASE",
    "sara_LANGFUSE_SAMPLE_RATE",
    "sara_LANGFUSE_MAX_CHARS",
    "sara_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
...
from sara_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("sara_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_sara_home() / ".managed"
...
def is_managed() -> bool:
    """Check if sara is running in package-manager-managed mode.

    Two signals: the sara_MANAGED env var (set by the systemd service),
    or a .managed marker file in sara_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade sara-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "sara update"


def format_managed_message(action: str = "modify this sara installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("sara_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this sara installation is managed by NixOS "
            f"(sara_MANAGED={env_hint}).\n"
            "Edit services.sara-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this sara installation is managed by Homebrew "
            f"(sara_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade sara-agent"
        )

    return (
        f"Cannot {action}: this sara installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall sara."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from sara_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, sara_bin
    or None if container mode is not active, we're already inside the
    container, or sara_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("sara_DEV") == "1":
        return None
    ...
    container_mode_file = get_sara_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "sara-agent")
    exec_user = info.get("exec_user", "sara")
    sara_bin = info.get("sara_bin", "/data/current-package/bin/sara")
...
# Re-export from sara_constants — canonical definition lives there.
from sara_constants import get_sara_home  # noqa: F811,E402
from utils import atomic_replace

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_sara_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_sara_home() / ".env"
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the sara group can
    share state with the gateway service.

    The mode can be overridden via the sara_HOME_MODE environment variable
    (e.g. sara_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse sara_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("sara_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When sara runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("sara_CONTAINER") or os.environ.get("sara_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set sara_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into sara_HOME if the user doesn't have one yet."""
...
def ensure_sara_home():
    """Ensure ~/.sara directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_sara_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_sara_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"sara_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the sara-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, sara auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), sara sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when sara
        ...
        # bundled sara image, which drops to the `sara` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, sara sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.sara/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, sara sweeps at startup (at most once per
        # min_interval_hours) and deletes:
        #   * orphan repos: sara_WORKDIR no longer exists on disk
    ...
    # single read_file page exceeds these limits, sara truncates the
    ...
    "auxiliary": {
        ...
        # model; override via `sara model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `sara --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `sara -c` muscle memory.  Default off so existing
        # users aren't surprised.  sara_TUI_RESUME=<id> always wins.
        ...
        # e.g. `model · 68% · ~/projects/sara`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.sara/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.sara/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # response. If not, sara feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before sara auto-pauses the goal and
    ...
    # always goes to ~/.sara/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${sara_SKILL_DIR} and ${sara_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `sara curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.sara/skills/ into
        # ~/.sara/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `sara curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for sara-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "⚕ *sara Agent*" header.
    ...
    # stored approval from ~/.sara/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or sara_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via sara_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `sara -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     sara-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.sara/logs/.
    ...
    "model_catalog": {
        "enabled": True,
        "url": "https://sara-agent.NexvisoraResearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `sara model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.sara/state.db.
    ...
    "sessions": {
        ...
        # default of ``sara sessions prune``.
        "retention_days": 90,
    ...
    # ``sara update`` behaviour.
    "updates": {
        # Run a full ``sara backup``-style zip of sara_HOME before every
        # ``sara update``.  Backups land in ``<sara_HOME>/backups/`` and
        # can be restored with ``sara import <path>``.  Off by default —
        # on large sara_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'sara model' for endpoint-specific config)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @sara:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. sara_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #sara)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: sara-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'sara-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote sara API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote sara API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote sara API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # sara_TOOL_PROGRESS and sara_TOOL_PROGRESS_MODE are deprecated —
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.sara.config`` entries, then checks
    ...
    try:
        ...
        # should never break `sara update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older sara versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``sara_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'sara setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — sara won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'sara doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("sara_HOME", "~/.sara")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("sara_TOOL_PROGRESS")
            old_mode = get_env_value("sara_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from sara_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from sara_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("sara_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from sara_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$sara_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_sara_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `sara plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `sara curator status` has no
    ...
    #   3. Creates `~/.sara/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_sara_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_sara_home() / "logs" / "curator"
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in ("y", "yes"):
                ...
                print("  Set later with: sara config set <key> <value>")
    ...
    # metadata.sara.config in their SKILL.md frontmatter.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _sara_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.sara/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.sara/config.yaml.
    ...
    (which change ``sara_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_sara_home()
    config_path = get_config_path()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.sara/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.sara/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real sara env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.sara/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.sara/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.sara/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.sara/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to sara — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known sara vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.sara/.env or environment."""
...
def show_config():
    ...
    print(color("│              ⚕ sara Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(color("  sara config edit     # Edit config file", Colors.DIM))
    print(color("  sara config set <key> <value>", Colors.DIM))
    print(color("  sara setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: sara config set <key> <value>")
            print()
            print("Examples:")
            print("  sara config set model anthropic/claude-sonnet-4")
            print("  sara config set terminal.backend docker")
            print("  sara config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'sara config migrate' to add them")
        ...
        print("  sara config           Show current configuration")
        print("  sara config edit      Open config in editor")
        print("  sara config set <key> <value>   Set a config value")
        print("  sara config check     Check for missing/outdated config")
        print("  sara config migrate   Update config with new options")
        print("  sara config path      Show config file path")
        print("  sara config env-path  Show .env file path")
        sys.exit(1)
