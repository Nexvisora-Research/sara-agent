"""CLI subcommand: `sara curator <subcommand>`.
...
def _cmd_run(args) -> int:
    ...
    if dry:
        print(
            ...
            "`sara curator status` and run `sara curator run` (no flag) to apply."
        )
    return 0
...
def _cmd_backup(args) -> int:
    ...
    if snap is None:
        ...
        return 1
    print(f"curator: snapshot created at ~/.sara/skills/.curator_backups/{snap.name}")
    return 0
...
def _cmd_rollback(args) -> int:
    ...
    if target_path is None:
        ...
        if not rows:
            print(
                ...
                "`sara curator backup` or wait for the next curator run."
            )
        else:
            print(
    ...
    print(
        "\nThis will replace the current ~/.sara/skills/ tree (a safety "
...
# ---------------------------------------------------------------------------
# argparse wiring (called from sara_cli.main)
# ---------------------------------------------------------------------------
...
def register_cli(parent: argparse.ArgumentParser) -> None:
    ...
    p_backup = subs.add_parser(
        "backup",
        help="Take a manual tar.gz snapshot of ~/.sara/skills/ "
    ...
    p_rollback = subs.add_parser(
        "rollback",
        help="Restore ~/.sara/skills/ from a curator snapshot "
...
def cli_main(argv=None) -> int:
    """Standalone entry (also usable by sara_cli.main fallthrough)."""
    parser = argparse.ArgumentParser(prog="sara curator")
    register_cli(parser)
