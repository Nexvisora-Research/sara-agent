...
# Add .agents to path for credential_pool and related modules
...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "nexvisora", "openai-codex", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "nexvisora", "openai-codex", "qwen-oauth", "google-gemini-cli", "minimax-oauth"} else AUTH_TYPE_API_KEY
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_sara_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:sara_pkce",
    ...
    if provider == "nexvisora":
        creds = auth_mod._nexvisora_device_code_login(
        ...
        # Honor `--label <name>` so Nexvisoramatches other providers' UX.  The
        # helper embeds this into providers.Nexvisoraso that label_from_token
        # doesn't overwrite it on every subsequent load_pool("nexvisora").
        custom_label = (getattr(args, "label", None) or "").strip() or None
        entry = auth_mod.persist_nexvisora_credentials(creds, label=custom_label)
    ...
    if provider == "openai-codex":
        # Clear any existing suppression marker so a re-link after `sara auth
    ...
    if provider == "google-gemini-cli":
        from google_oauth import run_gemini_oauth_login_pure
    ...
    if provider == "minimax-oauth":
        from sara_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    raise SystemExit(f"`sara auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source sara reads from
    # (env vars, external OAuth files, auth.json blocks, custom config)
    # has a RemovalStep registered in credential_sources.  The step
    ...
    from credential_sources import find_removal_step
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `sara auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `sara auth` is called bare."""
