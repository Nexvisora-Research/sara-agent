...
- **Aggregators** (OpenRouter, nexvisora, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  sara revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, nexvisora, etc.).
#
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to sara' canonical ids."""
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical sara provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``sara_cli.models.normalize_provider()``.
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from sara_cli.models import normalize_copilot_model_id
