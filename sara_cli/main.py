#!/usr/bin/env python3
"""
sara CLI - Main entry point.

Usage:
    sara                     # Interactive chat (default)
    sara chat                # Interactive chat
    sara gateway             # Run gateway in foreground
    sara gateway start       # Start gateway as service
    sara gateway stop        # Stop gateway service
    sara gateway status      # Show gateway status
    sara gateway install     # Install gateway service
    sara gateway uninstall   # Uninstall gateway service
    sara setup               # Interactive setup wizard
    sara logout              # Clear stored authentication
    sara status              # Show status of all components
    sara cron                # Manage cron jobs
    sara cron list           # List cron jobs
    sara cron status         # Check if cron scheduler is running
    sara doctor              # Check configuration and dependencies
    sara honcho setup                    # Configure Honcho AI memory integration
    sara honcho status                   # Show Honcho config and connection status
    sara honcho sessions                 # List directory → session name mappings
    sara honcho map <name>               # Map current directory to a session name
    sara honcho peer                     # Show peer names and dialectic settings
    sara honcho peer --user NAME         # Set user peer name
    sara honcho peer --ai NAME           # Set AI peer name
    sara honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    sara honcho mode                     # Show current memory mode
    sara honcho mode [hybrid|honcho|local]  # Set memory mode
    sara honcho tokens                   # Show token budget settings
    sara honcho tokens --context N       # Set session.context() token cap
    sara honcho tokens --dialectic N     # Set dialectic result char cap
    sara honcho identity                 # Show AI peer identity representation
    sara honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    sara honcho migrate                  # Step-by-step migration guide: OpenClaw native → sara + Honcho
    sara version             Show version
    sara update              Update to latest version
    sara uninstall           Uninstall sara Agent
    sara acp                 Run as an ACP server for editor integration
    sara sessions browse     Interactive session picker with search

    sara claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to sara_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (sara tools, sara setup, sara model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'sara {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any sara module import.
#
# Many modules cache sara_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("sara_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.sara/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set sara_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If sara_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("sara_HOME"):
        return

    # 2. If no flag, check active_profile in the sara root
    if profile_name is None:
        try:
            from sara_constants import get_default_sara_root

            active_path = get_default_sara_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set sara_HOME
    if profile_name is not None:
        try:
            from sara_cli.profiles import resolve_profile_env

            sara_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent sara from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["sara_HOME"] = sara_home
...
# Load .env from ~/.sara/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from sara_cli.config import get_sara_home
from sara_cli.env_loader import load_sara_dotenv

load_sara_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → sara_REDACT_SECRETS env
# var BEFORE sara_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "sara_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_sara_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["sara_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `sara` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from sara_logging import setup_logging as _setup_logging
...
try:
    from sara_cli.config import load_config as _load_config_early
    from sara_constants import apply_ipv4_preference as _apply_ipv4
...
from sara_cli import __version__, __release_date__
from sara_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from sara_cli.config import get_env_path, get_sara_home, load_config
    from sara_cli.auth import get_auth_status

    # Determine whether sara itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from sara_cli.config import DEFAULT_CONFIG
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_sara_config = _model_name and _model_name != _DEFAULT_MODEL

    # Check env vars (may be set by .env or shell).
    # OPENAI_BASE_URL alone counts — local models (vLLM, llama.cpp, etc.)
    # often don't require an API key.
    from sara_cli.auth import PROVIDER_REGISTRY
    ...
    # Only count these if sara has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants sara to use their tokens.
    if _has_sara_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, sara_bin
        cli_args: the original CLI arguments (everything after 'sara')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    sara_bin = container_info["sara_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo sara {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo sara {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  sara --tui --resume {target}")
    if title:
        print(f'  sara --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @sara/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@sara" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: sara_TUI_DIR first, else repo ui-tui."""
    env = os.environ.get("sara_TUI_DIR")
    if env:
        p = Path(env)
...
def _tui_build_needed(tui_dir: Path) -> bool:
    if _sara_ink_bundle_stale(tui_dir):
        return True
...
def _sara_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "sara-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``sara_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("sara_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    sara_home = os.environ.get("sara_HOME") or str(Path.home() / ".sara")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "sara_HOME": sara_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(sara_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (sara_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("sara_NODE")
    ...
    # pre-built dist + node_modules (nix / full sara_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = os.environ.get("sara_TUI_DIR")
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
    ...
    if _tui_need_npm_install(tui_dir):
        if not os.environ.get("sara_QUIET"):
    ...
    if tui_dev:
        if _sara_ink_bundle_stale(tui_dir):
            result = subprocess.run(
                [npm, "run", "build", "--prefix", "packages/sara-ink"],
                cwd=str(tui_dir),
                capture_output=True,
                text=True,
            )
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from sara_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="sara-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["sara_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["sara_PYTHON_SRC_ROOT"] = os.environ.get(
        "sara_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("sara_PYTHON", sys.executable)
    env.setdefault("sara_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["sara_MODEL"] = model
        env["sara_INFERENCE_MODEL"] = model
    if provider:
        env["sara_TUI_PROVIDER"] = provider
        env["sara_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["sara_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["sara_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("sara_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'sara sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like sara isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  sara setup")
        print()

        from sara_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'sara setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from sara_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["sara_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.sara/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["sara_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["sara_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["sara_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from sara_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from sara_cli.config import get_env_value, save_env_value
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with sara?")
        print()
    ...
    session_dir = get_sara_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: sara gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  sara gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ sara Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  sara gateway")
        ...
        print()
        print("  Or install as a service: sara gateway install")
    else:
        print("⚠ Pairing may not have completed. Run 'sara whatsapp' to try again.")


def cmd_setup(args):
    """Interactive setup wizard."""
    from sara_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``sara model``) and the setup wizard
    ...
    from sara_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("sara_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'sara model' for "
                "available providers, or run 'sara doctor' to diagnose config "
                "issues."
            )
    ...
    from sara_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from sara_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.sara/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.sara/.env if the active provider is not 'custom'.
    ...
    """
    from sara_cli.config import get_env_value, save_env_value, load_config
...
#
# sara uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `sara model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `sara model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``sara model`` flow, then route aux tasks to them here.
    """
    from sara_cli.config import load_config
    from sara_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from sara_cli.auth import _prompt_model_selection
    from sara_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from sara_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from sara_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from sara_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from sara_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `sara auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from sara_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from sara_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from sara_cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        try:
            from google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from sara_cli.auth import _save_model_choice, deactivate_provider
    from sara_cli.config import get_env_value, load_config, save_config
    ...
    from sara_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `sara model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from sara_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from sara_cli.config import get_env_value, save_env_value, load_config, save_config
    from sara_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  sara will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from sara_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from sara_cli.config import save_env_value, load_config, save_config
    from sara_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from sara_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from sara_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates sara turns to `copilot --acp`.")
    print("  sara currently starts its own ACP subprocess for each request.")
    print("  sara uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set sara_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so sara knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from gemini_native_adapter import probe_gemini_tier
        except Exception:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   sara typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with sara, enable billing on your "
    ...
    if provider_id == "lmstudio":
        from sara_cli.auth import AuthError
        from sara_cli.models import fetch_lmstudio_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from sara_constants import display_sara_home as _dhh_fn

            print(
                f"    sara will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from sara_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'sara model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate sara CLI with a provider."""
    from sara_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from sara_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from sara_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from sara_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from sara_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from sara_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``sara slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: sara slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `sara slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from sara_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from sara_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from sara_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from sara_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from sara_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from sara_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from sara_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up sara home directory to a zip file."""
    if getattr(args, "quick", False):
        from sara_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from sara_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a sara backup from a zip file."""
    from sara_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"sara Agent v{__version__} ({__release_date__})")
...
def cmd_uninstall(args):
    """Uninstall sara Agent."""
    _require_tty("uninstall")
    from sara_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``sara update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``sara_cli/web_dist/`` (per vite.config.ts
    outDir: "../sara_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "sara_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``sara update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``sara web``).
    ...
    if r1.returncode != 0:
        print(
            ...
            + ("" if fatal else " (sara web will not be available)")
        )
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``sara dashboard`` processes other than ourselves.

    ``sara dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``sara update`` replaces files on disk, the running
    ...
    patterns = [
        "sara dashboard",
        "sara_cli.main dashboard",
        "sara_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "sara.*dashboard"`) keeps us consistent
            # with `sara_cli.gateway._scan_gateway_pids` and avoids the
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `sara update`.
    ...
    print("  Preview now:  sara curator run --dry-run")
    print("  Pause it:     sara curator pause")
    print("  Docs:         https://sara-agent.NexvisoraResearch.com/docs/user-guide/features/curator")


def _kill_stale_dashboard_processes(
    reason: str = "the running backend no longer matches the updated frontend",
) -> None:
    """Kill running ``sara dashboard`` processes.

    Called at the end of ``sara update`` (default ``reason``) and also
    from ``sara dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    sara dashboard --port <port>")
...
def _update_via_zip(args):
    """Update sara Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/NexvisoraResearch/sara-agent/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="sara-update-")
        zip_path = os.path.join(tmp_dir, f"sara-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to sara-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"sara-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "sara-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if sara behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes sara completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but sara couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but sara couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if sara behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `sara update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/NexvisoraResearch/sara-agent.git",
    "git@github.com:NexvisoraResearch/sara-agent.git",
    "https://github.com/NexvisoraResearch/sara-agent",
    "git@github.com:NexvisoraResearch/sara-agent",
}
OFFICIAL_REPO_URL = "https://github.com/NexvisoraResearch/sara-agent.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_sara_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_sara_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official sara repository.")
        print("  This means you may miss updates from NexvisoraResearch/sara-agent.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/NexvisoraResearch/sara-agent.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/NexvisoraResearch/sara-agent.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``sara update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from sara_constants import get_default_sara_root

    default_home = get_default_sara_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "sara-agent[matrix]" or "package-name[extra]".
...
class _UpdateOutputStream:
    """Stream wrapper used during ``sara update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.sara/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``sara update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``sara update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.sara/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``sara update --gateway``) the update is already
    ...
    try:
        ...
        # sara_cli.config.get_sara_home to simulate setup failure.
        from sara_cli.config import get_sara_home as _get_sara_home

        logs_dir = _get_sara_home() / "logs"
...
def _cmd_update_check():
    """Implement ``sara update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``sara update`` without requiring a reinstall.  The
    ...
    ``sara`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/sara.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v sara'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/sara, code at /usr/local/lib/sara-agent).
    fhs_link = Path("/usr/local/bin/sara")
    ...
    path_comment = (
        "# sara Agent — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of sara_HOME before running the update.
    ...
    sara_HOME directories.  The ``--backup`` flag on ``sara update``
    ...
    try:
        from sara_cli.backup import create_pre_update_backup
    except Exception as exc:
    ...
    # Render path using display_sara_home so the user sees ~/.sara/...
    try:
        from sara_constants import get_sara_home, display_sara_home
        home = get_sara_home()
        try:
            display_path = f"{display_sara_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  sara import {out_path}")
...
def cmd_update(args):
    """Update sara Agent to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from sara_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update sara Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_sara_home added to sara_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload sara_constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_sara_home() added since the last release.
        try:
            import importlib
            import sara_constants as _hc
        ...
        if needs_migration:
            ...
            if assume_yes:
                ...
                print(
                    "    Run 'sara config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if (gateway_mode or assume_yes) and missing_env:
                    print("  ℹ API keys require manual entry: sara config migrate")
            else:
                print()
                print("Skipped. Run 'sara config migrate' later to configure.")
        else:
        ...
        # When running as ``sara update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_sara_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            # Discover all sara-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. sara-gateway.service or sara-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: sara gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: sara -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy sara gateway unit files are still installed.
        # When both sara.service (from a pre-rename install) and the
        # current sara-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `sara update` surfaces the issue until the user migrates.
        try:
            from sara_cli.gateway import (
                has_legacy_sara_units,
                _find_legacy_sara_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_sara_units():
                print()
                print("⚠ Legacy sara gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_sara_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (sara.service) fight the current")
                print("  sara-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    sara gateway migrate-legacy")
                print()
        ...
        print("  sara model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``sara -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    from sara_constants import display_sara_home

    action = getattr(args, "profile_action", None)

    if action is None:
        # Bare `sara profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_sara_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → sara -p {p.name}")
                break
        print()
        return
    ...
    if action == "setup":
        ...
        profile_name = getattr(args, "profile_name", "default") or "default"
        try:
            from memory.user_profile import interactive_onboard, get_profile_summary
        except Exception:
            print("Onboarding helper not available.")
            return
        interactive_onboard(profile_name)
        print("\nProfile summary:\n")
        print(get_profile_summary(profile_name))
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.sara)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  sara profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     sara -p {name} chat")
                else:
        ...
        from sara_cli.profiles import profile_exists
        ...
        from sara_cli.profiles import rename_profile
        ...
        from sara_cli.profiles import export_profile
        ...
        from sara_cli.profiles import import_profile
...
def _report_dashboard_status() -> int:
    """Print ``sara dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``sara dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No sara dashboard processes running.")
        return 0

    print(f"{len(pids)} sara dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No sara dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `sara update`.
    ...
    if "sara_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from sara_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("sara_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from sara_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter sara log files."""
    from sara_cli.logs import tail_log, list_logs
...
def main():
    ...
    from sara_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from sara_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://sara-agent.NexvisoraResearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `sara model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy sara.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy sara gateway unit files "
            "(e.g. sara.service) left over from older installs. Profile "
            "units (sara-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure sara Agent with an interactive wizard. "
        "Run a specific section: sara setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'sara setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for sara.",
    )
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $sara_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "sara")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for sara CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: sara-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate sara with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set sara_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of sara Agent components",
    )
    ...
    # =========================================================================
    from sara_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.sara/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.sara/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with sara Agent setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your sara setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for sara Agent. Use 'sara debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sara debug share              Upload debug report and print URL
    sara debug share --lines 500  Include more log lines
    sara debug share --expire 30  Keep paste for 30 days
    sara debug share --local      Print report locally (no upload)
    sara debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'sara debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up sara home directory to a zip file",
        description="Create a zip archive of your entire sara configuration, "
        "skills, sessions, and data (excludes the sara-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/sara-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a sara backup from a zip file",
        description="Extract a previously created sara backup into your "
        "sara home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage sara Agent configuration",
    )
    ...
    def cmd_pairing(args):
        from sara_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.sara/skills/.bundled_manifest) "
            "so future 'sara update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from sara_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from sara_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/sara-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `sara plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from sara_cli.plugins_cmd import plugins_command
    ...
    try:
        from sara_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_sara_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_sara_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_sara_home()}/memories/\n")
        else:
            from sara_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'sara tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # sara tools disable <name...> [--platform cli]
    ...
    # sara tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from sara_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from sara_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run sara as an MCP server",
        description=(
            "Manage MCP server connections and run sara as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'sara mcp add' to connect to a new server, or\n"
            "'sara mcp serve' to expose sara conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run sara as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from sara_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_sara_home() / "sessions"
            ...
            # Launch sara --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from sara_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to sara",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to sara",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.sara/ (by default a "
        "single restore-point archive is written to ~/.sara/backups/ "
        "before apply; restorable with 'sara import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from sara_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update sara Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'sara config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall sara Agent",
        description="Remove sara Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run sara Agent as an ACP (Agent Client Protocol) server",
        description="Start sara Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch sara Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated sara instances",
    )
    ...
    # Interactive setup helper for profiles
    profile_setup = profile_subparsers.add_parser(
        "setup", help="Interactively configure a profile (onboarding)"
    )
    profile_setup.add_argument(
        "profile_name",
        nargs="?",
        default="default",
        help="Profile to configure (default: default)",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the sara Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `sara --tui` via PTY/WebSocket). "
            "Alternatively set sara_DASHBOARD_TUI=1."
        ),
    )
    ...
    # for `sara dashboard` cmdlines and SIGTERM them directly — the same
    # path `sara update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running sara dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running sara dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter sara log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    sara logs                    Show last 50 lines of agent.log
    sara logs -f                 Follow agent.log in real time
    sara logs errors             Show last 50 lines of errors.log
    sara logs gateway -n 100     Show last 100 lines of gateway.log
    sara logs --level WARNING    Only show WARNING and above
    sara logs --session abc123   Filter by session ID
    sara logs --component tools  Only show tool-related lines
    sara logs --since 1h         Lines from the last hour
    sara logs --since 30m -f     Follow, starting from 30 min ago
    sara logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``sara -c Pokemon Agent Dev`` → ``sara -c 'Pokemon Agent Dev'``
    ...
    from sara_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'sara -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (sara hooks list, cron
    ...
    if (
        ...
        try:
            from sara_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
        ...
        try:
            from sara_cli.config import load_config
            from shell_hooks import register_from_config
    ...
    if getattr(args, "oneshot", None):
        from sara_cli.oneshot import run_oneshot
    ...
    # Execute the command
    # If running interactively and the active profile hasn't been onboarded,
    # offer to run the interactive profile setup. This only triggers for
    # interactive agent entry points (chat/default) so management commands
    # aren't interrupted.
    try:
        from sara_cli.profiles import get_active_profile_name
        from memory.user_profile import is_onboarded, interactive_onboard
    except Exception:
        get_active_profile_name = None
        is_onboarded = None
        interactive_onboard = None

    try:
        active_profile = get_active_profile_name() if get_active_profile_name else "default"
        if (
            sys.stdin.isatty()
            and is_onboarded is not None
            and not is_onboarded(active_profile)
            and (args.command is None or args.command == "chat")
        ):
            try:
                resp = input("It looks like your profile is not set up. Run interactive setup now? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                resp = "n"
            if resp in ("", "y", "yes"):
                try:
                    interactive_onboard(active_profile)
                except Exception:
                    print("Interactive setup failed; continue without onboarding.")
...
def cli_main():
    """Console-script entrypoint kept for backward compatibility."""
    main()
