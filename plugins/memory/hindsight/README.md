...
sara spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.sara/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p sara ui start
```
...
Config file: `~/.sara/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `sara` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `sara-{profile}` isolates memory per active sara profile. Empty placeholders collapse cleanly (e.g. `sara-{user}` with no user becomes `sara`). |
...
| `retain_async` | `true` | Process retain asynchronexvisoraly on the Hindsight server |
...
The LLM API key is stored in `~/.sara/.env` as `HINDSIGHT_LLM_API_KEY`.
