...
Config file: `$sara_HOME/supermemory.json`

| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `sara` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags (e.g. `sara-{identity}` → `sara-coder`). |
...
Use `{identity}` in the `container_tag` to scope memories per sara profile:

```json
{
  "container_tag": "sara-{identity}"
}
```

For a profile named `coder`, this resolves to `sara-coder`. The default profile resolves to `sara-default`. Without `{identity}`, all profiles share the same container.
