...
def _load_supermemory_config(sara_home: str) -> dict:
    config = _default_config()
    config_path = Path(sara_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, sara_home: str) -> None:
    config_path = Path(sara_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `sara memory setup`.
        # All other options are documented for $sara_HOME/supermemory.json
    ...
    def save_config(self, values, sara_home):
        ...
        _save_supermemory_config(sanitized, sara_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from sara_constants import get_sara_home
        self._sara_home = kwargs.get("sara_home") or str(get_sara_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._sara_home)
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "sara", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "sara_memory", "target": target, "type": "explicit_memory"},
