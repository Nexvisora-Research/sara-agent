...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/sara>
...
```bash
sara honcho setup    # full interactive wizard (cloud or local)
sara memory setup    # generic picker, also works
```

Or manually:
```bash
sara config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.sara/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$sara_HOME/honcho.json` | Profile-local (isolated sara instances) |
| 2 | `~/.sara/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active sara profile: `sara` (default) or `sara.<profile>`.
...
| 4 | `per-session` strategy | sara session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`sara-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`sara`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-sara-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening sara in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — sara session ID (timestamp + hex). Every `sara` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple sara profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`sara`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active sara profile: `sara` (default) or `sara.<profile>` (e.g. `sara -p coder` → host key `sara.coder`).
...
| `sara_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `sara honcho setup` | Full interactive setup wizard |
| `sara honcho status` | Show resolved config for active profile |
| `sara honcho enable` / `disable` | Toggle Honcho for active profile |
| `sara honcho mode <mode>` | Change recall or observation mode |
| `sara honcho peer --user <name>` | Update user peer name |
| `sara honcho peer --ai <name>` | Update AI peer name |
| `sara honcho tokens --context <N>` | Set context token budget |
| `sara honcho tokens --dialectic <N>` | Set dialectic max chars |
| `sara honcho map <name>` | Map current directory to a session name |
| `sara honcho sync` | Create host blocks for all sara profiles |
