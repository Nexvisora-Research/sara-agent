...
Handles: sara honcho setup | status | sessions | map | peer
"""
...
def cmd_enable(args) -> None:
    ...
    cfg = _read_config()
    host = _host_key()
    label = f"[{host}] " if host != "sara" else ""
...
def cmd_sync(args) -> None:
    ...
    Scans all sara profiles and creates host blocks for any that don't
    ...
    cfg = _read_config()
    if not cfg:
        print("  No Honcho config found. Run 'sara honcho setup' first.\n")
        return
    ...
    if not default_block and not has_key:
        print("  Honcho not configured on default profile. Run 'sara honcho setup' first.\n")
        return
...
def sync_honcho_profiles_quiet() -> int:
    ...
    Called from `sara update` -- no output, no exceptions.
    """
    try:
        from sara_cli.profiles import list_profiles
        profiles = list_profiles()
...
def _host_key() -> str:
    """Return the active Honcho host key, derived from the current sara profile."""
...
def _local_config_path() -> Path:
    ...
    Always returns $sara_HOME/honcho.json so each profile/instance gets
    ...
    """
    return get_sara_home() / "honcho.json"
...
def cmd_setup(args) -> None:
    ...
    print("\nHoncho memory setup\n" + "─" * 40)
    print("  Honcho gives sara persistent cross-session memory.")
    ...
    sara_host = hosts.setdefault(_host_key(), {})
    ...
    if is_local:
        ...
        if not cfg.get("apiKey"):
            ...
            print("  Run 'sara honcho setup' again once you have a key.\n")
            return

    # --- 3. Identity ---
    current_peer = sara_host.get("peerName") or cfg.get("peerName", "")
    new_peer = _prompt("Your name (user peer)", default=current_peer or os.getenv("USER", "user"))
    if new_peer:
        sara_host["peerName"] = new_peer

    current_ai = sara_host.get("aiPeer") or cfg.get("aiPeer", "sara")
    new_ai = _prompt("AI peer name", default=current_ai)
    if new_ai:
        sara_host["aiPeer"] = new_ai

    current_workspace = sara_host.get("workspace") or cfg.get("workspace", "sara")
    new_workspace = _prompt("Workspace ID", default=current_workspace)
    if new_workspace:
        sara_host["workspace"] = new_workspace

    # --- 4. Observation mode ---
    current_obs = sara_host.get("observationMode") or cfg.get("observationMode", "directional")
    print("\n  Observation mode:")
    ...
    if new_obs in ("unified", "directional"):
        sara_host["observationMode"] = new_obs
    else:
        sara_host["observationMode"] = "directional"

    # --- 5. Write frequency ---
    current_wf = str(sara_host.get("writeFrequency") or cfg.get("writeFrequency", "async"))
    print("\n  Write frequency:")
    ...
    try:
        sara_host["writeFrequency"] = int(new_wf)
    except (ValueError, TypeError):
        sara_host["writeFrequency"] = new_wf if new_wf in ("async", "turn", "session") else "async"

    # --- 6. Recall mode ---
    _raw_recall = sara_host.get("recallMode") or cfg.get("recallMode", "hybrid")
    ...
    if new_recall in ("hybrid", "context", "tools"):
        sara_host["recallMode"] = new_recall

    # --- 7. Context token budget ---
    current_ctx_tokens = sara_host.get("contextTokens") or cfg.get("contextTokens")
    ...
    if new_ctx_tokens.strip().lower() in ("none", "uncapped", "no limit"):
        sara_host.pop("contextTokens", None)
        ...
        try:
            ...
            if val >= 0:
                sara_host["contextTokens"] = val
        except (ValueError, TypeError):
            pass  # keep current

    # --- 7b. Dialectic cadence ---
    current_dialectic = str(sara_host.get("dialecticCadence") or cfg.get("dialecticCadence") or "2")
    print("\n  Dialectic cadence:")
    ...
    try:
        ...
        if val >= 1:
            sara_host["dialecticCadence"] = val
    except (ValueError, TypeError):
        sara_host["dialecticCadence"] = 2

    # --- 7c. Dialectic reasoning level ---
    current_reasoning = (
        sara_host.get("dialecticReasoningLevel")
    ...
    if new_reasoning in ("minimal", "low", "medium", "high", "max"):
        sara_host["dialecticReasoningLevel"] = new_reasoning
    else:
        sara_host["dialecticReasoningLevel"] = "low"

    # --- 8. Session strategy ---
    current_strat = sara_host.get("sessionStrategy") or cfg.get("sessionStrategy", "per-session")
    print("\n  Session strategy:")
    ...
    if new_strat in ("per-session", "per-repo", "per-directory", "global"):
        sara_host["sessionStrategy"] = new_strat

    sara_host["enabled"] = True
    sara_host.setdefault("saveMessages", True)
    ...
    try:
        ...
        sara_config = load_config()
        sara_config.setdefault("memory", {})["provider"] = "honcho"
        save_config(sara_config)
        print("  Memory provider set to 'honcho' in config.yaml")
    except Exception as e:
        print(f"  Could not auto-enable in config.yaml: {e}")
        print("  Run: sara config set memory.provider honcho")
    ...
    print("\n  Other commands:")
    print("    sara honcho status     -- show full config")
    print("    sara honcho mode       -- change recall/observation mode")
    print("    sara honcho tokens     -- tune context and dialectic budgets")
    print("    sara honcho peer       -- update peer names")
    print("    sara honcho map <name> -- map this directory to a session name\n")


def _active_profile_name() -> str:
    """Return the active sara profile name (respects --target-profile override)."""
    if _profile_override:
...
def cmd_status(args) -> None:
    ...
    try:
        ...
        print("  honcho-ai is not installed. Run: sara honcho setup\n")
        return
    ...
    if not cfg:
        ...
        print("  Run 'sara honcho setup' to configure.\n")
        return
...
def cmd_sessions(args) -> None:
    ...
    if not sessions:
        ...
        print("  Add one with: sara honcho map <session-name>")
...
def cmd_peer(args) -> None:
    ...
    if user_name is None and ai_name is None and reasoning is None:
        # Show current values
        hosts = cfg.get("hosts", {})
        sara = hosts.get(_host_key(), {})
        user = sara.get('peerName') or cfg.get('peerName') or '(not set)'
        ai = sara.get('aiPeer') or cfg.get('aiPeer') or _host_key()
        lvl = sara.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        max_chars = sara.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        print("\nHoncho peers\n" + "─" * 40)
        print(f"  User peer:   {user}")
        print("    Your identity in Honcho. Messages you send build this peer's card.")
        print(f"  AI peer:     {ai}")
        print("    sara' identity in Honcho. Seed with 'sara honcho identity <file>'.")
...
def cmd_mode(args) -> None:
    ...
    if mode_arg is None:
        ...
        print(f"\n  Set with: sara honcho mode [hybrid|context|tools]\n")
        return
...
def cmd_strategy(args) -> None:
    ...
    if strat_arg is None:
        ...
        print(f"\n  Set with: sara honcho strategy [per-session|per-directory|per-repo|global]\n")
        return
...
def cmd_tokens(args) -> None:
    ...
    if context is None and dialectic is None:
        ctx_tokens = sara.get("contextTokens") or cfg.get("contextTokens") or "(Honcho default)"
        d_chars = sara.get("dialecticMaxChars") or cfg.get("dialecticMaxChars") or 600
        d_level = sara.get("dialecticReasoningLevel") or cfg.get("dialecticReasoningLevel") or "low"
        print("\nHoncho budgets\n" + "─" * 40)
        print()
        ...
        print("    AI-to-AI inference. sara asks Honcho's AI peer a question")
        ...
        print("\n  Set with: sara honcho tokens [--context N] [--dialectic N]\n")
        return
...
def cmd_identity(args) -> None:
    ...
    if not _resolve_api_key(cfg):
        print("  No API key configured. Run 'sara honcho setup' first.\n")
        return
    ...
    if show:
        ...
        if ai_rep.get("representation"):
            ...
            print("  Run 'sara honcho identity <file>' to seed one.")
        print()
        return
    ...
    if not file_path:
        ...
        print(f"  AI peer:   {hcfg.ai_peer}")
        print()
        print("    sara honcho identity --show        — show both peer representations")
        print("    sara honcho identity <file>        — seed AI peer from SOUL.md or any .md/.txt\n")
        return
...
def cmd_migrate(args) -> None:
    """Step-by-step migration guide: OpenClaw native memory → sara + Honcho."""
    from pathlib import Path
    ...
    print("\nHoncho migration: OpenClaw native memory → sara\n" + "─" * 50)
    print()
    ...
    if has_key:
        ...
        print("  Honcho is a cloud memory service that gives sara persistent memory")
        print("  across sessions. You need an API key to use it.")
        print()
        print("  1. Get your API key at https://app.honcho.dev")
        print("  2. Run:  sara honcho setup")
        print("     Paste the key when prompted.")
        print()
        answer = _prompt("  Run 'sara honcho setup' now?", default="y")
        if answer.lower() in ("y", "yes"):
            ...
            print()
            print("  Run 'sara honcho setup' when ready, then re-run this walkthrough.")
    ...
    if user_files or agent_files:
        ...
        print("  or seed them manually:  sara honcho identity <path/to/file>")
    ...
    if user_files:
        ...
        print()
        print("  These are picked up automatically the first time you run 'sara'")
        print("  with Honcho configured and no prior session history.")
        print("  (sara calls migrate_memory_files() on first session init.)")
        print()
        print("  If you want to migrate them now without starting a session:")
        for f in user_files:
            print("    sara honcho migrate  — this step handles it interactively")
        if has_key:
            ...
            print("  Run 'sara honcho setup' first, then re-run this step.")
    else:
    ...
    print()
    print("  In sara, they are seeded once into Honcho's AI peer through the")
    ...
    if agent_files:
        ...
        if has_key:
            ...
            print("  Run 'sara honcho setup' first, then seed manually:")
            for f in agent_files:
                print(f"    sara honcho identity {f}")
    else:
        print("  No agent identity files detected.")
        print("  To seed manually:  sara honcho identity <path/to/SOUL.md>")
    ...
    print("    sara:   cloud-backed Honcho peers. Files can stay on disk as source")
    print("              of truth; Honcho holds the live representation.")
    print()
    print("  Context injection")
    print("    OpenClaw: file excerpts injected synchronexvisoraly before each LLM call.")
    print("    sara:   Honcho context fetched async at turn end, injected next turn.")
    ...
    print("    sara:   Honcho observes every message and updates representations")
    ...
    print("    sara:   per-session by default — each run gets its own session")
    print("              Map a custom name:  sara honcho map <session-name>")
    ...
    print()
    if not has_key:
        print("  1. sara honcho setup              — configure API key (required)")
        print("  2. sara honcho migrate            — re-run this walkthrough")
    else:
        print("  1. sara honcho status             — verify Honcho connection")
        print("  2. sara                           — start a session")
        print("     (user memory files auto-uploaded on first turn if not done above)")
        print("  3. sara honcho identity --show    — verify AI peer representation")
        print("  4. sara honcho tokens             — tune context and dialectic budgets")
        print("  5. sara honcho mode               — view or change memory mode")
    print()
...
def honcho_command(args) -> None:
    ...
    if sub == "setup":
        ...
        print("  Running 'sara memory setup'...\n")
        from sara_cli.memory_setup import cmd_setup_provider
...
def register_cli(subparser) -> None:
    """Build the ``sara honcho`` argparse subcommand tree.

    Called by the plugin CLI registration system during argparse setup.
    The *subparser* is the parser for ``sara honcho``.
    """
    ...
    subs.add_parser(
        "setup",
        help="Initial Honcho setup (redirects to sara memory setup)",
    )
    ...
    subs.add_parser(
        "migrate",
        help="Step-by-step migration guide from openclaw-honcho to sara Honcho",
    )
