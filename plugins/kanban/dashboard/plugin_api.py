...
``sara_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
bridge in ``sara_cli/web_server.py``. If you run the dashboard with
...
from sara_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from sara_cli import web_server as _ws
    except Exception:
...
def create_task(payload: CreateTaskBody):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from sara_cli.kanban import _check_dispatcher_presence
...
class UpdateTaskBody(BaseModel):
    ...
    # transitions to 'done'. Dashboard parity with ``sara kanban
...
@router.get("/config")
def get_config():
    """Return kanban dashboard preferences from ~/.sara/config.yaml.
...
def get_assignees():
    ...
    Returns the union of ``~/.sara/profiles/*`` on disk and every
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in sara_cli/web_server.py.
