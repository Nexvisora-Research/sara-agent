...
1. **Theme** — copy the theme YAML into your sara home:

   ```
   cp theme/strike-freedom.yaml ~/.sara/dashboard-themes/
   ```
...
2. **Plugin** — the `dashboard/` directory gets auto-discovered because
   ...
   whole plugin directory into `~/.sara/plugins/`:

   ```
   cp -r . ~/.sara/plugins/strike-freedom-cockpit
   ```
