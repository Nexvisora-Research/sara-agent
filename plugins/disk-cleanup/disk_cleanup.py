"""disk_cleanup — ephemeral file cleanup for sara Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under sara_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly sara_HOME and /tmp/sara-*
Never touches: ~/.sara/logs/ or any system directory.
"""
...
try:
    ...
    def get_sara_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("sara_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".sara").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$sara_HOME/logs/``."""
    return get_sara_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$sara_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under sara_HOME or ``/tmp/sara-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    sara_home = get_sara_home()
    try:
        path.resolve().relative_to(sara_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/sara-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("sara-"):
        return True
    return False
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside sara_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under sara_HOME (but leave sara_HOME itself and
    ...
    _PROTECTED_TOP_LEVEL = {
        ...
        "sara-agent", "backups", "profiles", ".worktrees",
    }
    empty_removed = 0
    try:
        for dirpath in sorted(sara_home.rglob("*"), reverse=True):
            if not dirpath.is_dir() or dirpath == sara_home:
                continue
            try:
                rel_parts = dirpath.relative_to(sara_home).parts
            except ValueError:
                continue
...
def guess_category(path: Path) -> Optional[str]:
    ...
    sara_home = get_sara_home()
    try:
        rel = path.resolve().relative_to(sara_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "sara-agent",
        }:
            return None
        ...
        # Path isn't under sara_HOME (e.g. /tmp/sara-*) — fall through.
        pass
