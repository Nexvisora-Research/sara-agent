...
Auto-tracks and cleans up ephemeral files created during sara Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$sara_HOME` and `/tmp/sara-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `sara_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
- `is_safe_path()` rejects anything outside `sara_HOME` or `/tmp/sara-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$sara_HOME/disk-cleanup/` is itself excluded
- `$sara_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
