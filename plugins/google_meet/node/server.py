...
Launched by ``sara meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$sara_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``sara meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_sara_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "sara-meet-node",
    ) -> None:
        self.host = host
        self.port = port
