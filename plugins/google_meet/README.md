...
Let the sara agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where sara runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `sara meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `sara meet` CLI |
...
| `cli.py` | `sara meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$sara_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `sara meet node {run,list,approve,remove,status,ping}` |
...
sara meet install                                      # pip + Chromium
sara meet setup                                        # preflight
sara meet auth                                         # optional
sara meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
sara meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.sara/.env
sara meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
sara meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
sara meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.sara/.env
sara meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, sara will **not** switch your system audio input automatically — the
...
sara plugins enable google_meet
sara meet node run --display-name my-mac --host 0.0.0.0 --port 18789
