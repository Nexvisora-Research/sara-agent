# sara Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: sara-achievements plugin
...
### `GET /api/plugins/sara-achievements/achievements`
...
### `POST /api/plugins/sara-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/sara-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/sara-achievements/overview`
...
- Persist plugin data under:
  - `~/.sara/plugins/sara-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.sara/plugins/sara-achievements/scan_checkpoint.json`
...
- `~/.sara/plugins/sara-achievements/`
