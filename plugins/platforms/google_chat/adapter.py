...
googleapiclient is synchronexvisora. This keeps the event loop responsive.
...
# ``model_tools`` import time, so ``Sara status``, ``Sara chat``, etc.
...
# ``Sara_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Sara is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a Sara MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${Sara_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _Sara_home = _get_Sara_home()
        except (ModuleNotFoundError, ImportError):
            _Sara_home = _Path.home() / ".Sara"
        self._thread_count_store = _ThreadCountStore(
            _Sara_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Sara is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("Sara_HOME", str(_Path.home() / ".Sara"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${Sara_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a Sara MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Sara is thinking…' marker message.
        ...
        "Sara is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Sara is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Sara is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Sara is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Sara is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``Sara setup``.

    The setup wizard at ``Sara_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.Sara/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. Sara-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.Sara/.env")
    print_info("Restart the gateway: Sara gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``Sara cron`` running as a
    separate process from ``Sara gateway``).  Without this hook,
    ...
    # Bound the synchronexvisora urllib3-backed token refresh so a hung Google
...
def register(ctx) -> None:
    """Plugin entry point — called by the Sara plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'Sara-agent[google_chat]'",
        ...
        # the "Sara is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Sara is thinking…' marker message "
