"""
Microsoft Teams platform adapter for sara Agent.
...
class TeamsAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            self._app = App(
                ...
                client=ClientOptions(headers={"User-Agent": "sara"}),
            )
    ...
    async def _on_card_action(
        ...
        data = action.data or {}
        sara_action = data.get("sara_action", "")
        session_key = data.get("session_key", "")

        if not sara_action or not session_key:
        ...
        choice = choice_map.get(sara_action)
        if not choice:
            return InvokeResponse(
                status=200,
                body=AdaptiveCardActionMessageResponse(value="Unknown action."),
        ...
        card = (
            ...
            .with_actions([
                ExecuteAction(
                    title="Allow Once",
                    verb="sara_approve",
                    data={**btn_data_base, "sara_action": "approve_once"},
                    style="positive",
                ),
                ExecuteAction(
                    title="Allow Session",
                    verb="sara_approve",
                    data={**btn_data_base, "sara_action": "approve_session"},
                ),
                ExecuteAction(
                    title="Always Allow",
                    verb="sara_approve",
                    data={**btn_data_base, "sara_action": "approve_always"},
                ),
                ExecuteAction(
                    title="Deny",
                    verb="sara_approve",
                    data={**btn_data_base, "sara_action": "deny"},
                    style="destructive",
                ),
            ])
        )
...
def interactive_setup() -> None:
    ...
    print_info("  teams app create --name \"sara\" --endpoint \"https://<tunnel>/api/messages\"")
    print()
    ...
    print()
    print_success("Teams configuration saved to ~/.sara/.env")
