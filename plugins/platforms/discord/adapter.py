...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.Sara/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(os.getenv("Sara_DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(os.getenv("Sara_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Sara gateway
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_Sara_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Sara manages."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Sara session")
        ...
        @tree.command(name="status", description="Show Sara session status")
        ...
        @tree.command(name="stop", description="Stop the running Sara agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.Sara/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Sara Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Sara gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Sara session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Sara in the thread",
        ...
        # Sara_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from Sara_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from Sara_cli.commands import _iter_plugin_command_entries
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from Sara_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Sara session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Sara: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "Sara"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by Sara: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Sara handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``Sara update --gateway``
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``Sara update`` prompts.
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _expensive_warning_for(self, model_id: str):
            try:
                from Sara_cli.model_cost_guard import expensive_model_warning
...
# is not in this process (e.g. ``Sara cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from Sara_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``Sara_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import Sara_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Sara plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'Sara-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # Sara_cli/setup.py::_setup_discord function.  Same shape as Teams.
