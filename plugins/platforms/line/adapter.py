"""
LINE Messaging API platform adapter for Sara Agent.
...
to Sara Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``Sara_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            Sara_home = Path(get_Sara_home()).resolve()
        except Exception:
            Sara_home = Path.home().joinpath(".Sara").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``Sara status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``Sara status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Sara setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.Sara/.env`` via ``Sara_cli.config``.
    """
    print()
    ...
    try:
        from Sara_cli.config import get_env_var, set_env_var
    except ImportError:
        print("Sara_cli.config not available; set LINE_* vars manually in ~/.Sara/.env")
        return
