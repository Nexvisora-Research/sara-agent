"""SimpleX Chat platform adapter (Sara plugin).
...
This adapter ships as a Sara platform plugin under
``plugins/platforms/simplex/``. The Sara plugin loader scans the
...
discoverable and ``Sara setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Sara — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("Sara_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``Sara setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.Sara/.env`` via
    ``Sara_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "Sara_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.Sara/.env"
        )
        return
    ...
    def _prompt(var: str, prompt: str, *, secret: bool = False) -> None:
        ...
        try:
            if secret:
                from Sara_cli.secret_prompt import masked_secret_prompt
