"""ntfy platform adapter (Sara plugin).
...
``plugins/platforms/ntfy/``. The Sara plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "Sara-in"              # subscribe topic (incoming)
          publish_topic: "Sara-out"     # optional — defaults to topic
...
_ECHO_TAG = "Sara-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``Sara cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Sara dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `Sara gateway status` without
