"""
Photon Spectrum (iMessage) platform adapter for Sara Agent.
...
if TYPE_CHECKING:
    ...
    try:
        import httpx
        HTTPX_AVAILABLE = True
    except ImportError:  # pragma: no cover - httpx is already a Sara dep
...
def check_requirements() -> bool:
    ...
    if not (_SIDECAR_DIR / "node_modules").exists():
        # spectrum-ts not installed yet — `Sara photon setup` will
        # install it.  check_fn still returns False so the gateway
        # surfaces the missing-deps state in `Sara setup` / status.
        return False
    return True
...
class PhotonAdapter(BasePlatformAdapter):
    ...
    def _compile_mention_patterns(raw: Any) -> "list[re.Pattern]":
        ...
        list, or comma/newline-separated), or None (use Sara defaults).
    ...
    async def _inbound_loop(self) -> None:
        ...
        headers = {"X-Sara-Sidecar-Token": self._sidecar_token}
        backoff = 1.0
    ...
    def _pid_is_sidecar(pid: int) -> bool:
        ...
        try:
            ...
            return False
        # Checkout-agnostic: any Sara checkout's sidecar entry point.
    ...
    async def _start_sidecar(self) -> None:
        if not (_SIDECAR_DIR / "node_modules").exists():
            raise RuntimeError(
                ...
                f"cd {_SIDECAR_DIR} && npm install   (or `Sara photon setup`)"
            )
...
async def _standalone_send(
    ...
    *,
    ...
    headers = {"X-Sara-Sidecar-Token": token}
...
def register(ctx) -> None:
    """Called by the Sara plugin loader at startup."""
    # Local import to avoid argparse work at module load; reused for both the
    # gateway-setup hook and the `Sara photon` CLI command below.
    ...
    ctx.register_platform(
        ...
        install_hint=(
            "Run: Sara photon setup  (logs in via device flow, creates a "
            "Spectrum project, links your phone number, installs the "
            "spectrum-ts sidecar)."
        ),
        # Surfaces Photon in `Sara gateway setup` alongside every other
    ...
    # Register CLI subcommands — `Sara photon ...`
