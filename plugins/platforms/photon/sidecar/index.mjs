// Sara Agent — Photon Spectrum sidecar
//
...
//
// Inbound  (gRPC -> Sara): the SDK's `app.messages` async iterator is a
...
//   gateway connects.
// Outbound (Sara -> gRPC): `/send` drives `space.send(...)`; `/typing`
//   sends the documented `typing("start" | "stop")` content builder.
//
// Protocol (all requests require `X-Sara-Sidecar-Token: ${TOKEN}`):
...
//                          default off — toggle with `Sara photon telemetry`)
...
// instead of a cryptic module-resolution error during import. Apply Sara'
...
const server = http.createServer(async (req, res) => {
  if (!tokenOk(req.headers["x-Sara-sidecar-token"])) {
  ...
  try {
    ...
    if (req.url === "/send-attachment") {
      ...
      // overrides only when Sara supplied them so a known-good
