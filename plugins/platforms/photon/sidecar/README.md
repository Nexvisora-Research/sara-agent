...
Small Node helper that bridges Sara Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Sara is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Sara-Sidecar-Token`)
...
The Sara plugin's `Sara photon setup` command runs `npm install`
here automatically.
