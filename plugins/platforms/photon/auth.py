...
Credential storage mirrors every other Sara channel:

    * runtime SDK creds  -> ``~/.Sara/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.Sara/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a Sara dependency
...
# registers Sara as its own client_id.
...
# Default name of the project Sara provisions for the operator.
DEFAULT_PROJECT_NAME = "Sara Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of Sara-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.Sara/auth.json`` honouring the active Sara profile."""
    try:
        from Sara_constants import get_Sara_home
        return Path(get_Sara_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.Sara")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.Sara/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.Sara/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.Sara/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: Sara_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `Sara photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `Sara photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `Sara photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `Sara photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `Sara photon setup`)"
