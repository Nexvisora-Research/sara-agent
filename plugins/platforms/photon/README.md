...
This plugin connects Sara Agent to iMessage (and other Spectrum
...
TypeScript-only, Sara runs it inside a small supervised Node sidecar and
...
- **Outbound**: `send` / `send_typing` / reaction tapbacks are loopback POSTs
  ...
  `/unreact`), authenticated with a shared `X-Sara-Sidecar-Token`.

## First-time setup

```bash
# One-shot setup: device login (opens browser) + project + user + sidecar deps
Sara photon setup --phone +15551234567

# Start the gateway
Sara gateway start
```

`Sara photon setup` does, in order:

1. **Device login** (RFC 8628, `client_id=photon-cli`) — opens
   `https://app.photon.codes/` for approval and stores the bearer token.
2. **Find or create** the `Sara Agent` project on the Photon dashboard.
3. **Provision the project secret** — mint a fresh project secret (the
   dashboard reveals it only once) and persist it to `~/.Sara/.env` so the
...
There is no separate `login` command; like every other Sara channel,
...
Run `Sara photon status` to see what's configured.

## Credentials

Runtime SDK credentials live in `~/.Sara/.env` (the same place every other
...
Management metadata lives in `~/.Sara/auth.json` under `credential_pool`:
...
| `PHOTON_TELEMETRY`        | false                      | Spectrum SDK telemetry — toggle with `Sara photon telemetry on\|off` (restart the gateway to apply) |
...
5. Verify end-to-end: `Sara photon status`, a DM and a group roundtrip,
