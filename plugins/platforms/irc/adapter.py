"""
IRC Platform Adapter for sara Agent.

A plugin-based gateway adapter that connects to an IRC server and relays
messages to/from the sara agent.  Zero external dependencies — uses
...
class IRCAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config, **kwargs):
        ...
        self.nickname = os.getenv("IRC_NICKNAME") or extra.get("nickname", "sara-bot")
    ...
    async def connect(self) -> bool:
        ...
        await self._send_raw(f"USER {self.nickname} 0 * :sara Agent")
    ...
    async def disconnect(self) -> None:
        ...
        if self._writer and not self._writer.is_closing():
            try:
                await self._send_raw("QUIT :sara Agent shutting down")
    ...
    async def _handle_line(self, raw: str) -> None:
        ...
        if command == "433":
            # Retry with incrementing suffix: sara_, sara_1, sara_2...
...
def check_requirements() -> bool:
    ...
    # sara setup / requirements check path.
...
def interactive_setup() -> None:
    """Interactive `sara gateway setup` flow for the IRC platform.

    Lazy-imports ``sara_cli.setup`` helpers so the plugin stays importable
    ...
    print_info("Connect sara to an IRC network. Uses Python stdlib — no extra packages needed.")
    ...
    channel = prompt(
        "Channel to join (e.g. #sara — comma-separate for multiple)",
    ...
    print()
    print_success("IRC configuration saved to ~/.sara/.env")
    print_info("Restart the gateway for changes to take effect: sara gateway restart")
