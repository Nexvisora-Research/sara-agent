...
def _install_sidecar_publisher() -> None:
    ...
    Activated by `sara_TUI_SIDECAR_URL`, set by the dashboard's
    ...
    """
    url = os.environ.get("sara_TUI_SIDECAR_URL")
...
# tested; override via ``sara_TUI_GATEWAY_SHUTDOWN_GRACE_S`` if a
...
def _shutdown_grace_seconds() -> float:
    raw = (os.environ.get("sara_TUI_GATEWAY_SHUTDOWN_GRACE_S") or "").strip()
    if not raw:
...
def _log_signal(signum: int, frame) -> None:
    ...
    (``sara_TUI_GATEWAY_SHUTDOWN_GRACE_S``, default
