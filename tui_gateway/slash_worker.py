"""Persistent slash-command worker — one saraCLI per TUI session.
...
def _run(cli: saraCLI, command: str) -> str:
...
def main():
    ...
    os.environ["sara_SESSION_KEY"] = args.session_key
    os.environ["sara_INTERACTIVE"] = "1"

    with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
        cli = saraCLI(model=args.model or None, compact=True, resume=args.session_key, verbose=False)
