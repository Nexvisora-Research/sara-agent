...
//!
//! Mirrors `Sara_constants.get_Sara_home()` from the Python CLI:
//!   Windows: %LOCALAPPDATA%\Sara
//!   macOS:   ~/.Sara
//!   Linux:   ~/.Sara  (override via $Sara_HOME)
//!
//! NOTE (macOS): Python's get_Sara_home(), scripts/install.sh, and the
//! Electron desktop's resolveSaraHome() ALL use ~/.Sara on macOS — there
...
/// Returns the canonical Sara home directory, respecting $Sara_HOME if set.
pub fn Sara_home() -> PathBuf {
    if let Ok(override_path) = std::env::var("Sara_HOME") {
    ...
    {
        // %LOCALAPPDATA%\Sara — matches scripts/install.ps1's $SaraHome.
        if let Some(local_app_data) = dirs::data_local_dir() {
            return local_app_data.join("Sara");
        }
    }

    // macOS + Linux + fallback: ~/.Sara (matches Python get_Sara_home(),
    // install.sh, and the Electron desktop's resolveSaraHome()).
    if let Some(home) = dirs::home_dir() {
        return home.join(".Sara");
    }
...
pub fn bootstrap_cache_dir() -> PathBuf {
    Sara_home().join("bootstrap-cache")
}
...
/// Sara_HOME so it survives repo checkout deletion (unlike anything under
/// Sara-agent/).
///
/// On Windows this is `%LOCALAPPDATA%\Sara\Sara-setup.exe`; on other
...
pub fn copy_self_to_Sara_home() -> std::io::Result<()> {
    ...
    tracing::info!(?src, ?dest, "copied installer to Sara_HOME");
    Ok(())
}
...
/// We don't always know ACTIVE_Sara_ROOT until install.ps1 reports it, so
/// this is a probe helper, not a definitive path.
pub fn likely_bootstrap_marker(install_root: &Path) -> PathBuf {
    install_root.join(".Sara-bootstrap-complete")
}

/// Initializes tracing to bootstrap-installer.log under Sara_HOME/logs/.
...
pub fn init_logging() -> Option<WorkerGuard> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        ...
        eprintln!("[Sara-setup] could not create log dir {dir:?}: {err}");
        return None;
    }
    ...
    let env_filter = tracing_subscriber::EnvFilter::try_from_env("Sara_BOOTSTRAP_LOG")
...
#[tauri::command]
pub fn get_Sara_home() -> String {
    Sara_home().to_string_lossy().into_owned()
}
