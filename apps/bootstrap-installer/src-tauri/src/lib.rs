...
/// desktop app handing off via `Sara-Setup.exe --update`).
///
...
pub fn run() {
    // Tracing → bootstrap-installer.log under Sara_HOME/logs/ so install
    ...
    // Sara is already installed, so users can re-run setup to repair a broken
    ...
    tracing::info!(?mode, force_setup, "Sara installer starting");
    ...
    tauri::Builder::default()
        ...
        .setup(move |app| {
            ...
            // Sara is already installed should NOT show the installer or
            // rebuild — it should just open the app, so the /Applications
            // "Sara" doubles as a normal launcher (first run installs, every
            ...
            // Menu/Desktop "Sara" shortcuts that install.ps1 creates, and a
            // reliable detached relaunch there needs the DETACHED_PROCESS +
            // startup-grace handling used by launch_Sara_desktop — out of
            ...
            if cfg!(target_os = "macos") && mode == AppMode::Install && !force_setup {
                let install_root = paths::Sara_home().join("Sara-agent");
                if bootstrap::Sara_is_installed(&install_root) {
                    match bootstrap::spawn_installed_desktop(&install_root) {
                        Ok(()) => {
                            // Brief grace so the spawned app is registered
                            // before we exit (mirrors launch_Sara_desktop).
                            std::thread::sleep(std::time::Duration::from_millis(200));
                            tracing::info!(
                                "Sara already installed — relaunched desktop; exiting installer"
                            );
                            app.handle().exit(0);
                            return Ok(());
                        }
        ...
        .expect("error while running Sara Setup");
}
