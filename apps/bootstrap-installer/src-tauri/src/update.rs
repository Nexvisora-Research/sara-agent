...
//!
//! Driven when the installer is launched as `Sara-Setup.exe --update` (see
//! `AppMode` in lib.rs). The desktop app hands off to us — it exits, then we:
//!
//!   1. wait for the old Sara desktop process to fully exit (so both the
//!      venv shim and packaged app.asar are free; otherwise `Sara update`
//!      or repair bootstrap can race locked files),
//!   2. run `Sara update --yes --gateway` (Python/repo update; this does NOT
//!      rebuild apps/desktop by design — see cmd_update in Sara_cli/main.py),
//!   3. run `Sara desktop --build-only` (the rebuild step update skips),
...
//!
//! Cross-platform note: `Sara update` already handles macOS/Linux (git/pip).
//! The only OS-specific bits here are the venv shim path (resolve_Sara) and
...
/// `Sara update` exit code meaning "another Sara process is holding the
/// venv shim open / dirty precondition" — see _cmd_update_impl in
/// Sara_cli/main.py (sys.exit(2)). We surface a targeted message for this.
const UPDATE_EXIT_CONCURRENT: i32 = 2;

/// How long to wait for the old desktop process to release files under the
/// install tree before giving up and letting `Sara update`'s own guard decide.
...
pub async fn start_update(app: AppHandle) -> Result<(), String> {
        ...
        .is_err()
    {
        ...
        let mut stages = vec![
            stage_info("update", "Updating Sara"),
...
async fn run_update(app: AppHandle) -> Result<()> {
    let Sara_home = crate::paths::Sara_home();
    let install_root = Sara_home.join("Sara-agent");
    ...
    let Sara = resolve_Sara(&install_root).ok_or_else(|| {
        let msg = format!(
            "Could not find the Sara CLI under {}. Is Sara installed? \
    ...
    // async on Windows. If it still holds the venv shim, `Sara update`
    ...
    // ---- stage 1: Sara update -----------------------------------------
    // Pass --branch so `Sara update` targets the branch this installer was
    ...
    // without --branch, `Sara update` switches the checkout to `main` (a
    ...
    // --force skips `Sara update`'s Windows running-exe guard (which would
    ...
    // time `Sara update` runs there is no legitimate Sara.exe to protect,
    // and the guard would only produce a false "Sara is still running" stop.
    ...
    // Retry-once for the update-boundary crash. `Sara update` lazily imports
    ...
    // second `Sara update` runs clean because the now-current module is loaded
    ...
    // for the concurrent-instance guard (exit 2) — that's a "close Sara" state
    // a retry can't fix.
    ...
    match update.exit_code {
        ...
        Some(code) if code == UPDATE_EXIT_CONCURRENT => {
            let msg = "Sara is still running. Close all Sara windows and try \
                       the update again."
                .to_string();
            emit_stage(
                &app,
        ...
        other => {
            let msg = format!(
                "Sara update failed (exit {:?}). See {} for details.",
                other,
                crate::paths::Sara_home()
                    .join("logs")
                    .join("update.log")
                    .display()
            );
    ...
    // ---- stage 2: Sara desktop --build-only ----------------------------
    // `Sara update` deliberately does NOT build apps/desktop (it installs
    ...
    // retry-once `Sara update` already does above, and `Sara update`'s own
    ...
    if rebuild.exit_code != Some(0) {
        let msg = format!(
            "Rebuilding the desktop app failed (exit {:?}). The update was \
             applied but the app could not be rebuilt; run `Sara desktop` \
    ...
    if let Some(target_app) = launch_target {
        if let Err(err) = launch_macos_app_and_exit(&app, &target_app).await {
            emit_log(
                ...
                None,
                LogStream::Stderr,
                &format!("[update] could not auto-launch desktop: {err}. Launch Sara manually."),
            );
        }
    } else if let Err(err) =
        crate::bootstrap::launch_Sara_desktop(app.clone(), install_root.to_string_lossy().into_owned()).await
    {
...
pub(crate) async fn wait_for_install_locks_free(install_root: &Path, app: &AppHandle, stage: &str) {
    ...
    emit_log(app, Some(stage), LogStream::Stdout, "[handoff] waiting for Sara to exit…");
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Last resort: a backend Sara.exe (or the desktop Sara.exe
            ...
            // force-kill every Sara.exe except ourselves, then give the OS a
            ...
            emit_log(
                ...
                LogStream::Stdout,
                &format!(
                    "[handoff] Sara still holding install files ({}); force-killing stragglers…",
                    format_locked_paths(&locked)
                ),
            );
            force_kill_other_Sara();
...
fn install_lock_probe_paths(install_root: &Path) -> Vec<PathBuf> {
    let mut paths = vec![venv_Sara(install_root)];
...
fn desktop_app_payload_paths(install_root: &Path) -> Vec<PathBuf> {
    ...
    if cfg!(target_os = "windows") {
        ...
        vec![
            release.join("mac").join("Sara.app").join("Contents").join("Resources").join("app.asar"),
            release.join("mac-arm64").join("Sara.app").join("Contents").join("Resources").join("app.asar"),
        ]
    } else {
...
/// Force-kill any `Sara.exe` other than this process. Windows-only; a no-op
...
/// knew its children — so we kill the whole `Sara.exe` image tree via
/// taskkill, excluding our own PID.
///
/// Safe w.r.t. our own update child: this runs inside the install-lock wait,
/// which completes BEFORE we spawn `venv\Scripts\Sara.exe update`. At this
/// point no update-driven Sara.exe exists yet, so the only Sara.exe images
...
/// Spawn `Sara <args>` from `cwd`, stream stdout/stderr as Log events on the
...
/// Path to the venv Sara shim under an install root, regardless of existence.
fn venv_Sara(install_root: &Path) -> PathBuf {
    if cfg!(target_os = "windows") {
        install_root.join("venv").join("Scripts").join("Sara.exe")
    } else {
        install_root.join("venv").join("bin").join("Sara")
    }
}

/// Resolve the Sara CLI to drive. Prefer the venv shim in the install we
/// just updated; fall back to `Sara` on PATH.
fn resolve_Sara(install_root: &Path) -> Option<PathBuf> {
    let shim = venv_Sara(install_root);
    if shim.exists() {
        return Some(shim);
    }
    // PATH fallback. which-style probe via env, kept dependency-free.
    let exe = if cfg!(target_os = "windows") { "Sara.exe" } else { "Sara" };
...
fn update_child_env(install_root: &Path) -> Vec<(String, OsString)> {
    ...
    let mut envs = vec![(
        "Sara_HOME".to_string(),
        Sara_home.as_os_str().to_os_string(),
    )];
    if let Some(path) = path_with_prepended_entries(&[
        Sara_home.join("node").join("bin"),
...
async fn install_macos_app_update(
    ...
    let rebuilt_app = crate::bootstrap::resolve_Sara_desktop_app(install_root).ok_or_else(|| {
        anyhow!(
            "desktop rebuild succeeded but no Sara.app was found under {}",
    ...
    let tmp = PathBuf::from(format!("{}.Sara-update-new", target_app.display()));
    let old = PathBuf::from(format!("{}.Sara-update-old", target_app.display()));
    ...
    let ditto = Command::new("/usr/bin/ditto")
        .arg(&rebuilt_app)
        .arg(&tmp)
        .current_dir(crate::paths::Sara_home())
        .status()
        .await
...
mod tests {
    ...
    #[test]
    fn venv_Sara_is_under_install_root() {
        let root = Path::new("/x/Sara-agent");
        let shim = venv_Sara(root);
    ...
    fn lock_probe_paths_include_desktop_app_payload() {
        ...
        assert!(
            probes.iter().any(|p| p == &venv_Sara(root)),
    ...
    fn locked_paths_ignores_missing_payloads() {
        let root = Path::new("/nonexistent/Sara-agent");
        let probes = install_lock_probe_paths(root);
    ...
    fn parses_only_app_targets() {
        assert_eq!(
            target_app_from_args(["--update", "--target-app", "/Applications/Sara.app"]),
            Some(PathBuf::from("/Applications/Sara.app"))
        );
    ...
    async fn swap_installs_new_bundle_and_cleans_up() {
        ...
        let target = base.join("Sara.app");
        let tmp = base.join("Sara.app.Sara-update-new");
        let old = base.join("Sara.app.Sara-update-old");
    ...
    async fn swap_failure_never_leaves_target_missing() {
        ...
        let target = base.join("Sara.app");
        let tmp = base.join("Sara.app.Sara-update-new"); // intentionally absent
        let old = base.join("Sara.app.Sara-update-old");
        write_marker(&target, "OLD");
    ...
    async fn swap_rolls_back_when_install_step_fails() {
        ...
        let target = base.join("Sara.app");
        let tmp = base.join("Sara.app.Sara-update-new"); // absent
        let old = base.join("Sara.app.Sara-update-old");
        write_marker(&target, "OLD");
