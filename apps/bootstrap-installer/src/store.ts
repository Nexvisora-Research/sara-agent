...
export const $SaraHome = atom<string | null>(null)
...
export async function initialize(): Promise<void> {
  ...
  try {
    const [logPath, SaraHome, mode] = await Promise.all([
      invoke<string>('get_log_path'),
      invoke<string>('get_Sara_home'),
      invoke<AppMode>('get_mode')
    ])
    $logPath.set(logPath)
    $SaraHome.set(SaraHome)
    $mode.set(mode)
  } catch (err) {
  ...
  unlisten = await listen<BootstrapEvent>('bootstrap', (event) => {
    ...
    switch (payload.type) {
      ...
      case 'complete':
        ...
        // Install: show the "launch Sara" success screen. Update: this is a
...
export async function startUpdate(): Promise<void> {
  // Update is driven by the desktop handing off (Sara-Setup.exe --update);
...
export async function launchSaraDesktop(): Promise<void> {
  const installRoot = $bootstrap.get().installRoot
  if (!installRoot) throw new Error('no install root')
  await invoke('launch_Sara_desktop', { installRoot })
}
