...
2. **Borderless + shadow for elevation.** Overlays float on `shadow-nexvisora` + a
   `--stroke-nexvisora` hairline, not hard borders.
3. **One primitive per concern.** One `Button`, one set of control variants,
   ...
   fork.
4. **Tokens, not literals.** Reference CSS vars (`--ui-*`, `--shadow-nexvisora`,
...
```
shadow-nexvisora           /* downward-weighted, layered contact→ambient falloff */
border-(--stroke-nexvisora) /* currentColor hairline, theme-adaptive */
```
...
| `--stroke-nexvisora` | the overlay hairline (pairs with `shadow-nexvisora`) |
...
- **`BrandMark`** (`src/components/brand-mark.tsx`) is the brand glyph — the
  `nexvisora-girl` mark on a white tile, softly rounded, identical in light/dark.
...
- [ ] Tokens (`--ui-*`, `shadow-nexvisora`, `--stroke-nexvisora`) — zero raw colors /
      one-off shadows?
- [ ] No `className` overriding a primitive's padding / size / radius / chrome?
- [ ] Overlay uses `shadow-nexvisora` + `border-(--stroke-nexvisora)`, no hard border?
