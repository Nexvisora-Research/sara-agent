 ...
 * product name (`Sara`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/Sara'
 *
 * This is a hard failure with no obvious cause for the user — `Sara desktop`
 ...
 * (the mac-unpacked Sara.app bundle) and Windows (win-unpacked), so we
