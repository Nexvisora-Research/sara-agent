 ...
 *
 * Stamps the Sara icon + identity onto the packed Windows Sara.exe via
 * rcedit (delegated to set-exe-identity.cjs). This runs for EVERY packed build
 * — first install, `Sara desktop`, the installer's --update rebuild, and a
 ...
 *   - packager.appInfo.productFilename: the exe basename (e.g. 'Sara')
 */
...
exports.default = async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'Sara'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); Sara.exe keeps the stock Electron icon`)
  }
}
