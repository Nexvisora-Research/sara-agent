...
// no longer bundles the Sara Agent Python payload (that's fetched at first
// launch via install.ps1 / install.sh, per the Phase 1 thin-installer flow).
const APP = (() => {
  if (PLATFORM === 'darwin') {
    const appPath = path.join(RELEASE_ROOT, `mac-${ARCH}`, 'Sara.app')
    return {
      appPath,
      binary: path.join(appPath, 'Contents', 'MacOS', 'Sara'),
  ...
  if (PLATFORM === 'win32') {
    ...
    return {
      appPath: unpacked,
      binary: path.join(unpacked, 'Sara.exe'),
  ...
  return {
    appPath: unpacked,
    binary: path.join(unpacked, 'Sara'),
    resourcesPath: path.join(unpacked, 'resources'),
    asarPath: path.join(unpacked, 'resources', 'app.asar'),
    unpackedDistIndex: path.join(unpacked, 'resources', 'app.asar.unpacked', 'dist', 'index.html')
  }
...
// Default Sara_HOME for non-sandboxed runs -- matches main.cjs's
// resolveSaraHome(). On Windows it's %LOCALAPPDATA%\Sara; elsewhere
// it's ~/.Sara. The fresh-install sandbox launchFresh() sets its own
// Sara_HOME and never touches this.
const DEFAULT_Sara_HOME = (() => {
  if (PLATFORM === 'win32' && process.env.LOCALAPPDATA) {
    return path.join(process.env.LOCALAPPDATA, 'Sara')
  }
  return path.join(os.homedir(), '.Sara')
})()
const VENV_ROOT = path.join(DEFAULT_Sara_HOME, 'Sara-agent', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'Sara-desktop-fresh-install')
...
function ensurePackagedApp() {
  if (process.env.Sara_DESKTOP_SKIP_BUILD === '1' && exists(APP.binary)) {
    return
  }
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `Sara-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `Sara-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `Sara-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'Sara-${version}-${os}-${arch}.${ext}'
...
function ensureDmg() {
  ...
  if (process.env.Sara_DESKTOP_SKIP_BUILD === '1' && exists(resolveDmgPath())) {
    return
  }
...
function ensureNsis() {
  ...
  if (process.env.Sara_DESKTOP_SKIP_BUILD === '1' && resolveNsisPath()) {
    return
  }
...
function launchFresh() {
  ...
  const SaraHome = path.join(sandbox, 'Sara-home')
  const cwd = path.join(sandbox, 'workspace')

  fs.mkdirSync(userDataDir, { recursive: true })
  fs.mkdirSync(SaraHome, { recursive: true })
  ...
  env.Sara_DESKTOP_CWD = cwd
  env.Sara_DESKTOP_IGNORE_EXISTING = '1'
  env.Sara_DESKTOP_TEST_MODE = 'fresh-install'
  env.Sara_DESKTOP_USER_DATA_DIR = userDataDir
  env.Sara_HOME = SaraHome
  delete env.Sara_DESKTOP_Sara
  delete env.Sara_DESKTOP_Sara_ROOT
  ...
  console.log(`  Sara_HOME: ${SaraHome}`)
  console.log(`  cwd: ${cwd}`)

  return { runtimeRoot: path.join(SaraHome, 'Sara-agent', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The Sara Agent Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  // present anymore. If a stray ship of Sara_cli sneaks back in we want
  // to fail loudly rather than re-introduce the 400MB delta we just removed.
  const staleFactoryMarker = path.join(APP.resourcesPath, 'Sara-agent', 'Sara_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing Sara
  npm run test:desktop:fresh     # build packaged app, launch with temp userData + Sara_HOME
...
Fast rerun (skip rebuild if the packaged app already exists):
  Sara_DESKTOP_SKIP_BUILD=1 npm run test:desktop:fresh
`)
}
