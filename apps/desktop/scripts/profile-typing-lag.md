...
cd apps/desktop
XCURSOR_SIZE=24 Sara_DESKTOP_DEV_SERVER=http://127.0.0.1:5174 \
...
node apps/desktop/scripts/profile-typing.mjs \
  --chars=400 --cps=30 --out=/tmp/Sara-typing
# → /tmp/Sara-typing.cpuprofile  (open in Chrome DevTools Performance)
# → /tmp/Sara-typing.before.heapsnapshot
# → /tmp/Sara-typing.after.heapsnapshot
```
...
1. **Per-keystroke `scrollHeight` read removed.** The expansion useEffect
   ...
   (forces synchronexvisora layout). Replaced with a `draft.length > 60`
