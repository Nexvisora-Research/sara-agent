#!/usr/bin/env node
// set-exe-identity.cjs — stamp the Sara icon + version metadata onto the
// built Sara.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
// skips rcedit, so the unpacked Sara.exe keeps the stock Electron icon and
...
// so EVERY packed build — first install, `Sara desktop`, the installer's
...
// path (which rebuilds via `Sara desktop --build-only`, never install.ps1)
// shipped a stock "Electron" exe. Keeping it in afterPack closes that gap.
//
// Also runnable standalone for ad-hoc re-stamping:
//   node scripts/set-exe-identity.cjs <path-to-Sara.exe>
//
...
// Stamp the Sara icon + identity onto `exe`. Resolves on success, throws on
...
async function stampExeIdentity(exe, desktopRoot = path.resolve(__dirname, '..')) {
  ...
  await rcedit(exe, {
    ...
    'version-string': {
      ProductName: 'Sara',
      FileDescription: 'Sara',
      CompanyName: 'nexvisora Research',
      LegalCopyright: 'Copyright (c) 2026 nexvisora Research'
    }
  })

  console.log('[set-exe-identity] done — Sara icon + identity stamped')
}
