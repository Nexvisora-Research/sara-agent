 ...
 *
 * Drives apps/desktop's first-launch install of Sara Agent by spawning
...
function bootstrapCacheDir(SaraHome) {
  return path.join(SaraHome, 'bootstrap-cache')
}

// The install.sh / install.ps1 that ships inside the already-installed agent
// checkout under ~/.Sara/Sara-agent. Used as a last-resort fallback when
// the pinned commit can't be fetched from GitHub (e.g. a locally-built desktop
// app stamped to an unpushed HEAD).
function installedAgentInstallScript(SaraHome) {
  if (!SaraHome) return null
  const candidate = path.join(SaraHome, 'Sara-agent', 'scripts', installScriptName())
  try {
    fs.accessSync(candidate, fs.constants.R_OK)
    return candidate
  } catch {
...
function cachedScriptPath(SaraHome, commit) {
  return path.join(bootstrapCacheDir(SaraHome), `install-${commit}.${process.platform === 'win32' ? 'ps1' : 'sh'}`)
}
...
function downloadInstallScript(commit, destPath) {
  ...
  const url = `https://raw.githubusercontent.com/nexvisoraResearch/Sara-agent/${commit}/scripts/${scriptName}`
...
async function resolveInstallScript({ installStamp, sourceRepoRoot, SaraHome, emit, _download = downloadInstallScript }) {
  ...
  const cached = cachedScriptPath(SaraHome, installStamp.commit)
  try {
  ...
  try {
    ...
    const installed = installedAgentInstallScript(SaraHome)
    if (installed) {
      emit({
        type: 'log',
        line:
...
function spawnPowerShell(scriptPath, args, { emit, stageName, abortSignal, SaraHome } = {}) {
  return new Promise((resolve, reject) => {
    ...
    const child = spawn(ps, fullArgs, hiddenWindowsChildOptions({
      ...
      env: {
        ...process.env,
        // Pass Sara_HOME through so install.ps1 respects the caller's
        // choice rather than re-computing the default.
        Sara_HOME: SaraHome || process.env.Sara_HOME || ''
      }
    }))
...
function spawnBash(scriptPath, args, { emit, stageName, abortSignal, SaraHome } = {}) {
  return new Promise((resolve, reject) => {
...
function buildPosixPinArgs({ installStamp, activeRoot, SaraHome }) {
  const args = ['--dir', activeRoot, '--Sara-home', SaraHome]
  if (installStamp && installStamp.branch) {
    args.push('--branch', installStamp.branch)
  }
  if (installStamp && installStamp.commit) {
...
async function fetchManifest({ scriptPath, installerKind, emit, SaraHome, activeRoot, installStamp }) {
  const isPosix = installerKind === 'posix'
  const args = isPosix
    ? ['--manifest', ...buildPosixPinArgs({ installStamp, activeRoot, SaraHome })]
...
async function runStage({ scriptPath, installerKind, stage, emit, SaraHome, activeRoot, abortSignal, installStamp }) {
  ...
  const args = isPosix
    ? [
        ...
        '--non-interactive',
        '--json',
        ...buildPosixPinArgs({ installStamp, activeRoot, SaraHome })
      ]
...
async function runBootstrap(opts) {
  ...
  const runLog = openRunLog(logRoot || path.join(SaraHome, 'logs'))
  ...
  try {
    ...
    const scriptInfo = await resolveInstallScript({ installStamp, sourceRepoRoot, SaraHome, emit })
