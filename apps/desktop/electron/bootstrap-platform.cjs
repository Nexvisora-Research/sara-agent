 ...
 * null to keep it enabled. `Sara_DESKTOP_DISABLE_GPU` overrides detection
...
function detectRemoteDisplay(options = {}) {
  ...
  const override = String(env.Sara_DESKTOP_DISABLE_GPU || '')
    .trim()
    .toLowerCase()
  if (GPU_OVERRIDE_ON.has(override)) return 'override (Sara_DESKTOP_DISABLE_GPU)'
