...
const { buildDesktopBackendEnv, normalizeSaraHomeRoot } = require('./backend-env.cjs')
...
const USER_DATA_OVERRIDE = process.env.Sara_DESKTOP_USER_DATA_DIR
...
const DEV_SERVER = process.env.Sara_DESKTOP_DEV_SERVER
...
// switches only apply pre-launch. Override with Sara_DESKTOP_DISABLE_GPU
...
if (REMOTE_DISPLAY_REASON) {
  ...
  console.log(
    `[Sara] remote display detected (${REMOTE_DISPLAY_REASON}); disabling GPU hardware acceleration to prevent flicker`
  )
}

ipcMain.handle('Sara:get-remote-display-reason', () => REMOTE_DISPLAY_REASON)
    ...
    try {
      ...
      if (parsed && typeof parsed === 'object' && typeof parsed.commit === 'string' && parsed.commit.length >= 7) {
        if (parsed.schemaVersion !== INSTALL_STAMP_SCHEMA_VERSION) {
          console.warn(
            `[Sara] install-stamp.json schemaVersion ${parsed.schemaVersion} != expected ${INSTALL_STAMP_SCHEMA_VERSION}; ignoring`
          )
          continue
        }
...
if (INSTALL_STAMP) {
  console.log(
    `[Sara] install stamp: ${INSTALL_STAMP.commit.slice(0, 12)}${INSTALL_STAMP.branch ? ` (${INSTALL_STAMP.branch})` : ''}${INSTALL_STAMP.dirty ? ' [DIRTY]' : ''} from ${INSTALL_STAMP.source || 'unknown'}`
  )
} else if (IS_PACKAGED) {
  ...
  console.error(
    '[Sara] WARNING: no install-stamp.json found in packaged build. First-launch bootstrap will not have a pinned ref to install.'
  )
}

// Sara_HOME — the user-facing root for everything Sara-related. Mirrors
// scripts/install.ps1's $SaraHome and scripts/install.sh's $Sara_HOME.
//
// Defaults:
//   Windows: %LOCALAPPDATA%\Sara (matches install.ps1)
//   macOS / Linux: ~/.Sara (matches install.sh)
//
// Special case for Windows: if the user has a legacy ~/.Sara directory
// (e.g., from a prior pip install or a manual setup) AND no
// %LOCALAPPDATA%\Sara yet, prefer the legacy path so we don't orphan their
// existing config / sessions / .env. New installs go to %LOCALAPPDATA%.
//
// Sara_DESKTOP_USER_DATA_DIR (used by test:desktop:fresh) puts the sandbox
// Sara_HOME beneath the throwaway userData dir so a fresh-install run never
// touches the user's real ~/.Sara / %LOCALAPPDATA%\Sara.
function resolveSaraHome() {
  if (process.env.Sara_HOME) return normalizeSaraHomeRoot(process.env.Sara_HOME)
  if (USER_DATA_OVERRIDE) return path.join(path.resolve(USER_DATA_OVERRIDE), 'Sara-home')
  if (IS_WINDOWS) {
    // A GUI app launched from Explorer inherits the environment block captured
    // at login, so a Sara_HOME set via `setx` AFTER login is invisible in
    // process.env even though the CLI (a fresh shell) sees it. Without this the
    // backend silently falls back to %LOCALAPPDATA%\Sara and reports "No
    // inference provider configured" despite a valid configured home (#45471).
    // Consult the live User-scoped registry value before the default below.
    const fromRegistry = readWindowsUserEnvVar('Sara_HOME')
    if (fromRegistry) return normalizeSaraHomeRoot(fromRegistry)
  }
  if (IS_WINDOWS && process.env.LOCALAPPDATA) {
    const localappdata = path.join(process.env.LOCALAPPDATA, 'Sara')
    const legacy = path.join(app.getPath('home'), '.Sara')
    // Migrate transparently to LOCALAPPDATA, but honour an existing legacy
    // ~/.Sara setup (no LOCALAPPDATA install yet) so users don't lose state.
    if (!directoryExists(localappdata) && directoryExists(legacy)) return legacy
    return localappdata
  }
  return path.join(app.getPath('home'), '.Sara')
}

const Sara_HOME = resolveSaraHome()

function SaraManagedNodePathEntries() {
  // NOTE: keep this ordering in sync with iter_Sara_node_dirs() in
  // Sara_constants.py — this Node main process cannot import the Python
  // module, so the platform-ordering rule is mirrored here.
  const root = path.join(Sara_HOME, 'node')
  const bin = path.join(root, 'bin')
  const entries = IS_WINDOWS ? [root, bin] : [bin, root]
  return entries.filter(directoryExists)
}

function pathWithSaraManagedNode(...entries) {
  return [...SaraManagedNodePathEntries(), ...entries, process.env.PATH]
    .filter(Boolean)
    .join(path.delimiter)
}

// ACTIVE_Sara_ROOT — the canonical mutable Sara install. Same path
// install.ps1 / install.sh use, so a desktop-only user and a CLI-only user end
// up with identical layouts and can share one install.
const ACTIVE_Sara_ROOT = path.join(Sara_HOME, 'Sara-agent')
// VENV_ROOT — venv lives inside the repo, exactly like install.ps1 does it.
const VENV_ROOT = path.join(ACTIVE_Sara_ROOT, 'venv')
...
// boots, going straight to `resolveSaraBackend()`. Missing or stale marker
...
//
// We deliberately put the marker INSIDE ACTIVE_Sara_ROOT (not alongside)
...
const BOOTSTRAP_COMPLETE_MARKER = path.join(ACTIVE_Sara_ROOT, '.Sara-bootstrap-complete')
const BOOTSTRAP_MARKER_SCHEMA_VERSION = 1

const DESKTOP_CONNECTION_CONFIG_PATH = path.join(app.getPath('userData'), 'connection.json')
const DESKTOP_UPDATE_CONFIG_PATH = path.join(app.getPath('userData'), 'updates.json')
// active-profile.json records which Sara profile the desktop launches its
// local backend as. When set, startSara() passes `Sara --profile <name>
// dashboard …`, which deterministically pins Sara_HOME (see
// _apply_profile_override in Sara_cli/main.py) and bypasses the sticky
// ~/.Sara/active_profile file. Unset (null) preserves the legacy behavior:
// no --profile flag, so the backend honors active_profile / default.
const DESKTOP_PROFILE_CONFIG_PATH = path.join(app.getPath('userData'), 'active-profile.json')
// Mirrors Sara_cli.profiles._PROFILE_ID_RE so we never hand the backend a
// value its profile resolver would reject and exit on.
const PROFILE_NAME_RE = /^[a-z0-9][a-z0-9_-]{0,63}$/
// Branch we track for self-update. The GUI work has merged to main, so this
// tracks main. User can also override at runtime via
// SaraDesktop.updates.setBranch().
const DEFAULT_UPDATE_BRANCH = 'main'
// desktop.log lives under Sara_HOME/logs/ so it sits next to agent.log,
// errors.log, gateway.log produced by Sara_logging.setup_logging — one log
// directory per user, regardless of which UI surface produced the line.
const DESKTOP_LOG_PATH = path.join(Sara_HOME, 'logs', 'desktop.log')
...
//
// Mirror the Python logs (Sara_logging.py RotatingFileHandler, maxBytes x
...
const BOOT_FAKE_MODE = process.env.Sara_DESKTOP_BOOT_FAKE === '1'
const BOOT_FAKE_STEP_MS = (() => {
  const raw = Number.parseInt(String(process.env.Sara_DESKTOP_BOOT_FAKE_STEP_MS || ''), 10)
...
// 'Sara:native-theme' ('dark' | 'light' | 'system'); we pin
...
// Seed the native About panel with the live Sara version. This is refreshed
// on every open via the explicit "About" menu handler (refreshAboutPanel), so
// an in-place `Sara update` mid-session is reflected without an app restart;
// the seed here just covers the first open and any non-menu invocation path.
app.setAboutPanelOptions({
  applicationName: APP_NAME,
  applicationVersion: resolveSaraVersion(),
  copyright: 'Copyright © 2026 nexvisora Research'
})
...
const MEDIA_PROTOCOL = 'Sara-media'
// Only audio/video may be streamed. Without this the handler would read any
// non-blocklisted local file (no size cap) for any `fetch(Sara-media://…)`.
...
let mainWindow = null
let SaraProcess = null
let connectionPromise = null
// Additional per-profile backends, keyed by profile name. The PRIMARY backend
// (the desktop's launch profile) stays managed by SaraProcess +
// connectionPromise + startSara(); this pool only holds EXTRA profile
...
const POOL_MAX_BACKENDS = Math.max(1, Number(process.env.Sara_DESKTOP_POOL_MAX) || 3)
const POOL_IDLE_MS = Math.max(60_000, Number(process.env.Sara_DESKTOP_POOL_IDLE_MS) || 10 * 60_000)
...
// onto the error so subsequent startSara() calls (e.g. the renderer's
...
let bootProgressState = {
  ...
  message: 'Waiting to start Sara backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
function rememberLog(chunk) {
  ...
  SaraLog.push(...lines)
  if (SaraLog.length > 300) {
    SaraLog.splice(0, SaraLog.length - 300)
  }
...
function ensureWslWindowsFonts() {
  ...
  try {
    ...
    const confPath = path.join(confDir, '99-Sara-wsl-windows-fonts.conf')
    let existing = ''
    try {
...
function broadcastBootProgress() {
  ...
  webContents.send('Sara:boot-progress', bootProgressState)
}
...
//
// The snapshot is queryable via the Sara:bootstrap:get IPC handler so a
...
function broadcastBootstrapEvent(ev) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Sara:bootstrap:event', ev)
}
...
function isSaraSourceRoot(root) {
  return directoryExists(root) && fileExists(path.join(root, 'sara_cli', 'main.py'))
}

function findPythonForRoot(root) {
  const override = process.env.Sara_DESKTOP_PYTHON
...
function findSystemPython() {
  ...
  // CPython but several Sara deps (notably pywinpty's Rust-built
...
// findGitBash — locate bash.exe on Windows. Sara' terminal tool requires
...
function findGitBash() {
  ...
  // install.ps1 drops PortableGit at %LOCALAPPDATA%\Sara\git\... — checked
  // first so users who installed via install.ps1 are detected before we
  // start probing system-wide locations.
  const localAppData = process.env.LOCALAPPDATA || ''
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'Sara', 'git', 'bin', 'bash.exe'))
    candidates.push(path.join(localAppData, 'Sara', 'git', 'usr', 'bin', 'bash.exe'))
  }
...
// install only has PortableGit under %LOCALAPPDATA%\Sara\git (never on
...
function resolveGitBinary() {
  ...
  const candidates = []
  if (localAppData) {
    candidates.push(path.join(localAppData, 'Sara', 'git', 'cmd', 'git.exe'))
    candidates.push(path.join(localAppData, 'Sara', 'git', 'bin', 'git.exe'))
  }
...
function recentSaraLog() {
  return SaraLog.slice(-20).join('\n')
}

// ─── Self-update (git-pull against the running backend's Sara root) ──────
...
// Dev → SOURCE_REPO_ROOT. Packaged/CLI install → ACTIVE_Sara_ROOT.
// Sara_DESKTOP_Sara_ROOT always wins so devs can pin a worktree.
function resolveUpdateRoot() {
  const candidates = [
    process.env.Sara_DESKTOP_Sara_ROOT && path.resolve(process.env.Sara_DESKTOP_Sara_ROOT),
    !IS_PACKAGED && isSaraSourceRoot(SOURCE_REPO_ROOT) ? SOURCE_REPO_ROOT : null,
    isSaraSourceRoot(ACTIVE_Sara_ROOT) ? ACTIVE_Sara_ROOT : null
  ].filter(Boolean)

  return candidates.find(c => directoryExists(path.join(c, '.git'))) || candidates[0] || ACTIVE_Sara_ROOT
}
...
function emitUpdateProgress(payload) {
  ...
  for (const window of BrowserWindow.getAllWindows()) {
    window.webContents.send('Sara:updates:progress', merged)
  }
}
...
// Sara_HOME/Sara-setup.exe on a successful install (see
// apps/bootstrap-installer paths::copy_self_to_Sara_home). That binary owns
// ALL repo mutation — running `Sara update` + rebuilding the desktop — so
...
function resolveUpdaterBinary() {
  const name = IS_WINDOWS ? 'Sara-setup.exe' : 'Sara-setup'
  const candidate = path.join(Sara_HOME, name)
...
// Path to the venv shim whose lock decides whether `Sara update` can write
// fresh entry points. On Windows this is the file the running backend
// `Sara.exe` holds open; on POSIX it's never mandatory-locked.
function venvSaraShimPath(updateRoot) {
  return IS_WINDOWS
    ? path.join(updateRoot, 'venv', 'Scripts', 'Sara.exe')
    : path.join(updateRoot, 'venv', 'bin', 'Sara')
}
...
// only signals the direct child, so on Windows a backend `Sara.exe` that
// spawned its own grandchildren (a `Sara` REPL, a pty terminal session, the
// gateway) would survive and keep the venv shim locked. taskkill /T /F reaps
// the whole tree synchronexvisoraly. Windows-only: this is called solely from the
...
// `SaraProcess.kill('SIGTERM')` + `app.quit()` fire-and-forget: SIGTERM on
// Windows doesn't reap the backend's grandchildren, and quit didn't wait for
// teardown, so the updater raced a still-locked `Sara.exe`, the quarantine
...
// problem: the desktop's backend (and the grandchildren IT spawned — a Sara
// REPL, a pty terminal, the gateway) keep `Sara.exe` and other files in the
...
async function releaseBackendLock(updateRoot, tag) {
  ...
  const pids = []
  if (SaraProcess && Number.isInteger(SaraProcess.pid)) pids.push(SaraProcess.pid)
  ...
  if (SaraProcess && !SaraProcess.killed) {
    try {
      SaraProcess.kill('SIGTERM')
    } catch {
      void 0
    }
  }
  ...
  const shim = venvSaraShimPath(updateRoot)
...
// `Sara update`). Instead we spawn the staged Sara-Setup binary with
// --update and quit, so it can run `Sara update` (which refuses while we
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater && !IS_WINDOWS) {
      // macOS/Linux drag-install: no staged Tauri Sara-setup. Unlike Windows
      ...
      // whole update itself: `Sara update` (backend) + `Sara desktop
    ...
    if (!updater) {
      ...
      // `Sara desktop`, never the Tauri installer that self-copies
      // Sara-setup.exe into Sara_HOME). They DO have a working `Sara`
      ...
      // checkout they're on — bare `Sara update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'Sara update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `Sara update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `Sara update` if branch detection fails.
      }
      rememberLog(`[updates] no staged updater; surfacing manual \`${command}\` for CLI install at ${updateRoot}`)
      emitUpdateProgress({ stage: 'manual', message: command, percent: null })
      return { ok: true, manual: true, command, SaraRoot: updateRoot }
    }

    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the Sara updater…', percent: 100 })
    ...
    // Sara.exe (held by the backend child / its grandchildren) and the update
    // bricks. See releaseBackendLockForUpdate for the full failure analysis.
    await releaseBackendLockForUpdate(updateRoot)

    // Detached so the updater outlives this process — it needs us GONE before
    // `Sara update` will run (the venv shim is locked while we live).
    const child = spawn(updater, updaterArgs, {
      cwd: Sara_HOME,
      env: {
        ...process.env,
        Sara_HOME,
        PATH: pathWithSaraManagedNode(venvBin)
      },
      detached: true,
      stdio: 'ignore',
      windowsHide: false
...
async function handOffWindowsBootstrapRecovery(reason) {
  ...
  const venvBin = path.join(updateRoot, 'venv', IS_WINDOWS ? 'Scripts' : 'bin')
  const venvSara = path.join(venvBin, IS_WINDOWS ? 'Sara.exe' : 'Sara')
  const updaterArgs = fileExists(venvSara) ? ['--update', '--branch', branch] : ['--repair', '--branch', branch]
...
// Resolve the Sara CLI to drive an in-app update: prefer the venv shim in
// the install we're updating, fall back to `Sara` on PATH.
function resolveSaraCliBinary(updateRoot) {
  const venvSara = path.join(updateRoot, 'venv', 'bin', 'Sara')
  if (fileExists(venvSara)) return venvSara
  return findOnPath('Sara') || null
}
...
// macOS/Linux in-app update: backend (`Sara update`) + OS-aware GUI rebuild
// (`Sara desktop --build-only`), then atomically swap the running .app bundle
// with the freshly built one and relaunch. Degrades to "backend updated,
// restart to load the new GUI" if the swap can't be performed.
async function applyUpdatesPosixInApp() {
  const updateRoot = resolveUpdateRoot()
  const Sara = resolveSaraCliBinary(updateRoot)
  if (!Sara) {
    emitUpdateProgress({ stage: 'manual', message: 'Sara update', percent: null })
    return { ok: true, manual: true, command: 'Sara update', SaraRoot: updateRoot }
  }

  // Put the Sara-managed Node and the venv on PATH so `Sara desktop`'s
  // npm build can find them on a machine with no system Node. Windows portable
  // Node lives directly under %LOCALAPPDATA%\Sara\node, not node\bin.
  const env = {
    Sara_HOME,
    PATH: pathWithSaraManagedNode(path.join(updateRoot, 'venv', 'bin'))
  }

  // `Sara update` reaps stale `Sara dashboard` backends (a code update
  ...
  // _kill_stale_dashboard_processes reads Sara_DESKTOP_CHILD_PID and excludes
  ...
  if (SaraProcess && Number.isInteger(SaraProcess.pid)) {
    desktopChildPids.push(SaraProcess.pid)
  }
  for (const entry of backendPool.values()) {
  ...
  if (desktopChildPids.length) {
    env.Sara_DESKTOP_CHILD_PID = desktopChildPids.join(',')
  }
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating Sara (git + dependencies)…', percent: 10 })
  const updated = await runStreamedUpdate(Sara, ['update', '--yes', ...branchArgs], {
    cwd: updateRoot,
    env,
    stage: 'update'
  })
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'Sara update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'Sara update failed' }
  }
  ...
  const rebuilt = await runRebuildWithRetry(attempt => {
    ...
    return runStreamedUpdate(Sara, ['desktop', '--build-only'], { cwd: updateRoot, env, stage: 'rebuild' })
  })
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart Sara to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'Sara.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'Sara.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart Sara to load the new version.',
      percent: 100
    })
...
if [ "$SRC" != "$DST" ]; then
  if /usr/bin/ditto "$SRC" "$DST.Sara-update-new"; then
    rm -rf "$DST.Sara-update-old" 2>/dev/null || true
    mv "$DST" "$DST.Sara-update-old" 2>/dev/null || rm -rf "$DST"
    mv "$DST.Sara-update-new" "$DST"
    rm -rf "$DST.Sara-update-old" 2>/dev/null || true
  fi
fi
...
/usr/bin/open "$DST"
`
  const scriptPath = path.join(app.getPath('temp'), `Sara-desktop-update-${Date.now()}.sh`)
  try {
    fs.writeFileSync(scriptPath, swapScript, { mode: 0o755 })
  } catch (err) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart Sara to load the new version.',
      percent: 100
    })
...
function isBootstrapComplete() {
  ...
  // pinned commit -- users update via the in-app update path or `Sara
  ...
  return isSaraSourceRoot(ACTIVE_Sara_ROOT) && fileExists(getVenvPython(VENV_ROOT))
}
...
function resolveWebDist() {
  const override = process.env.Sara_DESKTOP_WEB_DIST
  ...
  if (IS_PACKAGED && /app\.asar(?=$|[\\/])/.test(fallback) && !directoryExists(fallback)) {
    rememberLog(
      `[web-dist] dashboard frontend dir resolved to an asar-internal path that ` +
        ...
        `Ensure dist/** is unpacked (asarUnpack) or set Sara_DESKTOP_WEB_DIST.`
    )
  }
  return fallback
}
...
function resolveRendererIndex() {
  ...
  rememberLog(
    `[renderer] index.html not found — the desktop app was packaged without a ` +
      ...
      `Rebuild with: Sara desktop --force-build`
  )
  return candidates[0]
}
...
function resolveSaraCwd() {
  // In a packaged build, `process.cwd()` resolves to the install root (e.g.
  // `…/win-unpacked` on Windows or `/Applications/Sara.app/Contents/...`
...
function sanitizeWorkspaceCwd(cwd) {
  ...
  if (!trimmed || isPackagedInstallPath(trimmed)) {
    return { cwd: resolveSaraCwd(), sanitized: Boolean(trimmed) }
  }
...
function createPythonBackend(root, label, dashboardArgs, options = {}) {
  ...
  return {
    ...
    label,
    command: python,
    args: ['-m', 'sara_cli.main', ...dashboardArgs],
...
// createActiveBackend — build a backend pointing at ACTIVE_Sara_ROOT, the
...
function createActiveBackend(dashboardArgs) {
  ...
  return {
    kind: 'python',
    label: `Sara at ${ACTIVE_Sara_ROOT}`,
    command: fileExists(venvPython) ? venvPython : findSystemPython(),
    args: ['-m', 'sara_cli.main', ...dashboardArgs],
    env: buildDesktopBackendEnv({
      SaraHome: Sara_HOME,
      pythonPathEntries: [ACTIVE_Sara_ROOT],
      venvRoot: VENV_ROOT
    }),
    root: ACTIVE_Sara_ROOT,
    bootstrap: true,
    shell: false
  }
}

function resolveSaraBackend(dashboardArgs) {
  // 1. Explicit override -- Sara_DESKTOP_Sara_ROOT points at a developer
  //    checkout. Honour it as-is (no bootstrap; the user is driving).
  const overrideRoot = process.env.Sara_DESKTOP_Sara_ROOT && path.resolve(process.env.Sara_DESKTOP_Sara_ROOT)
  if (overrideRoot && isSaraSourceRoot(overrideRoot)) {
    const backend = createPythonBackend(overrideRoot, `Sara source at ${overrideRoot}`, dashboardArgs)
  ...
  //    installed `Sara` on PATH so local Python edits are actually exercised.
  //    (In dev with no checkout, SOURCE_REPO_ROOT won't pass isSaraSourceRoot.)
  if (!IS_PACKAGED && isSaraSourceRoot(SOURCE_REPO_ROOT)) {
    const backend = createPythonBackend(SOURCE_REPO_ROOT, `Sara source at ${SOURCE_REPO_ROOT}`, dashboardArgs)
    if (backend) return backend
  }

  // 3. Bootstrap-complete ACTIVE_Sara_ROOT -- the canonical install at
  //    %LOCALAPPDATA%\Sara\Sara-agent (Windows) or ~/.Sara/Sara-agent.
  ...
  //    to spawning Sara. Updates flow through the in-app update path
  //    (applyUpdates -> git pull) or `Sara update` from the CLI.
  if (isBootstrapComplete()) {
    return createActiveBackend(dashboardArgs)
  }

  // 4. Existing `Sara` on PATH -- installed via install.ps1 / install.sh from
  //    a previous tool-only setup, or pip-installed system-wide. Use it but
  //    do NOT write a bootstrap marker; the user did this themselves and we
  //    don't want to take ownership of an install we didn't perform.
  //    Sara_DESKTOP_IGNORE_EXISTING=1 forces the bootstrap path for testing.
  if (process.env.Sara_DESKTOP_IGNORE_EXISTING !== '1') {
    let SaraCommand = null
    const SaraOverride = process.env.Sara_DESKTOP_Sara

    if (SaraOverride) {
      const resolvedOverride = findOnPath(SaraOverride)
      if (resolvedOverride) {
        SaraCommand = resolvedOverride
      } else if (!isWindowsBinaryPathInWsl(SaraOverride, { isWsl: IS_WSL })) {
        SaraCommand = SaraOverride
      } else {
        rememberLog(`Ignoring Windows Sara override under WSL: ${SaraOverride}`)
      }
    } else {
      SaraCommand = findOnPath('Sara')
    }

    if (SaraCommand) {
      if (looksLikeDesktopAppBinary(SaraCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving Sara CLI: ${SaraCommand}`)
        SaraCommand = null
      }
    }

    if (SaraCommand) {
      // Smoke-test the candidate before trusting it. A `Sara` shim
      ...
      const shellForProbe = isCommandScript(SaraCommand)
      if (verifySaraCli(SaraCommand, { shell: shellForProbe })) {
        return {
          label: `existing Sara CLI at ${SaraCommand}`,
          command: SaraCommand,
          args: dashboardArgs,
          bootstrap: false,
          env: {},
      ...
      rememberLog(
        `Ignoring existing Sara CLI at ${SaraCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }

  // 5. Last-ditch: pip-installed Sara_cli module via system Python.
  ...
  if (python) {
    ...
    // having Sara_cli installed -- common on dev boxes that have
    ...
    // a uv-managed 3.11 into %LOCALAPPDATA%\Sara\Sara-agent\venv.
    if (canImportSaraCli(python)) {
      return {
        kind: 'python',
        label: `installed Sara_cli module via ${python}`,
        command: python,
        args: ['-m', 'sara_cli.main', ...dashboardArgs],
        bootstrap: false,
        env: {},
    ...
    rememberLog(`Ignoring system Python ${python}: Sara_cli is not importable; falling through to bootstrap.`)
  }
  ...
  //    resolveSaraBackend was the old "no payload" path and forced the
  //    user into a dead end. With the bootstrap protocol, "no install yet"
  //    is a recoverable state the GUI can drive through.
  return {
    kind: 'bootstrap-needed',
    label: 'Sara Agent not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  // backend.kind === 'bootstrap-needed' means resolveSaraBackend couldn't
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no Sara install found; starting first-launch bootstrap')

    if (await handOffWindowsBootstrapRecovery('bootstrap-needed')) {
      const handoffError = new Error('Sara recovery was handed off to Sara Setup. The desktop will restart when recovery completes.')
    ...
    const bootstrapResult = await runBootstrap({
      ...
      SaraHome: Sara_HOME,
      logRoot: path.join(Sara_HOME, 'logs'),
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('Sara install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `Sara bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
          `${bootstrapResult.error || 'unknown error'}. ` +
          `Check ${path.join(Sara_HOME, 'logs', 'desktop.log')} for the full transcript.`
      )
      bootstrapError.isBootstrapFailure = true
      bootstrapError.failedStage = bootstrapResult.failedStage || null
      // Latch the failure so subsequent startSara() calls return this
      // same error without re-running install.ps1.  Cleared by the
      // Sara:bootstrap:reset IPC (renderer's "Reload and retry").
    ...
    return ensureRuntime(resolveSaraBackend(backend.args))
  }
  ...
  if (!isSaraSourceRoot(ACTIVE_Sara_ROOT)) {
    throw new Error(
      `Sara install at ${ACTIVE_Sara_ROOT} is missing or incomplete. ` +
        'Reinstall via the desktop installer or scripts/install.ps1.'
    )
  }

  // On Windows, preflight Git Bash. Sara' terminal tool calls bash.exe
  ...
  // %LOCALAPPDATA%\Sara\git\, which findGitBash() picks up, so for any
  // user who completed the bootstrap this is a no-op. For users who got
  // here via an external `Sara` on PATH, this check still helps.
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for Sara on Windows (provides Git Bash, ' +
  ...
  if (!fileExists(venvPython)) {
    ...
    // isSaraSourceRoot() and the bootstrap writes the marker only after
    // install.ps1 succeeds. If we hit this, the user (or a deleted venv)
    // broke the invariant; tell them to re-run the install.
    throw new Error(
      `Sara venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `Sara at ${ACTIVE_Sara_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'Sara runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported Sara backend URL protocol: ${parsed.protocol}`))
      return
    }
    ...
    const req = client.request(
      ...
      res => {
        ...
        res.on('end', () => {
          ...
          console.log(`[fetchJson] ${method} ${url} → ${res.statusCode}`)
          if ((res.statusCode || 500) >= 400) {
            console.error(`[fetchJson] ERROR ${res.statusCode}: ${(text || res.statusMessage).slice(0, 200)}`)
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the Sara backend.'
              )
            )
            return
          }
    ...
    req.on('error', err => {
      console.error(`[fetchJson] NETWORK ERROR ${method} ${url}: ${err.message}`)
      reject(err)
    })
    req.setTimeout(timeoutMs, () => {
      console.error(`[fetchJson] TIMEOUT ${method} ${url} after ${timeoutMs}ms`)
      req.destroy(new Error(`Timed out connecting to Sara backend after ${timeoutMs}ms`))
    })
...
function fetchPublicJson(url, options = {}) {
  ...
  // NO ``X-Sara-Session-Token`` header — used by the auth-mode probe before
...
function getLinkTitleSession() {
  ...
  linkTitleSession = session.fromPartition('Sara:link-titles', { cache: false })
...
async function previewFileTarget(rawTarget, baseDir) {
  ...
  const base = baseDir ? path.resolve(expandUserPath(baseDir)) : resolveSaraCwd()
...
function sendPreviewFileChanged(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Sara:preview-file-changed', payload)
}
...
async function waitForSara(baseUrl, token) {
  ...
  throw new Error(`Sara backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
function sendBackendExit(payload) {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Sara:backend-exit', payload)
}
...
function sendClosePreviewRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Sara:close-preview-requested')
}

// Tell the renderer the machine just woke. Sleep silently drops the
// renderer's WebSocket to the local backend; the renderer reconnects on this
// signal so the chat composer doesn't stay stuck on "Starting Sara...".
function sendPowerResume() {
  if (!mainWindow || mainWindow.isDestroyed()) return
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Sara:power-resume')
}
...
function sendOpenUpdatesRequested() {
  ...
  const { webContents } = mainWindow
  if (!webContents || webContents.isDestroyed()) return
  webContents.send('Sara:open-updates')
...
function sendWindowStateChanged(nextIsFullscreen) {
  ...
  webContents.send('Sara:window-state-changed', state)
}
...
const ZOOM_STORAGE_KEY = 'Sara:desktop:zoomLevel'
...
function installMediaPermissions() {
  ...
  // Synchronexvisora check handler: Chromium consults this for getUserMedia on
...
//
// Hosted Sara gateways gate the dashboard behind an OAuth provider (e.g.
// nexvisora Research) instead of a static session token. The auth model is
// fundamentally different from the token path:
//
//   * REST is authed by HttpOnly session cookies (``Sara_session_at``),
...
//   * nexvisora Portal now issues a 24h ROTATING, reuse-detected refresh token
//     alongside the ~15-min access token (Portal NAS #293 / Sara #37247).
//     Both are set as HttpOnly cookies (``Sara_session_at`` ~15 min,
//     ``Sara_session_rt`` 24h). When the AT cookie lapses but the RT cookie
...
const OAUTH_SESSION_PARTITION = 'persist:Sara-remote-oauth'
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to Sara backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
// a valid stored value (pins the root Sara_HOME explicitly); null means "no
...
async function sanitizeDesktopConnectionConfig(config = readDesktopConnectionConfig(), profile = null) {
  ...
  const envOverride = key ? false : Boolean(process.env.Sara_DESKTOP_REMOTE_URL)

  const remoteToken = decryptDesktopSecret(block.token)
  const authMode = normAuthMode(block.authMode)
  const remoteUrl = envOverride ? String(process.env.Sara_DESKTOP_REMOTE_URL || '') : String(block.url || '')
  ...
  return {
    ...
    // scope is never overridden by Sara_DESKTOP_REMOTE_URL.
    envOverride
  }
}
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    // Portal issues a 24h rotating refresh token (Sara #37247), and the
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote Sara gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote Sara gateway is selected, but no session token is saved. ' +
...
//   2. env override (Sara_DESKTOP_REMOTE_URL/_TOKEN) — applies app-wide
...
async function resolveRemoteBackend(profile) {
  ...
  const rawEnvUrl = process.env.Sara_DESKTOP_REMOTE_URL
  const rawEnvToken = process.env.Sara_DESKTOP_REMOTE_TOKEN
  if (rawEnvUrl) {
    if (!rawEnvToken) {
      throw new Error(
        'Sara_DESKTOP_REMOTE_URL is set but Sara_DESKTOP_REMOTE_TOKEN is not. ' +
          'Both must be provided to connect to a remote Sara backend.'
      )
    }
...
function globalRemoteActive() {
  if (process.env.Sara_DESKTOP_REMOTE_URL) {
    return true
  }
...
async function probeRemoteAuthMode(rawUrl) {
  ...
  // sending any credentials. ``/api/status`` is public on every Sara
  ...
  if (authRequired) {
    ...
    // button can read "Sign in with nexvisora Research" instead of a generic
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  if (wantRemote && block?.url) {
    ...
    const remote = (await resolveRemoteBackend(key)) || (await startSara())
  ...
  // connect to Sara gateway". Mirror the renderer's connect here so the test
...
// startSara() spawns fresh instead of racing the dying one. Shared by the
// connection-config and profile switch flows.
async function teardownPrimaryBackendAndWait() {
  // Capture the reference before resetSaraConnection() nulls SaraProcess.
  const dying = SaraProcess && !SaraProcess.killed ? SaraProcess : null
  resetSaraConnection()
...
// profile to startSara() (the window backend: boot UI, bootstrap, remote
...
// local-spawn portion of startSara() but without the boot-progress UI,
...
async function spawnPoolBackend(profile, entry) {
  ...
  if (remote) {
    await waitForSara(remote.baseUrl, remote.token)
    return {
      ...remote,
      profile,
      logs: SaraLog.slice(-80),
      ...getWindowState()
    }
  }

  const token = crypto.randomBytes(32).toString('base64url')
  // --profile wins over the inherited Sara_HOME env (see _apply_profile_override
  // step 3 in Sara_cli/main.py), so the child re-homes to this profile.
  // --port 0: the OS assigns an ephemeral port; the child announces it on stdout.
  const dashboardArgs = ['--profile', profile, 'dashboard', '--no-open', '--host', '127.0.0.1', '--port', '0']
  const backend = await ensureRuntime(resolveSaraBackend(dashboardArgs))
  const SaraCwd = resolveSaraCwd()
  const webDist = resolveWebDist()

  rememberLog(`Starting Sara backend for profile "${profile}" via ${backend.label}`)
  ...
  const child = spawn(
    ...
    hiddenWindowsChildOptions({
      ...
      env: {
        ...
        TERMINAL_CWD: SaraCwd,
        Sara_DASHBOARD_SESSION_TOKEN: token,
        // Marks this dashboard backend as desktop-spawned so it runs the cron
        // scheduler tick loop (the gateway isn't running under the app).
        Sara_DESKTOP: '1',
        Sara_WEB_DIST: webDist,
        sara_DASHBOARD_TUI: '1',
      },
      shell: backend.shell,
      stdio: ['ignore', 'pipe', 'pipe']
    })
  ...
  child.once('error', error => {
    rememberLog(`Sara backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`Sara backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(
        new Error(`Sara backend for profile "${profile}" exited before it became ready (${signal || code}).`)
      )
    }
  })
  ...
  await Promise.race([waitForSara(baseUrl, token), startFailed])
  ready = true
  const authToken = await adoptServedDashboardToken(baseUrl, token, {
    childAlive: () => child.exitCode === null && !child.killed,
    label: `Sara backend for profile "${profile}"`,
    rememberLog
  })
...
async function startSara() {
  ...
  // process, every subsequent startSara() call re-throws the same error
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving Sara backend', 8)
    // Resolve for the desktop's primary profile so a per-profile remote
    // override on the active profile is honored (falls back to env / global).
    const remote = await resolveRemoteBackend(primaryProfileKey())
    if (remote) {
      await advanceBootProgress('backend.remote', `Connecting to remote Sara backend at ${remote.baseUrl}`, 24)
      await waitForSara(remote.baseUrl, remote.token)
      updateBootProgress({
        phase: 'backend.ready',
        message: 'Remote Sara backend is ready',
        progress: 94,
        running: true,
        error: null
      })
    ...
    // deterministic (it wins over the sticky ~/.Sara/active_profile file) and
    // resolves Sara_HOME the same way `Sara -p <name>` does on the CLI. An
    ...
    await advanceBootProgress('backend.runtime', 'Resolving Sara runtime', 28)
    const backend = await ensureRuntime(resolveSaraBackend(dashboardArgs))
    const SaraCwd = resolveSaraCwd()
    const webDist = resolveWebDist()

    await advanceBootProgress('backend.spawn', `Starting Sara backend via ${backend.label}`, 84)
    rememberLog(`Starting Sara backend via ${backend.label}`)
    ...
    SaraProcess = spawn(
      ...
      hiddenWindowsChildOptions({
        ...
        env: {
          ...process.env,
          // Explicitly pin Sara_HOME for the child so Python's get_Sara_home()
          // resolves to the SAME location our resolveSaraHome() picked. Without
          // this pin, Python falls back to ~/.Sara on every platform — fine on
          // mac/linux (where our default matches), but on Windows our default is
          // %LOCALAPPDATA%\Sara, which differs from C:\Users\<u>\.Sara.
          // Mismatch would split config / sessions / .env / logs across two
          // directories. install.ps1 sets Sara_HOME via setx; the desktop
          ...
          Sara_DESKTOP: '1',
          Sara_WEB_DIST: webDist,
          // Enable the embedded-chat WebSocket route so the renderer can connect.
          sara_DASHBOARD_TUI: '1',
          // Unbuffer Python stdout so the DASHBOARD_READY port signal is
          // delivered to Electron immediately, not stuck in a pipe buffer.
          PYTHONUNBUFFERED: '1'
        },
        shell: backend.shell,
        stdio: ['ignore', 'pipe', 'pipe']
    ...
    SaraProcess.stdout.on('data', rememberLog)
    SaraProcess.stderr.on('data', rememberLog)
    let backendReady = false
    let rejectBackendStart = null
    const backendStartFailed = new Promise((_resolve, reject) => {
      rejectBackendStart = reject
    })
    SaraProcess.once('error', error => {
      rememberLog(`Sara backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `Sara backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    SaraProcess.once('exit', (code, signal) => {
      rememberLog(`Sara backend exited (${signal || code})`)
      SaraProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `Sara backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
        ...
        rejectBackendStart?.(
          new Error(
            `Sara backend exited before it became ready (${signal || code}). Log: ${DESKTOP_LOG_PATH}\n${recentSaraLog()}`
          )
        )
      }
    })

    await advanceBootProgress('backend.port', 'Waiting for Sara backend to launch', 86)
    // Discover the ephemeral port the child bound to
    const port = await Promise.race([waitForDashboardPort(SaraProcess), backendStartFailed])

    const baseUrl = `http://127.0.0.1:${port}`
    await advanceBootProgress('backend.wait', 'Waiting for Sara backend to become ready', 90)
    await Promise.race([waitForSara(baseUrl, token), backendStartFailed])
    backendReady = true
    const authToken = await adoptServedDashboardToken(baseUrl, token, {
      // The exit/error handlers null SaraProcess when the child dies.
      childAlive: () => SaraProcess !== null && SaraProcess.exitCode === null && !SaraProcess.killed,
      rememberLog
    })
    updateBootProgress({
      phase: 'backend.ready',
      message: 'Sara backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
function createWindow() {
  ...
  mainWindow.webContents.once('did-finish-load', () => {
    ...
    startSara().catch(error => rememberLog(error.stack || error.message))
  })
}

ipcMain.handle('Sara:connection', async (_event, profile) => ensureBackend(profile))
...
// "Starting Sara…". Before the renderer's backoff loop reconnects, it asks us
// to confirm the cached PRIMARY backend is still reachable; if a remote one is
// not, we drop the cache so the next getConnection() rebuilds it. Local backends
// self-heal via their child 'exit' handler, so we never touch them here.
ipcMain.handle('Sara:connection:revalidate', async () => {
  ...
  try {
    ...
    // tick rebuilds a fresh, reachable descriptor. resetSaraConnection only
    // nulls connectionPromise for a remote (no child to SIGTERM).
    rememberLog('Cached remote Sara backend failed liveness probe; dropping stale connection.')
    resetSaraConnection()
    return { ok: true, rebuilt: true }
  }
})
ipcMain.handle('Sara:backend:touch', async (_event, profile) => {
  touchPoolBackend(profile)
  return { ok: true }
})
ipcMain.handle('Sara:gateway:ws-url', async (_event, profile) => freshGatewayWsUrl(profile))
ipcMain.handle('Sara:window:openSession', async (_event, sessionId, opts) => {
...
ipcMain.handle('Sara:window:openNewSession', async () => {
  createNewSessionWindow()

  return { ok: true }
})
ipcMain.handle('Sara:bootstrap:reset', async () => {
  // Renderer's "Reload and retry" path. Clear the latched failure and
  // reset connection state so the next startSara() call restarts the
...
ipcMain.handle('Sara:bootstrap:repair', async () => {
  // Forceful repair: drop the bootstrap-complete marker so the next
  // startSara() re-runs the full installer (refreshing a broken/partial
...
ipcMain.handle('Sara:bootstrap:cancel', async () => {
...
ipcMain.handle('Sara:boot-progress:get', async () => bootProgressState)
ipcMain.handle('Sara:bootstrap:get', async () => getBootstrapState())
ipcMain.handle('Sara:connection-config:get', async (_event, profile) =>
  sanitizeDesktopConnectionConfig(readDesktopConnectionConfig(), profile)
)
ipcMain.handle('Sara:connection-config:test', async (_event, payload) => testDesktopConnectionConfig(payload))
ipcMain.handle('Sara:connection-config:probe', async (_event, rawUrl) => probeRemoteAuthMode(rawUrl))
ipcMain.handle('Sara:connection-config:oauth-login', async (_event, rawUrl) => {
...
ipcMain.handle('Sara:connection-config:oauth-logout', async (_event, rawUrl) => {
...
ipcMain.handle('Sara:connection-config:save', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)

  return sanitizeDesktopConnectionConfig(config, payload?.profile)
})
ipcMain.handle('Sara:connection-config:apply', async (_event, payload) => {
  const config = coerceDesktopConnectionConfig(payload)
  writeDesktopConnectionConfig(config)
...
ipcMain.handle('Sara:profile:get', async () => ({ profile: readActiveDesktopProfile() }))
ipcMain.handle('Sara:profile:set', async (_event, name) => {
  const next = writeActiveDesktopProfile(name)

  // Switching profiles is a backend re-home: relaunch the dashboard under the
  // new Sara_HOME. Pool backends keep their own homes, so only the primary
  // is torn down.
  await teardownPrimaryBackendAndWait()
  mainWindow?.reload()
...
ipcMain.on('Sara:previewShortcutActive', (_event, active) => {
  previewShortcutActive = Boolean(active)
})

ipcMain.handle('Sara:requestMicrophoneAccess', async () => {
...
ipcMain.handle('Sara:api', async (_event, request) => {
...
ipcMain.handle('Sara:notify', (_event, payload) => {
  ...
  notification.on('click', () => {
    ...
    if (payload?.sessionId) {
      mainWindow.webContents.send('Sara:focus-session', payload.sessionId)
    }
  })
  notification.on('action', (_actionEvent, index) => {
    ...
    if (action?.id) {
      mainWindow.webContents.send('Sara:notification-action', { sessionId: payload?.sessionId, actionId: action.id })
    }
  })
  notification.show()
  return true
...
ipcMain.handle('Sara:readFileDataUrl', async (_event, filePath) => {
...
ipcMain.handle('Sara:readFileText', async (_event, filePath) => {
...
ipcMain.handle('Sara:selectPaths', async (_event, options = {}) => {
...
ipcMain.handle('Sara:writeClipboard', (_event, text) => {
  clipboard.writeText(String(text || ''))
  return true
})

ipcMain.handle('Sara:saveImageFromUrl', (_event, url) => saveImageFromUrl(String(url || '')))

ipcMain.handle('Sara:saveImageBuffer', async (_event, payload) => {
...
ipcMain.handle('Sara:saveClipboardImage', async () => {
...
ipcMain.handle('Sara:normalizePreviewTarget', (_event, target, baseDir) =>
  normalizePreviewTarget(String(target || ''), baseDir ? String(baseDir) : '')
)

ipcMain.handle('Sara:watchPreviewFile', (_event, url) => watchPreviewFile(String(url || '')))

ipcMain.handle('Sara:stopPreviewFileWatch', (_event, id) => stopPreviewFileWatch(String(id || '')))

ipcMain.on('Sara:titlebar-theme', (_event, payload) => {
...
ipcMain.on('Sara:native-theme', (_event, mode) => {
...
ipcMain.on('Sara:translucency', (_event, payload) => {
...
ipcMain.handle('Sara:openExternal', (_event, url) => {
...
// it via writeDefaultProjectDir so resolveSaraCwd picks it up on the next
// session spawn (no app restart needed).
ipcMain.handle('Sara:setting:defaultProjectDir:get', async () => ({
  dir: readDefaultProjectDir(),
  defaultLabel: app.getPath('home'),
  resolvedCwd: resolveSaraCwd()
}))

ipcMain.handle('Sara:workspace:sanitize', async (_event, cwd) => sanitizeWorkspaceCwd(cwd))

ipcMain.handle('Sara:setting:defaultProjectDir:set', async (_event, dir) => {
...
ipcMain.handle('Sara:setting:defaultProjectDir:pick', async () => {
...
ipcMain.handle('Sara:fetchLinkTitle', (_event, url) => fetchLinkTitle(url))

ipcMain.handle('Sara:logs:reveal', async () => {
  try {
    await fs.promises.mkdir(path.dirname(DESKTOP_LOG_PATH), { recursive: true })
...
ipcMain.handle('Sara:logs:recent', async () => ({ path: DESKTOP_LOG_PATH, lines: SaraLog.slice(-200) }))
...
function terminalShellCommand() {
  // Sara_DESKTOP_SHELL is the cross-platform escape hatch (a path or a bare
  ...
  const override = (process.env.Sara_DESKTOP_SHELL || (IS_WINDOWS ? '' : process.env.SHELL) || '').trim()
...
function terminalShellEnv() {
  ...
  // /TERM=dumb; some terminals set COLORFGBG which would flip Sara' TUI into
  ...
  env.TERM_PROGRAM = 'Sara'
  env.TERM_PROGRAM_VERSION = app.getVersion()

  // Let a Sara/--tui launched in this pane know it's embedded in the desktop
  // GUI (build_environment_hints surfaces this). Distinct from Sara_DESKTOP,
  // which marks the agent *backend* and gates cron/gateway behavior.
  env.Sara_DESKTOP_TERMINAL = '1'

  return env
}

function terminalChannel(id, suffix) {
  return `Sara:terminal:${id}:${suffix}`
}
...
ipcMain.handle('Sara:fs:readDir', async (_event, dirPath) => readDirForIpc(dirPath))

ipcMain.handle('Sara:fs:gitRoot', async (_event, startPath) => gitRootForIpc(startPath))

ipcMain.handle('Sara:fs:worktrees', async (_event, cwds) => worktreesForIpc(cwds))

ipcMain.handle('Sara:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart Sara.')
  }
...
ipcMain.handle('Sara:terminal:write', (_event, id, data) => {
...
ipcMain.handle('Sara:terminal:resize', (_event, id, size = {}) => {
  const sessionInfo = terminalSessions.get(String(id || ''))
...
ipcMain.handle('Sara:terminal:dispose', (_event, id) => disposeTerminalSession(String(id || '')))

ipcMain.handle('Sara:updates:check', async () =>
...
ipcMain.handle('Sara:updates:apply', async (_event, payload) =>
...
ipcMain.handle('Sara:updates:branch:get', async () => readDesktopUpdateConfig())

ipcMain.handle('Sara:updates:branch:set', async (_event, name) => {
  const branch = typeof name === 'string' && name.trim() ? name.trim() : DEFAULT_UPDATE_BRANCH
  writeDesktopUpdateConfig({ branch })
  return { branch }
})

// Resolve the canonical Sara version (the one `release.py` bumps in
// Sara_cli/__init__.py + pyproject.toml) so the desktop About panel shows the
// real Sara version instead of the Electron app's own package.json version,
// which historically drifted (stuck at 0.0.2). Falls back to app.getVersion()
// when the source tree can't be read (e.g. a packaged build without the repo).
function resolveSaraVersion() {
  try {
    const root = resolveUpdateRoot()
    const initPath = path.join(root, 'sara_cli', '__init__.py')
...
// Re-resolve the live Sara version and push it into the native About panel
// just before showing it, so an in-place `Sara update` is reflected without
...
ipcMain.handle('Sara:version', async () => ({
  appVersion: resolveSaraVersion(),
  electronVersion: process.versions.electron,
  nodeVersion: process.versions.node,
  platform: process.platform,
  SaraRoot: resolveUpdateRoot()
}))
...
// the actual removal via `Sara uninstall …` so the cross-platform PATH /
// registry / service / node-symlink cleanup all lives in one place
// (Sara_cli/uninstall.py + Sara_cli/gui_uninstall.py).
//
...
async function getUninstallSummary() {
  ...
  const agentRoot = ACTIVE_Sara_ROOT
  ...
  const fallback = () => ({
    Sara_home: Sara_HOME,
    agent_installed: isSaraSourceRoot(agentRoot) && fileExists(py),
    gui_installed: true,
  ...
  return new Promise(resolve => {
    ...
    try {
      const child = spawn(
        py,
        ['-m', 'sara_cli.main', 'uninstall', '--gui-summary'],
        hiddenWindowsChildOptions({
          cwd: agentRoot,
          env: { ...process.env, Sara_HOME, NO_COLOR: '1' },
          stdio: ['ignore', 'pipe', 'ignore']
        })
      )
      child.stdout.on('data', chunk => {
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no Sara agent venv at ${VENV_ROOT}.`
    }
  }
  ...
  // Python with PYTHONPATH=<agentRoot> so `import Sara_cli` resolves from
  ...
  try {
    await releaseBackendLock(ACTIVE_Sara_ROOT, 'uninstall')
  } catch (error) {
  ...
  try {
    if (IS_WINDOWS) {
      scriptPath = path.join(app.getPath('temp'), `Sara-uninstall-${Date.now()}.cmd`)
      ...
      scriptPath = path.join(app.getPath('temp'), `Sara-uninstall-${Date.now()}.sh`)
...
ipcMain.handle('Sara:uninstall:summary', async () => getUninstallSummary())
ipcMain.handle('Sara:uninstall:run', async (_event, payload) => {
...
ipcMain.handle('Sara:vscode-theme:fetch', async (_event, id) => fetchMarketplaceThemes(String(id || '')))

// Search the Marketplace for color-theme extensions (empty query = top installs).
ipcMain.handle('Sara:vscode-theme:search', async (_event, query) => searchMarketplaceThemes(String(query || ''), 20))

// ---------------------------------------------------------------------------
// Sara:// deep links (e.g. Sara://blueprint/morning-brief?time=08:00).
// A docs/dashboard "Send to App" button opens this URL; we route it into the
// running app's chat composer. Three delivery paths: macOS 'open-url',
// Win/Linux running-app 'second-instance' (argv), Win/Linux cold-start argv.
// ---------------------------------------------------------------------------
const Sara_PROTOCOL = 'Sara'
let _pendingDeepLink = null
let _rendererReadyForDeepLink = false

function _extractDeepLink(argv) {
  if (!Array.isArray(argv)) return null
  return argv.find(a => typeof a === 'string' && a.startsWith(`${Sara_PROTOCOL}://`)) || null
}
...
function handleDeepLink(url) {
  ...
  // Sara://blueprint/<key>?slot=val  -> host="blueprint", path="/<key>"
  ...
  try {
    ...
    mainWindow.focus()
    mainWindow.webContents.send('Sara:deep-link', payload)
...
ipcMain.handle('Sara:deep-link-ready', () => {
  _rendererReadyForDeepLink = true
  if (_pendingDeepLink) {
    const queued = _pendingDeepLink
    _pendingDeepLink = null
    handleDeepLink(
      `${Sara_PROTOCOL}://${queued.kind}/${encodeURIComponent(queued.name)}` +
...
function registerDeepLinkProtocol() {
  try {
    if (process.defaultApp && process.argv.length >= 2) {
      ...
      app.setAsDefaultProtocolClient(Sara_PROTOCOL, process.execPath, [path.resolve(process.argv[1])])
    } else {
      app.setAsDefaultProtocolClient(Sara_PROTOCOL)
    }
  } catch (err) {
...
// second-instance argv. Without the lock a second `Sara://` launch spawns a
...
app.whenReady().then(() => {
  ...
  // Win/Linux cold start: the launching Sara:// URL is in our own argv.
