...
function mkTmpHome() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'Sara-bootstrap-test-'))
}
...
test('runBootstrap bails immediately when the signal is already aborted', async () => {
  ...
  const result = await runBootstrap({
    installStamp: null,
    activeRoot: '/tmp/Sara-runner-test',
    sourceRepoRoot: null,
    SaraHome: '/tmp/Sara-runner-test',
    logRoot: '/tmp/Sara-runner-test',
...
test('installedAgentInstallScript resolves the installer in the agent checkout', () => {
  ...
  try {
    ...
    const scriptsDir = path.join(home, 'Sara-agent', 'scripts')
