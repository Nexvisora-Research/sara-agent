const _READY_RE = /^SARA_DASHBOARD_READY port=(\d+)/m

/**
 * Watch a child process's stdout for the `SARA_DASHBOARD_READY port=<N>`
...
function waitForDashboardPort(child, timeoutMs = 45_000) {
  return new Promise((resolve, reject) => {
    ...
    function onExit(code, signal) {
      cleanup()
      reject(new Error(`Sara backend: exited before port announcement (${signal || code})`))
    }
    ...
    const timer = setTimeout(() => {
      cleanup()
      reject(new Error(`Timed out waiting for Sara backend port announcement (${timeoutMs}ms)`))
    }, timeoutMs)
