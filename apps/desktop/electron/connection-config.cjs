 ...
 *     `X-Sara-Session-Token` header; WS uses `?token=`.
...
// Sara_cli/dashboard_auth/cookies.py.
//
// Two cookies are in play (see that module):
//   - Sara_session_at: the OAuth access token. Short-lived (~15 min); its
//     Max-Age tracks the access-token TTL, so the cookie jar drops it the
//     instant the AT expires.
//   - Sara_session_rt: the OAuth refresh token. Long-lived (24h rotating,
//     reuse-detected — Portal NAS #293 / Sara #37247). When the AT cookie
...
const AT_COOKIE_VARIANTS = ['__Host-Sara_session_at', '__Secure-Sara_session_at', 'Sara_session_at']
const RT_COOKIE_VARIANTS = ['__Host-Sara_session_rt', '__Secure-Sara_session_rt', 'Sara_session_rt']
...
function pathWithGlobalRemoteProfile(path, profile, opts = {}) {
  ...
  let parsed
  try {
    parsed = new URL(rawPath, 'http://Sara.local')
  } catch {
    return path
  }
...
/**
 * True if any cookie in `cookies` is a Sara session ACCESS-token cookie
