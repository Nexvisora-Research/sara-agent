...
// even though a fresh shell — and the Sara CLI — sees it immediately. The
// desktop's Sara_HOME resolution relies on process.env, so that stale-snapshot
// gap silently sends the backend to the default %LOCALAPPDATA%\Sara. Reading
