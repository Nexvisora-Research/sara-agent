...
import { BUILTIN_THEME_LIST, BUILTIN_THEMES, DEFAULT_SKIN_NAME, DEFAULT_TYPOGRAPHY, nexvisoraTheme } from './presets'
...
const SKIN_KEY = 'Sara-desktop-theme-v2'
const MODE_KEY = 'Sara-desktop-mode-v1'
...
const PROFILE_SKINS_KEY = 'Sara-desktop-profile-themes-v1'
const PROFILE_MODES_KEY = 'Sara-desktop-profile-modes-v1'
// Last active profile, recorded so the boot-time paint can pick that profile's
// theme before the gateway reports which profile actually launched.
const LAST_PROFILE_KEY = 'Sara-desktop-active-profile-v1'
const RETIRED_SKINS = new Set(['nexvisora-light', 'default', 'gold'])
...
export function getBaseColors(skinName: string, mode: 'light' | 'dark'): DesktopThemeColors {
  const seed = resolveTheme(skinName) ?? nexvisoraTheme
...
function applyTheme(theme: DesktopTheme, mode: 'light' | 'dark') {
  ...
  const typo = { ...DEFAULT_TYPOGRAPHY, ...nexvisoraTheme.typography, ...theme.typography }
  ...
  root.dataset.SaraTheme = skinName
  root.dataset.SaraMode = rendered
  ...
  window.SaraDesktop?.setTitleBarTheme?.({
  ...
  try {
    window.localStorage.setItem('Sara-boot-background', c.background)
    window.localStorage.setItem('Sara-boot-color-scheme', rendered)
  } catch {
  ...
  if (typo.fontUrl && !INJECTED_FONT_URLS.has(typo.fontUrl)) {
    ...
    link.dataset.SaraThemeFont = 'true'
...
const syncNativeTheme = (pref: ThemeMode, rendered: 'light' | 'dark') =>
  window.SaraDesktop?.setNativeTheme?.(pref === 'system' ? 'system' : rendered)
...
/** Sync the desktop skin with the active Sara backend theme on connect. */
