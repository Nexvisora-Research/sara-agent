...
import { getGlobalModelOptions } from '@/Sara'
...
import type { ModelOptionProvider, OAuthProvider } from '@/types/Sara'
...
const API_KEY_OPTIONS: ApiKeyOption[] = [
  ...
  },
  {
    ...
    docsUrl: 'https://github.com/nexvisoraResearch/Sara-agent#bring-your-own-endpoint',
    placeholder: 'http://127.0.0.1:8000/v1'
  }
]

// Build the FULL API-key provider catalog from the backend model options so the
// onboarding / Providers key form lists every `api_key` provider `Sara model`
...
function useApiKeyCatalog(): ApiKeyOption[] {
  ...
  useEffect(() => {
    ...
    // Promise.resolve().then so a synchronexvisora throw (e.g. no desktop bridge in
...
const PROVIDER_DISPLAY: Record<string, { order: number; title: string }> = {
  nexvisora: { order: 0, title: 'nexvisora Portal' },
    ...
    >
      <div
        className={cn(
          ...
          bare
            ? ''
            : 'overflow-hidden rounded-xl border border-(--stroke-nexvisora) bg-(--ui-chat-bubble-background) shadow-nexvisora',
...
export const FEATURED_ID = 'nexvisora'
const SHOW_ALL_KEY = 'Sara-onboarding-show-all-v1'
...
export function Picker({ ctx }: { ctx: OnboardingContext }) {
  ...
  // Collapse the secondary providers behind a disclosure only when nexvisora
...
export function FeaturedProviderRow({
  ...
  return (
    <button
      ...
      type="button"
    >
      <span aria-hidden className="arc-border arc-reverse arc-nexvisora" />
          ...
          <span
            className={cn(
              ...
              copied
                ...
                : 'border-(--stroke-nexvisora) text-foreground group-hover:border-(--ui-stroke-secondary)'
            )}
            key={i}
          >
            {ch}
...
function CodeBlock({ copied, onCopy, text }: { copied: boolean; onCopy: () => void; text: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md border border-(--stroke-nexvisora) px-3 py-2">
...
function HackeryButton({
  ...
  return (
    <button
      className={cn(
        'group inline-flex items-center gap-2 rounded-md border border-(--stroke-nexvisora) px-6 py-2.5',
