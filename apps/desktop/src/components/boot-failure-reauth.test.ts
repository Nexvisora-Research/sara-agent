...
describe('deriveProviderShape', () => {
  ...
  it('OAuth shape when the provider is a redirect IDP', () => {
    expect(deriveProviderShape([{ name: 'nexvisora', displayName: 'nexvisora Research', supportsPassword: false }])).toEqual({
      isPassword: false,
      providerLabel: 'nexvisora Research'
    })
  })
  ...
  it('mixed deployment keeps generic OAuth copy (not every provider is password)', () => {
    const shape = deriveProviderShape([
      ...
      { name: 'nexvisora', displayName: 'nexvisora Research', supportsPassword: false }
    ])

    expect(shape.isPassword).toBe(false)
    expect(shape.providerLabel).toBe('Username & Password / nexvisora Research')
  })
...
describe('signInLabel', () => {
  ...
  it('OAuth gateway names the provider', () => {
    expect(signInLabel({ url: 'x', isPassword: false, providerLabel: 'nexvisora Research' })).toBe(
      'Sign in with nexvisora Research'
    )
  })
