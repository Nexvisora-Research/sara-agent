...
export function BootFailureOverlay() {
  ...
  const retry = async () => {
    setBusy('retry')
    await window.SaraDesktop?.resetBootstrap().catch(() => undefined)
    window.location.reload()
  }

  const repair = async () => {
    setBusy('repair')
    await window.SaraDesktop?.repairBootstrap().catch(() => undefined)
    window.location.reload()
  }
  ...
  const switchToLocalGateway = async () => {
    ...
    await window.SaraDesktop?.applyConnectionConfig({ mode: 'local' }).catch(() => undefined)
    setBusy(null)
  }
  ...
  const signInRemote = async () => {
    ...
    try {
      const result = await window.SaraDesktop?.oauthLoginConnectionConfig(remoteReauth.url)
  ...
  const openLogs = () => void window.SaraDesktop?.revealLogs().catch(() => undefined)
  ...
  return (
    <div className="fixed inset-0 z-[1400] flex items-center justify-center bg-(--ui-chat-surface-background) p-6">
      <div className="w-full max-w-[40rem] overflow-hidden rounded-xl border border-(--stroke-nexvisora) bg-(--ui-chat-bubble-background) shadow-nexvisora">
