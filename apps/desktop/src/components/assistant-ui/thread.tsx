...
import { Square as StopIcon } from '@/lib/icons'
import {
  type ClipboardEvent,
  type ComponentProps,
  type FC,
...
import { DirectiveContent, SaraDirectiveFormatter } from '@/components/assistant-ui/directive-text'
...
import type { SaraGateway } from '@/Sara'
...
const StreamStallIndicator: FC = () => {
  ...
  return (
    <StatusRow
      ...
      label={compacting ? COMPACTION_LABEL : 'Sara is thinking'}
    >
      <span aria-hidden="true" className="dither inline-block size-3 rounded-[2px] text-midground/80 animate-pulse" />
      {compacting && <CompactionHint />}
      <ActivityTimerText seconds={elapsed} />
...
const StopGlyph = <StopIcon aria-hidden className="size-3.5 -translate-y-px" fill="currentColor" />
...
const UserMessage: FC<{
  ...
  const measureClamp = useCallback((entries: readonly ResizeObserverEntry[]) => {
    ...
    // `scrollHeight` outside RO timing forces a synchronexvisora layout, and with
