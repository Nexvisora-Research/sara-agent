...
const Sara_REF_TYPES = ['file', 'folder', 'url', 'image', 'tool', 'line', 'terminal', 'session'] as const
type SaraRefType = (typeof Sara_REF_TYPES)[number]
...
const ICON_PATHS: Record<SaraRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as SaraRefType] ?? ICON_FALLBACK
...
const Sara_DIRECTIVE_RE = new RegExp(
...
export const SaraDirectiveFormatter: Unstable_DirectiveFormatter = {
...
function parseDirectiveText(text: string): Unstable_DirectiveSegment[] {
  const matches = [
    ...
    ...Array.from(text.matchAll(Sara_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as SaraRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: SaraRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
function safeDirectiveSegments(text: string): Unstable_DirectiveSegment[] {
  try {
    return [...SaraDirectiveFormatter.parse(text)]
  } catch {
...
/**
 * Renders text containing Sara directives (`@file:...`, `@image:...`) as
...
const DirectiveImage: FC<{ id: string; label: string }> = ({ id, label }) => {
  ...
  useEffect(() => {
    ...
    const load =
      window.SaraDesktop && isRemoteGateway()
        ? gatewayMediaDataUrl(id)
        : window.SaraDesktop?.readFileDataUrl(id)
