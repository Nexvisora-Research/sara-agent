...
import { formatRefValue, SaraDirectiveFormatter } from './directive-text'
...
describe('SaraDirectiveFormatter.parse', () => {
  it('keeps quoted file paths whole when parsing', () => {
    const segments = SaraDirectiveFormatter.parse('see @image:`apple-touch-icon (1).png` for the icon')
  ...
  it('still parses unquoted paths', () => {
    const segments = SaraDirectiveFormatter.parse('@file:src/main.tsx the entry point')
