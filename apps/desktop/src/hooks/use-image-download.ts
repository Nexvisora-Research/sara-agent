...
function isMissingIpcHandler(error: unknown): boolean {
  ...
  return message.includes("No handler registered for 'Sara:saveImageFromUrl'")
}
...
export function useImageDownload(src?: string) {
  ...
  const download = useCallback(async () => {
    ...
    try {
      if (window.SaraDesktop?.saveImageFromUrl) {
        if (await window.SaraDesktop.saveImageFromUrl(src)) {
