...
const checkSaraUpdateSpy = vi.fn()
const updateSaraSpy = vi.fn()
const getActionStatusSpy = vi.fn()

vi.mock('@/Sara', () => ({
  checkSaraUpdate: (...args: unknown[]) => checkSaraUpdateSpy(...args),
  updateSara: (...args: unknown[]) => updateSaraSpy(...args),
...
describe('checkBackendUpdates', () => {
  ...
  it('maps the backend /update/check onto the backend status, including commits', async () => {
    setRemote(true)
    checkSaraUpdateSpy.mockResolvedValue({
      install_method: 'git',
      current_version: '0.16.0',
      behind: 2,
      update_available: true,
      can_apply: true,
      update_command: 'Sara update',
      message: null,
    ...
    expect(checkSaraUpdateSpy).toHaveBeenCalled()
  ...
  it('is a no-op in local mode (backend check only runs when remote)', async () => {
    ...
    expect(checkSaraUpdateSpy).not.toHaveBeenCalled()
  })
})
...
describe('applyBackendUpdate recovery', () => {
  ...
  it('waits for the backend to return after the restart drops the connection, then clears the overlay', async () => {
    updateSaraSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkSaraUpdateSpy.mockResolvedValue({ install_method: 'git', current_version: '0.16.0', behind: 0, update_available: false, can_apply: true, update_command: 'Sara update', message: null })
  ...
  it('surfaces an error when the backend never comes back after the restart', async () => {
    updateSaraSpy.mockResolvedValue({ ok: true, name: 'update', pid: 1 })
    getActionStatusSpy.mockRejectedValue(new Error('ECONNREFUSED'))
    checkSaraUpdateSpy.mockRejectedValue(new Error('ECONNREFUSED'))
