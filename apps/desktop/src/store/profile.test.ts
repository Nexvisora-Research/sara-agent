...
const remoteConn = (over: Partial<SaraConnection> = {}): SaraConnection =>
  ({ baseUrl: 'https://Sara-roy.tail.ts.net', mode: 'remote', profile: 'vps-remote', ...over }) as SaraConnection

const localConn = (over: Partial<SaraConnection> = {}): SaraConnection =>
  ({ baseUrl: '', mode: 'local', profile: 'default', ...over }) as SaraConnection

const getConnection = vi.fn<(profile?: string | null) => Promise<SaraConnection>>()
...
beforeEach(() => {
  ...
  vi.stubGlobal('window', { SaraDesktop: { getConnection } })
})
