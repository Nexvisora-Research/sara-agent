...
import type { SaraConnection } from '@/global'
import type { ChatMessage } from '@/lib/chat-messages'
import { persistBoolean, persistString, storedBoolean, storedString } from '@/lib/storage'
import type { SessionInfo, UsageStats } from '@/types/Sara'

type Updater<T> = T | ((current: T) => T)

const WORKSPACE_CWD_KEY = 'Sara.desktop.workspace-cwd'
...
const COMPOSER_MODEL_KEY = 'Sara.desktop.composer.model'
const COMPOSER_PROVIDER_KEY = 'Sara.desktop.composer.provider'
const COMPOSER_EFFORT_KEY = 'Sara.desktop.composer.reasoning-effort'
const COMPOSER_FAST_KEY = 'Sara.desktop.composer.fast'

let configuredDefaultProjectDir = ''

function workspaceCwdKey(connection: SaraConnection | null = $connection.get()): string {
...
export async function syncConfiguredDefaultProjectDir(): Promise<string> {
  const settings = window.SaraDesktop?.settings?.getDefaultProjectDir
...
export async function ensureDefaultWorkspaceCwd(): Promise<void> {
  const sanitize = window.SaraDesktop?.sanitizeWorkspaceCwd
...
export const $connection = atom<SaraConnection | null>(null)
...
export const setConnection = (next: Updater<SaraConnection | null>) => updateAtom($connection, next)
