...
const STORAGE_KEY = 'Sara:native-notifications'
...
// "Backgrounded" = the user isn't on Sara. `document.hidden` only flips when
...
export function dispatchNativeNotification(input: NativeNotificationInput): void {
  ...
  void window.SaraDesktop?.notify({
...
export async function sendTestNativeNotification(title: string, body: string): Promise<boolean> {
  const bridge = window.SaraDesktop
