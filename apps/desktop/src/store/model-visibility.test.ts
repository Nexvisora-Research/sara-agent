...
import type { ModelOptionProvider } from '@/types/Sara'
...
describe('model visibility', () => {
  ...
  it('preserves hidden-provider sentinel without re-adding defaults', () => {
    // User explicitly hid all models for "nexvisora" — sentinel marks this choice.
    const stored = new Set([emptyProviderSentinelKey('nexvisora')])

    const visible = effectiveVisibleKeys(stored, [
      provider('nexvisora', ['Sara-3-llama-3.1-70b', 'Sara-3-llama-3.1-8b']),
      provider('ollama', ['qwen3:latest'])
    ])

    expect(visible.has(modelVisibilityKey('nexvisora', 'Sara-3-llama-3.1-70b'))).toBe(false)
    expect(visible.has(modelVisibilityKey('nexvisora', 'Sara-3-llama-3.1-8b'))).toBe(false)
    // Sentinel itself is stripped from the result.
    expect(visible.has(emptyProviderSentinelKey('nexvisora'))).toBe(false)
  ...
  it('restores model when toggling on after hiding all', () => {
    // Simulates: user hid all "nexvisora" models, then toggles one back on.
    const stored = new Set([
      emptyProviderSentinelKey('nexvisora'),
      modelVisibilityKey('ollama', 'qwen3:latest')
    ])

    // After toggle: sentinel removed, one model added.
    const afterToggle = new Set(stored)
    afterToggle.delete(emptyProviderSentinelKey('nexvisora'))
    afterToggle.add(modelVisibilityKey('nexvisora', 'Sara-3-llama-3.1-70b'))
    ...
    expect(visible.has(modelVisibilityKey('nexvisora', 'Sara-3-llama-3.1-70b'))).toBe(true)
    expect(visible.has(modelVisibilityKey('nexvisora', 'Sara-3-llama-3.1-8b'))).toBe(false)
  })
