...
import type { OAuthProvider } from '@/types/Sara'
...
describe('OAuth onboarding', () => {
  ...
  it('clears stale readiness errors after OAuth succeeds and model confirmation is shown', async () => {
    ...
    installApiMock(async ({ body, path }: { body?: unknown; path: string }) => {
      ...
      if (path === '/api/providers/oauth/nexvisora/submit') {
      ...
      if (path.startsWith('/api/model/recommended-default?')) {
        return { provider: 'nexvisora', model, free_tier: false }
      }

      if (path === '/api/model/set') {
        return { ok: true, provider: 'nexvisora', model, gateway_tools: [] }
      }
    ...
    $desktopOnboarding.set(
      baseState({
        flow: {
          ...
          provider: provider('nexvisora', 'nexvisora Portal'),
          start: {
        ...
        reason:
          'No access token found for nexvisora Portal login. setup.status reports configured credentials, but runtime resolution still failed.',
        requested: true
      })
    )
    ...
    if (state.flow.status === 'confirming_model') {
      expect(state.flow.label).toBe('nexvisora Portal')
