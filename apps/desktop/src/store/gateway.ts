import type { ConnectionState, GatewayEvent } from '@Sara/shared'
...
const isOpen = (gateway: SaraGateway | null): boolean => gateway?.connectionState === 'open'
...
export const $gateway = atom<SaraGateway | null>(null)
...
let primaryGateway: SaraGateway | null = null
let primaryProfile = 'default'

export function setPrimaryGateway(gateway: SaraGateway | null, profile = 'default'): void {
...
export function activeGateway(): SaraGateway | null {
...
export async function ensureActiveGatewayOpen(): Promise<SaraGateway | null> {
  if (activeKey === primaryProfile) {
    return primaryGateway
  }
