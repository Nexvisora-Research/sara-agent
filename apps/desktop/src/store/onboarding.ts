...
import type { ModelOptionProvider, OAuthProvider, OAuthStartResponse } from '@/types/Sara'
...
const CONFIGURED_CACHE_KEY = 'Sara-desktop-onboarded-v1'
const SKIP_CACHE_KEY = 'Sara-onboarding-skipped-v1'
const POLL_MS = 2000
...
function notifyReady(provider: string) {
  notify({ kind: 'success', title: 'Sara is ready', message: `${provider} connected.` })
}

// Human-friendly labels for tools auto-routed through the nexvisora Tool Gateway,
// mirroring Sara_cli/nexvisora_subscription._GATEWAY_TOOL_LABELS so the GUI and
...
// When switching to nexvisora auto-routes unconfigured tools through the Tool
...
function notifyGatewayTools(tools: string[] | undefined) {
  ...
  notify({
    durationMs: 8000,
    kind: 'info',
    message: `${list} now run through your nexvisora subscription — no separate API keys needed.`,
...
async function fetchProviderDefaultModel(
  ...
  // `Sara model` does (for nexvisora it honors the user's free/paid tier, so a
...
function providerResolutionFailure(reason: null | string) {
  ...
  return detail
    ? `Connected, but Sara still cannot resolve a usable provider. ${detail}`
    : 'Connected, but Sara still cannot resolve a usable provider.'
}
...
async function openSignInUrl(url: string) {
  if (window.SaraDesktop?.openExternal) {
    try {
      await window.SaraDesktop.openExternal(url)
...
export async function recheckExternalSignin(ctx: OnboardingContext) {
  ...
  await completeWithModelConfirm(ctx, provider.name, [provider.id], reason =>
    setFlow({
      ...
      message:
        reason?.trim() ||
        `Sara still cannot reach ${provider.name}. Run \`${provider.cli_command}\` in a terminal first.`
    })
  )
}
...
export async function saveOnboardingLocalEndpoint(baseUrl: string, apiKey: string, ctx: OnboardingContext) {
  ...
  try {
    ...
    if (!runtime.ready) {
      ...
      return { ok: false, message: detail || `Saved, but Sara still cannot reach ${url}.` }
    }
