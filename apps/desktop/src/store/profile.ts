...
import { getProfiles, setApiRequestProfile } from '@/Sara'
...
import type { ProfileInfo } from '@/types/Sara'
...
// /api/profiles/active `current`). "default" is the root ~/.Sara. This is the
...
const PROFILE_ORDER_STORAGE_KEY = 'Sara.desktop.profileOrder'
...
const PROFILE_COLORS_STORAGE_KEY = 'Sara.desktop.profileColors'
...
export async function refreshActiveProfile(): Promise<void> {
  try {
    const res = await window.SaraDesktop.api<ActiveProfileResponse>({ path: '/api/profiles/active' })
...
// Persist the choice and relaunch the backend under the new Sara_HOME. The
...
export async function switchProfile(name: string): Promise<void> {
  ...
  await window.SaraDesktop.profile.set(name)
}
...
async function syncConnectionToActiveProfile(profile: string): Promise<void> {
  const getConnection = window.SaraDesktop?.getConnection
...
const SHOW_ALL_PROFILES_STORAGE_KEY = 'Sara.desktop.showAllProfiles'
...
// Switch to the default (root ~/.Sara) profile — bound to ⌘1.
...
export function touchActiveGatewayBackend(): void {
  ...
  void window.SaraDesktop?.touchBackend?.(target).catch(() => undefined)
}
