...
declare global {
  interface Window {
    SaraDesktop: {
      ...
      getConnection: (profile?: string | null) => Promise<SaraConnection>
      ...
      profile: {
        ...
        // backend under the new Sara_HOME (reloads the window). Pass null to
        // clear the preference.
        set: (name: string | null) => Promise<DesktopActiveProfile>
      }
      api: <T>(request: SaraApiRequest) => Promise<T>
      notify: (payload: SaraNotification) => Promise<boolean>
      requestMicrophoneAccess: () => Promise<boolean>
      readFileDataUrl: (filePath: string) => Promise<string>
      readFileText: (filePath: string) => Promise<SaraReadFileTextResult>
      selectPaths: (options?: SaraSelectPathsOptions) => Promise<string[]>
      writeClipboard: (text: string) => Promise<boolean>
      saveImageFromUrl: (url: string) => Promise<boolean>
      saveImageBuffer: (data: ArrayBuffer | Uint8Array, ext: string) => Promise<string>
      saveClipboardImage: () => Promise<string>
      getPathForFile: (file: File) => string
      normalizePreviewTarget: (target: string, baseDir?: string) => Promise<SaraPreviewTarget | null>
      watchPreviewFile: (url: string) => Promise<SaraPreviewWatch>
      stopPreviewFileWatch: (id: string) => Promise<boolean>
      setTitleBarTheme?: (payload: SaraTitleBarTheme) => void
      ...
      readDir: (path: string) => Promise<SaraReadDirResult>
      ...
      worktrees?: (cwds: string[]) => Promise<Record<string, SaraWorktreeInfo | null>>
      terminal: {
        dispose: (id: string) => Promise<boolean>
        onData: (id: string, callback: (payload: string) => void) => () => void
        onExit: (id: string, callback: (payload: SaraTerminalExit) => void) => () => void
        resize: (id: string, size: { cols: number; rows: number }) => Promise<boolean>
        start: (options?: { cols?: number; cwd?: string; rows?: number }) => Promise<SaraTerminalSession>
      ...
      onWindowStateChanged?: (callback: (payload: SaraWindowState) => void) => () => void
      onFocusSession?: (callback: (sessionId: string) => void) => () => void
      onNotificationAction?: (callback: (payload: { actionId: string; sessionId?: string }) => void) => () => void
      onPreviewFileChanged: (callback: (payload: SaraPreviewFileChanged) => void) => () => void
...
export interface SaraTerminalSession {
  cwd: string
  id: string
  shell: string
}

export interface SaraTerminalExit {
  code: number | null
  signal: string | null
}
...
export interface DesktopUpdateApplyResult {
   ...
   *  `Sara update` themselves. `command` is the exact line to run. */
  manual?: boolean
  command?: string
  SaraRoot?: string
}
...
export interface SaraConnection {
  baseUrl: string
...
export interface SaraTitleBarTheme {
  background: string
  foreground: string
}
...
export interface SaraApiRequest {
  path: string
  method?: string
  body?: unknown
  timeoutMs?: number
...
export interface SaraNotification {
  title?: string
  body?: string
  silent?: boolean
  kind?: string
...
export interface SaraPreviewTarget {
  binary?: boolean
  byteSize?: number
  kind: 'file' | 'url'
  label: string
...
export interface SaraReadFileTextResult {
  binary?: boolean
  byteSize?: number
  language?: string
  mimeType?: string
...
export interface SaraPreviewWatch {
  id: string
  path: string
}

export interface SaraWorktreeInfo {
...
export interface SaraReadDirEntry {
  name: string
  path: string
  isDirectory: boolean
}

export interface SaraReadDirResult {
  entries: SaraReadDirEntry[]
  error?: string
}

export interface SaraPreviewFileChanged {
  id: string
  path: string
  url: string
}

export interface SaraSelectPathsOptions {
  title?: string
  defaultPath?: string
  directories?: boolean
  multiple?: boolean
