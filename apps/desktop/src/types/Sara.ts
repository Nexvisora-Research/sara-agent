...
export interface OAuthProvider {
   ...
   *  embedded terminal. Null when Sara doesn't know how to remove it. */
...
export interface EnvVarInfo {
  ...
  // in Sara_cli/provider_catalog.py). When present, the Keys tab groups by
  // this provider identity — the SAME one `Sara model` uses — instead of
...
export type SaraConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
   ...
   *  the `Sara model` CLI / onboarding OAuth flow. */
  auth_type?: string
  ...
  /** nexvisora only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** nexvisora only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
  ...
  requires_nexvisora_auth: boolean
  /** True when this is the provider currently written to config (mirrors the
   *  CLI `Sara tools` active-provider detection). */
  is_active: boolean
}
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the nexvisora Tool Gateway as a result of
   *  switching the main provider to nexvisora. Empty unless provider === 'nexvisora'
...
export interface MemoryGraphResponse {
  ok: boolean
  graph: { nodes: { id: string; label: string; type: string }[]; edges: { source: string; target: string; predicate: string }[] }
  space?: string
  error?: string
}
