...
import { getSaraConfigRecord, type SaraConfigRecord, saveSaraConfig } from '@/Sara'
...
export interface I18nConfigClient {
  getConfig: () => Promise<SaraConfigRecord>
  saveConfig: (config: SaraConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.SaraDesktop?.api) {
      return Promise.resolve({})
    }

    return getSaraConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.SaraDesktop?.api) {
...
export function getConfigDisplayLanguage(config: SaraConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: SaraConfigRecord, locale: Locale): SaraConfigRecord {
