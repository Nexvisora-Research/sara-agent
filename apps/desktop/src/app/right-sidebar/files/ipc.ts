...
import type { SaraReadDirEntry, SaraReadDirResult } from '@/global'

export type ProjectTreeEntry = SaraReadDirEntry
...
function ignoredBy(rules: GitignoreRule[], entry: SaraReadDirEntry) {
...
async function filterIgnored(entries: SaraReadDirEntry[], rootPath: string, dirPath: string) {
...
export async function readProjectDir(dirPath: string, rootPath = dirPath): Promise<SaraReadDirResult> {
  if (!window.SaraDesktop) {
