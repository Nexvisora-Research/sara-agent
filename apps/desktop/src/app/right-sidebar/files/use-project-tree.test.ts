...
import type { SaraReadDirResult } from '@/global'

import { clearProjectDirCache, readProjectDir } from './ipc'
import { resetProjectTreeState, useProjectTree } from './use-project-tree'

const readDir = vi.fn<(path: string) => Promise<SaraReadDirResult>>()

beforeEach(() => {
  $connection.set(null)
  resetProjectTreeState()
  readDir.mockReset()
  ;(window as unknown as { SaraDesktop: { readDir: typeof readDir } }).SaraDesktop = { readDir }
})
...
afterEach(() => {
  ...
  $connection.set(null)
  resetProjectTreeState()
  delete (window as unknown as { SaraDesktop?: unknown }).SaraDesktop
})

function ok(entries: { name: string; path: string; isDirectory: boolean }[]): SaraReadDirResult {
  return { entries }
}
...
describe('useProjectTree', () => {
  ...
  it('reads gitignore from the real path while caching per connection', async () => {
    ...
    ;(window as unknown as { SaraDesktop: unknown }).SaraDesktop = { gitRoot, readDir, readFileDataUrl }
  ...
  it('dedupes concurrent loadChildren calls for the same id', async () => {
    ...
    let resolveChildren: ((value: SaraReadDirResult) => void) | undefined
    readDir.mockImplementationOnce(
      () =>
        new Promise<SaraReadDirResult>(resolve => {
  ...
  it('falls back to the sanitized workspace dir when the session cwd is gone', async () => {
    ...
    ;(window as unknown as { SaraDesktop: unknown }).SaraDesktop = { readDir, sanitizeWorkspaceCwd }
  ...
  it('returns no-bridge gracefully when window.SaraDesktop is missing', async () => {
    delete (window as unknown as { SaraDesktop?: unknown }).SaraDesktop
