...
const Sara_PATHS_MIME = 'application/x-Sara-paths'
...
function transferHasDropCandidates(t: DataTransfer): boolean {
  if (t.types?.includes(Sara_PATHS_MIME)) {
    return true
  }
...
function collectDroppedPaths(t: DataTransfer): string[] {
  ...
  try {
    const raw = t.getData(Sara_PATHS_MIME)
...
export function useTerminalSession({ cwd, onAddSelectionToChat }: UseTerminalSessionOptions) {
  ...
  useEffect(() => {
    ...
    const terminalApi = window.SaraDesktop?.terminal
    ...
    const term = new Terminal({
      ...
      letterSpacing: 0,
      lineHeight: 1.12,
      // Full-screen TUIs (Sara --tui, vim) grab the mouse, so a plain drag
    ...
    // synchronexvisoraly while sibling panes are mid-transition (e.g. file browser
    ...
    const mount = () => {
      ...
      try {
        ...
        console.warn('[Sara-terminal] WebGL unavailable; falling back to DOM', err)
      }
  ...
  useEffect(() => {
    ...
    return $terminalInjection.subscribe(command => {
      ...
      void window.SaraDesktop?.terminal?.write(id, `${command}\r`)
