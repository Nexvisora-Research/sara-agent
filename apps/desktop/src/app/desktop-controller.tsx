...
import { getCronJobs, getSessionMessages, listAllProfileSessions, type SessionInfo, triggerCronJob } from '../Sara'
...
import { useSaraConfig } from './session/hooks/use-sara-config'
...
export function DesktopController() {
  ...
  useEffect(() => {
    window.SaraDesktop?.setPreviewShortcutActive?.(Boolean(chatOpen && (filePreviewTarget || previewTarget)))
  }, [chatOpen, filePreviewTarget, previewTarget])

  useEffect(() => {
    startUpdatePoller()
    const unsubscribe = window.SaraDesktop?.onOpenUpdatesRequested?.(() => openUpdatesWindow())
  ...
  useEffect(() => {
    const unsubscribe = window.SaraDesktop?.onFocusSession?.(sessionId => {
      if (sessionId) {
  ...
  useEffect(() => {
    const unsubscribe = window.SaraDesktop?.onNotificationAction?.(({ actionId, sessionId }) => {
  ...
  // Sara:// deep links (e.g. a docs "Send to App" button for an automation blueprint).
  ...
  useEffect(() => {
    const unsubscribe = window.SaraDesktop?.onDeepLink?.(payload => {
    ...
    void window.SaraDesktop?.signalDeepLinkReady?.()
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.SaraDesktop?.onClosePreviewRequested?.(closeActiveRightRailTab)
  ...
  // synchronexvisoraly (agent tool call or the cron UI), so refreshing here right
  ...
  const { refreshSaraConfig, sttEnabled, voiceMaxRecordingSeconds } = useSaraConfig({
    activeSessionIdRef,
  ...
  useEffect(() => {
    if (gatewayState === 'open' && !activeSessionId && freshDraftReady) {
      void refreshCurrentModel()
      void refreshSaraConfig()
    }
  }, [activeSessionId, freshDraftReady, gatewayState, refreshCurrentModel, refreshSaraConfig])
      ...
      <PaneMain>
        <Routes>
          ...
          <Route
            element={
              <Suspense fallback={null}>
                <MemoryView setStatusbarItemGroup={setStatusbarItemGroup} />
              </Suspense>
            }
            path="memory"
          />
