...
const STATUSBAR_ACTION_CLASS =
  'inline-flex h-full items-center gap-1 rounded-sm px-1.5 text-[0.6875rem] text-(--ui-text-tertiary) transition-colors hover:bg-(--chrome-action-hover) hover:text-foreground disabled:cursor-default disabled:opacity-45'
...
export function StatusbarControls({ className, leftItems = [], items = [], ...props }: StatusbarControlsProps) {
  ...
  return (
    <footer
      className={cn(
        'flex h-5 shrink-0 items-stretch justify-between gap-2 border-t border-(--ui-stroke-tertiary) bg-[color-mix(in_srgb,var(--ui-sidebar-surface-background)_88%,transparent)] px-1 py-0 text-(--ui-text-tertiary) backdrop-blur-xl [-webkit-app-region:no-drag]',
        className
      )}
      {...props}
    >
