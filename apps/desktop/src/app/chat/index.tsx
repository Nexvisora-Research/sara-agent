...
import { getGlobalModelOptions, type SaraGateway } from '@/Sara'
...
import type { ModelOptionsResponse } from '@/types/Sara'
...
import { requestComposerFocus, requestComposerInsert, requestComposerInsertRefs } from './composer/focus'
...
export function ChatView({
  ...
  const modelOptionsQuery = useQuery<ModelOptionsResponse>({
    ...
    queryFn: () => {
      ...
      if (!gateway) {
        throw new Error('Sara gateway unavailable')
      }
  ...
  if (showIntro) {
    return (
      ...
      >
        <div className="relative min-h-0 max-w-full flex-1 overflow-hidden contain-[layout_paint]">
          <ChatRuntimeBoundary
            busy={busy}
            onCancel={onCancel}
            onEdit={onEdit}
          ...
          >
            <AnimatedHero onNewChat={onNewChat} />
            {showChatBar && (
              <div className="absolute bottom-0 left-0 right-0 z-20 flex justify-center pb-4 pointer-events-none">
                <div className="pointer-events-auto w-full max-w-[var(--composer-width)]">
