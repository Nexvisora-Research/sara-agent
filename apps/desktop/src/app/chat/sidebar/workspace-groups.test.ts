...
import type { SaraWorktreeInfo } from '@/global'
import type { SessionInfo } from '@/types/Sara'
...
describe('workspaceGroupsFor', () => {
  it('groups by full cwd, not by basename — same-named folders are separate groups', () => {
    const groups = workspaceGroupsFor(
      [makeSession('/a/Sara-agent/apps/desktop'), makeSession('/a/Sara-agent-wt-rtl/apps/desktop')],
      'No workspace'
    )
  ...
  it('disambiguates colliding basenames by walking up the path', () => {
    expect(
      labels([makeSession('/a/Sara-agent/apps/desktop'), makeSession('/a/Sara-agent-wt-rtl/apps/desktop')])
    ).toEqual(['Sara-agent/apps/desktop', 'Sara-agent-wt-rtl/apps/desktop'])
  })

  it('leaves a unique basename as its short label', () => {
    expect(labels([makeSession('/a/Sara-agent/apps/desktop'), makeSession('/b/heval-py')])).toEqual([
      'desktop',
      'heval-py'
    ])
  })
...
const info = (over: Partial<SaraWorktreeInfo> & Pick<SaraWorktreeInfo, 'repoRoot' | 'worktreeRoot'>): SaraWorktreeInfo => ({
  branch: null,
...
describe('workspaceTreeFor', () => {
  it('heuristic nests `<repo>-wt-<branch>` under its sibling repo', () => {
    const tree = workspaceTreeFor(
      [makeSession('/www/Sara-agent'), makeSession('/www/Sara-agent-wt-rtl')],
      'No workspace'
    )

    expect(tree).toHaveLength(1)
    expect(tree[0].label).toBe('Sara-agent')
    expect(tree[0].groups.map(g => g.label).sort()).toEqual(['Sara-agent', 'rtl'])
  })
  ...
  it('git metadata is authoritative — worktrees group by repoRoot regardless of directory naming', () => {
    const resolver: WorktreeResolver = cwd => {
      if (cwd === '/www/Sara-agent') {
        return info({ repoRoot: '/www/Sara-agent', worktreeRoot: '/www/Sara-agent', isMainWorktree: true, branch: 'main' })
      }

      if (cwd === '/elsewhere/ha-rtl') {
        return info({ repoRoot: '/www/Sara-agent', worktreeRoot: '/elsewhere/ha-rtl', branch: 'rtl' })
      }
    ...
    const tree = workspaceTreeFor(
      [makeSession('/www/Sara-agent'), makeSession('/elsewhere/ha-rtl')],
      'No workspace',
      resolver
    )
    ...
    expect(tree[0].groups.map(g => g.label)).toEqual(['Sara-agent', 'rtl'])
  })
