...
export const Sara_PATHS_MIME = 'application/x-Sara-paths'

/**
 * Eagerly resolve files from a drop event into [File?, path, isDirectory?]
 * triples. Internal Sara sources (e.g. the project tree) ride on a custom
 * MIME and produce path-only entries; OS drops produce File-bearing entries.
 *
 * Must be called synchronexvisoraly from inside the drop handler — `DataTransfer`
...
export function extractDroppedFiles(transfer: DataTransfer): DroppedFile[] {
  ...
  const getPath = window.SaraDesktop?.getPathForFile
  ...
  try {
    const internalRaw = transfer.getData(Sara_PATHS_MIME)
...
export function useComposerActions({ activeSessionId, currentCwd, requestGateway }: ComposerActionsOptions) {
  ...
  const pickContextPaths = useCallback(
    async (kind: 'file' | 'folder') => {
      const paths = await window.SaraDesktop?.selectPaths({
  ...
  const attachImagePath = useCallback(async (filePath: string) => {
    ...
    try {
      const previewUrl = await window.SaraDesktop?.readFileDataUrl(filePath)
  ...
  const attachImageBlob = useCallback(
    async (blob: Blob) => {
      ...
      try {
        ...
        const savedPath = await window.SaraDesktop?.saveImageBuffer(data, blobExtension(blob))
  ...
  const pasteClipboardImage = useCallback(async () => {
    try {
      const path = await window.SaraDesktop?.saveClipboardImage()
  ...
  const attachDroppedItems = useCallback(
    async (candidates: DroppedFile[]) => {
      ...
      for (const candidate of candidates) {
        ...
        const fallbackPath =
          !knownPath && window.SaraDesktop?.getPathForFile ? window.SaraDesktop.getPathForFile(file) : ''
