...
import { SaraDirectiveFormatter, type SlashChipKind } from '@/components/assistant-ui/directive-text'
...
import { extractDroppedFiles, Sara_PATHS_MIME, partitionDroppedFiles } from '../hooks/use-composer-actions'
...
export function ChatBar({
  ...
  // Distinguish a cold start ("Starting Sara...") from a dropped connection
  ...
  // Set synchronexvisoraly in keydown when the open trigger popover consumes a
  ...
  const replaceTriggerWithChip = (item: Unstable_TriggerItem) => {
    ...
    const serialized = SaraDirectiveFormatter.serialize(item)
  ...
  const handleDragEnter = (event: ReactDragEvent<HTMLFormElement>) => {
    if (!onAttachDroppedItems || !dragHasAttachments(event.dataTransfer, Sara_PATHS_MIME)) {
      return
    }
  ...
  const handleInputDragOver = (event: ReactDragEvent<HTMLDivElement>) => {
    if (!dragHasAttachments(event.dataTransfer, Sara_PATHS_MIME)) {
      return
    }
  ...
  const input = (
    <div className={cn('relative', stacked ? 'w-full' : 'min-w-(--composer-input-inline-min-width) flex-1')}>
      ...
      {/* assistant-ui requires ComposerPrimitive.Input somewhere in the tree
        ...
        forcing two synchronexvisora layouts per keystroke for an element the
