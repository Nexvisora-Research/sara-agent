...
import { Bookmark, BookmarkFilled, Download, Trash2 } from '@/lib/icons'
...
import type { ActionStatusResponse, AnalyticsResponse, StatusResponse } from '@/Sara'
...
export function CommandCenterView({ initialSection, onClose, onDeleteSession, onOpenSession }: CommandCenterViewProps) {
  ...
  const runSystemAction = useCallback(
    async (kind: 'restart' | 'update') => {
      ...
      try {
        const started = kind === 'restart' ? await restartGateway() : await updateSara()
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-1 text-[length:var(--conversation-caption-font-size)] text-(--ui-text-tertiary)">
                          {cc.SaraActiveSessions(status.version, status.active_sessions)}
                        </div>
                      </div>
      ...
      <div className="min-h-0 flex-1">
        {loading ? (
          ...
          <EmptyPanel
            ...
            description={cc.nexvisoraage(period)}
          />
        )}
      </div>
    )
