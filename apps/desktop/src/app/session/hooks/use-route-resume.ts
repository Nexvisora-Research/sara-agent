...
export function useRouteResume({
  ...
  useEffect(() => {
    ...
    if (routedSessionId) {
      ...
      // stuck empty forever. selectedStoredSessionIdRef is set synchronexvisoraly at
  ...
  // sets it synchronexvisoraly at entry) and the pathname/gateway are unchanged, so
