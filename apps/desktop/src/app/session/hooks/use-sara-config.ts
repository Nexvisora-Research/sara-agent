...
import { getSaraConfig, getSaraConfigDefaults } from '@/Sara'
...
interface SaraConfigOptions {
  activeSessionIdRef: MutableRefObject<string | null>
  refreshProjectBranch: (cwd: string) => Promise<void>
}

export function useSaraConfig({ activeSessionIdRef, refreshProjectBranch }: SaraConfigOptions) {
  const [voiceMaxRecordingSeconds, setVoiceMaxRecordingSeconds] = useState(DEFAULT_VOICE_SECONDS)
  const [sttEnabled, setSttEnabled] = useState(true)

  const refreshSaraConfig = useCallback(async () => {
    try {
      const [config, defaults] = await Promise.all([getSaraConfig(), getSaraConfigDefaults().catch(() => ({}))])
  ...
  return { refreshSaraConfig, sttEnabled, voiceMaxRecordingSeconds }
}
