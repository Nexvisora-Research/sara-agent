...
import { deleteSession, getSession, getSessionMessages, setSessionArchived } from '@/Sara'
...
import type { SessionCreateResponse, SessionInfo, SessionResumeResponse, SessionRuntimeInfo, UsageStats } from '@/types/Sara'
...
export function useSessionActions({
  ...
  const resumeSession = useCallback(
    async (storedSessionId: string, replaceRoute = false) => {
      ...
      // also what use-route-resume's self-heal assumes ("set synchronexvisoraly at
      // resume entry").
      setFreshDraftReady(false)
      clearNotifications()
