...
import { getProfiles, transcribeAudio } from '@/Sara'
...
async function readFileDataUrlForAttach(filePath: string): Promise<string | null> {
  const reader = window.SaraDesktop?.readFileDataUrl
...
export function usePromptActions({
  ...
  // separate `Sara gateway` process performs the actual transfer, so we
