...
import { getSessionMessages, listAllProfileSessions } from '@/Sara'
...
import type { SessionInfo, SessionMessage } from '@/types/Sara'
  ...
  const openArtifact = useCallback(async (href: string) => {
    try {
      if (window.SaraDesktop?.openExternal) {
        await window.SaraDesktop.openExternal(href)
      } else {
