...
// bug that drives the REAL useGatewayBoot hook + REAL SaraGateway through a
...
function Harness() {
  useGatewayBoot({
    ...
    refreshSaraConfig: async () => undefined,
...
beforeEach(() => {
  ...
  ;(window as { SaraDesktop?: unknown }).SaraDesktop = fakeDesktop()
...
afterEach(() => {
  ...
  delete (window as { SaraDesktop?: unknown }).SaraDesktop
})
...
describe('useGatewayBoot remote reconnect loop (real hook, fake socket)', () => {
  it('INITIAL boot against a dead VPS: getConnection hangs (waitForSara) → app sits in the connecting combo, then fails', async () => {
    // The report's actual path: a fresh launch pointed at an unreachable VPS.
    // startSara()'s remote branch awaits waitForSara() for 45s before it
    ...
    ;(window as { SaraDesktop?: unknown }).SaraDesktop = desktop
    ...
    // After ~45s waitForSara gives up and getConnection rejects → boot()
    // catch → failDesktopBoot → the BootFailureOverlay recovery surface.
    await act(async () => {
      rejectConn(new Error('Sara backend did not become ready: timeout'))
      await vi.advanceTimersByTimeAsync(0)
    })
