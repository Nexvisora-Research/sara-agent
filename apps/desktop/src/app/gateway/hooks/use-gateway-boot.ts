...
import type { RpcEvent } from '@/types/Sara'

interface GatewayBootOptions {
  handleGatewayEvent: (event: RpcEvent) => void
  onConnectionReady: (
    connection: Awaited<ReturnType<NonNullable<typeof window.SaraDesktop>['getConnection']>> | null
  ) => void
  onGatewayReady: (gateway: SaraGateway | null) => void
  refreshSaraConfig: () => Promise<void>
...
export function useGatewayBoot({
  ...
  useEffect(() => {
    let cancelled = false
    const desktop = window.SaraDesktop

    const publish = (next: SaraConnection | null) => {
    ...
    // composer stayed disabled forever on "Starting Sara...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting Sara…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to Sara gateway". resolveGatewayWsUrl
        ...
        await callbacksRef.current.refreshSaraConfig().catch(() => undefined)
    ...
    async function boot() {
      try {
        ...
        await callbacksRef.current.refreshSaraConfig()
