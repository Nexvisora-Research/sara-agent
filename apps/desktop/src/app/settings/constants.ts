...
export const EMPTY_SELECT_VALUE = '__Sara_empty__'
export const CONTROL_TEXT = 'text-xs'

export const PROVIDER_GROUPS: ProviderPrefix[] = [
  {
    prefix: 'nexvisora_',
    name: 'nexvisora Portal',
    description: 'Hosted Sara & nexvisora-trained models',
    docsUrl: 'https://portal.nexvisoraresearch.com',
    priority: 0
  },
  {
    prefix: 'OPENROUTER_',
  ...
  { prefix: 'Sara_GEMINI_', name: 'Gemini', priority: 4 },
  {
    prefix: 'DEEPSEEK_',
    name: 'DeepSeek',
  ...
  { prefix: 'Sara_QWEN_', name: 'DashScope (Qwen)', priority: 6 },
  {
    prefix: 'GLM_',
    name: 'GLM / Z.AI',
...
export const ENUM_OPTIONS: Record<string, string[]> = {
  ...
  // Sara_cli/config.py (local/groq/openai/mistral/elevenlabs).
...
export const FIELD_DESCRIPTIONS: Record<string, string> = defineFieldCopy({
  ...
  timezone: 'Used when Sara needs local time context. Blank uses the system timezone.',
  agent: {
    imageInputMode: 'Controls how image attachments are sent to the model.',
    maxTurns: 'Upper bound for tool-calling turns before Sara stops a run.'
  },
  terminal: {
  ...
  fileReadMaxChars: 'Maximum characters Sara can read from one file request.',
  approvals: {
    mode: 'How Sara handles commands that need explicit approval.',
  ...
  updates: {
    nonInteractiveLocalChanges:
      'When Sara updates itself from the app (no terminal prompt), keep local source edits (stash) or throw them away (discard). Terminal updates always ask.'
  }
})
