...
import type { EnvVarInfo, OAuthProvider } from '@/types/Sara'
...
function provider(id: string, loggedIn: boolean, patch: Partial<OAuthProvider> = {}): OAuthProvider {
  return {
    cli_command: `Sara auth add ${id}`,
    disconnectable: true,
    docs_url: '',
    flow: 'device_code',
    id,
    name: id === 'nexvisora' ? 'nexvisora Portal' : 'MiniMax',
    status: {
      logged_in: loggedIn
    },
    ...patch
...
beforeEach(() => {
  ...
  disconnectOAuthProvider.mockResolvedValue({ ok: true, provider: 'nexvisora' })
  listOAuthProviders.mockResolvedValue({
    providers: [provider('nexvisora', true), provider('minimax-oauth', false)]
  })
...
describe('ProvidersSettings', () => {
  it('disconnects a connected provider account and refreshes the accounts list', async () => {
    ...
    const remove = await screen.findByRole('button', { name: 'Remove nexvisora Portal' })
    fireEvent.click(remove)

    await waitFor(() => expect(disconnectOAuthProvider).toHaveBeenCalledWith('nexvisora'))
  ...
  it('keeps provider selection separate from account removal', async () => {
    ...
    fireEvent.click(await screen.findByText('nexvisora Portal'))

    expect(startManualProviderOAuth).toHaveBeenCalledWith('nexvisora')
    ...
    listOAuthProviders.mockResolvedValue({
      providers: [
        provider('qwen-oauth', true, {
          ...
          disconnect_hint: 'Use `Sara auth add qwen-oauth` or that provider\'s CLI to remove it.',
