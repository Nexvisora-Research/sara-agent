...
describe('settings helpers', () => {
  ...
  it('reads and writes nested config paths', () => {
    const config: SaraConfigRecord = { display: { theme: 'mono' } }
  ...
  describe('providerGroup', () => {
    it('maps a provider env var to its labeled group', () => {
      ...
      expect(providerGroup('nexvisora_API_KEY')).toBe('nexvisora Portal')
    ...
    it('prefers the longest matching prefix so CN/regional buckets win', () => {
      ...
      // Sara_QWEN_ and Sara_GEMINI_ both share the Sara_ stem.
      expect(providerGroup('Sara_QWEN_BASE_URL')).toBe('DashScope (Qwen)')
      expect(providerGroup('Sara_GEMINI_CLIENT_ID')).toBe('Gemini')
    })
