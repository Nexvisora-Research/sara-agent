...
import { disconnectOAuthProvider, listOAuthProviders } from '@/Sara'
...
const canRunInTerminal = () => typeof window !== 'undefined' && Boolean(window.SaraDesktop?.terminal)
...
//      in Sara_cli/provider_catalog.py) — the SAME provider identity
//      `Sara model` uses. This is authoritative: a provider tagged by the
...
function ConnectedProviderRow({
  ...
  // Sara can clear this provider's creds via the API.
  const canDisconnect = provider.disconnectable ?? provider.flow !== 'external'
  // External (CLI-managed) provider Sara can't clear via the API, but ships a
...
export function ProvidersSettings({ onClose, onViewChange, view }: ProvidersSettingsProps) {
  ...
  // Sara never deletes creds another tool owns behind a silent API call.
