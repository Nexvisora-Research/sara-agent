...
const getSaraConfigRecord = vi.fn()
const saveSaraConfig = vi.fn()
...
vi.mock('@/Sara', () => ({
  ...
  getSaraConfigRecord: () => getSaraConfigRecord(),
  saveSaraConfig: (config: unknown) => saveSaraConfig(config)
}))
...
beforeEach(() => {
  getGlobalModelInfo.mockResolvedValue({ provider: 'nexvisora', model: 'Sara-4' })
  getGlobalModelOptions.mockResolvedValue({
    providers: [
      {
        name: 'nexvisora',
        slug: 'nexvisora',
        models: ['Sara-4', 'Sara-4-mini'],
        authenticated: true,
        capabilities: { 'Sara-4': { reasoning: true, fast: true } }
      },
  ...
  getAuxiliaryModels.mockResolvedValue({
    main: { provider: 'nexvisora', model: 'Sara-4' },
    tasks: [{ task: 'vision', provider: 'auto', model: '', base_url: '' }]
  })
  setModelAssignment.mockResolvedValue({ provider: 'nexvisora', model: 'Sara-4', gateway_tools: [] })
  getRecommendedDefaultModel.mockResolvedValue({ provider: 'deepseek', model: 'deepseek-chat', free_tier: null })
  setEnvVar.mockResolvedValue({ ok: true })
  getSaraConfigRecord.mockResolvedValue({ agent: { reasoning_effort: 'medium', service_tier: 'normal' } })
  saveSaraConfig.mockResolvedValue({ ok: true })
})
...
describe('ModelSettings', () => {
  it('loads the current main model and lists the full provider universe', async () => {
    ...
    // "nexvisora" shows in both the trigger and the open list; the unconfigured
    // provider + its setup hint are the unique signal of the full universe.
    expect((await screen.findAllByText('nexvisora')).length).toBeGreaterThan(0)
  ...
  it('writes the profile default speed (service_tier) when the fast switch is toggled', async () => {
    await renderModelSettings()
    await waitFor(() => expect(getSaraConfigRecord).toHaveBeenCalled())

    const fastSwitch = await screen.findByRole('switch')
    fireEvent.click(fastSwitch)

    await waitFor(() =>
      expect(saveSaraConfig).toHaveBeenCalledWith(
  ...
  it('hides the reasoning/speed defaults when the main model reports no capabilities', async () => {
    getGlobalModelOptions.mockResolvedValueOnce({
      providers: [{ name: 'nexvisora', slug: 'nexvisora', models: ['Sara-4'], authenticated: true, capabilities: { 'Sara-4': { reasoning: false, fast: false } } }]
    })
  ...
  it('warns when a main switch leaves auxiliary tasks pinned to another provider', async () => {
    setModelAssignment.mockResolvedValueOnce({
      ...
      gateway_tools: [],
      stale_aux: [{ task: 'compression', provider: 'nexvisora', model: 'Sara-4' }]
    })
    ...
    expect(screen.getByText('nexvisora')).toBeTruthy()
  })
