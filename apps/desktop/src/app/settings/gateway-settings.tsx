...
export function GatewaySettings() {
  ...
  const save = async (apply: boolean) => {
    ...
    try {
      const next = apply
        ? await window.SaraDesktop.applyConnectionConfig(payload())
        : await window.SaraDesktop.saveConnectionConfig(payload())
  ...
  const signIn = async () => {
    ...
    try {
      ...
      const saved = await window.SaraDesktop.saveConnectionConfig({
        mode: state.mode,
        profile: scope ?? undefined,
      ...
      const result = await window.SaraDesktop.oauthLoginConnectionConfig(trimmedUrl)

      if (result.connected) {
        const refreshed = await window.SaraDesktop.getConnectionConfig(scope)
        setState(refreshed)
  ...
  const signOut = async () => {
    ...
    try {
      await window.SaraDesktop.oauthLogoutConnectionConfig(trimmedUrl || undefined)
      const refreshed = await window.SaraDesktop.getConnectionConfig(scope)
      setState(refreshed)
  ...
  const testRemote = async () => {
    ...
    try {
      const result = await window.SaraDesktop.testConnectionConfig({
        mode: 'remote',
        profile: scope ?? undefined,
        remoteAuthMode: authMode,
        remoteToken: authMode === 'token' ? remoteToken.trim() || undefined : undefined,
  ...
  if (!window.SaraDesktop?.getConnectionConfig) {
    return (
      <EmptyState
  ...
  return (
    <SettingsContent>
        ...
        <ListRow
          action={
            <Input
              ...
              placeholder="https://gateway.example.com/Sara"
      ...
      <div className="mt-6 grid gap-1">
        <ListRow
          action={
            <Button onClick={() => void window.SaraDesktop?.revealLogs()} size="sm" variant="textStrong">
              <FileText />
              {g.openLogs}
            </Button>
          }
