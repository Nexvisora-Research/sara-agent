import { Download, RefreshCw, Upload } from '@/lib/icons'
import { useRef } from 'react'

import { Tip } from '@/components/ui/tooltip'
import { getSaraConfigDefaults, getSaraConfigRecord, saveSaraConfig } from '@/Sara'
...
export function SettingsView({ gateway, onClose, onConfigSaved, onMainModelChanged }: SettingsPageProps) {
  ...
  const exportConfig = async () => {
    try {
      const cfg = await getSaraConfigRecord()
      const blob = new Blob([JSON.stringify(cfg, null, 2)], { type: 'application/json' })
      const url = URL.createObjectURL(blob)
      const a = document.createElement('a')
      a.href = url
      a.download = 'Sara-config.json'
      a.click()
  ...
  const resetConfig = async () => {
    ...
    try {
      await saveSaraConfig(await getSaraConfigDefaults())
      triggerHaptic('success')
      onConfigSaved?.()
    } catch (err) {
