...
import type { ActionStatusResponse, ToolEnvVar, ToolProvider, ToolsetConfig } from '@/types/Sara'
 ...
 * log inline — the GUI equivalent of the install step `Sara tools` runs
...
export function ToolsetConfigPanel({ toolset, onConfiguredChange }: ToolsetConfigPanelProps) {
  ...
  // panel highlighted the first keyless provider (e.g. nexvisora Portal) even when
          ...
          <div className="overflow-hidden rounded-xl bg-background/60" key={provider.name}>
            ...
            {isActive && (
              <div className="grid gap-2 bg-muted/20 p-3">
                ...
                {provider.requires_nexvisora_auth && (
                  <p className="text-[0.72rem] text-muted-foreground">
                    {copy.nexvisoraIncluded}
                  </p>
                )}
