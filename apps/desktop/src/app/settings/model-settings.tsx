...
import type { AuxiliaryModelsResponse, ModelOptionProvider, StaleAuxAssignment } from '@/Sara'
import { useI18n } from '@/i18n'
import { AlertTriangle, Cpu, Loader2 } from '@/lib/icons'
import { cn } from '@/lib/utils'
import { notifyError } from '@/store/notifications'
import { startManualLocalEndpoint, startManualProviderOAuth } from '@/store/onboarding'
import type { SaraConfigRecord } from '@/types/Sara'

import { CONTROL_TEXT } from './constants'
import { getNested, setNested } from './helpers'
import { ListRow, LoadingState, Pill, SectionHeading } from './primitives'

// Sara' reasoning levels (VALID_REASONING_EFFORTS); `none` = thinking off.
// Empty config = Sara default (medium), shown as Medium.
...
// backend now surfaces the full `Sara model` universe (every canonical
...
// Mirrors `_AUX_TASK_SLOTS` in Sara_cli/web_server.py. Friendly labels and
...
export function ModelSettings({ onMainModelChanged }: ModelSettingsProps) {
  ...
  const [config, setConfig] = useState<SaraConfigRecord | null>(null)
  ...
  const activateApiKeyProvider = useCallback(async () => {
    ...
    try {
      ...
      // `Sara model` curation). Best-effort — fall through to the refreshed
        ...
        {needsSetup && !setupIsApiKey && (
          <p className="mt-2 text-xs text-muted-foreground">
            {selectedProviderRow?.auth_type === 'api_key'
              ...
              : `${selectedProviderRow?.name} signs in through your browser — Sara runs the flow for you.`}
          </p>
        )}
