...
There are two ways to add a platform to the sara gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.sara/plugins/` with a `PLUGIN.yaml` and
...
requires **zero changes to core sara code**.
...
Checklist for integrating a platform directly into the sara core.
...
"sara-your-platform": {
    ...
    "tools": _sara_CORE_TOOLS,
    "includes": []
},
```

And add it to the `sara-gateway` composite:

```python
"sara-gateway": {
    "includes": [..., "sara-your-platform"]
}
```
...
## 12. Status Display (`sara_cli/status.py`)
...
## 13. Gateway Setup Wizard (`sara_cli/gateway.py`)
...
grep -r "telegram\|discord\|whatsapp\|slack" gateway/ tools/ agent/ cron/ sara_cli/ toolsets.py \
  --include="*.py" -l | sort -u
