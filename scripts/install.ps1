# ============================================================================
# sara Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://sara-agent.nexvisoraresearch.com/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$saraHome = $(if ($env:sara_HOME) { $env:sara_HOME } else { "$env:LOCALAPPDATA\sara" }),
    [string]$InstallDir = $(if ($env:sara_HOME) { "$env:sara_HOME\sara-agent" } else { "$env:LOCALAPPDATA\sara\sara-agent" }),
    ...
    # builds apps/desktop into a launchable sara.exe.
    #
    # Why opt-in:
    #   * sara-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched sara.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live sara.exe
    ...
    #     `sara desktop` already builds on demand.
...
# 5.1's progress UI repaints synchronexvisoraly on every received byte, which
...
$RepoUrlSsh = "git@github.com:nexvisoraResearch/sara-agent.git"
$RepoUrlHttps = "https://github.com/nexvisoraResearch/sara-agent.git"
$PythonVersion = "3.11"
$NodeVersion = "22"
...
function Write-Banner {
    ...
    Write-Host "|             * sara Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host "|  An open source AI agent by nexvisora Research.              |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
    Write-Host ""
}
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $saraHome)) {
        New-Item -ItemType Directory -Force -Path $saraHome | Out-Null
    }
    $envFile = Join-Path $saraHome ".env"
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $saraHome "node"
...
function Install-Uv {
    # sara owns its own uv at $saraHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sara_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `sara update` stay in sync.
    $managedUv = Join-Path $saraHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $saraHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $saraHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $saraHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $saraHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and sara_HOME wasn't propagated).
...
function Install-Git {
    ...
    which sara uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\sara\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    sara needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the sara copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\sara\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``sara_GIT_BASH_PATH`` (User scope) so sara can find it in a fresh
    ...
    # Download PortableGit into $saraHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $saraHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent sara features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  sara needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``sara_GIT_BASH_PATH`` (User env scope) so sara can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $saraHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $saraHome\git\usr\bin\bash.exe
    $candidates += "$saraHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$saraHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("sara_GIT_BASH_PATH", $candidate, "User")
            $env:sara_GIT_BASH_PATH = $candidate
            Write-Info "Set sara_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- sara may not find Git Bash."
    Write-Info "If needed, set sara_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$saraHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$saraHome\node;$env:Path"
        Write-Success "Node.js $version found (sara-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing sara-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $saraHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $saraHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\sara-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$saraHome\node") { Remove-Item -Recurse -Force "$saraHome\node" }
                Move-Item $extractedDir.FullName "$saraHome\node"
                ...
                $version = & "$saraHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $saraHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\sara-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `sara
                    # update` path (#4735).
                    ...
                    $stashName = "sara-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if sara behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running sara processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/nexvisoraResearch/sara-agent/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/nexvisoraResearch/sara-agent/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/nexvisoraResearch/sara-agent/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\sara-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\sara-agent-extract"
    ...
    # next `sara update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    if (Test-Path "venv") {
        ...
        # DLLs by any running sara process. Windows denies deletion of loaded
        # DLLs, so kill any sara.exe tree before removing the venv.
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running sara processes before recreating venv..."
            & taskkill /F /T /IM sara.exe /FI "PID ne $myPid" 2>$null | Out-Null
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `sara.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install sara-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and sara.exe broken with
    ...
    # here, not 30 seconds later when the user runs `sara`.
    if (-not $NoVenv) {
    ...
    # users hit and lazy-import errors from `sara dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `sara dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `sara dashboard` should now work."
            } else {
...
function Set-PathVariable {
    Write-Info "Setting up sara command..."
    
    if ($NoVenv) {
        $saraBin = "$InstallDir"
    } else {
        $saraBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so sara is globally available
    # On Windows, the sara.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$saraBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$saraBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $saraBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set sara_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\sara instead
    # of the Unix default ~/.sara
    $currentsaraHome = [Environment]::GetEnvironmentVariable("sara_HOME", "User")
    if (-not $currentsaraHome -or $currentsaraHome -ne $saraHome) {
        [Environment]::SetEnvironmentVariable("sara_HOME", $saraHome, "User")
        Write-Success "Set sara_HOME=$saraHome"
    }
    $env:sara_HOME = $saraHome
    
    # Update current session
    $env:Path = "$saraBin;$env:Path"
    
    Write-Success "sara command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.sara-bootstrap-complete which tells the sara
    ...
    $markerPath = Join-Path $InstallDir ".sara-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the sara_HOME directory structure ($saraHome, default %LOCALAPPDATA%\sara)
    New-Item -ItemType Directory -Force -Path "$saraHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$saraHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$saraHome\.env"
    ...
    # Create config.yaml
    $configPath = "$saraHome\config.yaml"
    ...
    # (the default PS5 behaviour), and sara's prompt-injection scanner
...
"@
    ...
    Write-Success "Configuration directory ready: $saraHome"
    
    # Seed bundled skills into $saraHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $saraHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $saraHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$saraHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $saraHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (sara-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'sara setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\sara-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\sara-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\sara-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `sara` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable sara.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_sara_desktop command
    # resolves apps/desktop/release/win-unpacked/sara.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the sara-managed
    ...
    # The --dir mode produces an unpacked sara.exe in
    ...
    # are NOT trying to work around). The sara icon + product name are
    # stamped onto sara.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\sara-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\sara.exe",
        "$desktopDir\release\win-arm64-unpacked\sara.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no sara.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The sara icon + identity are stamped onto sara.exe by the
    ...
    #    sara.exe. We deliberately do NOT point them at `sara desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'sara.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'sara.lnk')
        )
        ...
        # with the sara icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.sara/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'sara setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m sara_cli.main setup
    } else {
        python -m sara_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $saraCmd = "$InstallDir\venv\Scripts\sara.exe"
    ...
    $whatsappSession = "$saraHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'sara whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: sara gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$saraHome\logs\gateway.log"
            Start-Process -FilePath $saraCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$saraHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: sara gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: sara gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$saraHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$saraHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$saraHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$saraHome\sara-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   sara              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   sara setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   sara config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   sara config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   sara gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   sara update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning sara repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (sara-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding sara to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://sara-agent.nexvisoraresearch.com/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
