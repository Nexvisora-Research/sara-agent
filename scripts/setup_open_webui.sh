...
# Bootstrap Open WebUI against sara Agent's OpenAI-compatible API server.
#
# Idempotent by design:
# - ensures ~/.sara/.env has API server settings
# - installs Open WebUI into ~/.local/open-webui-venv
# - writes a reusable launcher at ~/.local/bin/start-open-webui-sara.sh
...
#   sara_API_PORT=8642
#   sara_API_HOST=127.0.0.1
#   sara_API_MODEL_NAME='sara Agent'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-sara Agent WebUI}"
...
sara_ENV_FILE="${sara_ENV_FILE:-$HOME/.sara/.env}"
sara_API_PORT="${sara_API_PORT:-8642}"
sara_API_HOST="${sara_API_HOST:-127.0.0.1}"
sara_API_CONNECT_HOST="${sara_API_CONNECT_HOST:-127.0.0.1}"
sara_API_MODEL_NAME="${sara_API_MODEL_NAME:-sara Agent}"
sara_API_BASE_URL="http://${sara_API_CONNECT_HOST}:${sara_API_PORT}/v1"
LAUNCHER_PATH="$HOME/.local/bin/start-open-webui-sara.sh"
LOG_DIR="$HOME/.sara/logs"
...
write_launcher() {
  ...
  quoted_base_url="$(shell_quote "$sara_API_BASE_URL")"
...
ensure_env_permissions() {
  chmod 600 "$sara_ENV_FILE" 2>/dev/null || true
}

install_launchd_service() {
  local plist="$HOME/Library/LaunchAgents/ai.openwebui.sara.plist"
...
<dict>
  <key>Label</key>
  <string>ai.openwebui.sara</string>
...
EOF
  ...
  launchctl enable "gui/$(id -u)/ai.openwebui.sara"
  launchctl kickstart -k "gui/$(id -u)/ai.openwebui.sara"
}

install_systemd_user_service() {
  require_cmd systemctl
  local unit_dir="$HOME/.config/systemd/user"
  local unit="$unit_dir/openwebui-sara.service"
  mkdir -p "$unit_dir"
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to sara Agent
After=default.target

[Service]
Type=simple
ExecStart=/bin/bash %h/.local/bin/start-open-webui-sara.sh
Restart=always
RestartSec=3
WorkingDirectory=%h
StandardOutput=append:%h/.sara/logs/openwebui.log
StandardError=append:%h/.sara/logs/openwebui.error.log

[Install]
WantedBy=default.target
EOF
  systemctl --user daemon-reload
  systemctl --user enable --now openwebui-sara.service
}
...
main() {
  ...
  local api_key
  api_key="$(get_env_value API_SERVER_KEY "$sara_ENV_FILE")"
  if [[ -z "$api_key" ]]; then
    api_key="$(generate_secret)"
  fi

  log 'Ensuring sara API server is configured...'
  upsert_env API_SERVER_ENABLED true "$sara_ENV_FILE"
  upsert_env API_SERVER_HOST "$sara_API_HOST" "$sara_ENV_FILE"
  upsert_env API_SERVER_PORT "$sara_API_PORT" "$sara_ENV_FILE"
  upsert_env API_SERVER_MODEL_NAME "$sara_API_MODEL_NAME" "$sara_ENV_FILE"
  upsert_env API_SERVER_KEY "$api_key" "$sara_ENV_FILE"
  ensure_env_permissions

  log 'Restarting sara gateway so API server settings take effect...'
  sara gateway restart >/dev/null 2>&1 || true
  sleep 4
  if ! curl -fsS "http://${sara_API_CONNECT_HOST}:${sara_API_PORT}/health" >/dev/null; then
    log 'sara API server did not answer on the first check. Trying to start gateway in the background...'
    nohup sara gateway run >/dev/null 2>&1 &
    sleep 6
  fi
  curl -fsS "http://${sara_API_CONNECT_HOST}:${sara_API_PORT}/health" >/dev/null
  ...
  log "sara API endpoint: ${sara_API_BASE_URL}"
