#!/usr/bin/env python3
"""Live test harness for sara Agent's Tool Search feature.
...
# Force-isolate the test environment BEFORE any sara imports.
ORIGINAL_HOME = os.environ.get("sara_HOME")
ORIGINAL_AUTH = Path.home() / ".sara" / "auth.json"
...
def setup_isolated_home(enabled: bool) -> Path:
    """Create a fresh ~/.sara/ for one test, copying minimal credentials.

    Also reads OPENROUTER_API_KEY from the user's real ``~/.sara/.env`` so
    the agent can authenticate against OpenRouter inside the isolated home.
    """
    home_dir = Path(tempfile.mkdtemp(prefix="sara_ts_live_"))
    sara_home = home_dir / ".sara"
    sara_home.mkdir(parents=True)

    if ORIGINAL_AUTH.exists():
        shutil.copy(ORIGINAL_AUTH, sara_home / "auth.json")

    # Copy .env so OPENROUTER_API_KEY (or others) are visible to the agent
    # running inside the isolated home.
    real_env_file = Path.home() / ".sara" / ".env"
    if real_env_file.exists():
        shutil.copy(real_env_file, sara_home / ".env")
        ...
        load_sara_dotenv(sara_home=str(Path.home() / ".sara"))
    ...
    (sara_home / "config.yaml").write_text(_yaml_dump(cfg), encoding="utf-8")
    return sara_home
...
def reset_module_state():
    """Drop cached modules so the new sara_HOME takes effect."""
    keys = [k for k in sys.modules.keys()
            if k.startswith(("tools.", "model_tools", "toolsets",
                             "sara_cli", "agent.", "run_agent"))]
    for k in keys:
        del sys.modules[k]
...
def main():
    ...
    # Restore original sara_HOME
    if ORIGINAL_HOME is not None:
        os.environ["sara_HOME"] = ORIGINAL_HOME
    else:
        os.environ.pop("sara_HOME", None)
