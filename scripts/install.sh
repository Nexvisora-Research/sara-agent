#!/bin/bash
# ============================================================================
# sara Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://sara-agent.nexvisoraresearch.com/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. sara terminal tool). A pre-set PYTHONPATH
...
# Configuration
REPO_URL_SSH="git@github.com:nexvisoraResearch/sara-agent.git"
REPO_URL_HTTPS="https://github.com/nexvisoraResearch/sara-agent.git"
sara_HOME="${sara_HOME:-$HOME/.sara}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${sara_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$sara_INSTALL_DIR"
...
#   code at /usr/local/lib/sara-agent, command at /usr/local/bin/sara,
#   data still at /root/.sara (sara_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$sara_HOME/.no-bundled-skills so future"
            echo "                   'sara update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> sara.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.sara/sara-agent"
            echo "                   default (root, Linux): /usr/local/lib/sara-agent"
            echo "  --sara-home PATH  Data directory (default: ~/.sara, or \$sara_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, sara installs the code under"
            echo "  /usr/local/lib/sara-agent and links the command into"
            echo "  /usr/local/bin/sara (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$sara_HOME"
            echo "  (default /root/.sara).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$sara_HOME/sara-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps (comma-separated)"
            echo "                   Supported: node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs sara setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
print_banner() {
    ...
    echo "│             ⚕ sara Agent Installer                    │"
    echo "├─────────────────────────────────────────────────────────┤"
    echo "│  An open source AI agent by nexvisora Research.              │"
    echo "└─────────────────────────────────────────────────────────┘"
    echo -e "${NC}"
}
...
# intentional, but they leave the checkout dirty — which forces `sara update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (sara-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download sara Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install sara command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `sara` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $sara_HOME/sara-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $sara_HOME/sara-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/sara-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $sara_HOME/sara-agent — then preserve it)
#
# Always no-op when the user set --dir or $sara_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in sara_HOME.
    if is_termux; then
        INSTALL_DIR="$sara_HOME/sara-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$sara_HOME/sara-agent/.git" ]; then
            INSTALL_DIR="$sara_HOME/sara-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/sara-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/sara-agent"
        ...
        # /usr/local/bin/sara wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/sara"
        log_info "  Data:    $sara_HOME (unchanged)"
...
# Point a sara-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $sara_HOME/node/bin — which is NOT on
...
# sara's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no sara-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$sara_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$sara_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$sara_HOME/node/etc/npmrc"
}

get_sara_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/sara" ]; then
        echo "$link_dir/sara"
    else
        echo "sara"
    fi
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://sara-agent.nexvisoraresearch.com/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # sara owns its own uv at $sara_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (sara_cli/managed_uv.py) looks in the same
    # place, so install.sh and `sara update` stay in sync.
    local _managed_uv="$sara_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $sara_HOME/bin ..."
    mkdir -p "$sara_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/sara-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/sara-uv-installer.$$.sh")"
    ...
    # directly into $sara_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$sara_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the sara-
...
check_node() {
    ...
    # Repair pre-existing sara-managed installs where `npm install -g` lands
    ...
    # Prefer a sara-managed Node from a previous run over a too-old system one.
    if [ -x "$sara_HOME/node/bin/node" ] && node_satisfies_build "$("$sara_HOME/node/bin/node" --version)"; then
        export PATH="$sara_HOME/node/bin:$PATH"
        log_success "Node.js $("$sara_HOME/node/bin/node" --version) found (sara-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing sara-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.sara/node/..."
    ...
    # Place into ~/.sara/node/ and symlink binaries into the same bin dir
    # the sara command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$sara_HOME/node"
    mkdir -p "$sara_HOME"
    mv "$extracted_dir" "$sara_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$sara_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$sara_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$sara_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$sara_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.sara/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. sara install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "sara Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `sara update` path
                # (#4735).
                ...
                local stash_name
                stash_name="sara-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if sara behaves unexpectedly."
                    else
...
setup_path() {
    log_info "Setting up sara command..."

    if [ "$USE_VENV" = true ]; then
        sara_BIN="$INSTALL_DIR/venv/bin/sara"
    else
        sara_BIN="$(which sara 2>/dev/null || echo "")"
        if [ -z "$sara_BIN" ]; then
            log_warn "sara not found on PATH after install"
            return 0
        fi
    fi
    ...
    if [ ! -x "$sara_BIN" ]; then
        log_warn "sara entry point not found at $sara_BIN"
    ...
    # Create a user-facing shim for the sara command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $sara_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$sara_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/sara"
    cat > "$command_link_dir/sara" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$sara_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/sara"
    log_success "Installed sara launcher → $command_link_display_dir/sara"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "sara command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v sara' \
                >/dev/null 2>&1; then
        ...
        log_info "sara not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# sara Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# sara Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# sara Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so sara works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.sara directory structure (config at top level, code in subdir)
    mkdir -p "$sara_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.sara/.env (top level, easy to find)
    if [ ! -f "$sara_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$sara_HOME/.env"
            log_success "Created ~/.sara/.env from template"
        else
            touch "$sara_HOME/.env"
            log_success "Created ~/.sara/.env"
        fi
    else
        log_info "~/.sara/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$sara_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.sara/config.yaml (top level, easy to find)
    if [ ! -f "$sara_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$sara_HOME/config.yaml"
            log_success "Created ~/.sara/config.yaml from template"
        fi
    else
        log_info "~/.sara/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file)
    if [ ! -f "$sara_HOME/SOUL.md" ]; then
        cat > "$sara_HOME/SOUL.md" << 'SOUL_EOF'
# sara Agent Persona

<!--
This file defines the agent's personality and tone.
The agent will embody whatever you write here.
Edit this to customize how sara communicates with you.
...
-->
SOUL_EOF
        log_success "Created ~/.sara/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.sara/"

    # Seed bundled skills into ~/.sara/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `sara update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'sara update'." \
            > "$sara_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $sara_HOME/.no-bundled-skills"
        log_info "  Future 'sara update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.sara/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.sara/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$sara_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$sara_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.sara/skills/"
            fi
        fi
    fi
}
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# sara Agent browser tools — use the system Chrome/Chromium binary."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/package.json" ]; then
        ...
        if [ -n "$DETECTED_BROWSER_EXECUTABLE" ]; then
            ...
            log_info "Skipping Playwright browser download; sara will use the system browser."
        else
            case "$DISTRO" in
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        npm install --silent 2>/dev/null || {
            log_warn "TUI npm install failed (sara --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `sara update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'sara setup' after install."
        return 0
    fi
    ...
    # Run sara setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m sara_cli.main setup < /dev/tty
    else
        python -m sara_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$sara_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for sara to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$sara_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'sara whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                sara_CMD="$(get_sara_command_path)"
                $sara_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'sara whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'sara gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $sara_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $sara_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: sara gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: sara gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $sara_CMD gateway > "$sara_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.sara/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: sara gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: sara gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $sara_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $sara_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $sara_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}sara${NC}              Start chatting"
    echo -e "   ${GREEN}sara setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}sara config${NC}       View/edit configuration"
    echo -e "   ${GREEN}sara config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}sara gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}sara update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'sara' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'sara' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'sara' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$sara_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$sara_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$sara_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    local log_file
    log_file="$(mktemp)"
    if ! "$npm_bin" install -g --prefix "$sara_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$sara_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up sara for pip install"
    ...
    sara_CMD="$(command -v sara 2>/dev/null || echo "")"
    if [ -n "$sara_CMD" ]; then
        log_info "Running sara setup..."
        "$sara_CMD" setup
    else
        log_warn "sara command not found on PATH"
        log_info "Try: python -m sara_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`sara` rename
...
# _purge_electron_build_cache() used by `sara desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the sara-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `sara desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/sara" ]; then
            app="$desktop_dir/release/linux-unpacked/sara"
        elif [ -x "$desktop_dir/release/linux-unpacked/sara" ]; then
            app="$desktop_dir/release/linux-unpacked/sara"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/sara.app" \
            "$desktop_dir/release/mac/sara.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("sara is damaged and can't be opened").
    ...
    # checkout stays clean for the next `sara update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the sara-managed Node
            # provisioned during prerequisites/node-deps (at $sara_HOME/node/bin)
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $sara_HOME.
    # $sara_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'sara update' on this host install.
