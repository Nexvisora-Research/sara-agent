 ...
 *
 * Usage:
 *   node bridge.js --port 3000 --session ~/.sara/whatsapp/session
 */
...
const SESSION_DIR = getArg('session', path.join(process.env.HOME || '~', '.sara', 'whatsapp', 'session'));
// Cache directories: the Python gateway passes the profile-aware paths via
// env (sara_HOME-aware, new cache/ layout).  Fall back to the legacy
// hardcoded locations for bridges launched outside the gateway.
const IMAGE_CACHE_DIR = process.env.sara_IMAGE_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sara', 'image_cache');
const DOCUMENT_CACHE_DIR = process.env.sara_DOCUMENT_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sara', 'document_cache');
const AUDIO_CACHE_DIR = process.env.sara_AUDIO_CACHE_DIR
  || path.join(process.env.HOME || '~', '.sara', 'audio_cache');
...
// `sara update` updates bridge.js on disk but a long-lived bridge process
...
const DEFAULT_REPLY_PREFIX = '⚕ *sara Agent*\n────────────\n';
...
async function startSocket() {
  ...
  sock = makeWASocket({
    ...
    browser: ['sara Agent', 'Chrome', '120.0'],
  ...
  sock.ev.on('messages.upsert', async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      ...
      // Ignore sara' own reply messages in self-chat mode to avoid loops.
  ...
  try {
    ...
    switch (type) {
      ...
      case 'audio': {
        ...
        let tmpPath = null;
        if (needsConversion) {
          tmpPath = path.join(tmpdir(), `sara_voice_${randomBytes(6).toString('hex')}.ogg`);
          try {
            execSync(
