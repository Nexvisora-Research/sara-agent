#!/usr/bin/env bash
# Canonical test runner for sara-agent. Run this instead of calling
...
#   * Proper venv activation (probes .venv, venv, then ~/.sara/...)
#
# Usage:
...
# ── Activate venv ───────────────────────────────────────────────────────────
VENV=""
for candidate in "$REPO_ROOT/.venv" "$REPO_ROOT/venv" "$HOME/.sara/sara-agent/venv"; do
...
if [ -f "$HOME/.sara/pytest_live_guard.py" ]; then
  EXTRA_PYTHONPATH="$HOME/.sara"
