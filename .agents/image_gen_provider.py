...
as ``kind: backend``) or ``~/.sara/plugins/image_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class ImageGenProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``sara tools``. Defaults to ``name.title()``."""
    ...
    def list_models(self) -> List[Dict[str, Any]]:
        """Return catalog entries for ``sara tools`` model picker.
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``sara tools`` picker.
...
def _images_cache_dir() -> Path:
    """Return ``$sara_HOME/cache/images/``, creating parents as needed."""
    from sara_constants import get_sara_home

    path = get_sara_home() / "cache" / "images"
...
def save_b64_image(
    ...
    *,
    ...
    """Decode base64 image data and write it under ``$sara_HOME/cache/images/``.
