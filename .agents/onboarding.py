...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time sara starts and finds ``~/.openclaw/``.

    Points users at ``sara claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``sara claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to sara, run "
        "`sara claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `sara claw cleanup` (renames it to ~/.openclaw.pre-migration — "
