...
Data resolution order (like TypeScript OpenCode):
  ...
  2. Disk cache (~/.sara/models_dev_cache.json)
...
# ---------------------------------------------------------------------------
# Provider ID mapping: sara ↔ models.dev
# ---------------------------------------------------------------------------

# sara provider names → models.dev provider IDs
...
# Reverse mapping: models.dev → sara (built lazily)
...
def _get_cache_path() -> Path:
    ...
    return get_sara_home() / "models_dev_cache.json"
...
def _get_provider_models(provider: str) -> Optional[Dict[str, Any]]:
    """Resolve a sara provider ID to its models dict from models.dev.
...
def list_provider_models(provider: str) -> List[str]:
    ...
    """
    from sara_cli.models import normalize_provider
...
# Gemma models whose TPM quotas are too small for normal sara agent traffic.
...
def get_provider_info(provider_id: str) -> Optional[ProviderInfo]:
    ...
    Accepts either a sara provider ID (e.g. "kilocode") or a models.dev
    ID (e.g. "kilo").  Returns None if the provider is not in the catalog.
    """
    # Resolve sara ID → models.dev ID
...
def get_model_info(
    ...
    Accepts sara or models.dev provider ID.  Tries exact match then
