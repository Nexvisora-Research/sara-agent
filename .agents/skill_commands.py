...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    Resolves from (in order) ``sara_PLATFORM`` env var and
    ``sara_SESSION_PLATFORM`` from the gateway session context. Returns
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("sara_PLATFORM")
            or get_session_env("sara_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("sara_PLATFORM")
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.sara.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_sara_home()}/config.yaml):"]
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.sara/skills/ and return a mapping of /command -> skill info.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.sara/skills/`` and any ``skills.external_dirs`` so the
