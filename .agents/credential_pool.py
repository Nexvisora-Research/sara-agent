...
from sara_cli.config import get_env_value, load_env
import sara_cli.auth as auth_mod
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        Meanwhile the user may run ``sara model`` / ``sara auth`` which
        ...
        Mirrors the nexvisora/Anthropic resync paths above.  Only applies to
    ...
    def _sync_nexvisora_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a Nexvisorapool entry from auth.json if tokens differ.

        NexvisoraOAuth refresh tokens are single-use.  When another process
        (e.g. a concurrent cron) refreshes the token via
        ``resolve_nexvisora_runtime_credentials``, it writes fresh tokens to
        auth.json under ``_auth_store_lock``.  The pool entry's tokens
        become stale.  This method detects that and adopts the newer pair,
        avoiding a "refresh token reuse" revocation on the NexvisoraPortal.
        """
        if self.provider != "nexvisora" or entry.source != "device_code":
            return entry
        try:
            with _auth_store_lock():
                auth_store = _load_auth_store()
            ...
            if store_refresh and store_refresh != entry.refresh_token:
                logger.debug(
                    "Pool entry %s: syncing tokens from auth.json (Nexvisorarefresh token changed)",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "access_token": store_access,
            ...
            logger.debug("Failed to sync Nexvisoraentry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently Nexvisoraand OpenAI Codex).
        """
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from anthropic_adapter import refresh_anthropic_oauth_pure

                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("sara_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if entry.source == "claude_code":
                    try:
                        from anthropic_adapter import _write_claude_code_credentials
            ...
            elif self.provider == "nexvisora":
                synced = self._sync_nexvisora_entry_from_auth_store(entry)
                ...
                refreshed = auth_mod.refresh_nexvisora_oauth_from_state(
                    nexvisora_state,
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("sara_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For nexvisora: another process may have consumed the refresh token
            # between our proactive sync and the HTTP call.  Re-sync from
            # auth.json and adopt the fresh tokens if available.
            if self.provider == "nexvisora":
                synced = self._sync_nexvisora_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("Nexvisorarefresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "nexvisora":
            # Nexvisorarefresh/mint can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other sara profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
                ...
                if synced is not entry:
                    entry = synced
                    cleared_any = True
            # For Nexvisoraentries, sync from auth.json before status checks.
            # Another process may have successfully refreshed via
            # resolve_nexvisora_runtime_credentials(), making this entry's
            ...
            # re-authed via `sara model` / `sara auth` after a 429/401,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `sara auth remove <provider> <N>` is stable across all source types.
    try:
        from sara_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, sara PKCE)
        # when the user has explicitly configured anthropic as their provider.
        # Without this gate, auxiliary client fallback chains silently read
        # ~/.claude/.credentials.json without user consent.  See PR #4210.
        try:
            from sara_cli.auth import is_provider_explicitly_configured
            if not is_provider_explicitly_configured("anthropic"):
                return changed, active_sources
        except ImportError:
            pass

        from anthropic_adapter import read_claude_code_credentials, read_sara_oauth_credentials

        for source_name, creds in (
            ("sara_pkce", read_sara_oauth_credentials()),
        ...
        if state and not _is_suppressed(provider, "device_code"):
            ...
            # (set by persist_nexvisora_credentials(label=...) when the user ran
            # `sara auth add Nexvisora--label <name>`).  Fall back to the
        ...
        # sara auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from sara_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.sara/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `sara auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `sara auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the sara auth store.  Without this gate the removal is instantly
        ...
        # sara owns its own Codex auth state — we do NOT auto-import from
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.sara/.env over os.environ — the user's config file is the
    # authoritative source for sara credentials. Stale env vars from parent
    ...
    # Honour user suppression — `sara auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.sara/.env.
    ...
    try:
        from sara_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.sara/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "sara_pkce"}
        )
    ]
