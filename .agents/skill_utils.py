...
from sara_constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"telegram"``).  When
            *None*, resolves from ``sara_PLATFORM`` or
            ``sara_SESSION_PLATFORM`` env vars.  Falls back to the
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.sara/skills/`` are silently skipped.
    """
    config_path = get_config_path()
    if not config_path.exists():
        return []
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against sara_HOME, not cwd
        if not p.is_absolute():
            p = (sara_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.sara/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    sara = metadata.get("sara") or {}
    if not isinstance(sara, dict):
        sara = {}
    return {
        "fallback_for_toolsets": sara.get("fallback_for_toolsets", []),
        "requires_toolsets": sara.get("requires_toolsets", []),
        "fallback_for_tools": sara.get("fallback_for_tools", []),
        "requires_tools": sara.get("requires_tools", []),
    }
