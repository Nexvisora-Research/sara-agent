...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (sara Agent)",
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``sara_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired Codex OAuth access token from sara auth store.
    ...
    try:
        from sara_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from sara_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `sara setup` or `sara model` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `sara model` but the
    #    old OPENAI_BASE_URL lingers in ~/.sara/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: sara model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.sara/.env",
...
def resolve_provider_client(
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: sara model)")
                return None, None
    ...
    try:
        from sara_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("sara_cli.auth not available for provider %s", provider)
        return None, None
