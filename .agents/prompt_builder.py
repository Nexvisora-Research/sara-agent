...
from sara_constants import get_sara_home, get_skills_dir, is_wsl
...
_sara_MD_NAMES = (".sara.md", "sara.md")


def _find_sara_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest ``.sara.md`` or ``sara.md``.
...
KANBAN_GUIDANCE = (
    ...
    "You were spawned by the sara Kanban dispatcher to execute ONE task from "
    "the shared board at `~/.sara/kanban.db`. Your task id is in "
    "`$sara_KANBAN_TASK`; your workspace is `$sara_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $sara_KANBAN_WORKSPACE` before "
    ...
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `sara kanban <verb>` for board operations. Use "
...
def _skills_prompt_snapshot_path() -> Path:
    return get_sara_home() / ".skills_prompt_snapshot.json"
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.sara/skills/`` directory.  External dirs
    ...
    _platform_hint = (
        os.environ.get("sara_PLATFORM")
        or get_session_env("sara_SESSION_PLATFORM")
        or ""
    )
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot sara Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `sara-agent` skill "
            "first. It has the actual commands (e.g. `sara config set …`, `sara tools`, "
            "`sara setup`) so you don't have to guess or invent workarounds.\n"
...
def load_soul_md() -> Optional[str]:
    """Load SOUL.md from sara_HOME and return its content, or None.
    ...
    """
    try:
        from sara_cli.config import ensure_sara_home
        ensure_sara_home()
    except Exception as e:
        logger.debug("Could not ensure sara_HOME before loading SOUL.md: %s", e)

    soul_path = get_sara_home() / "SOUL.md"
...
def _load_sara_md(cwd_path: Path) -> str:
    """.sara.md / sara.md — walk to git root."""
    sara_md_path = _find_sara_md(cwd_path)
    if not sara_md_path:
        return ""
    try:
        content = sara_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = sara_md_path.name
        try:
            rel = str(sara_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(result, ".sara.md")
    except Exception as e:
        logger.debug("Could not read %s: %s", sara_md_path, e)
        return ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .sara.md / sara.md  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from sara_HOME is independent and always included when present.
    ...
    # SOUL.md from sara_HOME only — skip when already loaded as identity
    if not skip_soul:
