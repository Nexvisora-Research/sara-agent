...
# `export sara_REDACT_SECRETS=true`) cannot enable/disable redaction
# mid-session.  OFF by default — user must opt in via
# `security.redact_secrets: true` in config.yaml (bridged to this env var
# in sara_cli/main.py and gateway/run.py) or `sara_REDACT_SECRETS=true`
# in ~/.sara/.env.
_REDACT_ENABLED = os.getenv("sara_REDACT_SECRETS", "").lower() in ("1", "true", "yes", "on")
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across sara — used by
    ``sara config``, ``sara status``, ``sara dump``, and anywhere
