"""Unified removal contract for every credential source sara reads from.

sara seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.sara/.env
    claude_code   — ~/.claude/.credentials.json
    sara_pkce   — ~/.sara/.anthropic_oauth.json
    device_code   — auth.json providers.<provider> (nexvisora, openai-codex, ...)
    qwen-cli      — ~/.qwen/oauth_creds.json
    gh_cli        — gh auth token
    config:<name> — custom_providers config entry
    model_config  — model.api_key when model.provider == "custom"
    manual        — user ran `sara auth add`
...
unify here is **removal**:
    ...
    ``sara auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, Nexvisoradevice_code (partial), sara_pkce, copilot gh_cli, and
...
class RemovalStep:
    ...
    Attributes:
        provider: Provider pool key (``"xai"``, ``"anthropic"``, ``"nexvisora"``, ...).
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.sara/.env  → clear it
    ...
    """
    from sara_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            "launchd plist, etc.) or it will keep being visible to sara.",
            f"  The pool entry is now suppressed — sara will ignore "
            f"{env_var} until you run `sara auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so sara stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `sara auth add anthropic` to re-enable if needed.",
    ])


def _remove_sara_pkce(provider: str, removed) -> RemovalResult:
    """~/.sara/.anthropic_oauth.json is ours — delete it outright."""
    from sara_constants import get_sara_home

    result = RemovalResult()
    oauth_file = get_sara_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared sara Anthropic OAuth credentials")
...
def _remove_nexvisora_device_code(provider: str, removed) -> RemovalResult:
    """NexvisoraOAuth lives in auth.json providers.Nexvisora— clear it and suppress.

    We suppress in addition to clearing because nothing else stops the
    user's next `sara login` run from writing providers.Nexvisoraagain
    before they decide to.  Suppression forces them to go through
    `sara auth add nexvisora` to re-engage, which is the documented re-add
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as nexvisora: single-source OAuth state with refresh tokens.
...
def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    ...
    the sara auth store is not enough — _seed_from_singletons() would
    ...
    via ``sara auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from sara_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Run `sara auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `sara auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    sara stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `sara auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="sara_pkce",
        remove_fn=_remove_sara_pkce,
        description="~/.sara/.anthropic_oauth.json",
    ))
    register(RemovalStep(
        provider="nexvisora", source_id="device_code",
        remove_fn=_remove_nexvisora_device_code,
        description="auth.json providers.nexvisora",
    ))
    register(RemovalStep(
