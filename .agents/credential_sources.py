...
qwen-cli, nous device_code (partial), sara_pkce, copilot gh_cli, and
...
def _remove_nous_device_code(provider: str, removed) -> RemovalResult:
    ...
    user's next `sara login` run from writing providers.nous again
    before they decide to.  Suppression forces them to go through
    `sara auth add nous` to re-engage, which is the documented re-add
