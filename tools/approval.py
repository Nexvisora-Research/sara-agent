...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("sara_SESSION_KEY", default)

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $sara_HOME.
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_sara_ENV_PATH = (
    r'(?:~\/\.sara/|'
    r'(?:\$home|\$\{home\})/\.sara/|'
    r'(?:\$sara_home|\$\{sara_home\})/)'
    r'\.env\b'
)
...
DANGEROUS_PATTERNS = [
    ...
    (r'\bsara\s+gateway\s+(stop|restart)\b', "stop/restart sara gateway (kills running agents)"),
    (r'\bsara\s+update\b', "sara update (restarts gateway, kills running agents)"),
    # Gateway protection: never start gateway outside systemd management
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart sara-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart sara-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(sara|gateway|cli\.py)\b', "kill sara/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill sara` but not
    # `kill -9 $(pgrep -f sara)` because the substitution is opaque
...
# =========================================================================
# Blocking gateway approval (mirrors CLI's synchroNexvisorainput() flow)
# =========================================================================
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("sara_YOLO_MODE")) or is_current_session_yolo_enabled():
        return {"approved": True, "message": None}
    ...
    is_cli = os.getenv("sara_INTERACTIVE")
    is_gateway = os.getenv("sara_GATEWAY_SESSION")

    if not is_cli and not is_gateway:
        # Cron sessions: respect cron_mode config
        if os.getenv("sara_CRON_SESSION"):
    ...
    if is_gateway or os.getenv("sara_EXEC_ASK"):
...
def check_all_command_guards(command: str, env_type: str,
    ...
    if is_truthy_value(os.getenv("sara_YOLO_MODE")) or is_current_session_yolo_enabled() or approval_mode == "off":
        return {"approved": True, "message": None}

    is_cli = os.getenv("sara_INTERACTIVE")
    is_gateway = os.getenv("sara_GATEWAY_SESSION")
    is_ask = os.getenv("sara_EXEC_ASK")
    ...
    # responds with /approve or /deny, mirroring the CLI's synchronexvisora
