...
# (sara-* prefixed), and scenario toolsets.
#
...
def _build_child_agent(
    ...
    model on OpenRouter while the parent runs on NexvisoraPortal).
    """
    ...
    # Extract parent's API key so subagents inherit auth (e.g. NexvisoraPortal).
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.sara/logs/subagent-<sid>-<ts>.log``
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Available providers: openrouter, nexvisora, zai, kimi-coding, minimax."
        ) from exc
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'sara auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (sara_cli/config.py load_config()) so that
...
DELEGATE_TASK_SCHEMA = {
    ...
    "description": (
        ...
        "notify_on_complete=True) instead. delegate_task runs SYNCHROnexvisoraLY "
