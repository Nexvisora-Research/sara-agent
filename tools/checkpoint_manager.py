...
Architecture:
    ~/.sara/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                        — standard git internals
        sara_WORKDIR                               — original dir path
...
CHECKPOINT_BASE = get_sara_home() / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("sara_CHECKPOINT_TIMEOUT", "30"))))
...
def _git_env(shadow_repo: Path, working_dir: str) -> dict:
    ...
    The shadow repo is internal sara infrastructure — it must NOT inherit
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "sara@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "sara Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "sara_WORKDIR").write_text(
...
#   * orphan:  the ``sara_WORKDIR`` path no longer exists on disk
...
def _read_workdir_marker(shadow_repo: Path) -> Optional[str]:
    """Read ``sara_WORKDIR`` from a shadow repo, or None if missing/unreadable."""
    try:
        return (shadow_repo / "sara_WORKDIR").read_text(encoding="utf-8").strip()
...
def prune_checkpoints(
    ...
    * ``delete_orphans=True`` and its ``sara_WORKDIR`` path no longer
...
def maybe_auto_prune_checkpoints(
    ...
    bounded regardless of how many times sara is invoked per day.
