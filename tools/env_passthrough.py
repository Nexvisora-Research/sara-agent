...
def _is_sara_provider_credential(name: str) -> bool:
    """True if ``name`` is a sara-managed provider credential (API key,
    token, or similar) per ``_sara_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-sara API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    in the blocklist and remain legitimately registerable — skills that
    wrap third-party APIs still work.
    """
    try:
        from tools.environments.local import _sara_PROVIDER_ENV_BLOCKLIST
    except Exception:
        return False
    return name in _sara_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are sara-managed provider credentials (from
    ``_sara_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a sara-managed
    ...
    Non-sara third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_sara_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register sara provider "
                "credential %r (blocked by _sara_PROVIDER_ENV_BLOCKLIST). "
