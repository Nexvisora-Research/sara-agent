...
running under the dispatcher (env var ``sara_KANBAN_TASK`` set). A
normal ``sara chat`` session sees **zero** kanban tools in its schema.

Why tools instead of just shelling out to ``sara kanban``?

1. **Backend portability.** A worker whose terminal tool points at Docker
   / Modal / Singularity / SSH would run ``sara kanban complete …``
   inside the container, where ``sara`` isn't installed and the DB
   isn't mounted. Tools run in the agent's Python process, so they
   always reach ``~/.sara/kanban.db`` regardless of terminal backend.
...
Humans continue to use the CLI (``sara kanban …``), the dashboard
(``sara dashboard``), and the slash command (``/kanban …``) — all
...
def _check_kanban_mode() -> bool:
    """Tools are available iff the current process has ``sara_KANBAN_TASK``
    set in its env, which the dispatcher sets when spawning a worker.

    Humans running ``sara chat`` see zero kanban tools. Workers spawned
    by the kanban dispatcher (gateway-embedded by default) see all seven.
    """
    return bool(os.environ.get("sara_KANBAN_TASK"))
...
def _default_task_id(arg: Optional[str]) -> Optional[str]:
    ...
    if arg:
        return arg
    env_tid = os.environ.get("sara_KANBAN_TASK")
...
def _handle_show(args: dict, **kw) -> str:
    ...
    if not tid:
        return tool_error(
            "task_id is required (or set sara_KANBAN_TASK in the env)"
        )
    try:
        kb, conn = _connect()
        try:
...
def _handle_comment(args: dict, **kw) -> str:
    ...
    author = args.get("author") or os.environ.get("sara_PROFILE") or "worker"
    try:
        kb, conn = _connect()
        try:
...
def _handle_create(args: dict, **kw) -> str:
    ...
    tenant = args.get("tenant") or os.environ.get("sara_TENANT")
    ...
    try:
        ...
        try:
            new_tid = kb.create_task(
                ...
                skills=skills,
                created_by=os.environ.get("sara_PROFILE") or "worker",
            )
...
_DESC_TASK_ID_DEFAULT = (
    "Task id. If omitted, defaults to sara_KANBAN_TASK from the env "
        ...
        "properties": {
            ...
            "tenant": {
                ...
                "description": (
                    ...
                    "Defaults to sara_TENANT env if set."
                ),
            },
            "priority": {
                "type": "integer",
