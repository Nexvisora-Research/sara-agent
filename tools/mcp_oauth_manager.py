...
`mcp_tool.py`, and `sara_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# saraMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_sara_provider_class() -> Optional[type]:
    ...
    class saraMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._sara_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`saraTokenStorage` persisting an absolute
        ...
        async def _prefetch_oauth_metadata(self) -> None:
            ...
            but runs synchronexvisoraly before the first request instead of
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._sara_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        logger.debug(
                            ...
                            "token_endpoint=%s",
                            self._sara_server_name, asm.token_endpoint,
                        )
                        break
    ...
    return saraMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_sara_PROVIDER_CLS: Optional[type] = _make_sara_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`saraMCPOAuthProvider` directly using the helpers
        ...
        storage = saraTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``sara mcp remove <name>`` and (indirectly) by
        ``sara mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
