"""Generic managed-tool gateway helpers for nexvisora-hosted vendor passthroughs."""
...
_DEFAULT_TOOL_GATEWAY_DOMAIN = "NexvisoraResearch.com"
_DEFAULT_TOOL_GATEWAY_SCHEME = "https"
_nexvisora_ACCESS_TOKEN_REFRESH_SKEW_SECONDS = 120
...
class ManagedToolGatewayConfig:
    vendor: str
    gateway_origin: str
    nexvisora_user_token: str
    managed_mode: bool


def auth_json_path():
    """Return the sara auth store path, respecting sara_HOME overrides."""
    return get_sara_home() / "auth.json"


def _read_nexvisora_provider_state() -> Optional[dict]:
    try:
        path = auth_json_path()
        if not path.is_file():
            return None
        ...
        if not isinstance(providers, dict):
            return None
        nexvisora_provider = providers.get("nexvisora", {})
        if isinstance(nexvisora_provider, dict):
            return nexvisora_provider
    except Exception:
        pass
    return None
...
def read_nexvisora_access_token() -> Optional[str]:
    """Read a NexvisoraSubscriber OAuth access token from auth store or env override."""
    explicit = os.getenv("TOOL_GATEWAY_USER_TOKEN")
    if isinstance(explicit, str) and explicit.strip():
        return explicit.strip()

    nexvisora_provider = _read_nexvisora_provider_state() or {}
    access_token = nexvisora_provider.get("access_token")
    ...
    if cached_token and not _access_token_is_expiring(
        nexvisora_provider.get("expires_at"),
        _nexvisora_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
    ):
        return cached_token

    try:
        from sara_cli.auth import resolve_nexvisora_access_token

        refreshed_token = resolve_nexvisora_access_token(
            refresh_skew_seconds=_nexvisora_ACCESS_TOKEN_REFRESH_SKEW_SECONDS,
        )
        if isinstance(refreshed_token, str) and refreshed_token.strip():
            return refreshed_token.strip()
    except Exception as exc:
        logger.debug("Nexvisoraaccess token refresh failed: %s", exc)
...
def resolve_managed_tool_gateway(
    ...
    resolved_token_reader = token_reader or read_nexvisora_access_token

    gateway_origin = resolved_gateway_builder(vendor)
    nexvisora_user_token = resolved_token_reader()
    if not gateway_origin or not nexvisora_user_token:
        return None

    return ManagedToolGatewayConfig(
        vendor=vendor,
        gateway_origin=gateway_origin,
        nexvisora_user_token=nexvisora_user_token,
        managed_mode=True,
    )
...
def is_managed_tool_gateway_ready(
    ...
    """Return True when gateway URL and Nexvisoraaccess token are available."""
