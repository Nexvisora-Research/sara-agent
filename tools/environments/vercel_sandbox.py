...
Uses the Vercel Python SDK to run commands in cloud sandboxes through sara'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``sara_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_sara_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            ...
            container_base = f"{self._remote_home.rstrip('/')}/.sara"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_sara = (
            "/.sara"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.sara"
        )
        archive_member = remote_sara.lstrip("/")
        remote_tar = f"/tmp/.sara_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
