...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    """Gateway-owned Modal sandbox with sara-compatible execute/cleanup."""
    ...
    def __init__(
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway and Nexvisorauser token")

        self._gateway_origin = gateway.gateway_origin.rstrip("/")
        self._nexvisora_user_token = gateway.nexvisora_user_token
    ...
    def _request(self, method: str, path: str, *,
        ...
        headers = {
            "Authorization": f"Bearer {self._nexvisora_user_token}",
