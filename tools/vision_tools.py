...
nexvisora, Codex, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("sara_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from sara_cli.config import cfg_get, load_config
        cfg = load_config()
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $sara_HOME/cache/vision/ and cleaned up
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_sara_dir("cache/vision", "temp_vision_images")
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, nexvisora, Codex, Anthropic, or a custom OpenAI-compatible endpoint).")
        exit(1)
    else:
