...
This module provides the glue:
    - ``saraTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "sara Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses sara_HOME so each profile gets its own OAuth tokens.
    Layout: ``sara_HOME/mcp-tokens/``
    """
    try:
        from sara_constants import get_sara_home
        base = Path(get_sara_home())
    except ImportError:
        base = Path(os.environ.get("sara_HOME", str(Path.home() / ".sara")))
...
# ---------------------------------------------------------------------------
# saraTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class saraTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        sara_HOME/mcp-tokens/<server_name>.json         -- tokens
        sara_HOME/mcp-tokens/<server_name>.client.json   -- client info
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # sara records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to sara.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "sara Agent")
    scope = cfg.get("scope")
