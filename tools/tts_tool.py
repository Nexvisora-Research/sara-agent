...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.sara/config.yaml``. sara
...
Configuration is loaded from ~/.sara/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``sara_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from sara_cli.config import get_env_value as _get_env_value
    except ImportError:
        return os.getenv(name, default)
    value = _get_env_value(name)
    return default if value is None else value
from tools.managed_tool_gateway import resolve_managed_tool_gateway
from tools.tool_backend_helpers import managed_nexvisora_tools_enabled, prefers_gateway, resolve_openai_audio_api_key
from tools.xai_http import sara_xai_user_agent
...
def _get_default_output_dir() -> str:
    from sara_constants import get_sara_dir
    return str(get_sara_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.sara/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.sara/config.yaml.
    ...
    try:
        ...
        logger.debug("sara_cli.config not available, using default TTS config")
        return {}
...
# custom voice-cloning scripts, etc.) into sara without any Python code
...
#           output_format: wav
#
# sara writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__sara_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    # output_format when sara actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": sara_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where sara caches Piper voice models.

    Resolves to ``~/.sara/cache/piper-voices/`` under the active
    sara_HOME so voice downloads follow profile boundaries.
    """
    from sara_constants import get_sara_dir
    root = Path(get_sara_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.sara/config.yaml (tts: section).
    ...
    platform = get_session_env("sara_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'sara-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run sara setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'sara setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'sara tools' and select Piper under TTS, or install manually: "
...
def _resolve_openai_audio_client_config() -> tuple[str, str]:
    ...
    return managed_gateway.nexvisora_user_token, urljoin(
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_sara_home()}/audio_cache/<timestamp>.mp3"
            }
        },
        "required": ["text"]
    }
