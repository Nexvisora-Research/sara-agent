...
it is automatically downloaded from GitHub releases to $sara_HOME/bin/tirith.
...
def _get_sara_home() -> str:
    """Return the sara home directory, respecting sara_HOME env var."""
    return str(get_sara_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_sara_home(), ".tirith-install-failed")
...
def _sara_bin_dir() -> str:
    """Return $sara_HOME/bin, creating it if needed."""
    d = os.path.join(_get_sara_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $sara_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_sara_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $sara_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $sara_HOME/bin/tirith
    ...
    sara_bin = os.path.join(_sara_bin_dir(), "tirith")
    if os.path.isfile(sara_bin) and os.access(sara_bin, os.X_OK):
        _resolved_path = sara_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return sara_bin
