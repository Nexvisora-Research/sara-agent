...
# All skills live in ~/.sara/skills/ (seeded from bundled skills/ on install).
# This is the single source of truth -- agent edits, hub installs, and bundled
# skills all coexist here without polluting the git repo.
sara_HOME = get_sara_home()
SKILLS_DIR = sara_HOME / "skills"
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from sara_HOME/.env."""
...
def _is_gateway_surface() -> bool:
    if os.getenv("sara_GATEWAY_SESSION"):
        return True
    from gateway.session_context import get_session_env
    return bool(get_session_env("sara_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        ...
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_sara_home()}/.env manually."
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.sara/skills/mlops/axolotl/SKILL.md -> "mlops"
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``sara_SESSION_PLATFORM``.
    """
    try:
        from gateway.session_context import get_session_env
        return get_session_env("sara_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``sara_PLATFORM`` environment variable
    3. ``sara_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from sara_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("sara_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.sara/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``sara skills`` config UI). Default False
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_sara_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from sara_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: sara plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            ...
            from sara_cli.plugins import discover_plugins, get_plugin_manager
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.sara/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `sara skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        # Check metadata.sara.* first (agentskills.io convention), fall back to top-level
        sara_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            sara_meta = metadata.get("sara", {}) or {}

        tags = _parse_tags(sara_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            sara_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
