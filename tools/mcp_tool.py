...
discovers their tools, and registers them into the sara-agent tool registry
so the agent can call them like any built-in tool.

Configuration is read from ~/.sara/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.sara/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            sara_home = os.path.expanduser(
                os.getenv(
                    "sara_HOME", os.path.join(os.path.expanduser("~"), ".sara")
                )
            )
            candidates = [
                os.path.join(sara_home, "node", "bin", resolved_command),
...
class MCPServerTask:
                ...
                if _MCP_NOTIFICATION_TYPES and isinstance(message, ServerNotification):
                    match message.root:
                        case ToolListChangedNotification():
                            ...
                            # request. Refreshing synchronexvisoraly inside the SDK
    ...
    async def _refresh_tools(self):
        ...
        After the initial ``await`` (list_tools), all mutations are synchronexvisora
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `sara mcp login {server_name}` (or delete the tokens "
            f"file under ~/.sara/mcp-tokens/ and restart). Do NOT retry "
...
# can be cleaned up asynchronexvisoraly by _kill_orphaned_mcp_children().
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the sara config file.
    ...
    ``os.environ`` (which includes ``~/.sara/.env`` loaded at startup).
    """
    try:
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves sara's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the sara registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``sara tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the sara registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    survivors, avoiding shared-resource collisions when multiple sara
