...
from tools.tool_backend_helpers import managed_nexvisora_tools_enabled, prefers_gateway
...
class BrowserUseProvider(CloudBrowserProvider):
    ...
    # ------------------------------------------------------------------
    # Config resolution (direct API key OR managed Nexvisoragateway)
    # ------------------------------------------------------------------
    ...
    def _get_config_or_none(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            "api_key": managed.nexvisora_user_token,
    ...
    def create_session(self, task_id: str) -> Dict[str, object]:
        ...
        # default to a long Browser-Use timeout when sara only needs a task-
        ...
        if managed_mode:
            _clear_pending_create_key(task_id)
        session_name = f"sara_{task_id}_{uuid.uuid4().hex[:8]}"
