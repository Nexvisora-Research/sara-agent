...
# 5. Symlinks the 'sara' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ sara Agent Setup${NC}"
echo ""
...
if is_termux; then
    ...
    elif [ -x "$HOME/snap/code/237/.local/bin/uv" ]; then
        UV_CMD="$HOME/snap/code/237/.local/bin/uv"
    fi
...
# ============================================================================
# Environment files (project + per-user)
# ============================================================================

SARA_HOME_DIR="${sara_HOME:-$HOME/.sara}"
mkdir -p "$SARA_HOME_DIR"
...
if [ -z "$ENV_TEMPLATE" ]; then
    echo -e "${YELLOW}⚠${NC} No env template found (.env.example or example.env)"
else
    # Project-local .env for repo runs
    if [ ! -f ".env" ]; then
        cp "$ENV_TEMPLATE" .env
        chmod 600 .env 2>/dev/null || true
        echo -e "${GREEN}✓${NC} Created project .env from $ENV_TEMPLATE"
    else
        chmod 600 .env 2>/dev/null || true
        echo -e "${GREEN}✓${NC} Project .env exists"
    fi

    # Per-user env used by sara_cli.env_loader (preferred for multi-user machines)
    USER_ENV_FILE="$SARA_HOME_DIR/.env"
    if [ ! -f "$USER_ENV_FILE" ]; then
        cp "$ENV_TEMPLATE" "$USER_ENV_FILE"
        chmod 600 "$USER_ENV_FILE" 2>/dev/null || true
        echo -e "${GREEN}✓${NC} Created user env: $USER_ENV_FILE"
    else
        chmod 600 "$USER_ENV_FILE" 2>/dev/null || true
        echo -e "${GREEN}✓${NC} User env exists: $USER_ENV_FILE"
    fi
fi

# ============================================================================
# PATH setup — symlink sara into a user-facing bin dir
# ============================================================================
...
# ============================================================================
# Seed bundled skills into ~/.sara/skills/
# ============================================================================

sara_SKILLS_DIR="${sara_HOME:-$HOME/.sara}/skills"
mkdir -p "$sara_SKILLS_DIR"
...
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    ...
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$sara_SKILLS_DIR/" 2>/dev/null || true
