...
- **Two separate arrowhead markers**: `#arr-call` is a filled triangle (`<polygon>`) for synchroNexvisoracalls; `#arr-ret` is an open chevron (`fill="none"`) for dashed return messages — both use `context-stroke` to inherit line color
...
<svg width="100%" viewBox="0 0 680 648" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <!-- Filled triangle arrowhead — synchroNexvisoracalls -->
  ...
  <text class="ts" x="154" y="613" opacity=".75">SynchroNexvisoracall</text>
...
| SynchroNexvisoracall arrows | `var(--text-primary)` + filled triangle | High contrast for calls — the primary interaction direction |
